...
require "zoo.gamePlay.mode.TravelMode"
require "zoo.gamePlay.mode.WeeklyRace2020Mode"
require "zoo.gamePlay.mode.AngryBirdMode"
require "zoo.gamePlay.mode.TreasureHuntMode"
require "zoo.gamePlay.mode.BridgeCrossMode"
require "zoo.gamePlay.mode.DestinationMode"
require "zoo.gamePlay.mode.GoldenPodBattleMode"
require "zoo.gamePlay.mode.NationalDayBunnyMode"
...
function GameModeFactory:create(mainLogic)
    ...
    elseif gameMode == GameModeTypeId.TRAVEL_MODE_ID then
        return TravelMode.new(mainLogic)
    elseif gameMode == GameModeTypeId.WEEKLY_RACE_2020_MODE_ID then
        return WeeklyRace2020Mode.new(mainLogic)        -- 多屏挖地模式
    elseif gameMode == GameModeTypeId.ANGRY_BIRD_MODE_ID then
        return AngryBirdMode.new(mainLogic)
    elseif gameMode == GameModeTypeId.TREASURE_HUNT_MODE_ID then
        return TreasureHuntMode.new(mainLogic)
    elseif gameMode == GameModeTypeId.BRIDGE_CROSS_MODE_ID then
        return BridgeCrossMode.new(mainLogic)
    elseif gameMode == GameModeTypeId.DESTINATION_MODE_ID then
        return DestinationMode.new(mainLogic)
    elseif gameMode == GameModeTypeId.GOLDEN_POD_BATTLE_MODE_ID then
        return GoldenPodBattleMode.new(mainLogic)
    elseif gameMode == GameModeTypeId.NATIONAL_DAY_BUNNY_MODE_ID then
        return NationalDayBunnyMode.new(mainLogic)
    else
