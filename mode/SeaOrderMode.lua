...
SeaAnimalConfig = {
    ...
    [SeaAnimalType.kPopcorn_32] = {area = {3, 2}},
    [SeaAnimalType.kPopcorn_23] = {area = {2, 3}},
}
...
function SeaOrderMode:initModeSpecial(config)
    ...
    for i = #self.mainLogic.theOrderList, 1, -1 do
        ...
        if order.key1 == 6 and (order.key2 == 5 or order.key2 == 6 or order.key2 == 7 or order.key2 == 8 or order.key2 == 9 ) then
            if seaOrder then 
                if order.key2 == 7 and not LevelType:isPopcornLevel(self.mainLogic.level) then
				...
				if canPutFlag then
					for yIndex = r, r + yAdd do
						for xIndex =c, c + xAdd do
							if xIndex > colAmount or yIndex > rowAmount then
								canPutFlag = false
								break
							end
...
function SeaOrderMode:getScoreStarLevel()
	if self.mainLogic and self.mainLogic.level and ( LevelType:isSpringFes2020Level(self.mainLogic.level)
		or LevelType:isSixYear2020Level(self.mainLogic.level) )  then
		return 1
	else
		return OrderMode.getScoreStarLevel(self)
	end
end

-- 获取被海洋生物占据的格子列表
function SeaOrderMode:getBoardList()
	local list = {}
	local map = {}
	local seaAnimals = self.allSeaAnimals or {}
