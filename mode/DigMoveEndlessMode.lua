...
function DigMoveEndlessMode:initModeSpecial(config)
	...
	self.groundPool = {}
	for i=1, 9*max_generate_row do 	--挖地关如将来默认屏配置超过9行，请重新确认生成规则
...
function DigMoveEndlessMode:getExtraMap(passedRow, additionRow)
	...
	for row = 1, normalRowCount do
		...
		if row == 1 then
			colCount = #self.mainLogic.gameItemMap[row]
		end
	end
	...
	if rowCountUsingGenerator > 0 then
		...
		for k, v in pairs(generatedItems) do 
			...
			local rowIndex = math.ceil(k / colCount)
			local colIndex = k - (rowIndex - 1) * colCount 
...
function DigMoveEndlessMode:generateGroundRow(rowCount)
	...
	local rowAmount, colAmount = GameBoardUtil:getRowAndColAmountOfBoard(self.mainLogic)
	local length = colAmount * rowCount
	local usedIndex = {}
...
function DigMoveEndlessMode:upgradeGround()
	if self.rowCountSinceLastGroundUpgrade >= ground_upgrade_interval then
		...
		local counter  = 0
		local rowAmount, colAmount = GameBoardUtil:getRowAndColAmountOfBoard(self.mainLogic)
		local length = colAmount * max_generate_row
