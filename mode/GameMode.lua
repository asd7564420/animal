...
function GameMode:getScoreStarLevel()
 ...
 --onlyCheck为true时，代表时关卡内的每帧检测是否达到过关目标 这个检测不应该触发非预期的赋值
function GameMode:reachEndCondition(onlyCheck)
  return false
end
...
function GameMode:revertDataFromBackProp()
	...
	if mainLogic.saveRevertData.level then
		mainLogic.level = mainLogic.saveRevertData.level
	end
	
	mainLogic.entranceLevel = mainLogic.saveRevertData.entranceLevel
	...
	if mainLogic.saveRevertData.defaultProductRuleGroupId  then
		mainLogic.defaultProductRuleGroupId = mainLogic.saveRevertData.defaultProductRuleGroupId
	end

	if mainLogic.saveRevertData.productRuleConfig  then
		mainLogic.productRuleConfig = mainLogic.saveRevertData.productRuleConfig
	end

	if mainLogic.saveRevertData.productRuleGlobalConfig  then
		mainLogic.productRuleGlobalConfig = mainLogic.saveRevertData.productRuleGlobalConfig
	end
	...
	mainLogic.generateFirecrackerTemp = mainLogic.saveRevertData.generateFirecrackerTemp or 0
	...
	mainLogic.curWaterBucketChargingGroupId = mainLogic.saveRevertData.curWaterBucketChargingGroupId or 0
	mainLogic.squidOnBoard = nil	-- 重新从棋盘获取一遍鱿鱼数据
	mainLogic.globalScoreBuffVal = mainLogic.saveRevertData.globalScoreBuffVal
	mainLogic.hadAutoSnowMonsterBuff = mainLogic.saveRevertData.hadAutoSnowMonsterBuff
	mainLogic.hadAutoBuffActived = mainLogic.saveRevertData.hadAutoBuffActived
	mainLogic.lastSlyBunnyFleeingMoves = mainLogic.saveRevertData.lastSlyBunnyFleeingMoves
	
	mainLogic.collectTargetNums = mainLogic.saveRevertData.collectTargetNums
	mainLogic:setExtraGainedActCollectionFromRevertData(mainLogic.saveRevertData.extraGainedActCollections)
	mainLogic.moveBonusHarvested = mainLogic.saveRevertData.moveBonusHarvested
	mainLogic.inObservingMode = mainLogic.inObservingMode

	mainLogic.shellGiftData = mainLogic.saveRevertData.shellGiftData
	...
	LocalActCoreModel.getOrCreateInstance():notify(ActInterface.kReadSectionData, mainLogic.saveRevertData)

	if mainLogic.saveRevertData.stagePropsUsedInfo then
		StageInfoLocalLogic:revertPropsUsedInLevel(mainLogic.saveRevertData.stagePropsUsedInfo)
	end

	Notify:dispatch("revertGameBoard", mainLogic , mainLogic.saveRevertData )

	mainLogic.noBiscuitInBoard = false --重新检测
...
function GameMode:revertUIFromBackProp()
	...
	if mainLogic.PlayUIDelegate then
		...
		if mainLogic.PlayUIDelegate.scoreProgressBar then
	...
	LocalActCoreModel.getOrCreateInstance():notify(ActInterface.kCPBRevertData, _data)
end
...
function GameMode:onStartGame()
	local _data = {
		...
		mainLogic = self.mainLogic,
	}
	LocalActCoreModel:getOrCreateInstance():notify(ActInterface.kGameBoardViewInited, _data)

	if not self.checkStrategy then
		hiddenStrategyManager.getInstance():checkStrategyIcon(self.mainLogic)
		self.checkStrategy = true
	end
	...
	self.mainLogic.PlayUIDelegate:onStartGame(self.mainLogic)
end

function GameMode:onGameInit()
	local context = self
	local isAct5003Effective = StarBuffYEManager.getInstance():isBuffIngameEffective()
	StarBuffYEManager.getInstance():setIngameFlag(false)

	hiddenStrategyManager.getInstance():checkStrategyIcon(self.mainLogic)
	self.checkStrategy = true

	if self.mainLogic.replayMode == ReplayMode.kNone then
    	ReplayDataManager:updateRedCrossContext()
	end
	...
	local function actPreActionAnim()
		local _data = {
			levelId = self.mainLogic.level,
			mainLogic = self.mainLogic,
			endCallback = setGameStart,
		}
		if LocalActCoreModel:getOrCreateInstance():notifyMayBlock(ActInterface.kAddInitAction, _data) then
			--maybe do sth
		else
			setGameStart()
		end
	...
	local function matchFestivalVipAnim()
		if LevelType:isMatchFestivalLevel(context.mainLogic.level) and context.mainLogic.PlayUIDelegate.topArea then
			context.mainLogic.PlayUIDelegate.topArea:playVipAnim(context.mainLogic, actPreActionAnim) 
		else
			actPreActionAnim()
		end
	end

	local function dhc_end()
		if isAct5003Effective then
			StarBuffYEManager.getInstance():playBuffAnim(context.mainLogic, matchFestivalVipAnim)
		elseif LocalActCoreModel.getInstance():notifyMayBlock(ActInterface.kUseSpecialSkin, LevelType:getLevelTypeByLevelId(self.mainLogic.level)) then
			if context.mainLogic.PlayUIDelegate.topArea and context.mainLogic.PlayUIDelegate.topArea.playAddBuffAnim then
				context.mainLogic.PlayUIDelegate.topArea:playAddBuffAnim(context.mainLogic, actPreActionAnim) 
			end
		else
			matchFestivalVipAnim()
		end
	end

	local function handleGameTopPartAni()
		if TurnTable2019DHCManager.getInstance():isSupportIngameTip(self.mainLogic.level) and self.mainLogic.replayMode == ReplayMode.kNone then
			TurnTable2019DHCManager.getInstance():playTipAnim(context.mainLogic, dhc_end)
		else
			dhc_end()
		end
	end
...
function GameMode:onModeStartWaitingState()
	-- override it plz
end

function GameMode:onModeStopWaitingState()
	-- override it plz
end
