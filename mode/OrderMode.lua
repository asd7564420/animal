...
function OrderMode:initModeSpecial(config)
    ...
    if ChannelWaterLogic.isEnable() then
        local _tileMap = config.tileMap
        for r = 1, #_tileMap do
            if self.mainLogic.boardmap[r] == nil then self.mainLogic.boardmap[r] = {} end
...
function OrderMode:reachEndCondition(onlyCheck)
    if self:targetIsVenom() 
        and not self:hasOctopus() 
        and not self:venomTargetReached() 
        and self:getVenomCount() == 0 
        and not self._isWaitingBackProp
        and not onlyCheck then
        -- 弹过提示就不再弹
        --[[
