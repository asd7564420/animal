...
function DigMoveMode:onGameInit()
	...
	local function dhc_end()
		if isAct5003Effective then
			StarBuffYEManager.getInstance():playBuffAnim(context.mainLogic, actPreActionAnim)
...
function DigMoveMode:doScrollDigGround(moveUpRow, stableScrollCallback)
	...
	local function scrollCallback()
		...
		if self.handleDataAfterScrollCallBack then
			self:handleDataAfterScrollCallBack()
		end
		...
		mainLogic.PlayUIDelegate:afterDigGroudScroll(mainLogic)
...
function DigMoveMode:handleDataAfterScrollCallBack()
