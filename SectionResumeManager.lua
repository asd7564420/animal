...
function SectionResumeManager:addSection()
	...
	if _G.useSectionWhenCrash 
		...
		or (self.mainlogic.replayMode == ReplayMode.kMcts and not _G.AIAutoCheckReplayCheck) 
		...
		for k,v in pairs( tab ) do
			...
			elseif opr == 2 then
				...
				if numKey then
					...
					recodeTab[k] = v 	-- weightList的Key就是string
				end
			end
		end
		return recodeTab
...
function SectionResumeManager:encodeBySection( section )
	...
	local function cloneTable(oringinTable , targetTable)
		...
		for k,v in pairs(oringinTable) do
			...
			elseif k == "blocker199Dirs" or k == "blocker83Dirs" then
				local fixedList = {}
				for k1 , v1 in ipairs(v) do
	...
	for r = 1, #section.gameItemMap do
		cloneGameItemMapForRevert[r] = {}
		for c = 1, #section.gameItemMap[r] do
	...
	for r = 1, #section.boardmap do
		cloneBoardMapForRevert[r] = {}
		for c = 1, #section.boardmap[r] do
	...
	local rowAmount = #section.gameItemMap
	local colAmount = 9
	if section.gameItemMap[1] then
		colAmount = #section.gameItemMap[1]
	end
...
function SectionResumeManager:decodeByTable( dataTable )
	...
	for r = 1, #dataTable.gameItemMap do
		cloneGameItemMapForRevert[r] = {}
		for c = 1, #dataTable.gameItemMap[r] do
	...
	for r = 1, #dataTable.boardmap do
		cloneBoardMapForRevert[r] = {}
		for c = 1, #dataTable.boardmap[r] do
	...
	local rowAmount = #dataTable.gameItemMap
	local colAmount = 9
	if dataTable.gameItemMap[1] then
		colAmount = #dataTable.gameItemMap[1]
	end
