require "zoo.gamePlay.UploadReplayDataManager"
...
local UploadReplayDataManager = require "zoo.gamePlay.UploadReplayDataManager"
ReplayDataManager.UploadReplayDataManager = UploadReplayDataManager
	...
	if tonumber(version) == 2 then
		...
		if replaySteps then
			for _, v in ipairs(replaySteps) do
				...
				elseif params[1] == "swch" then --"swch" for "switch player"
					table.insert(ret, {switchPlayer = tonumber(params[2])})	-- value for player ID (not really used, though)
				else
...
function ReplayDataManager:updatePropBagInfo()
	...
	check( ItemType.TIMELIMIT_ADD_FIVE_STEP_BY_WEEKLY_RACE_2020 )
...
function ReplayDataManager:updatePropUseInfo( propId )
	if self.currLevelReplayData and self.currLevelReplayData.propUseInfo then
		...
		if propId == ItemType.ADD_FIVE_STEP or
			...
			propId == ItemType.TIMELIMIT_ADD_FIVE_STEP or
			propId == ItemType.ADD_FIVE_STEP_BY_WEEKLY_RACE_2020 or 
			propId == ItemType.TIMELIMIT_ADD_FIVE_STEP_BY_WEEKLY_RACE_2020 or
			propId == ItemType.ADD_10_STEP_BY_AUTUMN2020 or
			propId == ItemType.ADD_15_STEP_BY_AUTUMN2020 or
			propId == ItemType.Autumn2020_Resurrect
			or ItemType:isActivityAddFiveStep(propId)
			then
...
function ReplayDataManager:onStartLevel(gameBoardLogic)
	...
	if gameBoardLogic.replayMode == ReplayMode.kNone 
		...
		or (gameBoardLogic.replayMode == ReplayMode.kMcts and _G.launchCmds.domain and (not _G.AIAutoCheckReplayCheck) )
		then
		...
		replay.SCLogicVer = SpecialCoverLogicVer or 0
		...
		replay.shellGift = ShellGiftLogic.getDataForReplay()
		...
		replay.hadAutoSnowMonsterBuff = gameBoardLogic.hadAutoSnowMonsterBuff


		replay.actContext = {}

		LocalActCoreModel.getOrCreateInstance():notify(ActInterface.kWriteReplayData, replay)
		...
		replay.strategyDCInfo.nd4 = LevelDifficultyAdjustManager:getLastFuuuUnactivateReason()
		...
		if AngryBird2020Manager and AngryBird2020Manager.getInstance() then
			replay.angryBirdsLevelData = gameBoardLogic.PlayUIDelegate.angryBirdsLevelData
		end
		
		-- 目前新周赛没有结束时间的相关设定
		if gameBoardLogic.PlayUIDelegate then
			replay.weeklyRace2020LevelData = gameBoardLogic.PlayUIDelegate.weeklyRace2020LevelData
			-- if replay.weeklyRace2020LevelData then
			-- 	replay.weeklyRace2020LevelData.gottenRewards = {}
			-- end
		end
		...
		if StarBuffYEManager.getInstance():isBuffIngameEffective() then
			replay.act5003Effctive = true
			if StarBuffYEManager.getInstance():hasInfiniteBuffState() then
				replay.act5003EffctiveLastTime = Localhost:timeInSec()
			end
		end
		...
		for k,v in pairs(featureMap) do
			if not featureStr then
				featureStr = tostring( v.tileId )
			else
				featureStr = featureStr .. "_" .. tostring( v.tileId )
			end
		end
		replay.features = featureStr
		replay.reslist = {}
		-- printx( 1 , "featureMap =" , table.tostring(featureMap) )
		-- printx( 1 , "featureStr =" , featureStr )
	...
	ReplayDataManager.UploadReplayDataManager.maxReplayNum = maxReplayNum
...
function ReplayDataManager:updateGamePlayContext(forceFlush, noIO)
	if self.gameBoardLogic and ( forceFlush or self.gameBoardLogic.replayMode == ReplayMode.kNone 
											or self.gameBoardLogic.replayMode == ReplayMode.kResume
											or self.gameBoardLogic.replayMode == ReplayMode.kSectionResume ) then
		if self.currLevelReplayData then
...
function ReplayDataManager:updateGameScore( forceFlush, noIO )
	if self.gameBoardLogic 
		...
		( 
			forceFlush 
			or 
			(self.gameBoardLogic.PlayUIDelegate and (self.gameBoardLogic.PlayUIDelegate.replayMode == ReplayMode.kNone or (not self.gameBoardLogic.PlayUIDelegate.replayMode)))
		) then
...
function ReplayDataManager:updateRedCrossContext( ... )



	if self.gameBoardLogic and self.gameBoardLogic.PlayUIDelegate and (self.gameBoardLogic.PlayUIDelegate.replayMode == ReplayMode.kNone or (not self.gameBoardLogic.PlayUIDelegate.replayMode)) then
		if self.currLevelReplayData then
			self.currLevelReplayData.rcCtx = RedCross:getOrCreateInstance():getRevertData()
			self:flushReplayCache()
		end
	end
end
...
function ReplayDataManager:onPassLevel(result , totalScore)
	...
	UploadReplayDataManager:checkUploadByDpTags()
...
function ReplayDataManager:addReplayStep( item )
	...
	if item and self.setWriteReplayEnable and self.currLevelReplayData and self.currLevelReplayData.replaySteps then
		...
		elseif item.switchPlayer then
			formatItem = string.format("swch:%d", item.switchPlayer)
		else
...
function ReplayDataManager:checkNeedFlushReplayDataByCache( checkResultCallback )
	-- printx( 1 , "ReplayDataManager:checkNeedFlushReplayDataByCache 111" )
	local function doCheckNeedFlushReplayDataByCache()
		...
		-- printx( 1 , "ReplayDataManager:checkNeedFlushReplayDataByCache replayCache = " , replayCache )
		
		if replayCache and replayCache.level and type(replayCache.level) == "number" then
			-- Need Flush Replay Data By Cache

			-- printx( 1 , "ReplayDataManager:checkNeedFlushReplayDataByCache 222" )
...
function ReplayDataManager:flushReplayDataByCache(replayCache)
	...
	local function removeFiles(path, isDir)
		if HeFileUtils:exists(path) then
			...
			if false then
				...
				if checkReplayCacheText then
					local _currTime = os.time()
					RemoteDebug:uploadLogWithTag( "REPLAY" , "last replay data id : " .. path .. "_" .. tostring(_currTime) )
					Localhost:safeWriteStringToFile( checkReplayCacheText , path .. "_" ..  tostriung( _currTime ) )
					-- Localhost:safeWriteStringToFile( checkReplayCacheText , "ErrorReplaySection_" ..  tostriung( _currTime ) )
				end
			end
	...
	local dataKey = tostring(replayCache.idStr) .. "_" .. tostring( #replayCache.replaySteps )
	...
	UploadReplayDataManager:addLogDataFlag( dataKey )

	self:uploadReplayIfNeeded(replayCache , 1)

	while #replayData.datas > maxReplayNum do
		table.remove( replayData.datas , 1 )
	end
	
	if not (replayData and replayData.datas and #replayData.datas > 0) then
		replayData = {}
		replayData.datas = {}
	end
...
function ReplayDataManager:readCheckReplayData( filePath )
	...
	local replayData = defaultData
    if checkReplayDataText then
         replayData = table.deserialize(checkReplayDataText) or defaultData
    end
...
function ReplayDataManager:setPopResume(value)
	self.hasPopedResumePanel = value
end

function ReplayDataManager:getPopResume()
	return self.hasPopedResumePanel
end
...
function ReplayDataManager:checkUploadReplayByCommonEvent(datas)
	...
	self:checkForceToUploadReplay( uploadReplay , ReplayUploadReason.kUserExtendFlag )
end

function ReplayDataManager:checkForceToUploadReplay( userExtendFlag , reason )
	-- printx( 1 , "ReplayDataManager:checkForceToUploadReplay     userExtendFlag" , userExtendFlag , "reason" , reason )
	...
	local userExtendFlag = userExtendFlag or 0
	if userExtendFlag == 0 and UserManager:getInstance() and UserManager:getInstance().userExtend and UserManager:getInstance().userExtend.uploadReplay then
		userExtendFlag = tonumber( UserManager:getInstance().userExtend.uploadReplay )
	end

	local targetType , levelId = UploadReplayDataManager:decodeUserExtendFlag( userExtendFlag )

	local uploadResult = UploadReplayDataManager:uploadReplayByLevelId( levelId , targetType , reason )
...
function ReplayDataManager:uploadReplayForReasons(replayCache, reasons , targetType)
	...
	local rpdata = replayCache

	if rpdata then
		local datalist = {}
		table.insert( datalist , rpdata )
		
		UploadReplayDataManager:uploadReplay( datalist , targetType , reasonCode )
	end
end

function ReplayDataManager:uploadReplayIfNeeded(replayCache , uploadMode)
	if not replayCache then return end

	if not uploadMode then uploadMode = 1 end

	if self:hasUploadReplayReasons() then
		self:uploadReplayForReasons(replayCache, self.uploadReplayReasons , uploadMode)
		self.uploadReplayReasons= {}
		return
	end
end
...
function ReplayDataManager:onTakeScreenSnapshot()
	if self.setWriteReplayEnable and self.gameBoardLogic and self.currLevelReplayData then
		...
		table.insert(userActs.sn, self.gameBoardLogic.realCostMove)
	 	...
	 	if _G.isLocalDevelopMode 
	 		or MaintenanceManager:getInstance():isEnabledInGroup("UploadReplayWhenSnapshot", "ON" , UserManager:getInstance().uid) then

	 		self:addUploadReplayReason( ReplayUploadReason.kScreenShot  )
	 		self:uploadReplayIfNeeded( self:getCurrLevelReplayDataCopyWithoutSectionData() , 2 ) --发生截图，立刻回传
	 	end
	end
end
...
function ReplayDataManager:updateShellGiftInfo()
	if self.currLevelReplayData then 
		self.currLevelReplayData.shellGift = ShellGiftLogic.getDataForReplay()
	 	self:flushReplayCache()
	end
end
...
function ReplayDataManager:checkNeedSetActContextWhenLevelStart( )
	...
	local _data = {
		levelId = self.currLevelReplayData.level,
		actContext = self.currLevelReplayData.actContext,
	} 
	LocalActCoreModel.getOrCreateInstance():notify(ActInterface.kSetReplayFlag, _data)
end
...
function ReplayDataManager:checkResumeEnableByActContext( actPathKey )
	if actPathKey == CloverRaceManager.getInstance():getReplayFlag() then
		return CloverRaceManager.getInstance():isActivitySupport()
	end

	local _data = {
		replayFlagEnabled = true,		--默认值为true
		actPathKey = actPathKey,
	}
	LocalActCoreModel.getOrCreateInstance():notify(ActInterface.kCheckReplayFlag, _data)

	return _data.replayFlagEnabled
end
...
function ReplayDataManager:checkProductItemLogicVersion( levelId , levelConfig )
	...
	if __productLogic == 0 then
		...
		-- if MaintenanceManager:getInstance():isEnabledInGroup("NewProductItemLogic" , "ON" , uid) then
			if GameGuide:hasGuideSeed(levelId) then
				logicVer = 1
			else
				logicVer = 2
			end
...
function ReplayDataManager:addMctsLogs( pos , datas )

	local log = {}
	log.pos = pos

	if datas.method == "ready" then
		log.method = "ready"
	elseif datas.method == "start" then
		log.method = "start"
		log.seed = datas.seed
		log.level = datas.level

		if _G.launchCmds then
			log.domain = _G.launchCmds.domain
			log.mock = _G.launchCmds.mock
		end
	elseif datas.method == "status" then
		if datas.result then
			log.method = "endLevel"
			log.score = datas.score
			log.star = datas.star
			log.result = datas.result
			log.failReason = datas.failReason
		else
			log.method = "status"
			log.actionList = datas.actionList
			log.score = datas.score
		end
	elseif datas.method == "nextStep" then
		log.method = "nextStep"
		log.action = datas.action
		log.newSeed = datas.newSeed
	elseif datas.method == "playout" then
		log.method = "playout"
		log.action = datas.action
		log.newSeed = datas.newSeed
	elseif datas.method == "doIndexSwap" then
		log.method = "doIndexSwap"
		log.idx = datas.idx
		log.r1 = datas.r1
		log.c1 = datas.c1
		log.r2 = datas.r2
		...
		log.method = datas.method
	end

	table.insert( mctsLogData ,  log)

	local str = table.serialize( mctsLogData )
	Localhost:safeWriteStringToFile( str , HeResPathUtils:getUserDataPath() .. "/MctsLog.ds")
end

local _tcount = 0
function ReplayDataManager:clearMctsLogs()
	if not AIGamePlayManager.DebugMode then
		os.remove( HeResPathUtils:getUserDataPath() .. "/MctsLog.ds" )
		mctsLogData = {}
	else
		_tcount = _tcount + 1
		if _tcount > 10 then
...
-- UploadReplayDataManager:init( ReplayDataManager )