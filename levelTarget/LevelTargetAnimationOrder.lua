...
function LevelTargetAnimationOrder:getIconFrameName(itemType, id, fname)
	...
	if fullname == "target.order4_1" then
		if SnowMatchFestivalLogic:isSnowMatchFestivalLevel() then
			fullname = fullname.."_mf"
		end
	end
...
function LevelTargetAnimationOrder:setTargetNumber( itemType, itemId, itemNum, animate, globalPosition, rotation, percent, subKey2 )
	for i=1,self.numberOfTargets do
		local item = self["c"..i]
		
		local function delaySetTargetNumber()
		...
		if item and item.itemId == itemId and item.itemType == itemType then
			if item.itemType == "order4" and itemId == 1
				and item.sprite and not item.sprite.isDisposed 
				then
				-- setTimeOut(delaySetTargetNumber, 0.7)	--setTimeOut里面难道是堆栈存储吗？后进先出，顺序全乱了
				-- item.sprite:runAction(CCSequence:createWithTwoActions(CCDelayTime:create(0.7), CCCallFunc:create(delaySetTargetNumber)))
				
				GameBoardLogic:getInstance().PlayUIDelegate:runAction(
					CCSequence:createWithTwoActions(CCDelayTime:create(35/30), CCCallFunc:create(delaySetTargetNumber)))
				...
				item:setTargetNumber(itemId, itemNum, animate, globalPosition, rotation, percent, subKey2)
			end
		end

		--TODO 四周年
		if item and item.itemType == 'order6' and ( itemId >= 5 and itemId <= 11 ) then
			item:setTargetNumber(itemId, itemNum, animate, globalPosition, rotation, percent)
		end
	end
end
...
function LevelTargetAnimationOrder:setNumberOfTargets( v, animationCallbackFunc, flyFinishedCallback , noAnimation, noFlyAnimation)
	-- body
...
function LevelTargetAnimationOrder:updateTargets( hideTop )
	-- body
	...
	for i=1,v do
		...
		if hideTop then
			tileIcon:setScale(1.6*1.5)
			tileArray:addObject(CCDelayTime:create(1.3 + i * 0.3))
			tileArray:addObject(CCScaleTo:create(0.3, 2.2*1.5))
			tileArray:addObject(CCScaleTo:create(0.3, 1.5*1.5))
		else
			tileArray:addObject(CCDelayTime:create(1.3 + i * 0.3))
	...
	if mainLogic and mainLogic.level and LevelType:isCoconutLevel(mainLogic.level) then
		if PlatformConfig:isPlatform(PlatformNameEnum.kMI) then
			self.tip_label:setString("获得足够的米兔摆件")
		elseif PlatformConfig:isPlatform(PlatformNameEnum.kHuaWei) then
			self.tip_label:setString("获得足够的萝卜")
		else
			self.tip_label:setString("获得足够的椰汁")
		end
	end
...
function LevelTargetAnimationOrder:getTargetTypeByTargets( ... )
	...
	if self.targets then
		...
		if selectedItem then
			...
			elseif selectedItem.type == kLevelTargetType.order7 then
				local wholeType = selectedItem.type
				if selectedItem.id then
					wholeType = wholeType.."_"..selectedItem.id
				end
				return kLevelTargetType[wholeType]
			end
