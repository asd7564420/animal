...
function LevelTargetAnimationBase:setTargets( targets, animationCallbackFunc, flyFinishedCallback  , noAnimation , noFlyAnimation)
	-- body
	self.targets = targets
	self:setNumberOfTargets(#targets, animationCallbackFunc, flyFinishedCallback , noAnimation,noFlyAnimation)
...
function LevelTargetAnimationBase:buildLevelPanel( ... )
	...
	local winSize = CCDirector:sharedDirector():getVisibleSize()
  	...
  	if WorldSceneShowManager:getInstance():isInAcitivtyTime() then
  		panel = ResourceManager:sharedInstance():buildGroup("level_end_pop_panels/spring2020/targets_panel")
  	else
  		panel = ResourceManager:sharedInstance():buildGroup("level_end_pop_panels/targets_panel")
  	end
  	-- local panel = ResourceManager:sharedInstance():buildGroup("level_end_pop_panels/targets_panel")
  	...
  	local tip_label_ph = panel:getChildByName("tip_label_ph")
  	if tip_label_ph then
  		local gb = tip_label_ph:getGroupBounds(panel)
  		local centerPos = {x = gb.origin.x + gb.size.width / 2, y = gb.origin.y + gb.size.height / 2}
  		tip_label.centerPos = centerPos
  		tip_label_ph:removeFromParentAndCleanup(true)
  	end
  	--stringKey, {level_number = self.levelId}
  	local tipSize = tip_label:getContentSize()
  	tip_label:setString("")
  	if tip_label.fntColor then
	  	tip_label:setColor(hex2ccc3(tip_label.fntColor))
  	end
  	tip_label:changeFntFile("fnt/tutorial_white.fnt")
  	tip_label:setAnchorPoint(ccp(0.5, 0.5))
  	if tip_label.centerPos then
  		tip_label:setPosition(ccp(tip_label.centerPos.x, tip_label.centerPos.y))
  	end
...
function LevelTargetAnimationBase:getTargetByType( itemType, itemId )
	-- print("LevelTargetAnimationBase:getTargetByType",itemType, itemId)
	if type(itemType)=="number" then
		itemType = "order" .. tostring(itemType)
	end
	for i=1,self.numberOfTargets do
        local item = self["c"..i]
		-- print("LevelTargetAnimationBase:getTargetByType-",i,item.itemId,item.itemType)
...
function LevelTargetAnimationBase:highlightTarget( itemType, itemId, enable, showRaccoon )
	-- body
	 for i=1,self.numberOfTargets do
        local item = self["c"..i]
        -- if _G.isLocalDevelopMode then printx(0, "LevelTargetAnimationBase:highlightTarget()setTargetNumber", item.itemId, item.itemType) end
        if item and item.itemId == itemId and item.itemType == itemType then
