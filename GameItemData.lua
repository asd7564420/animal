...
function GameItemData:ctor()
	...
	self.isEmpty = true 				-- 注意：有花藤的格子isEmpty居然是true...
	...
	self.lotusLevel = 0	--荷塘等级，对应1-3级（大概）--!!!! 注意：当等级为1时，刷新会错误地随着动物移动带走这个level，所以lotusLevel==1时这个值是不可信的，请用boardData中的相应值来判断 !!!!
	...
	self.shieldToColorFilter = false 	-- 【临时】屏蔽过滤器过滤
	...
	self.inChameleonTransform = false 		-- 变色龙变化中
    ...
    self.blocker83Dirs = nil

    self.isToBlastScoreBuffBottles = false 		--即将爆破的刷星瓶
    self.sunFlaskLevel = 0				-- 阳光瓶等级
    ...
    self.gyroDirection = 0 --陀螺方向
    self.gyroLevel = 0 --陀螺等级
    self.gyroCreaterPos = nil--生成陀螺的生成器位置
    self.gyroCreaterNextDirection = nil--生成陀螺的生成器下一个方向

    self.gyroCreaterData = nil--陀螺生成器属性
    self.gyroCreaterCurDirector = 0--陀螺生成器方向
    self.gyroGeneratedByStep = 0		--- 生成的陀螺数量（通过步数）
	self.gyroGeneratedByBoardMin = 0	---	生成的陀螺数量（通过棋盘最少限制）
	self.gyroCreaterChildList = {} ---生成的陀螺位置列表

	self.gyroPathBlock = 0 --陀螺消除路径上的block --已废弃

	self.waterLevel = 0
	self.canevine_data = {}
	self.can_attack_canevine = true

	self.waterBucketGroupId = 0
	self.waterBucketDestroying = false
	self.waterBucketCharge = 0
	self.waterBucketMaximumCharge = 9999999
	self.waterBucketChargingCache = 0

	self.windTunnelSwitchLevel = 0				-- 风洞开关的等级
	self.windTunnelSwitchTypeIsOff = false 		-- 风洞开关的类型：是否为关闭型开关

	self.activityCollectionItem_ItemId = -1     --感恩节收集物里的道具ID -1为不存在
	self.activityCollectionItem_lock = false 	--感恩节收集物锁定不能被旁消了

	self.tempRowShiftByHero = 0
	self.tempColShiftByHero = 0
	self.travelHeroReachEndActivated = false 	--临时，是否触发了到达终点状态
	self.cuckooBirdReachedEnd = false 	-- 布谷鸟是否已到达终点

	self.valentineIngredientID = 0		--2020情人节食材ID
	self.SakuraIngredientID = 0

	self.planeDirection = 0		-- 飞机方向
	self.planeCountDown = 0		-- 飞机起飞倒计时
	self.planeTookOff = false   -- [临时]飞机是否已起飞

	self.weeklyRace2020ChestData = WeeklyRace2020Logic:initWeeklyRace2020ChestData()

    self.catteryDirection = 0   --猫窝方向
    self.catterySize = 0        --猫窝大小
    self.catteryState = 0       --猫窝状态
    self.canAttackCattery = true --猫窝受到攻击锁
    self.meowLevel = 0          --喵喵等级

    self.birdEnergy = 0		--小鸟的能量
    self.birdType = 0
	self.tempRowShiftByAngryBird = 0
	self.tempColShiftByAngryBird = 0
	self.readyToShot = true

	self.treasureFishType = 0

	self.bunnyLaneLevel = 0				-- 狡兔草坪的等级
	self.hasSlyBunnyOnLane = false 		-- 狡兔草坪上是否有兔

	self.walkChickReachEndActivated = false
	self.tempRowShiftByWalkChick = 0
	self.tempColShiftByWalkChick = 0
	self.isHitShellGift = false  -- 贝壳礼包 已激活
	self.shellGiftInfo = nil   -- 贝壳礼包内容信息

	self.fireworkLevel = nil
	self.fireworkFinished = false
	self.fireworkPathData = nil

	self.batteryLevel = 0
	self.batteryCharged = false

	self.NDBunnyType = 0   --兔子类型
	self.NDBunnyHp = 0     --兔子血量
	self.NDBunnyMove = 0   --只有急速兔子为2 其他兔子都是1
	self.NDBunnyDir = 0    --兔子移动方向
	self.NDSnowBunnyReadyLevel = 0 --雪兔子扔的雪块等级
	self.NDBunnySnowLevel = 0 --兔子雪块等级
	self.NDBunnyFullHp = 0 --兔子满血数，用于计算血量进度条

	self.puffedRiceHp = 0  --爆米花血量
	self.puffedRiceMoveableSteps = 0 --爆米花可移动的步数
	self.puffedRiceBoom = false --爆米花是否在爆炸中
	self.puffedRiceJumped = nil --爆米花移动过的格子
end
...
function GameItemData:create()
	...
	-- watchObj(v, {'ItemType', 'canevine_data'})
	return v
end
...
function GameItemData:resetDatas(mode)
	...
	self.colorFilterBLock = false
	self.shieldToColorFilter = false
	...
	self.lockBoxRopeUp = false --是否显示配对锁的绳子
	self.lockBoxActive = false --配对锁是否为激活状态
	self.inChameleonTransform = false
    ...
    self.gyroDirection = 0 --陀螺方向
    self.gyroLevel = 0
    self.gyroCreaterPos = nil
    self.gyroCreaterNextDirection = nil
    self.gyroCreaterData = nil
    self.gyroCreaterCurDirector = 0
    self.gyroGeneratedByStep = 0
	self.gyroGeneratedByBoardMin = 0
	self.gyroCreaterChildList = {}
	self.gyroPathBlock = 0
	...
	self.windTunnelSwitchLevel = 0
	self.windTunnelSwitchTypeIsOff = false 

	self.activityCollectionItem_ItemId = -1
	self.activityCollectionItem_lock = false

	self.tempRowShiftByHero = 0
	self.tempColShiftByHero = 0
	self.cuckooBirdReachedEnd = false

	self.valentineIngredientID = 0
	self.SakuraIngredientID = 0

	self.planeDirection = 0
	self.planeCountDown = 0
	self.planeTookOff = false

	self.weeklyRace2020ChestData = WeeklyRace2020Logic:initWeeklyRace2020ChestData()

    self.catteryDirection = 0   
    self.catterySize = 0        
    self.catteryState = 0       
    self.canAttackCattery = true
    self.meowLevel = 0       
	...
	self.bunnyLaneLevel = 0
	self.hasSlyBunnyOnLane = false

	self.tempRowShiftByWalkChick = 0
	self.tempColShiftByWalkChick = 0
	self.isHitShellGift = false
	self.shellGiftInfo = false
	...
	self.NDBunnyMove = 0
	self.NDBunnyDir = 0
	self.NDSnowBunnyReadyLevel = 0
	self.NDBunnySnowLevel = 0
	self.NDBunnyFullHp = 0

	self.puffedRiceHp = 0
	self.puffedRiceMoveableSteps = 0
	self.puffedRiceBoom = false
	self.puffedRiceJumped = nil
end
...
function GameItemData.copyDatasFrom(toData, fromData , mode )
	...
	toData.shieldToColorFilter = fromData.shieldToColorFilter
	...
	toData.inChameleonTransform = fromData.inChameleonTransform
    ...
    toData.blocker83Dirs = fromData.blocker83Dirs
    ...
    toData.gyroDirection = fromData.gyroDirection
    toData.gyroLevel = fromData.gyroLevel
    toData.gyroCreaterPos = fromData.gyroCreaterPos
    toData.gyroCreaterNextDirection = fromData.gyroCreaterNextDirection
    toData.gyroCreaterData = fromData.gyroCreaterData
    toData.gyroCreaterCurDirector = fromData.gyroCreaterCurDirector
    toData.gyroGeneratedByStep = fromData.gyroGeneratedByStep
    toData.gyroGeneratedByBoardMin = fromData.gyroGeneratedByBoardMin
    toData.gyroCreaterChildList = fromData.gyroCreaterChildList
    toData.gyroPathBlock = fromData.gyroPathBlock

    toData.waterLevel = fromData.waterLevel
    toData.canevine_data = table.clone(fromData.canevine_data, true)
    toData.can_attack_canevine = fromData.can_attack_canevine

    toData.waterBucketGroupId = fromData.waterBucketGroupId
    toData.waterBucketDestroying = fromData.waterBucketDestroying
    toData.waterBucketCharge = fromData.waterBucketCharge
    toData.waterBucketMaximumCharge = fromData.waterBucketMaximumCharge
    toData.waterBucketChargingCache = fromData.waterBucketChargingCache

    toData.windTunnelSwitchLevel = fromData.windTunnelSwitchLevel
	toData.windTunnelSwitchTypeIsOff = fromData.windTunnelSwitchTypeIsOff 

	toData.activityCollectionItem_ItemId = fromData.activityCollectionItem_ItemId 
	toData.activityCollectionItem_lock = fromData.activityCollectionItem_lock 

	toData.valentineIngredientID = fromData.valentineIngredientID 
	toData.SakuraIngredientID = fromData.SakuraIngredientID

	toData.planeDirection = fromData.planeDirection 
	toData.planeCountDown = fromData.planeCountDown 

	if fromData.weeklyRace2020ChestData then
	    toData.weeklyRace2020ChestData = table.clone(fromData.weeklyRace2020ChestData, true)
	end

	toData.catteryDirection = fromData.catteryDirection
	toData.catterySize = fromData.catterySize  
    toData.catteryState = fromData.catteryState 
    toData.meowLevel = fromData.meowLevel 
    toData.canAttackCattery = fromData.canAttackCattery 

    toData.birdEnergy = fromData.birdEnergy
    toData.birdType = fromData.birdType
    toData.readyToShot = fromData.readyToShot

    toData.treasureFishType = fromData.treasureFishType

    toData.bunnyLaneLevel = fromData.bunnyLaneLevel
	toData.hasSlyBunnyOnLane = fromData.hasSlyBunnyOnLane

	toData.cuckooBirdReachedEnd = fromData.cuckooBirdReachedEnd
	toData.isHitShellGift = fromData.isHitShellGift
	toData.shellGiftInfo = fromData.shellGiftInfo

	toData.fireworkLevel = fromData.fireworkLevel
	toData.fireworkFinished = fromData.fireworkFinished
	if fromData.fireworkPathData then
		toData.fireworkPathData = table.clone(fromData.fireworkPathData,true)
	end

	toData.batteryLevel = fromData.batteryLevel
	toData.batteryCharged = fromData.batteryCharged

	toData.NDBunnyType = fromData.NDBunnyType
	toData.NDBunnyHp = fromData.NDBunnyHp
	toData.NDBunnyMove = fromData.NDBunnyMove
	toData.NDBunnyDir = fromData.NDBunnyDir
	toData.NDSnowBunnyReadyLevel = fromData.NDSnowBunnyReadyLevel
	toData.NDBunnySnowLevel = fromData.NDBunnySnowLevel
	toData.NDBunnyFullHp = fromData.NDBunnyFullHp

	toData.puffedRiceHp = fromData.puffedRiceHp
	toData.puffedRiceMoveableSteps = fromData.puffedRiceMoveableSteps
	toData.puffedRiceBoom = fromData.puffedRiceBoom
	toData.puffedRiceJumped = fromData.puffedRiceJumped
...
function GameItemData:initByConfig( tileDef )
	...
	elseif tileDef:hasProperty(TileConst.kDynamiteCrate) then self.ItemType = GameItemType.kDynamiteCrate self.isEmpty = false self.isBlock = false self.missileLevel = 1
	elseif tileDef:hasProperty(TileConst.kChameleon) then self.ItemType = GameItemType.kChameleon self.isEmpty = false 
	elseif tileDef:hasProperty(TileConst.kShellGift) then self.ItemType = GameItemType.kShellGift self.isEmpty = false  
    ...
    elseif tileDef:hasProperty(TileConst.kWanSheng) then 
        ...
        self.wanShengConfig = nil
        -- self.isBlock = true
    elseif tileDef:hasProperty(TileConst.kGyro) then
		self.ItemType = GameItemType.kGyro
		self.gyroDirection = tonumber(tileDef:getAttrOfProperty(TileConst.kGyro))
		self.gyroLevel = 0
		self.isBlock = true 
		self.isEmpty = false
	elseif tileDef:hasProperty(TileConst.kGyroCreater) then
		self.ItemType = GameItemType.kGyroCreater
		self.isBlock = true 
		self.isEmpty = false
	elseif tileDef:hasProperty(TileConst.kWater) then
		self.ItemType = GameItemType.kWater
		self.isEmpty = false
		self.waterLevel = math.max(1, tonumber(tileDef:getAttrOfProperty(TileConst.kWater)))
	elseif tileDef:hasProperty(TileConst.kCanevine) then
		self.ItemType = GameItemType.kCanevine
		self.isEmpty = false

    	local attr = tileDef:getAttrOfProperty(TileConst.kCanevine)
    	local level, direction, len = unpack(string.split(attr, '-'))
	...
	elseif tileDef:hasProperty(TileConst.kWindTunnelSwitch) then
		self.ItemType = GameItemType.kWindTunnelSwitch
		self.isBlock = true 
		self.isEmpty = false
		self.windTunnelSwitchLevel = 2
		local windTunnelSwitchType = tonumber(tileDef:getAttrOfProperty(TileConst.kWindTunnelSwitch))
		if windTunnelSwitchType == 1 then
			self.windTunnelSwitchTypeIsOff = false
		else
			self.windTunnelSwitchTypeIsOff = true
		end
	elseif tileDef:hasProperty(TileConst.kTravelHero) then
		self.ItemType = GameItemType.kTravelHero
    	self.isBlock = true
		self.isEmpty = false
	elseif tileDef:hasProperty(TileConst.kTravelEnergyBag) then 
		self.ItemType = GameItemType.kTravelEnergyBag 
		self.isEmpty = false  
	elseif tileDef:hasProperty(TileConst.kTravelFinishBuilding) then
		local buildingType = tonumber(tileDef:getAttrOfProperty(TileConst.kTravelFinishBuilding))
		if buildingType == 1 then
    		self.ItemType = GameItemType.kTravelFinishBuilding
		else
    		self.ItemType = GameItemType.kTravelFinishBuildingEmpty
		end
    	self.isBlock = true
    	self.isEmpty = false
    elseif tileDef:hasProperty(TileConst.kCuckooBird) then
		self.ItemType = GameItemType.kCuckooBird
    	self.isBlock = true
		self.isEmpty = false
		self.cuckooBirdReachedEnd = false
	elseif tileDef:hasProperty(TileConst.kCuckooWindupKey) then 
		self.ItemType = GameItemType.kCuckooWindupKey 
		self.isEmpty = false  
	elseif tileDef:hasProperty(TileConst.kCuckooClock) then
		self.ItemType = GameItemType.kCuckooClock
    	self.isBlock = true
    	self.isEmpty = false
    elseif tileDef:hasProperty(TileConst.kPlane) then 
    	self.ItemType = GameItemType.kPlane 
    	self.isEmpty = false 
    	self.isBlock = false
    	local planeType = tonumber(tileDef:getAttrOfProperty(TileConst.kPlane))
    	local direction, countDownLevel = PlaneLogic:getDirectionAndCountDownByConfig(planeType)
		-- printx(11, "Try init plane!", planeType, direction, countDownLevel)
    	self.planeDirection = direction
    	self.planeCountDown = countDownLevel
    	self.planeTookOff = false
	elseif tileDef:hasProperty(TileConst.kWeeklyRace2020Chest) then
		self.ItemType = GameItemType.kWeeklyRace2020Chest
		self.isBlock = true
		self.isEmpty = false 
		local chestType = tonumber(tileDef:getAttrOfProperty(TileConst.kWeeklyRace2020Chest))
		WeeklyRace2020Logic:setWeeklyRace2020FinalChestData(self, chestType)
	elseif tileDef:hasProperty(TileConst.kWeeklyRace2020JewelChest) then
		self.ItemType = GameItemType.kWeeklyRace2020JewelChest
		self.isBlock = true
		self.isEmpty = false 
		local chestType = tonumber(tileDef:getAttrOfProperty(TileConst.kWeeklyRace2020JewelChest))
		local addInfo = tileDef:getAddInfoOfProperty(TileConst.kWeeklyRace2020JewelChest)
		WeeklyRace2020Logic:setWeeklyRace2020JewelChestData(self, chestType, addInfo)
	elseif tileDef:hasProperty(TileConst.kCattery) then
		self.ItemType = GameItemType.kCattery
		self.isBlock = true
		self.isEmpty = false 
		local catteryType = tonumber(tileDef:getAttrOfProperty(TileConst.kCattery))
		local direction, size = CatteryLogic:getDirectionAndSizeByConfig(catteryType)
		self.catteryDirection = direction
		self.catterySize = size
		self.catteryState = CatteryState.kTwoLevel
		self.canAttackCattery = true
	elseif tileDef:hasProperty(TileConst.kCatteryEmpty) then 
		self.ItemType = GameItemType.kCatteryEmpty 
		self.isEmpty = false 
		self.isBlock = true
		local catteryType = tonumber(tileDef:getAttrOfProperty(TileConst.kCatteryEmpty))
		self.catterySize = catteryType
	elseif tileDef:hasProperty(TileConst.kMeow) then 
		self.ItemType = GameItemType.kMeow 
		self.isEmpty = false 
		self.isBlock = true
		self.meowLevel = 2
	elseif tileDef:hasProperty(TileConst.kAngryBird) then
		self.ItemType = GameItemType.kAngryBird 
		self.isEmpty = false
		self.isBlock = true
		self.birdEnergy = 0
		self.birdType = tonumber(tileDef:getAttrOfProperty(TileConst.kAngryBird))
	elseif tileDef:hasProperty(TileConst.kSling) then
    	self.ItemType = GameItemType.kSling
    	self.isBlock = true
    	self.isEmpty = false
    	self.readyToShot = true
	elseif tileDef:hasProperty(TileConst.kTreasureFish) then
		self.ItemType = GameItemType.kTreasureFish
		self.isEmpty = false
		self.isBlock = true
		self.treasureFishType = tonumber(tileDef:getAttrOfProperty(TileConst.kTreasureFish))
	elseif tileDef:hasProperty(TileConst.kSlyBunnyLane) then
		self.ItemType = GameItemType.kSlyBunnyLane
		self.isBlock = true 
		self.isEmpty = false
		local tileAttr = tonumber(tileDef:getAttrOfProperty(TileConst.kSlyBunnyLane))
		self.bunnyLaneLevel, self.hasSlyBunnyOnLane = SlyBunnyLogic:getBunnyLaneLevelAndBunnyByConfig(tileAttr)
	elseif tileDef:hasProperty(TileConst.kWalkChick) then
		self.ItemType = GameItemType.kWalkChick
		self.isEmpty = false
		self.isBlock = true
	elseif tileDef:hasProperty(TileConst.kWalkChickEnd) then
		self.ItemType = GameItemType.kWalkChickEnd
		self.isEmpty = false
		self.isBlock = true
	elseif tileDef:hasProperty(TileConst.kChannelTarget) then
		self.ItemType = GameItemType.kChannelTarget
		self.isEmpty = false
		self.isBlock = true
	elseif tileDef:hasProperty(TileConst.kBattery) then
		self.ItemType = GameItemType.kBattery
		self.isEmpty = false
		self.isBlock = false
		self.batteryCharged = false
		self.batteryLevel = tonumber(tileDef:getAttrOfProperty(TileConst.kBattery))
	elseif tileDef:hasProperty(TileConst.kNationalDayBunny) then
		self.ItemType = GameItemType.kNationalDayBunny
		self.isBlock = true
		self.isEmpty = false 
		self.NDBunnyHp = 1
		local config = {}
		...
		if mainLogic and mainLogic.NDbunnyGridConfig then
			local globalDir
			local globalSnowLevel
			if mainLogic.NDbunnyGlobalConfig then
				globalDir = mainLogic.NDbunnyGlobalConfig.dir
				globalSnowLevel = mainLogic.NDbunnyGlobalConfig.carrot
			end
			local girdData = mainLogic.NDbunnyGridConfig[self.y.."_"..self.x]
			if girdData then
				config = girdData.c or {}
				self.NDBunnyHp = girdData.h or 1
				dir = girdData.dir or globalDir
				if girdData.carrot and girdData.carrot > 0 then
					snowLevel = girdData.carrot
				else
					snowLevel = globalSnowLevel
				end
			end
		end
		self.NDBunnyFullHp = self.NDBunnyHp
		self.NDBunnyDir = NationalDayBunnyLogic:getDirByConfigWithRandom(dir,mainLogic)
--		self._encrypt.ItemColorType = AnimalTypeConfig.convertIndexToColorType(NationalDayBunnyLogic:getColorByConfig(config,mainLogic))
		local iconType = tonumber(tileDef:getAttrOfProperty(TileConst.kNationalDayBunny))
		self.NDBunnyType = NationalDayBunnyLogic:getBunnyType(iconType)
		self.NDBunnyMove = NationalDayBunnyLogic:getBunnyMove(iconType)
		self.NDSnowBunnyReadyLevel = snowLevel or 1
		--self.isLockColorOnInit = true
	elseif tileDef:hasProperty(TileConst.kNDBunnySnow) then 
		self.ItemType = GameItemType.kNDBunnySnow 
		self.isEmpty = false 
		self.isBlock = true

		local mainLogic = GameBoardLogic:getCurrentLogic()
		local level
		if mainLogic and mainLogic.NDbunnySnowGridConfig then
			local girdData = mainLogic.NDbunnySnowGridConfig[self.y.."_"..self.x]
			if girdData then
				level = girdData.carrot
			end
		end

		self.NDBunnySnowLevel = level or 1
	elseif tileDef:hasProperty(TileConst.kpuffedRice) then 
    	self.ItemType = GameItemType.kpuffedRice 
    	self.isEmpty = false 
    	self.isBlock = false
    	self.puffedRiceHp = 4
	end
	...
	if tileDef:hasProperty(TileConst.kBlockerCover) then
    	self.blockerCoverLevel = tonumber(tileDef:getAttrOfProperty(TileConst.kBlockerCover))
    	self.isBlock = true 
    	self.isEmpty = false
    end
	...
	if tileDef:hasProperty(TileConst.kWaterBucket) then
		local addInfo = tileDef:getAddInfoOfProperty(TileConst.kWaterBucket) or ''
		local addInfos = string.split2(addInfo, '|') or {}
		self.waterBucketGroupId = tonumber(addInfos[1] or 0) or 0
		self.waterBucketDestroying = false
		self.waterBucketCharge = 0
		self.waterBucketMaximumCharge = 9999999
		self.waterBucketChargingCache = 0
...
function GameItemData:initBlocker83AddInfo(addInfo)
	-- printx(15,"initBlocker83AddInfo",table.tostring(addInfo))
	if addInfo then
		-- addInfo = " "..addInfo
		addInfo = string.split2(addInfo, '|')
		if addInfo[1] then
			local colorInfos = string.split(addInfo[1], ':')
			if #colorInfos > 0 then
				...
				-- printx(15,"self.blocker83Colors",table.tostring(self.blocker83Colors))
			end 
		end
		...
		if addInfo[2] then
			...
			if #dirInfos > 0 then
				self.blocker83Dirs = {}
				for i,v in ipairs(dirInfos) do
					table.insert(self.blocker83Dirs, v)
				end
				-- printx(15,"self.blocker83Dirs",table.tostring(self.blocker83Dirs))
			end 
		end
	end
end
...
function GameItemData:changeToWater( waterLevel )
...
--转换为飞机
function GameItemData:changeToPlane(planeType)
    self:cleanAnimalLikeData() 
    self.ItemType = GameItemType.kPlane 
    self.isEmpty = false 
    self.isBlock = false
    if not planeType then planeType = 1 end
	local direction, countDownLevel = PlaneLogic:getDirectionAndCountDownByConfig(planeType)
	-- printx(11, "Try change to plane!", planeType, direction, countDownLevel)
	self.planeDirection = direction
	self.planeCountDown = countDownLevel
	self.planeTookOff = false
end
...
--转换为布谷鸟发条钥匙
function GameItemData:changeToCuckooWindupKey(targetColourIndex)
    self:cleanAnimalLikeData() 
    self.ItemType = GameItemType.kCuckooWindupKey 
    self.isEmpty = false

    if targetColourIndex and type(targetColourIndex) == "number" and targetColourIndex > 0 then
        local realColour = AnimalTypeConfig.convertIndexToColorType(targetColourIndex)
        self._encrypt.ItemColorType = realColour
    else
        local mainLogic = GameBoardLogic:getCurrentLogic()
        if mainLogic then
            self._encrypt.ItemColorType = mainLogic:randomColor()
...
function GameItemData:changeToBattery( )
	self:cleanAnimalLikeData() 
    self.ItemType = GameItemType.kBattery 
    self.isEmpty = false
    self.isBlock = false
...
--转换为爆米花
function GameItemData:changeToPuffedRice()
    self:cleanAnimalLikeData() 
    self.ItemType = GameItemType.kpuffedRice 
    self.isEmpty = false 
    self.isBlock = false
...
--转换为随机爆炸动物
function GameItemData:changeToWrapAnimal()
    ...
    if mainLogic then
        ...
        self.ItemSpecialType = AnimalTypeConfig.kWrap
    end
end

--转换为陀螺
function GameItemData:changeToGyro( arrow )
    self:cleanAnimalLikeData() 
    self.ItemType = GameItemType.kGyro
   	self.gyroDirection = arrow or 1
	self.gyroLevel = 0
	self.isBlock = true 
    self.isEmpty = false
end
...
--转换为感恩节收集物
function GameItemData:changeToActivityCollectionItem( activityCollectionItem_ItemId )
    self:cleanAnimalLikeData() 
    self.ItemType = GameItemType.kActivityCollectionItem
	self.activityCollectionItem_ItemId = activityCollectionItem_ItemId
	self.activityCollectionItem_lock = false
	-- self.isBlock = true
	self.isEmpty = false
end

function GameItemData:changeToFirework( fireworkLevel, fireworkPathData )
	if not fireworkLevel then
		fireworkLevel = 4
	end
	self.ItemType = GameItemType.kFirework
	self.isBlock = true
	self.isEmpty = false
	self.fireworkLevel = fireworkLevel
	self.fireworkFinished = false
	self.fireworkPathData = table.clone(fireworkPathData)
...
function GameItemData:canBeCoverByMatch()
	...
	if self.isHitShellGift then return false end
	if self:isReadyToCastingMagicLamp() then return false end 	--要发招的大眼仔不参与匹配
...
function GameItemData:canEffectLightUp()
	if (not self.isBlock or 
		...
		self.ItemType == GameItemType.kPuffer or
		self:hasMeowLock() or
		self:hasMeowHitLock())
		...
		and self.ItemType ~= GameItemType.kDynamiteCrate
        ...
        and self.ItemType ~= GameItemType.kWater
        and self.ItemType ~= GameItemType.kPlane
        and not CatteryLogic:isCatteryOrMeow(self.ItemType)
        and self.ItemType ~= GameItemType.kBattery
        and self.ItemType ~= GameItemType.kpuffedRice
		then
		return true
	end
	return false
...
function GameItemData:canEffectLotus(lotusLevel)
	...
	if WaterBucketLogic:hasBucket(self) then return false end
...
function GameItemData:canBeEliminateByMatch()
	...
	if self.ItemType == GameItemType.kShellGift then return false end
...
function GameItemData:canBeEliminateBySpecial()
	if (self.ItemType == GameItemType.kAnimal 		-----动物
		...
		or self.ItemType == GameItemType.kTravelEnergyBag
		or self.ItemType == GameItemType.kCuckooWindupKey
		)
...
function GameItemData:canBeCoverByBirdAnimal()
	...
	if (self.ItemType == GameItemType.kAnimal and self.ItemSpecialType ~= AnimalTypeConfig.kColor)
		...
		or self.ItemType == GameItemType.kShellGift
...
function GameItemData:isItemCanBeCoverByBirdBird()
	...
	if WaterBucketLogic:canAttackBucket(self) then
		return true
	end
	...
	if self.ItemType == GameItemType.kAnimal
		...
		or self.ItemType == GameItemType.kDynamiteCrate
		...
		or self.ItemType == GameItemType.kScoreBuffBottle
		or self.ItemType == GameItemType.kFirecracker
		or self.ItemType == GameItemType.kWater
		or self.ItemType == GameItemType.kTravelEnergyBag
		or self.ItemType == GameItemType.kCanevine
		or self.ItemType == GameItemType.kCuckooWindupKey
		then
		return true
	end
...
function GameItemData:isBlockerCanBeCoverByBirdBrid()
	...
	if self.ItemType == GameItemType.kSnow 
        ...
        or self.ItemType == GameItemType.kGyro
        or self.ItemType == GameItemType.kWindTunnelSwitch
		or self.ItemType == GameItemType.kPlane
		or WeeklyRace2020Logic:isWeeklyRace2020Chest(self.ItemType)
		or CatteryLogic:isCatteryOrMeow(self.ItemType)
		or self.ItemType == GameItemType.kAngryBird
		or self.ItemType == GameItemType.kSlyBunnyLane
		or self.ItemType == GameItemType.kFirework
		or self.ItemType == GameItemType.kBattery
		or self.ItemType == GameItemType.kNationalDayBunny
		or self.ItemType == GameItemType.kNDBunnySnow
		or self.ItemType == GameItemType.kpuffedRice
		then
		return true
	end
...
function GameItemData:canBeEffectByHammer(mainLogic)
	...
	if WaterBucketLogic:canChargeBucket(mainLogic, self) then
		return true
	end

	if (not self:hasBlocker206()) and CanevineLogic:is_occupy(self) then
		return true
	end
	...
	if self.ItemType == GameItemType.kNone 
		...
		or WaterBucketLogic:hasBucketLocked(self) and (not WaterBucketLogic:canChargeBucket(mainLogic, self))
		...
		or self.ItemType == GameItemType.kGyroCreater
		or self.ItemType == GameItemType.kTravelHero
		or self.ItemType == GameItemType.kTravelFinishBuilding
		or self.ItemType == GameItemType.kTravelEventBox
		or self.ItemType == GameItemType.kSling
		or self.ItemType == GameItemType.kTreasureFish
		or self.ItemType == GameItemType.kWalkChick
		or self.ItemType == GameItemType.kWalkChickEnd
		or self.ItemType == GameItemType.kChannelTarget
		or self.ItemType == GameItemType.kCuckooBird
		or self.ItemType == GameItemType.kCuckooClock
		then
		return false
	end
	return true
...
function GameItemData:canBeEffectByLineEffectProp()
	...
	if self:hasLockedWaterBucket() then return false end

	if self:hasActiveSuperCuteBall() 
		or self.blockerCoverLevel > 0 
		or self.colorFilterBLock 
		or self:isFreeGhost() 
		or self:hasActiveWaterBucket() 
		then
		return true
	end
	...
	if self:isVisibleAndFree() then
		if self.ItemType == GameItemType.kAnimal 
            ...
            or self.ItemType == GameItemType.kWindTunnelSwitch
            or self.ItemType == GameItemType.kTravelEnergyBag
            or self.ItemType == GameItemType.kActivityCollectionItem
            or self.ItemType == GameItemType.kCanevine
            or self.ItemType == GameItemType.kPlane
           	or self.ItemType == GameItemType.kAngryBird
            or WeeklyRace2020Logic:isWeeklyRace2020Chest(self.ItemType)
...
function GameItemData:getAnimalLikeDataFrom(data)
	...
	self.shieldToColorFilter = data.shieldToColorFilter
	...
	self.inChameleonTransform = data.inChameleonTransform
    ...
    self.blocker83Dirs = data.blocker83Dirs
    ...
    self.gyroDirection = data.gyroDirection
    self.gyroLevel = data.gyroLevel
    self.gyroCreaterPos = data.gyroCreaterPos
    self.gyroCreaterNextDirection = data.gyroCreaterNextDirection
    self.gyroCreaterData = data.gyroCreaterData
    self.gyroCreaterCurDirector = data.gyroCreaterCurDirector
    self.gyroGeneratedByStep = data.gyroGeneratedByStep
	self.gyroGeneratedByBoardMin = data.gyroGeneratedByBoardMin
	self.gyroCreaterChildList = data.gyroCreaterChildList
	self.gyroPathBlock = data.gyroPathBlock

	self.waterLevel = data.waterLevel
	self.canevine_data = table.clone(data.canevine_data, true)
	self.can_attack_canevine = data.can_attack_canevine


	self.waterBucketGroupId = data.waterBucketGroupId
	self.waterBucketDestroying = data.waterBucketDestroying
	self.waterBucketCharge = data.waterBucketCharge
	self.waterBucketMaximumCharge = data.waterBucketMaximumCharge
	self.waterBucketChargingCache = data.waterBucketChargingCache

	self.windTunnelSwitchLevel = data.windTunnelSwitchLevel
	self.windTunnelSwitchTypeIsOff = data.windTunnelSwitchTypeIsOff 

	self.activityCollectionItem_ItemId = data.activityCollectionItem_ItemId 
	self.activityCollectionItem_lock = data.activityCollectionItem_lock

	self.valentineIngredientID = data.valentineIngredientID
	self.SakuraIngredientID = data.SakuraIngredientID

	self.planeDirection = data.planeDirection
	self.planeCountDown = data.planeCountDown

	if data.weeklyRace2020ChestData then
	    self.weeklyRace2020ChestData = table.clone(data.weeklyRace2020ChestData, true)
	end
    self.catteryDirection = data.catteryDirection
    self.catterySize = data.catterySize
	self.catteryState = data.catteryState
	self.meowLevel = data.meowLevel
	self.canAttackCattery = data.canAttackCattery

	self.birdEnergy = data.birdEnergy
	self.birdType = data.birdType
	self.readyToShot = data.readyToShot

	self.treasureFishType = data.treasureFishType

	self.bunnyLaneLevel = data.bunnyLaneLevel
	self.hasSlyBunnyOnLane = data.hasSlyBunnyOnLane 

	self.cuckooBirdReachedEnd = data.cuckooBirdReachedEnd
	self.isHitShellGift = data.isHitShellGift
	self.shellGiftInfo = data.shellGiftInfo

	self.fireworkLevel = data.fireworkLevel
	self.fireworkFinished = data.fireworkFinished
	if data.fireworkPathData then
		self.fireworkPathData = table.clone(data.fireworkPathData, true)
	end

	self.batteryLevel = data.batteryLevel
	self.batteryCharged = data.batteryCharged

	self.NDBunnyType = data.NDBunnyType
	self.NDBunnyHp = data.NDBunnyHp
	self.NDBunnyMove = data.NDBunnyMove
	self.NDBunnyDir = data.NDBunnyDir
	self.NDSnowBunnyReadyLevel = data.NDSnowBunnyReadyLevel
	self.NDBunnySnowLevel = data.NDBunnySnowLevel
	self.NDBunnyFullHp = data.NDBunnyFullHp

	self.puffedRiceHp = data.puffedRiceHp
	self.puffedRiceMoveableSteps = data.puffedRiceMoveableSteps
	self.puffedRiceBoom = data.puffedRiceBoom
	self.puffedRiceJumped = data.puffedRiceJumped
end
 
function GameItemData:cleanAnimalLikeData()
	-- if _G.__DEBUG_CLEAN then
		-- print("GameItemData:cleanAnimalLikeData()",self.x,self.y,debug.traceback())
	-- end
	self.isNeedUpdate = true
	self.isEmpty = true
	self.ItemType = GameItemType.kNone
...
function GameItemData:checkBlock()--这个方法并不返回当前是不是isBlock，而是检测isBlock的值需不需要改变	
	...
	if self.snowLevel > 0 
		...
		or self.ItemType == GameItemType.kBlocker199
		or self:hasBlocker206()
		or WaterBucketLogic:hasBucket(self)
		or (self.ItemType == GameItemType.kBuffBoom and self.flag)
		or self.inChameleonTransform
		or self.ItemType == GameItemType.kPacman
		or self.ItemType == GameItemType.kPacmansDen
		...
		or self.ItemType == GameItemType.kGyro
		or self.ItemType == GameItemType.kGyroCreater
		or self.gyroPathBlock > 0
		or self.ItemType == GameItemType.kWindTunnelSwitch
		-- or self.ItemType == GameItemType.kActivityCollectionItem
		or self.ItemType == GameItemType.kTravelHero
		or self.ItemType == GameItemType.kTravelFinishBuilding
		or self.ItemType == GameItemType.kTravelFinishBuildingEmpty
		or self:hasFallingLockByTravelHero()
		or self:hasFallingLockByCuckoo()
		or self:hasFallingLockByNDBunny()
		or self:hasFallingLockByPuffedRice()
		or self.ItemType == GameItemType.kTravelEventBox
		or self.ItemType == GameItemType.kCanevine
		or (CanevineLogic:is_occupy(self) and self.isUsed )
		or self.planeTookOff
		or WeeklyRace2020Logic:isWeeklyRace2020Chest(self.ItemType)
		or CatteryLogic:isCatteryOrMeow(self.ItemType)
        or self:hasMeowLock()
        or self:hasMeowHitLock()
        or self.ItemType == GameItemType.kAngryBird
        or self.ItemType == GameItemType.kSling
        or self.ItemType == GameItemType.kTreasureFish
        or self.ItemType == GameItemType.kSlyBunnyLane
		...
		or self.ItemType == GameItemType.kCuckooClock
		or self.ItemType == GameItemType.kFirework
		or self.batteryCharged
		or self.ItemType == GameItemType.kNationalDayBunny
		or self.ItemType == GameItemType.kNDBunnySnow
		or self.puffedRiceBoom
		then 
		self.isBlock = true 
		self.isUsed = true 
	end
...
GameItemDataTopCoverName = {
	...
	k_Ghost = "typeGhost", 
	k_WaterBucket = "typeWaterBucket"
}
function GameItemData:isTopCover(typeName)
	local topCoverType = self:getTopCoverType()
	if topCoverType and (topCoverType == typeName) then
		return true
	end
	return false
end

function GameItemData:getTopCoverType()
	-- 层级由上到下，上层可覆盖下层
	-- 1层：挂锁
	if self:hasBlocker206() then
		return GameItemDataTopCoverName.k_Padlock
	else
		-- 2层：单面翻板，白毛球
		if self.isReverseSide or self:hasActiveSuperCuteBall() then 	
			if self.isReverseSide then return GameItemDataTopCoverName.k_TileBlocker end
			if self:hasActiveSuperCuteBall() then return GameItemDataTopCoverName.k_SuperFurball end
		else
			-- 3层：过滤器，小叶堆，魔法罐  （其实白毛球也与此层互斥）
			if self.colorFilterBLock or self.blockerCoverLevel > 0 or (WaterBucketLogic:hasBucket(self) and (not self.waterBucketDestroying)) then
				if self.colorFilterBLock then return GameItemDataTopCoverName.k_ColourFilter end
				if self.blockerCoverLevel > 0 then return GameItemDataTopCoverName.k_Leafpile end
				if (WaterBucketLogic:hasBucket(self) and (not self.waterBucketDestroying)) then return GameItemDataTopCoverName.k_WaterBucket end
			else
				-- 4层：幽灵
				if self:seizedByGhost() then
					return GameItemDataTopCoverName.k_Ghost
				end
			end
		end
	end
...
function GameItemData:hasCoveredByBlock()
	if self:hasBlocker206()
		or self.isReverseSide 
		or self:hasActiveSuperCuteBall()
		or self.colorFilterBLock
		or self.blockerCoverLevel > 0
...
function GameItemData:isAnimalLike()
	return self.ItemType == GameItemType.kAnimal
			or self.ItemType == GameItemType.kGift
			or self.ItemType == GameItemType.kNewGift
			or self.ItemType == GameItemType.kCrystal
			or self.ItemType == GameItemType.kBalloon
...
function GameItemData:canInfectByWanSheng( ... )--是否能被万生破裂后飞出的障碍替换
	...
	if self.ItemType == GameItemType.kAnimal --是小动物
		or self.ItemType == GameItemType.kShellGift
        -- and self.lotusLevel == 0 --荷塘  -- GameItemData中的lotusLevel会随着刷新乱窜，所以用boardData来判断
        then
		return true
	end
	return false
...
function GameItemData:isSpecial()
	return table.includes(AnimalTypeConfig.specialTypeList,self.ItemSpecialType) and not self:hasBlocker206() and not self:hasSquidLock() and (not WaterBucketLogic:hasBucket(self))
end
...
function GameItemData:isBigMonsterEffectPrior2( ... )
	...
	if self.venomLevel > 0 
		...
		or self.ItemType == GameItemType.kMoleBossSeed
		or self.ItemType == GameItemType.kGyro
		or WaterBucketLogic:canAttackWater(self)
		or self.ItemType == GameItemType.kWindTunnelSwitch
		or self.ItemType == GameItemType.kPlane
		or CatteryLogic:isCatteryOrMeow(self.ItemType)
		or (self.ItemType == GameItemType.kFirework and self.fireworkLevel and self.fireworkLevel > 0 )
		or self.ItemType == GameItemType.kBattery
		or self.ItemType == GameItemType.kpuffedRice
		then
		return true
...
function GameItemData:isBigMonsterEffectPrior3( boradHasJam, hasJam )
	-- body
	...
	--未涂果酱的类小动物（有果酱的雪怪）
	if not boradHasJam and hasJam and self:isAnimalLike() then
		return true
	end
...
function GameItemData:isReadyToCastingMagicLamp()
	if self.ItemType == GameItemType.kMagicLamp and self.lampLevel >= 5 then
		return true
	end
	return false
end
...
-----------------------------------------------------------
-- 旅行模式，与主人公交互的对象格子会有临时的掉落屏蔽
function GameItemData:addFallingLockByTravelHero()
	self.fallingLockByTravelHero = true
end

function GameItemData:removeFallingLockByTravelHero()
	self.fallingLockByTravelHero = false
end

function GameItemData:hasFallingLockByTravelHero()
	if self.fallingLockByTravelHero then
		return true
	end
	return false
end
...
-----------------------------------------------------------
-- 这么多障碍都在复用 fallingLockByTravelHero 好怕怕，只能另起炉灶了
function GameItemData:addFallingLockByCuckoo()
	-- printx(11, "+ + + + + add falling lock!", self.y, self.x)
	self.fallingLockByCuckoo = true
end

function GameItemData:removeFallingLockByCuckoo()
	-- printx(11, "- - - - - remove falling lock!", self.y, self.x)
	self.fallingLockByCuckoo = false
end

function GameItemData:hasFallingLockByCuckoo()
	if self.fallingLockByCuckoo then
		return true
	end
	return false
end
...
-----------------------------------------------------------
-- 国庆兔子，交互的对象格子会有临时的掉落屏蔽
function GameItemData:addFallingLockByNDBunny()
	self.fallingLockByNDBunny = true
end

function GameItemData:removeFallingLockByNDBunny()
	self.fallingLockByNDBunny = false
end

function GameItemData:hasFallingLockByNDBunny()
	if self.fallingLockByNDBunny then
		return true
	end
	return false
end
...
-----------------------------------------------------------
-- 爆米花，交互的对象格子会有临时的掉落屏蔽
function GameItemData:addFallingLockByPuffedRice()
	self.fallingLockByPuffedRice = true
end

function GameItemData:removeFallingLockByPuffedRice()
	self.fallingLockByPuffedRice = false
end

function GameItemData:hasFallingLockByPuffedRice()
	if self.fallingLockByPuffedRice then
		return true
	end
	return false
end
...
--是否能被暑期技能1打中
function GameItemData:canInfectByRailroadSkill1( ... )
	--可用并且 单面翻转地格没转
	if not self:isAvailable() then return false end
	if self.isUsed and self.ItemType ~= 0 then
		return true
	end
...
--是否能被暑期技能2打中
function GameItemData:canInfectByRailroadSkill2( ... )
	if not self:isAvailable() or self:hasLock() then return false end
	if self:hasFurball() then return false end
...
function GameItemData:hasLockedWaterBucket()
	if self:isTopCover(GameItemDataTopCoverName.k_WaterBucket) and (not WaterBucketLogic:canAttackBucket(self)) then
		-- printx(11, "+ + + hasLockedWaterBucket!!!", self.x, self.y)
		return true
	end
	return false
end

function GameItemData:hasActiveWaterBucket()
	if self:isTopCover(GameItemDataTopCoverName.k_WaterBucket) and WaterBucketLogic:canAttackBucket(self) then
		-- printx(11, "= = = hasActiveWaterBucket!!!", self.x, self.y)
		return true
	end
	return false
end
...
--是否能被暑期技能2打中
function GameItemData:canInfectByActivityColltionItem( ... )
	if not self:isAvailable() or self:hasLock() then return false end
	if self:hasFurball() then return false end
...
function GameItemData:canInfectByActivityColltionItem2( ... )
	if not self:isAvailable() or self:hasLock() then return false end
	if self:hasFurball() then return false end

	if self.ItemType == GameItemType.kCrystal then
        --水晶球
        return true
    end
...
function GameItemData:canInfectByThanksGivingSkill( ... )
	if self.isEmpty then return false end
	if not self:isAvailable() or self:hasLock() then return false end
	if self:hasFurball() then return false end
	if not self:isVisibleAndFree() then return false end

	if not self:canBeEffectByHammer() then return false end

	return true
end

function GameItemData:changeToTravelEventBox()
	self:cleanAnimalLikeData()
	self.ItemType = GameItemType.kTravelEventBox
	-- self._encrypt.ItemColorType = 0
	-- self.ItemSpecialType = 0
	self.isEmpty = false
	self.isBlock = true
...
-- function GameItemData:__on_value_change(k, v)
-- 	printx(11, "@@@@@@@@@@@@@ __on_value_change @@@@@@@@@@@@@", k, v, debug.traceback())
-- end
-- 	end
-- end
...
function GameItemData:addMeowLock(dir)
	if not self.meowLock or self.meowLock ~= dir then
		self.meowLock = dir
	end
end

function GameItemData:removeMeowLock()
	if self.meowLock then
		self.meowLock = nil
	end
end

function GameItemData:hasMeowLock()
	return self.meowLock
end


function GameItemData:addMeowHitLock()
	if not self.meowHitLock then
		self.meowHitLock = true
	end
end

function GameItemData:removeMeowHitLock()
	if self.meowHitLock then
		self.meowHitLock = nil
	end
end


function GameItemData:hasMeowHitLock()
	return self.meowHitLock
end

function GameItemData:afterChannelWater()
	-- self:cleanAnimalLikeData()
	self.ItemType = GameItemType.kNone
	self.isNeedUpdate = true
	self.isEmpty = true
