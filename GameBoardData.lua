...
function GameBoardData:ctor()
	...
	-- 行走路径：旅行模式、布谷鸟用
	self.travelRoadType = 0				-- 旅行路径类型
	self.prevTravelRoadType = 0			-- 路径前一格的旅行路径类型
	self.isTravelRoadStart = false		-- 是旅行路径起点
	self.isTravelRoadEnd = false		-- 是旅行路径终点
	self.isTravelEventTile = false 		-- 是否是旅行事件地格，产品改需求的历史遗迹，仅初始化时使用
    self.needTriggerTravelEvent = false -- 本地格的旅行事件是否需要触发

    self.birdRoadType = 0
    self.prevBirdRoadType = 0
    self.isBirdRoadStart = false
    self.isBirdRoadEnd = false

    self.bridgeRoadType = 0
    self.prevBridgeRoadType = 0
    self.isBridgeRoadStart = false
    self.isBridgeRoadEnd = false
    self.haveBoard = false
    self.isBoardFlyTarget = false
	...
	self.spawnQueueOfGrid = nil 		--该格子掉落口的生成序列状态（序列内容、本循环进度、剩余循环次数）
    ...
    self.windTunnelDir = nil 					-- 风洞配置的重力方向
    self.windTunnelActive = false 				-- 风洞的重力方向此时是否生效
    self.windTunnelWalls = nil 					-- 风区配置的边缘们
    self.windTunnelActiveWalls = nil 			-- 风区正在生效的边缘们

    self.refreshAreaFlag = 0					-- 刷新区域ID
    self.forbidProduceDiffChange = false 		-- 本格掉落是否屏蔽难度调整

    self.roostSelect = false --鸡窝x

    self.treasureRoadType = 0
    self.treasureRoadPreType = 0

    self.pathConfigs = {}			--通用的路径数据，以tileId作索引
    								--存储数据包括是否为路径起点终点，路径方向等。

    self.isBattleTrophyGridGenerater = false 	-- 抢豆荚模式下的收集物地格生成口
    self.battleTrophyGeneratedAmount = 0		-- 抢豆荚模式下收集物生地格成口的累计生成数量

    self.isNDBunnyProducer = false   --国庆兔子生成口
	self.isNDBunnyLeave  = false     --国庆兔子逃离口
	self.NDBunnyNumData = nil        --国庆兔子生成口属性
	self.NDBunnyIceLevel = 0         --国庆兔子专用冰层

	self.puffedRiceLow = false
...
function GameBoardData:resetDatas( mode )
    ...
    self.windTunnelDir = nil
    self.windTunnelActive = false 	
    self.windTunnelWalls = nil
    self.windTunnelActiveWalls = nil

    self.travelRoadType = 0
	self.prevTravelRoadType = 0
	self.isTravelRoadStart = false
	self.isTravelRoadEnd = false
	self.isTravelEventTile = false
	self.needTriggerTravelEvent = false
	...
	self.refreshAreaFlag = 0
    self.forbidProduceDiffChange = false

    self.roostSelect = false
    ...
    self.isBattleTrophyGridGenerater = false
    self.battleTrophyGeneratedAmount = 0

    self.isNDBunnyProducer = false
	self.isNDBunnyLeave  = false
	self.NDBunnyNumData = nil   
	self.NDBunnyIceLevel = 0
...
function GameBoardData.copyDatasFrom(toData, fromData , mode)
	...
	if fromData.spawnQueueOfGrid then
		toData.spawnQueueOfGrid = table.clone(fromData.spawnQueueOfGrid, true)
	else
		toData.spawnQueueOfGrid = nil
	end
    ...
    toData.windTunnelDir = fromData.windTunnelDir
    toData.windTunnelActive = fromData.windTunnelActive
    toData.windTunnelWalls = table.copyValues(fromData.windTunnelWalls)
    toData.windTunnelActiveWalls = table.copyValues(fromData.windTunnelActiveWalls)

    toData.travelRoadType = fromData.travelRoadType
	toData.prevTravelRoadType = fromData.prevTravelRoadType
	toData.isTravelRoadStart = fromData.isTravelRoadStart
	toData.isTravelRoadEnd = fromData.isTravelRoadEnd
    toData.isTravelEventTile = fromData.isTravelEventTile
    toData.needTriggerTravelEvent = fromData.needTriggerTravelEvent

    toData.birdRoadType = fromData.birdRoadType
    toData.prevBirdRoadType = fromData.prevBirdRoadType
    toData.isBirdRoadStart = fromData.isBirdRoadStart
    toData.isBirdRoadEnd = fromData.isBirdRoadEnd

    toData.refreshAreaFlag = fromData.refreshAreaFlag
    toData.forbidProduceDiffChange = fromData.forbidProduceDiffChange

    toData.roostSelect = fromData.roostSelect

    toData.treasureRoadType = fromData.treasureRoadType
    toData.treasureRoadPreType = fromData.treasureRoadPreType

    toData.isBridgeRoadStart = fromData.isBridgeRoadStart
    toData.isBridgeRoadEnd = fromData.isBridgeRoadEnd
    toData.haveBoard = fromData.haveBoard
    toData.isBoardFlyTarget = fromData.isBoardFlyTarget
    toData.bridgeRoadType = fromData.bridgeRoadType
    toData.prevBridgeRoadType = fromData.prevBridgeRoadType
    toData.isChannelWater = fromData.isChannelWater
    toData.isChannelTarget = fromData.isChannelTarget

    if fromData.pathConfigs then
    	toData.pathConfigs = table.clone(fromData.pathConfigs,true)
    else
    	toData.pathConfigs = {}
    end
	
    toData.isBattleTrophyGridGenerater = fromData.isBattleTrophyGridGenerater
    toData.battleTrophyGeneratedAmount = fromData.battleTrophyGeneratedAmount

    toData.isNDBunnyProducer = fromData.isNDBunnyProducer
	toData.isNDBunnyLeave  = fromData.isNDBunnyLeave
	toData.NDBunnyNumData = fromData.NDBunnyNumData 
	toData.NDBunnyIceLevel = fromData.NDBunnyIceLevel 

	toData.puffedRiceLow = fromData.puffedRiceLow 
end
...
function GameBoardData:initByConfig(tileDef)
	...
	self:_setWindTunnelData(tileDef)
    ...
    if tileDef:hasProperty(TileConst.kRoostSelect) then self.roostSelect = true end
    ...
    if tileDef:hasProperty(TileConst.kTravelEventTile) then
		self.isTravelEventTile = true
		self.needTriggerTravelEvent = false
	end

	if tileDef:hasProperty(TileConst.kRefreshAreaFlag) then
		self.refreshAreaFlag = tonumber(tileDef:getAttrOfProperty(TileConst.kRefreshAreaFlag))
	end

	if tileDef:hasProperty(TileConst.kSpawnForbidDiffChangeFlag) then
		self.forbidProduceDiffChange = true
	end

	if tileDef:hasProperty(TileConst.kBoard) then
		self.haveBoard = true
	end

	if tileDef:hasProperty(TileConst.kBottleTrophyGridGenerater) then
		self.isBattleTrophyGridGenerater = true
		self.battleTrophyGeneratedAmount = 0
	end

	-- 水到渠成
	if tileDef:hasProperty(TileConst.kChannelWater) then self.isChannelWater = true end
	if tileDef:hasProperty(TileConst.kChannelTarget) then self.isChannelTarget = true end

	if tileDef:hasProperty(TileConst.kNDBunnyProducer) then
		self.isNDBunnyProducer = true
	elseif tileDef:hasProperty(TileConst.kNDBunnyLeave) then
		self.isNDBunnyLeave  = true
	end

	if tileDef:hasProperty(TileConst.kNDBunnyIce) then 
		local mainLogic = GameBoardLogic:getCurrentLogic()
--		self.NDBunnyIceLevel = mainLogic.NDbunnyGlobalConfig.road
		self.NDBunnyIceLevel = 1
	end

	if tileDef:hasProperty(TileConst.kPuffedRice_Low) then self.puffedRiceLow = true end

end

function GameBoardData:_setWindTunnelData(tileDef)
	if not tileDef:hasProperty(TileConst.kWindTunnelTile) then return end

	local tileAttr = tileDef:getAttrOfProperty(TileConst.kWindTunnelTile)
	if not tileAttr then return end

	local windTunnelTileType = tonumber(tileAttr)
	if windTunnelTileType and windTunnelTileType > 0 and windTunnelTileType < 9 then
		-- on->off, up, right, down, left
		local initStatus = math.ceil(windTunnelTileType / 4)
		if initStatus == 1 then
			self.windTunnelActive = true
		else
			self.windTunnelActive = false
		end

		local dirType = windTunnelTileType - (initStatus - 1) * 4
		if dirType == 1 then
			self.windTunnelDir = BoardGravityDirection.kUp
		elseif dirType == 2 then
			self.windTunnelDir = BoardGravityDirection.kRight
		elseif dirType == 3 then
			self.windTunnelDir = BoardGravityDirection.kDown
		elseif dirType == 4 then
			self.windTunnelDir = BoardGravityDirection.kLeft
		end
		if self.windTunnelDir and self.windTunnelActive then
			self.gravity = self.windTunnelDir
		end

		self.gravitySkin = BoardGravitySkinType.kWindTunnel

		-- 风区边缘只在存在风区的地格才有意义，对于非风区地格，丢弃此属性不检测
		self.windTunnelWalls = {false, false, false, false} --init
		local hasWindTunnelWall = false
		local wallStartID = TileConst.kWindTunnelWall_Up
		for wallID = TileConst.kWindTunnelWall_Up, TileConst.kWindTunnelWall_Left do 
			if tileDef:hasProperty(wallID) then
				local wallDirIndex = wallID - wallStartID + 1
				self.windTunnelWalls[wallDirIndex] = true
				hasWindTunnelWall = true
			end
		end
		if not hasWindTunnelWall then
			self.windTunnelWalls = nil
		end
		-- if hasWindTunnelWall then printx(11, "WindWalls:", self.y, self.x, table.tostring(self.windTunnelWalls)) end
	end
end
...
-- 是否有非棋盘默认的重力方向生效。 目前仅处理了风洞。
function GameBoardData:getNonDefaultGravityActive()
	return self:isWindTunnelActive()
end

function GameBoardData:isWindTunnelActive()
	if self.windTunnelDir and self.windTunnelDir > 0 then
		return self.windTunnelActive
	end
	return false
end

function GameBoardData:isWindTunnelWallActive(direction)
	if self.windTunnelActiveWalls and self.windTunnelActiveWalls[direction] then
		return self.windTunnelActiveWalls[direction]
	end
	return false
end
...
function GameBoardData:changeDataAfterTrans(gameBoardData)
	...
	self.haveBoard = gameBoardData.haveBoard
...
function GameBoardData:hasChainInDirection(dir, chainLevel)
	local chain = self:getChainInDirection(dir)
	local _chainLevel = chainLevel or 0
	if chain and chain.level > _chainLevel then
		return true
	else
		return false
	end
...
function GameBoardData:initBoard(tileDef)
	if tileDef:hasProperty(TileConst.kBoard) then
		self.haveBoard = true
	end
end
...
-- 风区边缘视为可交换的隐形绳子，在非交换的场合考虑风区边缘
function GameBoardData:hasLeftRope(inSwapCheck)
	local windWallFlag = false
	if not inSwapCheck then
		windWallFlag = self:isWindTunnelWallActive(4)
	end
	local resultFlag = self:hasLeftRopeProperty() or self:hasChainInDirection(ChainDirConfig.kLeft) or windWallFlag
	return resultFlag
end

function GameBoardData:hasRightRope(inSwapCheck)
	local windWallFlag = false
	if not inSwapCheck then
		windWallFlag = self:isWindTunnelWallActive(2)
	end
	local resultFlag = self:hasRightRopeProperty() or self:hasChainInDirection(ChainDirConfig.kRight) or windWallFlag
	return resultFlag
end

function GameBoardData:hasTopRope(chainLevel, inSwapCheck)
	local windWallFlag = false
	if not inSwapCheck then
		windWallFlag = self:isWindTunnelWallActive(1)
	end
	local resultFlag = self:hasTopRopeProperty() or self:hasChainInDirection(ChainDirConfig.kUp, chainLevel) or windWallFlag
	return resultFlag
end

function GameBoardData:hasBottomRope(chainLevel, inSwapCheck)
	local windWallFlag = false
	if not inSwapCheck then
		windWallFlag = self:isWindTunnelWallActive(3)
	end
	local resultFlag = self:hasBottomRopeProperty() or self:hasChainInDirection(ChainDirConfig.kDown, chainLevel) or windWallFlag
	return resultFlag
end
...
function GameBoardData:hasJamSperad()
	return self.isJamSperad
end

function GameBoardData:jamSperad()
	self.isJamSperad = true
end
...
function GameBoardData:canTriggerTravelEvent()
	if self.isTravelEventTile and self.needTriggerTravelEvent then
		return true
	end
	return false
end
...
--
function GameBoardData:canInfectByActivityColltionItem( ... )

    if self.isUsed == false then return false end

	if self.preAndBuffFirecrackerPassSelect
        or self.preAndBuffLineWrapPassSelect
        or self.preAndBuffMagicBirdPassSelect
        then
	    return false
	end
	return true
...
function GameBoardData:canInfectThanksGivingSkill( ... )
    if self.isUsed == false then return false end
	return true
end
