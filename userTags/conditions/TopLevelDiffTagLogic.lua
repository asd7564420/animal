require "zoo.gamePlay.userTags.conditions.TopLevelDiffTagLogicConfig"
...
function TopLevelDiffTagLogic:updateContext()
	...
	if MaintenanceManager:getInstance():isEnabledInGroup( LevelDifficultyAdjustManager:getExpName() , "None" , self.context.uid ) then
		self.context.logicVer = 3
	elseif MaintenanceManager:getInstance():isEnabledInGroup( LevelDifficultyAdjustManager:getExpName() , "A17" , self.context.uid ) then
		-- self.context.logicVer = 5 --2019.8.12修改，v5迁移至v6
		self.context.logicVer = 6
	elseif MaintenanceManager:getInstance():isEnabledInGroup( LevelDifficultyAdjustManager:getExpName() , "A9" , self.context.uid ) then
		self.context.logicVer = 7
	elseif MaintenanceManager:getInstance():isEnabledInGroup( LevelDifficultyAdjustManager:getExpName() , "A18" , self.context.uid ) then
		self.context.logicVer = 8

	elseif MaintenanceManager:getInstance():isEnabledInGroup( LevelDifficultyAdjustManager:getExpName() , "V701" , self.context.uid ) then
		self.context.logicVer = 701
	elseif MaintenanceManager:getInstance():isEnabledInGroup( LevelDifficultyAdjustManager:getExpName() , "V801" , self.context.uid ) then
		self.context.logicVer = 801
...
function TopLevelDiffTagLogic:getContext()
	return self.context
end
...
function TopLevelDiffTagLogic:getDiffTagByConfig( levelId , failTimes , valueMap , diff , ver )

	local maxLevelId = self.context.maxLevelId

	DiffAdjustQAToolManager:print( 1 , "RRR" , "TopLevelDiffTagLogic:getDiffTagV" .. tostring(ver) .. " p1" , levelId , failTimes , diff , maxLevelId )
	...
	local TopLevelDiffTagLogicConfig = getTopLevelDiffTagLogicConfig( self.context.maxLevelId )

	for k , v in ipairs( TopLevelDiffTagLogicConfig["V" .. tostring(ver)] ) do
...
function TopLevelDiffTagLogic:getDiffTagV4( levelId , failTimes , valueMap , diff )
	return self:getDiffTagByConfig( levelId , failTimes , valueMap , diff , 4 )
end

function TopLevelDiffTagLogic:getDiffTagV5( levelId , failTimes , valueMap , diff )
	return self:getDiffTagByConfig( levelId , failTimes , valueMap , diff , 5 )
end

function TopLevelDiffTagLogic:getDiffTagV6( levelId , failTimes , valueMap , diff )
	return self:getDiffTagByConfig( levelId , failTimes , valueMap , diff , 6 )
end

function TopLevelDiffTagLogic:getDiffTagV7( levelId , failTimes , valueMap , diff )
	return self:getDiffTagByConfig( levelId , failTimes , valueMap , diff , 7 )
end

function TopLevelDiffTagLogic:getDiffTagV8( levelId , failTimes , valueMap , diff )
	return self:getDiffTagByConfig( levelId , failTimes , valueMap , diff , 8 )
end

function TopLevelDiffTagLogic:getDiffTagV701( levelId , failTimes , valueMap , diff )
	return self:getDiffTagByConfig( levelId , failTimes , valueMap , diff , 701 )
end

function TopLevelDiffTagLogic:getDiffTagV801( levelId , failTimes , valueMap , diff )
	return self:getDiffTagByConfig( levelId , failTimes , valueMap , diff , 801 )
end
...
function TopLevelDiffTagLogic:__check( levelId , failTimes )
	...
	-- TopLevelDiffTagLogicConfig.maxLevelId = self.context.maxLevelId
	...
	elseif self.context.logicVer == 5 then
		return self:getDiffTagV5( levelId , failTimes , valueMap , diff )
	elseif self.context.logicVer == 6 then
		return self:getDiffTagV6( levelId , failTimes , valueMap , diff )
	elseif self.context.logicVer == 7 then
		return self:getDiffTagV7( levelId , failTimes , valueMap , diff )
	elseif self.context.logicVer == 8 then
		return self:getDiffTagV8( levelId , failTimes , valueMap , diff )
	elseif self.context.logicVer == 701 then
		return self:getDiffTagV701( levelId , failTimes , valueMap , diff )
	elseif self.context.logicVer == 801 then
		return self:getDiffTagV801( levelId , failTimes , valueMap , diff )
	else
		return valueMap.kNormalDiff
	end
end
