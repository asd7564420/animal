...
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.WaterBucketLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.CanevineLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.GyroLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.WindTunnelLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.ActivityClollectionItemLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.TravelLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.SpringFes2020Logic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.PlaneLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.DynamiteCrateLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.ActivityPlayLogic.Valentine2020Logic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.CatteryLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.WeeklyRace2020Logic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.AngryBirdLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.TreasureHuntLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.BridgeCrossLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.FireworkLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.BatteryLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.NationalDayBunnyLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.PuffedRiceLogic"

require "zoo.gamePlay.GameBoardModel"
require "zoo.gamePlay.BeginnerPlugin"
...
local nativeImplement = require("hecore.nativeImplement")


NewGameBoardLogic = class2("NewGameBoardLogic")
local _currentLogic = nil
--local encryptIndex = 0


function NewGameBoardLogic:ctor()
	...
	self.gameMode = nil
	self.gameReachTarget = false --到达关卡目标
    ...
    self.actCollectRandFactory = RandomFactory:create("collect")
    self.travelMapRandFactory = RandomFactory:create("travel") 		--旅行模式下，关卡随机工厂
	
	self.PlayUIDelegate = nil;
	self.fallingMatchFrameId = 0

	------------------------------------------
	-- 这个table下的字段都会被link到 self.gameBoardModel 下的同名字段上
	-- GameBoardLogic不再储存和持有这些字段
	self.linkedKeysMap = nil --后面将根据self.linkedKeys自动生成
	self.linkedKeys = {
		"gameItemMap" ,
		"boardmap",
		"backItemMap",
		...
		"replay" ,
		"allReplay" ,
		"setWriteReplayEnable" ,
		"blockReplayReord" ,
		"AIPlayQAAdjustBreakAfterAll",

		"theCurMoves" ,
		"realCostMove" ,
		"realCostMoveWithoutBackProp" ,
		"leftMoves" ,
		"scoreTargets" ,
		...
		"EffectHelpMap" ,
		"EffectSHelpMap" ,
		"EffectLightUpHelpMap" ,
		"EffectChameleonHelpMap" ,

		"comboCount" ,
		"comboHelpDataSet" ,
		"comboHelpList" ,
		"comboHelpNumCountList" ,
		"comboSumBombScore" ,
		"totalScore" ,
		"oringinTotalScore" ,
		"bonusTimeScore",
		...
		"hasUseRevertThisRound" ,
		"isVenomDestroyedInStep" ,
		"questionMarkFirstBomb" ,
		"isFirstRefreshComplete" ,
		
		"snapshotModeEnable" ,

		"actCollectionNum" ,

		"lastCreateBuffBoomMoveSteps" ,
		...
		"pacmanConfig",
		"pacmanGeneratedByStep" ,
		"pacmanGeneratedByBoardMin" ,

		"ghostConfig" ,
		"ghostGeneratedByStep" ,
		"ghostGeneratedByBoardMin" ,

		"generatedScoreBuffBottle" ,
		"destroyedScoreBuffBottle" ,
		"scoreBuffBottleLeftSpecialTypes" ,
		"scoreBuffBottleInitAmount" ,
		"globalScoreBuffVal" ,
		...
		"travelEnergy",
		"skipTravelStateType",
		"travelEventBoxOpened",
		"currMapTravelRouteLength",
		"currMapTravelStep",
		"travelMapInitUsedMove",

		"blocker195Nums",
		"blocker199Cfg",
		"blocker206Cfg",
		"blocker207DestroyNum" ,
		...
		"NDbunnyGlobalConfig" ,
		"NDbunnyGridConfig" ,
		"NDbunnySnowGridConfig" ,

		"curWaterBucketChargingGroupId" ,

		"honeys" ,
		"missileSplit" ,
		"squidOnBoard" ,
		"blockerCoverMaterialTotalNum" ,
		"hadAutoSnowMonsterBuff" ,
		"hadAutoBuffActived" ,
		...
		"generateFirecrackerTemp" ,
		"generateFirecrackerTimes" ,
		"generateFirecrackerTimesForPreBuff" ,

		"needUpdateSeaAnimalStaticData" ,

		"getProps" ,

		"initAdjustData" ,

		"randomAnimalHelpList" ,
		...
		"fireworkEnergy" ,
		"forbidChargeFirework" ,
		"isFullFirework",

		"collectTargetNums",
		"extraGainedActCollections",
		"moveBonusHarvested",

		"moveTileCarryProductRuleMode",
...
function NewGameBoardLogic:__on_getter( k , rawData , ori_metatable )
	-- printx( 1 , "__on_getter  " , k )
	if self.linkedKeysMap[k] then
		-- if self.gameBoardModel then
		-- 	return self.gameBoardModel[k]
		-- else
		-- 	return nil
		-- end
...
function NewGameBoardLogic:__on_setter( k , v , rawData , ori_metatable )
	if self.linkedKeysMap[k] then
		-- if self.gameBoardModel then
		-- 	self.gameBoardModel[k] = v
		-- end
		self.gameBoardModel[k] = v
	else
		self[k] = v
...
function NewGameBoardLogic:create(replayMode)
	local gameBoardLogic = NewGameBoardLogic.new()
	gameBoardLogic:initBoard()

	gameBoardLogic.linkedKeysMap = {}
	for k2 , v2 in ipairs( gameBoardLogic.linkedKeys ) do
		gameBoardLogic.linkedKeysMap[v2] = true
	end

	buildGetterSetter( gameBoardLogic , gameBoardLogic.linkedKeys , "__on_setter" , "__on_getter" , 2 )

	gameBoardLogic.replayMode = replayMode or ReplayMode.kNone --顺序敏感，对gameBoardLogic的赋值操作应该在buildGetterSetter之后

	_currentLogic = gameBoardLogic

	return gameBoardLogic
end

function NewGameBoardLogic:getCurrentLogic()
	return _currentLogic
end

function NewGameBoardLogic:getInstance()--老子每次都记不住getCurrentLogic，受不了了！！！
	return self:getCurrentLogic()
end

function NewGameBoardLogic:getBoardModel()
	return self.gameBoardModel
end

function NewGameBoardLogic:initBoard()
	
	self.gameBoardModel = GameBoardModel:create()
...
function NewGameBoardLogic:dispose()
	NewGameBoardLogic._currentLogic = nil

	HanleBeforeFallingLogic:setNeedHandle(false)
	SmartGuideManager.getInstance():clear()
	self:stopTargetTip()
	...
	self.gameReachTarget = false

	LevelDifficultyAdjustManager:clearAICoreInfo()
	StarBuffYEManager.getInstance():setReplayFlag(false)
	ScoreBuffBottleLogic:clearInitAmountForReplay()		--清除刷星回放数据
	StarBuffYEManager.getInstance():unloadRes()
	Notify:dispatch("GameBoardLogicDispose",{})
	
	--self.initAdjustData = nil --这里如果删除，NewGameSceneUI中的onQuitCallback方法里会取不到，大量的旧逻辑在GameBoardLogic的dispose方法调用之后还在取值

	if self.replayUnresponsiveCheckTimerId then
		CCDirector:sharedDirector():getScheduler():unscheduleScriptEntry( self.replayUnresponsiveCheckTimerId )
		self.replayUnresponsiveCheckTimerId = nil
	end
end

function NewGameBoardLogic:reBuildGameBoardModel( levelConfig , replayMode , replayData , forceUseDropBuff )
	self.gameBoardModel:dispose()
	self.gameBoardModel = GameBoardModel:create()
	self.gameBoardModel:initByConfig( levelConfig , { replayMode = replayMode , replayData = replayData , forceUseDropBuff = forceUseDropBuff } )
end

function NewGameBoardLogic:buildSelfByGameBoardModel( isInLevelChangeBoard )

	local needInitRandomFactory = true
	local needInitGamePlayContext = true

	if isInLevelChangeBoard then
		needInitRandomFactory = false
		needInitGamePlayContext = false
	end
	---------------- pluginMode
	self.modeProcessor = LevelModeProcessor:create( self , self.gameBoardModel.modeProcessorDatas )
	

	---------------- randomSeed
	if needInitRandomFactory then
		self.randomSeed = self.gameBoardModel.levelConfig.randomSeed
		if self.randomSeed == nil then self.randomSeed = 0 end
	end
	

	if not self.gameBoardModel.replayMode or self.gameBoardModel.replayMode == ReplayMode.kNone then
  		self.stageStartTime = Localhost:timeInSec()
		
		if needInitRandomFactory then

			if GameGuide:hasGuideSeed(level) then
	...
	elseif self.gameBoardModel.replayData then

		if needInitRandomFactory then
			self.useGuideRandomSeed = self.gameBoardModel.replayData.useGuideRandomSeed
		end
		
  		self.stageStartTime = self.gameBoardModel.replayData.startTime
	end
	...
	if needInitRandomFactory then
	    ...
	    self.actCollectRandFactory:randSeed(self.randomSeed)
	    self.travelMapRandFactory:randSeed(self.randomSeed)
	end
	-- self.randomSeed = 47
 	
 	---------------- GamePlayContext doGameInit
 	if needInitGamePlayContext then
 	...
 	MACRO_DEV_START()
	printx(0, "level init id:", level, "randomSeed:", self.randomSeed)
	MACRO_DEV_END()

	-- 初始化神奇掉落规则
	self.dropBuffLogic = DropBuffLogic:create(self, self.gameBoardModel.levelConfig )
	...
	self.guideAtlas = AnimalGuideAtlas:create(self.gameBoardModel.levelConfig)
	
	ColorFilterLogic:init(self)
	ActCollectionLogic:init(self)
	GameMapInitialLogic:init( self , self.gameBoardModel.levelConfig , self.gameBoardModel.replayMode, self.gameBoardModel.replayData)
	FallingItemLogic:preUpdateHelpMap(self)

	self.gameMode = GameModeFactory:create(self)
	self:addAllItemsForMatchCheck()
	self.gameMode:initModeSpecial( self.gameBoardModel.levelConfig ,self.gameBoardModel.replayData)

	FUUUManager:update( self.gameMode )

	ProductItemLogic:init( self, self.gameBoardModel.levelConfig , self.logicVer or 1 )

	self.gamePlayEventTrigger = GamePlayEventTrigger:create()

	if self.gameBoardModel.replaying then
		self.gameBoardModel.initialMapData = self:getSaveDataForRevert()
	end
	...
	ObstacleFootprintManager:initData(self, self.gameBoardModel.levelConfig )

	self:generateExtraOrderItem()
end

function NewGameBoardLogic:setProductItemLogicVersion(logicVer)
...
----游戏逻辑更新循环
local _GARBAGE_MAP = {}
function NewGameBoardLogic:updateGame(dt)
	self.fsm:update(dt)
...
_FIELD_LOGIC_ID[GameItemType.kDynamiteCrate] = "GameItemType.kDynamiteCrate"
...
_FIELD_LOGIC_ID.gyroGenerate = "gyroGenerate"
_FIELD_LOGIC_ID.ActivityCollectionItem = "ActivityCollectionItem"
_FIELD_LOGIC_ID.travelHeroWalk = "travelHeroWalk"
_FIELD_LOGIC_ID.planeFly = "planeFly"
_FIELD_LOGIC_ID.catteryRoll = "catteryRoll"
_FIELD_LOGIC_ID.angryBirdWalk = "angryBirdWalk"
_FIELD_LOGIC_ID.walkChickWalk = "walkChickWalk"
_FIELD_LOGIC_ID.NDBunnyHandle = "NDBunnyHandle"
_FIELD_LOGIC_ID.NDBunnyProduce = "NDBunnyProduce"
_FIELD_LOGIC_ID.puffedRiceJump = "puffedRiceJump"

function NewGameBoardLogic:updateFieldLogicPossibility_activeAll()
...
function NewGameBoardLogic:updateFieldLogicPossibility()
	...
	for r = 1, m do
		...
		for c = 1, n do
			...
			if item then
				...
				if self._fieldLogicPossibility[_FIELD_LOGIC_ID.gyroGenerate] ~= 1 
					and itemType == GameItemType.kGyroCreater 
					and item:isVisibleAndFree() then
					self._fieldLogicPossibility[_FIELD_LOGIC_ID.gyroGenerate] = 1
				end
				if self._fieldLogicPossibility[_FIELD_LOGIC_ID.planeFly] ~= 1 
					and PlaneLogic:isPlaneReadyToTakeOff(self, item) then
					self._fieldLogicPossibility[_FIELD_LOGIC_ID.planeFly] = 1
				end
				if self._fieldLogicPossibility[_FIELD_LOGIC_ID.catteryRoll] ~= 1 
					and CatteryLogic:isCatteryReadyToRoll(self, item) then
					self._fieldLogicPossibility[_FIELD_LOGIC_ID.catteryRoll] = 1
				end

				if self._fieldLogicPossibility[_FIELD_LOGIC_ID.puffedRiceJump] ~= 1 
					and PuffedRiceLogic:isPuffedRiceReadyToJump(self, item) then
					self._fieldLogicPossibility[_FIELD_LOGIC_ID.puffedRiceJump] = 1
				end

				if itemType == GameItemType.kTravelHero 
					and item:isVisibleAndFree() 
					and self.travelEnergy and self.travelEnergy > 0
					then
					self._fieldLogicPossibility[_FIELD_LOGIC_ID.travelHeroWalk] = 1
				end

				if itemType == GameItemType.kAngryBird 
					and item:isVisibleAndFree() 
					and item.birdEnergy and item.birdEnergy > 0
					then
					self._fieldLogicPossibility[_FIELD_LOGIC_ID.angryBirdWalk] = 1
				end

				if itemType == GameItemType.kWalkChick 
					and item:isVisibleAndFree() 
					then
					self._fieldLogicPossibility[_FIELD_LOGIC_ID.walkChickWalk] = 1
				end

				if itemType == GameItemType.kNationalDayBunny and item:isVisibleAndFree() then
					self._fieldLogicPossibility[_FIELD_LOGIC_ID.NDBunnyHandle] = 1
				end
			...
			if boardData then
				...
				if boardData.isNDBunnyProducer then
					self._fieldLogicPossibility[_FIELD_LOGIC_ID.NDBunnyProduce] = 1
				end
...
function NewGameBoardLogic:updateGlobalCoreAction(dt)
...
function NewGameBoardLogic:fallingMatchUpdate(dt)
	-- printx(11 , "NewGameBoardLogic:fallingMatchUpdate  111  isFallingStable:["..tostring(self.isFallingStable).."]  isFallingStablePreFrame:["..tostring(self.isFallingStablePreFrame).."]  NEED CHECK:["..tostring(self.needCheckFalling).."]  dt:", dt)
	self.fallingMatchFrameId = self.fallingMatchFrameId + 1
	GameBoardActionRunner:runActionList(self, true)					-- 处理动作列表中的动作
	if self.needCheckFalling then	

		-- if UseNewFallingLogic then
		-- 	NewFallingItemLogic:clearVerticalFallingPlan()
		-- end
		...
		-- printx(11, "NewGameBoardLogic:fallingMatchUpdate  222  isFallingStable:"..tostring(self.isFallingStable))
		HanleBeforeFallingLogic:setNeedHandle(self.isFallingStable)
	...
	if ctx.endlessLoopLog and #ctx.endlessLoopLog > 0 then
		if self.replayMode == ReplayMode.kAutoPlayCheck or (self.replayMode == ReplayMode.kMcts and not _G.AIAutoCheckReplayCheck) then

			local lastLog = ctx.endlessLoopLog[#ctx.endlessLoopLog]
			local failReason = nil
			if lastLog.loopType == DeathLoopType.kPortal then
				failReason = AutoCheckLevelFinishReason.kEndlessLoopByPortal
			elseif lastLog.loopType == DeathLoopType.kColor then
				failReason = AutoCheckLevelFinishReason.kEndlessLoopByColor
			end

			self.PlayUIDelegate:replayResult(
									self.PlayUIDelegate.levelId, 
									self.totalScore, 
									self.gameMode:getScoreStarLevel(), 
...
--重置被特效打中的map
-----------------------------
function NewGameBoardLogic:resetSpecialEffectList(actid)
...
function NewGameBoardLogic:numDestroyAction()
	local count = 0
	for k, v in pairs(self.destroyActionList) do
		count = count + 1
	end
	return count
end

function NewGameBoardLogic:numFallingAction()
	local count = 0
	for k, v in pairs(self.fallingActionList) do
		count = count + 1
	end
	return count
end

function NewGameBoardLogic:getStableDestructionPlanListCount()
...
function NewGameBoardLogic:getActionListCount()
...
function NewGameBoardLogic:getGlobalCoreActionListCount()
	local n = 0
	if self.globalCoreActionList then
		for k, v in pairs(self.globalCoreActionList) do
			n = n + 1
		end
	end
	return n
...
--------------------------------------------------------
--	这个方法原名isItemAllStable，后来修改的时候发现掉落过程中被激活的闪电鸟可能无法通过判断，所以改名了
function NewGameBoardLogic:isItemAllStableExceptSuperTotems( passPosMap )
	if not passPosMap then passPosMap = {} end

	local passItemFilterFunc = function (item)
		if item and item.ItemType == GameItemType.kTotems and ( item:isActiveTotems() or item.totemsState == GameItemTotemsState.kWattingBomb ) and item:isAvailable() then
			return true
		end
		return false
	end
	...
	for r = 1, #self.gameItemMap do
		for c = 1, #self.gameItemMap[r] do
			...
			if not passPosMap[tostring(r) .. "_" .. tostring(c)] and tempItem and tempItem.isUsed and not tempItem.isEmpty 
				...
				-- and ( passItemFilterFunc and not passItemFilterFunc(tempItem) ) then

				-- return false
				if passItemFilterFunc then
					if not passItemFilterFunc(tempItem) then
						return false
					end
				else
					return false
...
function NewGameBoardLogic:segmentedActionCanMoveOn( )
	--	在老逻辑中，染色宝宝和魔力的放招action part2和part3会判断棋盘是否稳定
	--	判断依据是：GameBoardLogic:isItemAllStable的返回true + destructionPlanList中只有自己的action
	--	实际情况可能会出现：满足上述条件但棋盘中有其它队列中的action在运行，action结束后会改变棋盘状态，比如匹配消除
	--	修改后的逻辑会判断四个action队列的情况，并且连续10次条件判断都满足条件，才会认为棋盘稳定，可以进入action的下一阶段。

	if self:isItemAllStableExceptSuperTotems()
		and self:numDestrunctionPlan() == 1 
		and self:numDestroyAction() + self:numFallingAction() + self:getGlobalCoreActionListCount() == 0
		then
...
function NewGameBoardLogic:tryStableHandle()
	-- printx(11 , "NewGameBoardLogic:tryStableHandle   " , self.isFallingStable , self.isFallingStablePreFrame)
...
function NewGameBoardLogic:boardStableHandler()
...
function NewGameBoardLogic:onRealStable()
...
function NewGameBoardLogic:clearBlcoker195Data()
...
function NewGameBoardLogic:pauseBySnapshotMode()

	printx( 1 , "  ======================== NewGameBoardLogic:pauseBySnapshotMode ========================")
...
function NewGameBoardLogic:onAfterRefreshStableAndEndGame()
	--printx( 1 , "NewGameBoardLogic:onAfterRefreshStableAndEndGame --------------------" )
	...
	GamePlayContext:getInstance():resetEndlessLoopData()
end

function NewGameBoardLogic:onAfterRefreshStable()
	--printx( 1 , "NewGameBoardLogic:onAfterRefreshStable --------------------" )
...
function NewGameBoardLogic:onSnapshotModeContinue()
	printx( 1 , "   NewGameBoardLogic:onSnapshotModeContinue")
...
function NewGameBoardLogic:markVenomDestroyedInStep()
	self.isVenomDestroyedInStep = true
end

function NewGameBoardLogic:refreshComplete()
...
function NewGameBoardLogic:getIcePosList()
	local posList = {}
...
function NewGameBoardLogic:getSnowPosList()
	local posList = {}
	for r = 1, #self.gameItemMap do
		for c  = 1, #self.gameItemMap[r] do 
			local item = self.gameItemMap[r][c]
...
function NewGameBoardLogic:getHoneyPosList()
	local posList = {}
	for r = 1, #self.gameItemMap do
		for c  = 1, #self.gameItemMap[r] do 
			local item = self.gameItemMap[r][c]
...
function NewGameBoardLogic:getJamPosList()
	local posList = {} 
	for r = 1, #self.boardmap do
		for c  = 1, #self.boardmap[r] do 
			local board = self.boardmap[r][c]
			local item = self.gameItemMap[r][c]
			if board and (not board:hasJamSperad()) and board.isUsed and (item and not item.isBlock) then
...
function NewGameBoardLogic:clearTargetTip(realRm)
		...
		for i,v in ipairs(self.targetTipIcons) do
			if not v.isDisposed then
				if self.theGamePlayType == GameModeTypeId.JAMSPREAD_ID then
					if v.body then
						v.body:play('st2', 1)
						v.body:addEventListener(ArmatureEvents.COMPLETE, function ()
...
-- 得到这个scale
function NewGameBoardLogic:getSingleGridWidth(parentLayer)
...
function NewGameBoardLogic:startTargetTip()
	...
	local function checkSnowTarget()
		...
		if self.theOrderList then
			for index,orderData in ipairs(self.theOrderList) do
				...
				if orderData.key1 == GameItemOrderType.kOthers and orderData.key2 == GameItemOrderType_Others.kJamSperad then
					jamNumber = orderData.v1 - orderData.f1
					if jamNumber > 0  then 
						jamExist = true
					else
						jamExist = false
		...
		local totalTargetNumber = snowNumber+honeyNumber+jamNumber
		return totalTargetNumber>0 and totalTargetNumber<=3, snowTargetExist, honeyTargetExist, jamExist
	end

	local function playJamTargetTip(r, c)
		for _,ani in ipairs(self.targetTipIcons) do
			if ani.r == r and ani.c == c then return end
		end

		local targetAni = Layer:create()
		local animation = ArmatureNode:create("jam_level/tips")
		if not animation then
			return 
		end

		animation:update(0.0001)
		animation:play("st1")
		animation:setScale(1.05)

		for i = 1, 5 do
			local slot = animation.refCocosObj:getCCSlot(string.format("图层 %d", i))
			if slot and slot:getCCChildArmature() then
				slot:getCCChildArmature():advanceTime(0.001)
			end
		end

		targetAni:addChild(animation:wrapWithBatchNode())
		targetAni.body = animation

		local width = self:getSingleGridWidth(self.PlayUIDelegate.otherElementsLayer)
		targetAni:setScale(width/70)
		self.PlayUIDelegate.otherElementsLayer:addChild(targetAni)

		local pos = self:getGameItemPosInView(r, c)
		targetAni:setPosition(ccp(pos.x, pos.y))
		targetAni.r = r
		targetAni.c = c

		table.insert(self.targetTipIcons, targetAni)
	end
	...
	function showTargetTip()
		...
		if self.theGamePlayType == GameModeTypeId.ORDER_ID or self.theGamePlayType == GameModeTypeId.JAMSPREAD_ID then
			local checkEnable, snowTargetExist, honeyTargetExist, jamExist = checkSnowTarget()
			if checkEnable then
				...
				if jamExist then
					local jamPosList = self:getJamPosList()
					--if _G.isLocalDevelopMode then printx(0, "@@@@@@@@start to show honey tip!!!!!") end
					for _,pos in ipairs(jamPosList) do
						playJamTargetTip(pos.r, pos.c)
					end
					setTimeOut(function()
						self:clearTargetTip()
					end, 4)
	...
	local scheduleTargetTip = function ()
		...
		local targetTipScheduleId = CCDirector:sharedDirector():getScheduler():scheduleScriptFunc(showTargetTip, time, false)	
...
function NewGameBoardLogic:stopTargetTip()
...
function NewGameBoardLogic:hideTargetTip()
	--if _G.isLocalDevelopMode then printx(0, "todo: hide target tip!!!!!") end
	self:clearTargetTip()
end

function NewGameBoardLogic:startMoveTileEffect()
...
function NewGameBoardLogic:stopMoveTileEffect()
	if self.moveTileEffectScheduleId then
...
function NewGameBoardLogic:startEliminateAdvise()
	...
	-- 新手期在变速时保持间隔不变
	local beginnerTimer = nil
	local lastBeginnerCheck = nil
	if BeginnerPlugin.isEnabled() then
		beginnerTimer = Localhost:timeInMillis()
		delay = BeginnerPlugin.getAdviseDelay()
		interval = 0.5
	end

	local function showEliminateAdvise(dt)
		if beginnerTimer then
			local time = Localhost:timeInMillis()
			if time - beginnerTimer < delay*1000 then
				return
			end
			beginnerTimer = time
		end
		...
		if not self.targetPossibleSwap then 
			he_log_error("NewGameBoardLogic is disposed when showEliminateAdvise")
			return 
		end
		self:showEliminateAdvise(dt)

		BeginnerPlugin.showAdviseEffect(self)
	end
	...
	local eliminateAdviseScheduleId = CCDirector:sharedDirector():getScheduler():scheduleScriptFunc(showEliminateAdvise, interval, false)	
...
function NewGameBoardLogic:stopEliminateAdvise()
	--if _G.isLocalDevelopMode then printx(0, "----------stop advise") end
    BeginnerPlugin.stopAdviseEffect(true)
...
function NewGameBoardLogic:showEliminateAdvise(dt)
...
function NewGameBoardLogic:hideEliminateAdvise()
...
function NewGameBoardLogic:setNeedCheckFalling()
	self.needCheckFalling = true
end

function NewGameBoardLogic:getBossCount()
	local bossCount = 0
...
function NewGameBoardLogic:getTargetCount()
	local targetCount = 0
	if self.theGamePlayType == GameModeTypeId.MAYDAY_ENDLESS_ID
		or self.theGamePlayType == GameModeTypeId.HALLOWEEN_ID 
		or self.theGamePlayType == GameModeTypeId.HEDGEHOG_DIG_ENDLESS_ID
		...
		or self.theGamePlayType == GameModeTypeId.WUKONG_DIG_ENDLESS_ID 
		or self.theGamePlayType == GameModeTypeId.MOLE_WEEKLY_RACE_ID 
		or self.theGamePlayType == GameModeTypeId.WEEKLY_RACE_2020_MODE_ID 
		then
	...
	elseif self.levelType == GameLevelType.kFourYears then
     ...
     elseif self.levelType == GameLevelType.kSummerFish or self.levelType == GameLevelType.kSpringFes2020
    	or self.levelType == GameLevelType.kSixYear2020 or self.levelType == GameLevelType.kDragonBoat2020 then
		-- 鱼
		for _, v in ipairs(self.theOrderList) do
			if v.key1 == 6 and v.key2 == 4 then
				targetCount = v.v1
...
function NewGameBoardLogic:setGamePlayStatus(state)
	-- printx( 1 , "  ---------------------------   NewGameBoardLogic:setGamePlayStatus " , self.theGamePlayStatus , state , debug.traceback() , "\n")
	if (state ~= self.theGamePlayStatus) then
		if state == GamePlayStatus.kEnd then
			...
			if self.gameMode:reachTarget() then
				...
				if LevelType:isInfiniteMoveLevelType(self.levelType) then
					self:setGamePlayStatus(GamePlayStatus.kAferBonus)
				elseif self.theGamePlayType == GameModeTypeId.TASK_UNLOCK_DROP_DOWN_ID then
					self:setGamePlayStatus(GamePlayStatus.kAferBonus)
				else
					self:setGamePlayStatus(GamePlayStatus.kBonus)
				end

				self:getBoardModel().bonusTimeScore = self:getBoardModel().totalScore
			else
		...
		elseif state == GamePlayStatus.kFailed then
			...
			if self.__BUY_STEP_IN_PROCESS then
				he_log_error("ADD_STEP_BUY_POPOUT, level fail in buy process!")
			end
		...
		elseif state == GamePlayStatus.kBonus then
			...
			-- 		end)
			-- end
			SmartGuideManager.getInstance():onEnterBonusState()
			...
			if (BombItemLogic:getNumSpecialBomb(self) > 0 
				...
				or self.gameMode:is(AngryBirdMode)
				)
				and self.levelType ~= GameLevelType.kSpringFes2020
				and self.levelType ~= GameLevelType.kSixYear2020
				and self.levelType ~= GameLevelType.kDragonBoat2020
				and self.levelType ~= GameLevelType.kMidAutumn2020
				then
			 	self.comboCount = 0
		 	-- 	if _G.dev_kxxxl then
				...
				-- 有一些结束动画
				if self.levelType == GameLevelType.kSpringFes2020 then
					SpringFes2020Logic:calculateFinalCheeseNumAndRecordStep(self)
					SpringFes2020Logic:playReceiveCheeseAnimation(self)
					-- SpringFes2020Logic:checkPlayEnterAnimation(self.level, self)
					endGameDelay = 5
				end
		...
		elseif state == GamePlayStatus.kWin then
			...
			if self.theGamePlayType ~= GameModeTypeId.DIG_TIME_ID and 
				...
				self.theGamePlayType ~= GameModeTypeId.HEDGEHOG_DIG_ENDLESS_ID and
				self.theGamePlayType ~= GameModeTypeId.TRAVEL_MODE_ID
				then
		...
		elseif state == GamePlayStatus.kAferBonus then
			...
			local function gameResultShow( ... )
				self:_formattingScoreBeforeGameEnded()
			...
			elseif self.theGamePlayType == GameModeTypeId.WEEKLY_RACE_2020_MODE_ID then
				if _G.useNewAIAutoCheck then
					-- 周赛2020 的ai打关，非自动滚屏，将是否打完最终大宝箱区分为成功和失败，方便统计数据
					self:setGamePlayStatus(GamePlayStatus.kFailed)
				else
					-- 周赛2020 普通模式都是成功
					self:setGamePlayStatus(GamePlayStatus.kWin)
...
-- 在有分数加成条件下，结束关卡前按要求规范一下分数
function NewGameBoardLogic:_formattingScoreBeforeGameEnded()
	if self.globalScoreBuffVal and self.globalScoreBuffVal > 0 then
		local oldTotalScore = self.totalScore
		if oldTotalScore % 5 ~= 0 then
			local newTotalScore = oldTotalScore + ( 5 - oldTotalScore % 5 )
			-- printx(11, "formatting score:", oldTotalScore, newTotalScore)
			local deltaScore = newTotalScore - oldTotalScore
			if deltaScore > 0 then
				self.totalScore = newTotalScore
				-- update view..
				if self.PlayUIDelegate and self.PlayUIDelegate.scoreProgressBar then
					self.PlayUIDelegate:addScore(deltaScore, ccp(0, 0))
					-- self.PlayUIDelegate.scoreProgressBar:setScore(newTotalScore , ccp(0, 0))
				end
			end
		end
	end
...
function NewGameBoardLogic:getGainCoinNumber(levelId, coinNum)
...
function NewGameBoardLogic:generateExtraOrderItem( ... )
	if self.theOrderList then 
		for _, v in ipairs(self.theOrderList) do
...
function NewGameBoardLogic:getViewableOrderList( ... )
...
function NewGameBoardLogic:getSnailTotalCount( )
	-- body
...
function NewGameBoardLogic:getSnailOnScreenCount( ... )
	-- body
	local result = 0
	for r = 1, #self.gameItemMap do
		for c  = 1, #self.gameItemMap[r] do 
...
function NewGameBoardLogic:onGameInit()
	-- printx(11, "= = = NewGameBoardLogic:onGameInit", debug.traceback())
	...
	elseif self.levelType == GameLevelType.kWeeklyRace2020 then
		if not self.replayMode or self.replayMode == ReplayMode.kNone then
			local http = OpNotifyOffline.new()
			http:load(OpNotifyOfflineType.kWeeklyRace2020StartLevel) --必须在ReplayDataManager:onStartLevel之后
		end
	end
	...
	-- if _G.useNewAIAutoCheck then
	-- 	local log = {}
 --        log.method = "NewGameBoardLogic:onGameInit"
 --        ReplayDataManager:addMctsLogs("CC",log)
	-- end
	
end

function NewGameBoardLogic:getColorOfGameItem(r, c)--获取动物、水晶、gift、牢笼的颜色
	local color = 0
...
function NewGameBoardLogic:checkMatchQuick(r,c, color)	--快速检测如果r、c位置是color会怎样
...
----产生一个新的掉落数据
function NewGameBoardLogic:randomANewItemFallingData(r,c)
...
function NewGameBoardLogic:randomColor(forbidDropBuff)
	if not forbidDropBuff and self.dropBuffLogic and self.dropBuffLogic.dropBuffEnable then
		return self.dropBuffLogic:randomColor()
	end

	-- printx( 1 , "NewGameBoardLogic:randomColor ~~~~~~~~~~~~~~~~~~~ self.mapColorList:")
	-- for k , v in ipairs( self.mapColorList ) do
	-- 	printx( 1 , k ," : " , AnimalTypeConfig.convertColorTypeToIndex(v) )
	-- end

	local x = self.randFactory:rand(1,#self.mapColorList)

	-- printx( 1 , "self.randFactory:rand   x = " , x )
	return self.mapColorList[x]
end

function NewGameBoardLogic:randomColorByGlobalWeight()
	return self:_randomColorByWeight(self.mapColorWeightList)
end

function NewGameBoardLogic:_randomColorByWeight(colourWeightList, limitedColorList, forbidDropBuff)
	local function getRandomColourWithoutWeight()
		if limitedColorList and #limitedColorList > 0 then
			local idx = self.randFactory:rand(1, #limitedColorList)
			return limitedColorList[idx]
		else
			return self:randomColor(forbidDropBuff)
		end
	end

	if colourWeightList then
		local sumWeight = 0
		for i = 1, 6 do
			if colourWeightList and colourWeightList[i] then
				sumWeight = sumWeight + colourWeightList[i]
			end
		end

		local function getPickedColourIndex(randVal)
			local accuWeight = 0
			for cIdx = 1, 6 do
				if colourWeightList and colourWeightList[cIdx] then
					accuWeight = accuWeight + colourWeightList[cIdx]
					if randVal <= accuWeight then
						return cIdx
					end
				end
			end
		...
		local ranVal = self.randFactory:rand(1, sumWeight)
		local colourIndex = getPickedColourIndex(ranVal)
		-- printx(11, "_randomColorByWeight: sumWeight, ranVal", sumWeight, ranVal)
		-- printx(11, "result colour idx:", colourIndex)

		if colourIndex > 0 then
			return AnimalTypeConfig.colorTypeList[colourIndex]
		else
			return getRandomColourWithoutWeight()
		end
	else
		return getRandomColourWithoutWeight()
	end
...
function NewGameBoardLogic:isColorTypeValid(colorType)
	return AnimalTypeConfig.isColorTypeValid(colorType)
end

function NewGameBoardLogic:isSpecialTypeValid(specialType)
	return AnimalTypeConfig.isSpecialTypeValid(specialType)
end

function NewGameBoardLogic:getBoardMap()
	return self.boardmap
end

function NewGameBoardLogic:getItemMap()
	return self.gameItemMap
end

function NewGameBoardLogic:safeGetBoardData(r, c)
	if self.boardmap and self.boardmap[r] and self.boardmap[r][c] then
		return self.boardmap[r][c]
	end
	return nil
end

function NewGameBoardLogic:safeGetItemData(r, c)
	if self.gameItemMap and self.gameItemMap[r] and self.gameItemMap[r][c] then
		return self.gameItemMap[r][c]
	end
	return nil
end

function NewGameBoardLogic:isItemCanUsed(r,c)		--地图上可以被使用的地块
	if self.boardmap[r]
...
function NewGameBoardLogic:checkItemBlock(r,c)
	local item = self.gameItemMap[r][c];
	local board = self.boardmap[r][c];
...
function NewGameBoardLogic:setChainBreaked()
	self.chainBreaked = true
end

function NewGameBoardLogic:setTileMoved()
	self.tileMoved = true
end

----更新Block的状态来影响下落
function NewGameBoardLogic:updateFallingAndBlockStatus()
...
function NewGameBoardLogic:isItemCanMoved(r,c)		--地图上可以被移动的Item
...
function NewGameBoardLogic:isItemInTile(r, c)	-- 判断点击位置是否在棋盘有效位置中
...
--可以被交换，但是不一定有匹配
function NewGameBoardLogic:canBeSwaped(r1,c1,r2,c2)
...
function NewGameBoardLogic:canUseHammer(r, c)
	local item = self.gameItemMap[r][c]
	return item:canBeEffectByHammer(self)
end

function NewGameBoardLogic:canUseLineBrush(r, c)
...
function NewGameBoardLogic:canUseForceSwap(r1, c1, r2, c2)
...
function NewGameBoardLogic:canUseLineEffectOnGrid(r, c)
	local itemData = self.gameItemMap[r][c]
	return itemData:canBeEffectByLineEffectProp()
end

function NewGameBoardLogic:isNormal(item)
...
function NewGameBoardLogic:canUseRandomBird()
	for r=1, #self.gameItemMap do
		for c = 1, #self.gameItemMap[r] do
			local item = self.gameItemMap[r][c]
...
function NewGameBoardLogic:canUseBroom(r, c, isNewBroom)
	local canUseFlag = false

	if isNewBroom then
		if not self:isRowEmpty(r) then
			canUseFlag = true
		end
	else
		local rows = self:getBroomRows(r)
		if rows and #rows == 2 then
			canUseFlag = not (self:isRowEmpty(rows[1]) and self:isRowEmpty(rows[2]))
		end
	end
...
function NewGameBoardLogic:canUseOctopusForbid()
...
function NewGameBoardLogic:startTrySwapedItem(r1,c1,r2,c2 , callback)
...
function NewGameBoardLogic:startWaitingOperation()
	Notify:dispatch( "WaitingOperation" , { } )
...
function NewGameBoardLogic:stopWaitingOperation()
	self.isWaitingOperation = false
	self:onWaitingOperationChanged()
end

function NewGameBoardLogic:onWaitingOperationChanged()
	if self.isWaitingOperation == true then
		...
		if self.PlayUIDelegate then
			...
			self.PlayUIDelegate:checkUpdateDeactivatedProps()
...
----搞笑式移动两个方块----被绳子格挡住了
function NewGameBoardLogic:startTrySwapedItemFun(r1,c1,r2,c2)
	local gameBoardActionItem1 = IntCoord:create(r1,c1)
	local gameBoardActionItem2 = IntCoord:create(r2,c2)
	local theAction = GameBoardActionDataSet:createAs(
		GameActionTargetType.kGameBoardAction,		--动作发起主体	--能够进行普通交换ard为主体
...
--添加一个新的动作到队列里面
function NewGameBoardLogic:addGameAction(theAction)
	self:addActionToList(theAction, self.gameActionList)
end

function NewGameBoardLogic:addDestructionPlanAction(theAction)
	self:addActionToList(theAction, self.destructionPlanList)
end

function NewGameBoardLogic:addStableDestructionPlanAction(theAction)
	self:addActionToList(theAction, self.stableDestructionPlanList)
end

function NewGameBoardLogic:addDestroyAction(theAction)
	self:addActionToList(theAction, self.destroyActionList)
end

function NewGameBoardLogic:addFallingAction(theAction)
	self:addActionToList(theAction, self.fallingActionList)
end

function NewGameBoardLogic:addSwapAction(theAction)
	self:addActionToList(theAction, self.swapActionList)

	self.fsm:onStartSwap()
end

function NewGameBoardLogic:addPropAction(theAction)
	self:addActionToList(theAction, self.propActionList)
	SnapshotManager:stop()
end

function NewGameBoardLogic:addGlobalCoreAction(theAction)
	self:addActionToList(theAction, self.globalCoreActionList)
end

local totalActIndex = 1 
function NewGameBoardLogic:addActionToList(theAction, theList , type)
	--[[
	...
	printx( 1 , "   NewGameBoardLogic:addActionToList  " .. type .. "    actCount = " , actCount)
...
function NewGameBoardLogic:addNeedCheckMatchPoint(r, c)
	table.insert(self.needCheckMatchList, { r = r, c = c })
end

function NewGameBoardLogic:cleanNeedCheckMatchList()
	self.needCheckMatchList = nil
	self.needCheckMatchList = {}
end

function NewGameBoardLogic:getGamePlayType(...)
	assert(#{...} == 0)
...
--交换两个Item--并且尝试匹配消除
function NewGameBoardLogic:SwapedItemAndMatch(r1,c1,r2,c2, doSwap)	--doSwap==true表示确实进行交换，并且引起相应效果，doSwap==false表示仅仅判断是否能够交换
...
function NewGameBoardLogic:getSaveDataForRevert()
	local ret = SaveRevertData:create(self)
	return ret
end

function NewGameBoardLogic:getRevertPropInfo( ... )
	if self.saveRevertData then
		return self.saveRevertData:getUsePropInfo()
	end
end

function NewGameBoardLogic:setRevertGiftInfo(itemId,key)
	if self.saveRevertData then
		self.saveRevertData:addGiftInfo({ itemId=itemId,key=key })
	end
end

function NewGameBoardLogic:getRevertGiftInfos( ... )
	if self.saveRevertData then
...
------返回魔力鸟非主动触发时消除的颜色类型------
function NewGameBoardLogic:getBirdEliminateColor()
...
function NewGameBoardLogic:getPosListOfColor(theColor)
	local posList = {}
	local count = 0
...
function NewGameBoardLogic:getPosListOfItemType(ptype, noColor)--返回ItemType相同的坐标
	local posList = {}
	local count = 0
...
----使用某个道具
function NewGameBoardLogic:canUseProps()
...
function NewGameBoardLogic:revertProps( ... )
...
function NewGameBoardLogic:setBrushType(animalType)
	self.forceBrushAnimalType = animalType
	self.boardView:setBrushType(animalType)
end

function NewGameBoardLogic:useProps(propsType, r1, c1, r2, c2, propsUseType, isGuideRefresh, noReplayRecord, noSectionResumeRecord)
	if _G.isLocalDevelopMode then printx( 1 , "   NewGameBoardLogic:useProps   " , propsType, r1, c1, r2, c2,propsUseType, isGuideRefresh, noReplayRecord) end
	--printx( 1 , "NewGameBoardLogic:useProps  " , debug.traceback() )
	if self.boardView then
		self.boardView:stopOldSelectEffect()
	end
	...
	if self:getActionListCount() == 0 then
		...
		if propsType == GamePropsType.kRefresh 
			...
			then
			-- printx( 1 , "GameBoardLogic:useProps  tryRefresh")
			if RefreshItemLogic.tryRefresh(self, isGuideRefresh) then
				-----成功了----
				-- printx( 1 , "GameBoardLogic:useProps  runRefreshAction")
		...
		elseif propsType == GamePropsType.kAdd5TravelMode then
			if TravelLogic:triggerTravelAddStepSkill() then
				self.fsm:changeState(self.fsm.fallingMatchState)
			else

			end
		elseif propsType == GamePropsType.kAdd5WeeklyRace2020 or propsType == ItemType.TIMELIMIT_ADD_FIVE_STEP_BY_WEEKLY_RACE_2020 then
			WeeklyRace2020Logic:onWeeklyRace2020AddFive(self)
		elseif propsType == GamePropsType.kAdd5AngryBirds or propsType == GamePropsType.kAdd2AngryBirds then
			local isAddTwoSteps = (propsType == GamePropsType.kAdd2AngryBirds)
			AngryBirdLogic:onAngryBirdAddMoves(self, isAddTwoSteps)
		...
		elseif propsType == GamePropsType.kAdd10Autumn2020 then
			self.theCurMoves = self.theCurMoves + 10
			self.hasAddMoveStep = { source = "Prop" , steps = 10}
		...
		elseif propsType == GamePropsType.kAdd15Autumn2020 then
		...
		elseif propsType == GamePropsType.kAdd5
			or propsType == GamePropsType.kBombAdd5
			then
		...
		elseif propsType == GamePropsType.kAdd2 then

			self.theCurMoves = self.theCurMoves + 2
			self.hasAddMoveStep = { source = "Prop" , steps = 2 }
			
			if self.PlayUIDelegate and (not self.PlayUIDelegate.isDisposed) then
				self.PlayUIDelegate:useStepPropComplete(self.theCurMoves, nil, 2)
			end

			if self.gameMode:is(SpringHorizontalEndlessMode) then
				self.gameMode:logAddStep(2)
			end
		...
		elseif propsType == GamePropsType.kSwap 
			...
			if not r1 or not r2 or not c1 or not c2 then
				...
				SnapshotManager:removeLastUsePropAction()
				return false
			end
		...
		elseif propsType == GamePropsType.kRandomBird then
			...
			self:addPropAction(action)
			self.fsm:changeState(self.fsm.usePropState)
		elseif propsType == GamePropsType.kBroom 
			or propsType == GamePropsType.kBroom_l 
			or propsType == GamePropsType.kBroom_new 
			then

			local isNewBroom = false
			if propsType == GamePropsType.kBroom_new then 
				isNewBroom = true 
			end

			local rows = self:getBroomRows(r1, propsType)
			local action = GameBoardActionDataSet:createAs(
			        GameActionTargetType.kPropsAction, 
		...
		elseif propsType == GamePropsType.kMoleWeeklyRaceSPProp then
			...
			self:addPropAction(action)
			self.fsm:changeState(self.fsm.usePropState)
		elseif propsType == GamePropsType.kWeeklyRace2020SPProp then
			local action = GameBoardActionDataSet:createAs(
		        GameActionTargetType.kPropsAction, 
		        GamePropsActionType.kWeeklyRace2020SPProp,
		        nil, 
		        nil, 
		        GamePlayConfig_MaxAction_time)
			-- action.targetList, action.throwAddStepAmount, action.throwColourAmount, action.hpDamage = MoleWeeklyRaceLogic:getMoleSpecialPropsDatas(self)
		...
		elseif propsType == GamePropsType.kRowEffect 
			...
			self:addPropAction(action)
			self.fsm:changeState(self.fsm.usePropState)
		elseif propsType == GamePropsType.kGetTempProp 
			then
			local action = GameBoardActionDataSet:createAs(
		        GameActionTargetType.kPropsAction, 
		        GamePropsActionType.kGetTempProp,
		        IntCoord:create(r1,c1), 
		        nil, 
		        GamePlayConfig_MaxAction_time)
			action.tempInfo = r1
		...
		if (not self.replaying) 
			...
			or (self.replayMode == ReplayMode.kMcts and _G.launchCmds.domain and not _G.AIAutoCheckReplayCheck) then
			-- SnapshotManager:stop()
		...
		if self.replaying then
			if self.replayMode == ReplayMode.kMcts and _G.AutoCheckLeakInLevel then
				--propsType, r1, c1, r2, c2, propsUseType
				local str = "NewGameBoardLogic:useProps  propsType:" .. tostring(propsType) .. "   propsUseType:" .. tostring(propsUseType) .. " --> " .. tostring(r1)..tostring(c1)..tostring(r2)..tostring(c2)
				self.PlayUIDelegate:replayResult(
									self.PlayUIDelegate.levelId, 
									self.totalScore, 
									self.gameMode:getScoreStarLevel(), 
									0, 0, 0, false, AutoCheckLevelFinishReason.kUsePropWhenActionRunning , str )
			else
				setTimeOut( function () 
					self:Replay()
				end , 1 )
...
function NewGameBoardLogic:checkUpdateLevelDifficultyAdjustByUseProp( propsUseType , propId )
	--printx( 1 , "NewGameBoardLogic:checkUpdateLevelDifficultyAdjustByUseProp ~~~~~~~~!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" , propsUseType , propId )

	-- RemoteDebug:uploadLog( "NewGameBoardLogic" ,  "checkUpdateLevelDifficultyAdjustByUseProp  propsUseType" , propsUseType , 
	...
	if self.replayMode == ReplayMode.kSectionResume and self.replaying then
		...
		--RemoteDebug:uploadLog( "NewGameBoardLogic" ,  "checkUpdateLevelDifficultyAdjustByUseProp  111"  )
		if propsUseType == UsePropsType.NORMAL or propsUseType == UsePropsType.EXPIRE 
			...
			--RemoteDebug:uploadLog( "NewGameBoardLogic" ,  "checkUpdateLevelDifficultyAdjustByUseProp  222" ,LevelDifficultyAdjustManager.DAManager:getIsSatisfyPreconditions() )
			if LevelDifficultyAdjustManager:getDAManager():getIsSatisfyPreconditions() then
				--RemoteDebug:uploadLog( "NewGameBoardLogic" ,  "checkUpdateLevelDifficultyAdjustByUseProp  333" , LevelDifficultyAdjustManager:getDAManager():getIsPayUser()  )
...
function NewGameBoardLogic:isVirtualPropType(propType)
	if propType == kSpringPropItemID 
		...
		or propType == GamePropsType.kWeeklyRace2020SPProp
 		then
		 return true
	end
	return false
...
function NewGameBoardLogic:isUsePropsValid(propsType, r1, c1, r2, c2)
	if not propsType then return false end
	if self:getActionListCount() == 0 then
...
function NewGameBoardLogic:isRowEmpty(rowNum)
	local gameItemMap = self.gameItemMap
	local isEmpty = true
...
function NewGameBoardLogic:getBroomRows(r, propsType)
	local rowList = {}
	...
	if propsType == GamePropsType.kBroom_new then
		local upperRow = r - 1
		if upperRow > 0 then
			 table.insert(rowList, upperRow)
		end
		table.insert(rowList, r)
		local lowerRow = r + 1
		if lowerRow <= rowAmount then
			 table.insert(rowList, lowerRow)
		end
	else
		local r1 = r
		local r2 = r1 - 1
		if r2 < 1 or (self:isRowEmpty(r2) and r1 < rowAmount) then
			r2 = r1 + 1
		end
	    ...
	    table.insert(rowList, r1)
	    table.insert(rowList, r2)
	end
...
----使用一次移动
function NewGameBoardLogic:UseMoves()
...
--end
--performance:finish("NewGameBoardLogic:updateMoveTilesOnUseMove")
end

--开始移动回合前，重置所有步数稳定处理搜需要的标识位及数据
function NewGameBoardLogic:resetStepRecords()
	self.isInStep = true 
...
MACRO_DEV_START()
--此方法有真假两版，此处是假的，真的在LocalBox.lua
MACRO_DEV_END()

function NewGameBoardLogic:tryDoOrderList(r, c, key1, key2, v1, rotation, p_pos)
...
function NewGameBoardLogic:addReplayStep( item )
	-- body
...
function NewGameBoardLogic:ReadReplay(fileName, base)
	if not base then
...
--关闭replay
function NewGameBoardLogic:setWriteReplayOff(  )
	-- body
	self.setWriteReplayEnable = false
end

function NewGameBoardLogic:getReplayRecordsData()
	-- printx( 1 , "   NewGameBoardLogic:getReplayRecordsData ")
...
function NewGameBoardLogic:WriteReplay(fileName, base)
	--[[
	if not base then
		base = CCFileUtils:sharedFileUtils():fullPathForFilename("resource")
	end
...
function  NewGameBoardLogic:copyStep()
	...
	CCDirector:sharedDirector():setClipboard(table.serialize( replayData ))
end

function NewGameBoardLogic:setSnapshotModeEnable()
	self.snapshotModeEnable = true
	self.snapshotId = 1
end

function NewGameBoardLogic:ReplayStart(playMode)
...
function NewGameBoardLogic:countReplayStep()
	if self.replayStep then
		self.replayStep = self.replayStep + 1
	end
end

function NewGameBoardLogic:isReplayMode()
	return self.PlayUIDelegate and self.PlayUIDelegate:isReplayMode()
end

function NewGameBoardLogic:replayDoUseProp( r1, c1, r2, c2, propId , propsUseType , para )
	--self:startWaitingOperation()

	local usePropResult = nil

	if para then
		if para.lineBrushType then
			if para.lineBrushType == "H" then
				self.forceBrushAnimalType = AnimalTypeConfig.kLine
			elseif para.lineBrushType == "V" then
				self.forceBrushAnimalType = AnimalTypeConfig.kColumn
			end
		end
	end
	...
	elseif propId == GamePropsType.kSpringFirework 
		or propId == GamePropsType.kMoleWeeklyRaceSPProp 
		or propId == GamePropsType.kWeeklyRace2020SPProp 
		then
	...
	if self.PlayUIDelegate.replayMode == ReplayMode.kResume 
		...
		then
		if self.PlayUIDelegate.replayData.showMode then
			local propList = self.PlayUIDelegate.propList
			local itemFound, itemIndex = propList:findItemByItemID(propId)
		    if itemFound then
		    	-- printx( 1 , "????????????????????????  itemFound:increaseTemporaryItemNumber " , propId )
		    	local temPropId = propId
		    	for k,v in pairs(TempPropMap) do
		    		if v == propId then
		    			temPropId = k
		    			break
		    		end
		    	...
		    	itemFound:increaseTemporaryItemNumber(temPropId, -1)
		    else
		    end
		else
	...
	return usePropResult
end

function NewGameBoardLogic:Replay()

	-- if _G.useNewAIAutoCheck then
	-- 	local log = {}
 --        log.method = "NewGameBoardLogic:Replay"
 --        ReplayDataManager:addMctsLogs("CC",log)
	-- end
	...
	--printx(1 , "NewGameBoardLogic:Replay   self.replayStep = " , self.replayStep)
	if self.replayError then -- 发生了错误
		--printx(1 , "NewGameBoardLogic:Replay   111" )
		self.replaying = false
		...
		local function doUseProp(r1, c1, r2, c2, propId , propsUseType)
			self:replayDoUseProp( r1, c1, r2, c2, propId , propsUseType )
		end
		...
		local function doNextStep()
			...
			local switchPlayer = self.replaySteps[self.replayStep].switchPlayer

			--printx(1 , "NewGameBoardLogic:Replay   333" )
            if not propId and not switchPlayer then
				--printx(1 , "NewGameBoardLogic:Replay   444" )
			...
			elseif switchPlayer then
				GoldenPodBattleLogic:switchPlayer(self)
			else
				--printx(1 , "NewGameBoardLogic:Replay   555" )
				if self.replayMode == ReplayMode.kStrategy then 
		...
		local function doIndexSwap(index, needCheck)
			local rowAmount, colAmount = GameBoardUtil:getRowAndColAmountOfBoard(self)
			local swapChangeIndex = (colAmount - 1) * rowAmount --横向交换向竖向交换切换的Index
			-- Index从0开始，总Index数：(colAmount - 1) * rowAmount + (rowAmount - 1) * colAmount - 1

			local r1,c1,r2,c2
			if index >= swapChangeIndex then
				index = index - swapChangeIndex
				r1 = math.floor(index / colAmount) + 1
				c1 = index % colAmount + 1
				r2 = r1 + 1
				c2 = c1
			else
				r1 = math.floor(index / (colAmount - 1)) + 1
				c1 = index % (colAmount - 1) + 1
				r2 = r1
				c2 = c1 + 1
			end
			...
			-- local data = {}
			-- data.method = "doIndexSwap"
			-- data.idx = index
			-- data.r1 = r1
			-- data.c1 = c1
			-- data.r2 = r2
			-- data.c2 = c2
			-- ReplayDataManager:addMctsLogs( 6 , data )


			if needCheck then
				local errorMsg = "step " .. tostring(self.realCostMove) .. "   " .. tostring(r1) .. tostring(c1) .. " --> " .. tostring(r2)..tostring(c2)
				if not self:SwapedItemAndMatch(r1, c1, r2, c2 , false ) then
					return errorMsg
				end				
			end
		...
		local function getActions()
			...
			for i = 1, #possibleSwapList do 
				...
				if r1 == r2 then
					if c1 < c2 then
						index = (r1 - 1) * (colAmount - 1) + c1 - 1
					else
						index = (r2 - 1) * (colAmount - 1) + c2 - 1
					end
				end
				if c1 == c2 then
					if r1 < r2 then
						index = (r1 - 1) * colAmount + c1 - 1
					else
						index = (r2 - 1) * colAmount + c2 - 1
					end
					index = index + swapChangeIndex
				end
				actions[index] = true
		...
		if self.replayMode == ReplayMode.kSectionResume then
			...
			local function getSectionData()
				if self.PlayUIDelegate and self.PlayUIDelegate.sectionData then
					local sectionData = SectionResumeManager:decodeByTable(self.PlayUIDelegate.sectionData)
					return sectionData
				end
				return nil
			end
			...
			local function startRevert()
				local dorevertResult = dorevert( swapResultFun )
				if dorevertResult == 2 then
					--last action is swap
				else
					if dorevertResult == 0 then --dorevertResult == 0时棋盘不再进入waiting state
						SmartGuideManager.getInstance():dorevertResult() 
						NationalDayBunnyLogic:checkNDBunnyGuide(self)
					end
					replayEnd()
				end
				
				self:updateGlobalScoreBuffView()
			end

			local function onScrollComplete()
				-- self.fsm:initState()
				-- self:startWaitingOperation()
				-- printx( 1 , "Replay WTGFFFFFFFF  7")
				startRevert()
			end

			-- printx( 1 , "Replay WTGFFFFFFFF  1")
			-- currTravelMapIndex
			if self.theGamePlayType == GameModeTypeId.TRAVEL_MODE_ID then

				local xmas2019Data = self.PlayUIDelegate.replayData.xmas2019Data

				if xmas2019Data then

					if xmas2019Data.currTravelMapIndex == 1 then
						-- printx( 1 , "Replay WTGFFFFFFFF  2")
						startRevert()
					else
						local nextLevelID = xmas2019Data.levelList[ xmas2019Data.currTravelMapIndex or 1 ]
						if nextLevelID then
							-- printx( 1 , "Replay WTGFFFFFFFF  3")
							self.PlayUIDelegate:inLevelChangeBoard( nextLevelID , onScrollComplete , true )
						else
							-- printx( 1 , "Replay WTGFFFFFFFF  4")
							startRevert()
						end
					end
				else
					-- printx( 1 , "Replay WTGFFFFFFFF  5")
					startRevert()
				end
			elseif self.theGamePlayType == GameModeTypeId.WEEKLY_RACE_2020_MODE_ID then
				-- printx(11, "=========== start revert WeeklyRace2020 Level", self.level)

				local weeklyRace2020LevelData = self.PlayUIDelegate.replayData.weeklyRace2020LevelData
				local sectionData = getSectionData()
				-- printx(11, "=== weeklyRace2020LevelData, sectionData?", weeklyRace2020LevelData, sectionData)
				if weeklyRace2020LevelData and sectionData then
					local travelMapIndex = sectionData.currTravelMapIndex
					if travelMapIndex == 1 then
						-- printx(11, "=== currTravelMapIndex = 1, start revert directly.")
						startRevert()
					else
						if sectionData.level and type(sectionData.level) == "number" and sectionData.level > 0 then
							-- printx(11, "=== resume to currentMap, levelID:", sectionData.level)
							self.PlayUIDelegate:inLevelChangeBoard( sectionData.level , onScrollComplete , true )
						else
							-- printx(11, "=== no levelID of current map, start revert directly.")
							startRevert()
						end
					end
				else
					-- printx(11, "=== no weeklyRace2020LevelData or sectionData ...")
					startRevert()
				end
			elseif self.theGamePlayType == GameModeTypeId.ANGRY_BIRD_MODE_ID then
				local sectionData = getSectionData()
				
				if sectionData.currTravelMapIndex then
					if sectionData.currTravelMapIndex == 1 then
						startRevert()
					else
						self.PlayUIDelegate:inLevelChangeBoard( self.level+sectionData.currTravelMapIndex-1 , onScrollComplete , true )
					end
				else
					startRevert()
				end
			else
				-- printx( 1 , "Replay WTGFFFFFFFF  6")
				startRevert()
			end
		...
		--printx(1 , "NewGameBoardLogic:Replay   222" )
		...
		if self.replayMode == ReplayMode.kMcts then
			...
			if _G.useNewAIAutoCheck then
				...
				------------------------- AutoCheckLevelFinishReason.kUnexpectedStoped -----------------------------------
				local timeThreshold = 600

				local function unresponsiveCheck()
					local nowTime = Localhost:timeInSec()

					if nowTime > self.lastReplayTime and nowTime - self.lastReplayTime > (timeThreshold - 20) then

						local failReason = AutoCheckLevelFinishReason.kUnexpectedStoped 
						self.mctsReplayError = failReason
					end
				end

				if self.mctsReplayError then
					if self.mctsReplayError == AutoCheckLevelFinishReason.kUnexpectedStoped then
						if self.PlayUIDelegate then
							self.PlayUIDelegate:replayResult(
												self.PlayUIDelegate.levelId, 
												self.totalScore, 
												self.gameMode:getScoreStarLevel(), 
												0, 0, 0, false, self.mctsReplayError )
							return
						end
					end
				end

				self.lastReplayTime = Localhost:timeInSec()
				...
				self.replayUnresponsiveCheckTimerId = setTimeOut( unresponsiveCheck , timeThreshold )

				if (not _G.AIAutoCheckReplayCheck) and AIGamePlayManager.fixedStepData and self.realCostMove + 1 <= #AIGamePlayManager.fixedStepData then
					local obj = AIGamePlayManager.fixedStepData[ self.realCostMove + 1 ]
					if self:SwapedItemAndMatch( obj.r1 , obj.c1 , obj.r2 , obj.c2 , false ) then
						doSwap( obj.r1 , obj.c1 , obj.r2 , obj.c2 , nil )
					else
						local str = "step " .. tostring(self.realCostMove) .. "   " .. tostring(obj.r1) .. tostring(obj.c1) .. " --> " .. tostring(obj.r2)..tostring(obj.c2)
						self.PlayUIDelegate:replayResult(
											self.PlayUIDelegate.levelId, 
											self.totalScore, 
											self.gameMode:getScoreStarLevel(), 
											0, 0, 0, false, AutoCheckLevelFinishReason.kUnexpectedSwap , str )
					end
				else
					local function onGetNextStep( result , nextStepCmd )

						if _G.AIAutoCheckReplayCheck and not result then

							AIGamePlayManager:endLevelByErrorAndReboot( 
								AutoCheckLevelFinishReason.kReplayFinButNotOver , 
								nil ,
								self.totalScore, 
								self.gameMode:getScoreStarLevel()
							)
						...
						if nextStepCmd then
							...
							if nextStepCmd.method == "nextStep" then
								if nextStepCmd.dataType == "rc" then

									local r1 = nextStepCmd.action.r1
									local c1 = nextStepCmd.action.c1
									local r2 = nextStepCmd.action.r2
									local c2 = nextStepCmd.action.c2

         							if math.random() > 0.5 then
         								r1 = nextStepCmd.action.r2
										c1 = nextStepCmd.action.c2
										r2 = nextStepCmd.action.r1
										c2 = nextStepCmd.action.c1
         							end

									if self:SwapedItemAndMatch( r1, c1, r2, c2, false ) then
										-- ReplayDataManager:addMctsLogs( "onGetNextStep" , {method="onGetNextStep  222  " } )
										doSwap( r1, c1, r2, c2,nil)
									else
										-- ReplayDataManager:addMctsLogs( "onGetNextStep" , {method="onGetNextStep  333  " } )
										local str = "step " .. tostring(self.realCostMove) .. "   " .. tostring(r1) .. tostring(c1) .. " --> " .. tostring(r2)..tostring(c2)
										self.PlayUIDelegate:replayResult(
															self.PlayUIDelegate.levelId, 
															self.totalScore, 
															self.gameMode:getScoreStarLevel(), 
								...
								elseif nextStepCmd.dataType == "switchPlayer" then
									GoldenPodBattleLogic:switchPlayer(self)	--瞎写一下，不要在意
								elseif nextStepCmd.dataType == "function" then
									nextStepCmd.action()
								else
									if nextStepCmd.newSeed then
										self.randFactory:randSeed(nextStepCmd.newSeed)
									end
									local errorMsg = doIndexSwap(nextStepCmd.action, true)
									if errorMsg then
										self.PlayUIDelegate:replayResult(
											self.PlayUIDelegate.levelId, 
											self.totalScore, 
											self.gameMode:getScoreStarLevel(), 
											0, 0, 0, false, AutoCheckLevelFinishReason.kUnexpectedAISwap , errorMsg )
										return
									end

								end
							else
								he_log_error("MCT_" .. AIGamePlayManager:getVersion() .. " 123 ERROR  you must send nextStep now!")
							end
						else
							he_log_error("MCT_" .. AIGamePlayManager:getVersion() .. "  ERROR  nextStepCmd is nil !")
						end
					end

					if _G.AIAutoCheckReplayCheck then
						AIGamePlayManager:getNextStep( self , nil , onGetNextStep )
					else
						AIGamePlayManager:getNextStep( self , getActions() , onGetNextStep )
					end
				end
			...
			if not self.playout then
				...
				if not _G.launchCmds.mock then
					...
					if  _G.__startCmd then
						...
						-- ReplayDataManager:addMctsLogs( 2 , req)
				...
				-- ReplayDataManager:addMctsLogs( 3 , cmd )
			...
			if self.playout then
				if _G.launchCmds.cnn then
					...
					state = mime.b64(state)
					req.snap = state
					-- ReplayDataManager:addMctsLogs( 4 , req)
					StartupConfig:getInstance():sendMsg(simplejson.encode(req))
			        resp = StartupConfig:getInstance():receiveMsg()
			        local cmd = simplejson.decode(resp)
			        -- ReplayDataManager:addMctsLogs( 5  , cmd)
			        doIndexSwap(cmd.action)
			        return
			    end
		...
		if self.replayStep <= #self.replaySteps then
			...
			--self:startWaitingOperation()
			--printx(1 , "NewGameBoardLogic:Replay   666" )
			if self.replayMode == ReplayMode.kStrategy then 
				if self.PlayUIDelegate and type(self.PlayUIDelegate.onReplayEnd) == "function" then
					self.PlayUIDelegate:onReplayEnd()
				end
			...
			elseif (self.gameMode:is(ClassicMode) or self.gameMode:is(MaydayEndlessMode)) and self.replayMode ~= ReplayMode.kResume then
				--printx(1 , "NewGameBoardLogic:Replay   777" )
				self:setGamePlayStatus(GamePlayStatus.kEnd)
			else
				--printx(1 , "NewGameBoardLogic:Replay   888" )
...
function NewGameBoardLogic:checkReplayCanUseAddStep()
	-- printx( 1 , "NewGameBoardLogic:checkReplayCanUseAddStep !!!!!!!!!!!!!" )
	...
	if self.replaySteps and self.replaySteps[self.replayStep] then
		...
		for i=self.replayStep, maxStep do
			...
			if data.prop and (tonumber(data.prop) == GamePropsType.kAdd5 or 
								...
								tonumber(data.prop) == ItemType.TIMELIMIT_ADD_2_STEP or 
								tonumber(data.prop) == ItemType.TIMELIMIT_48_ADD_2_STEP or 
								tonumber(data.prop) == ItemType.ADD_15_STEP or
								tonumber(data.prop) == ItemType.ADD_FIVE_STEP_BY_ANGRY_BIRDS
							) 
			then

				-- printx( 1 , "NewGameBoardLogic:checkReplayCanUseAddStep i =" , i , "maxStep =" , maxStep , "data =" , table.tostring(data) )
...
function NewGameBoardLogic:checkReplayCanUseResurrectProp()
	if self.replayMode == ReplayMode.kSectionResume then
		return false
	end
	...
	if self.replaySteps and self.replaySteps[self.replayStep] then
		...
		for i=self.replayStep, maxStep do
			local data = self.replaySteps[i]
			if data.prop and (tonumber(data.prop) == ItemType.Autumn2020_Resurrect) 
			then			
				return true, data.prop
			end
		end
...
function NewGameBoardLogic:getGameItemPosInView_ForPreProp(r, c)
...
function NewGameBoardLogic:getGameItemPosInView(r, c)
	self.posAdd = self.posAdd or ccp(0, 0)
	local tempX = (c - 0.5) * GamePlayConfig_Tile_Width
	local tempY = (GamePlayConfig_Max_Item_Y - r - 0.5 ) * GamePlayConfig_Tile_Height
	local visibleOrigin = CCDirector:sharedDirector():getVisibleOrigin()
...
function NewGameBoardLogic:getGameItemPosInViewByOffset(r, c, offset)
	self.posAdd = self.posAdd or ccp(0, 0)
	local tempX = (c - 0.5) * GamePlayConfig_Tile_Width
	local tempY = (GamePlayConfig_Max_Item_Y - r - 0.5 ) * GamePlayConfig_Tile_Height
	local visibleOrigin = CCDirector:sharedDirector():getVisibleOrigin()
	return self.boardView:convertToWorldSpace(ccp(tempX+offset.x, tempY+offset.y))
end

-- 确定两个方块是否相邻
function NewGameBoardLogic:tileNextToEachOther(r1, c1, r2, c2)
...
function NewGameBoardLogic:isPosUsedByPreProp(x, y)
	local index = x*10+y
	return self.pre_prop_pos[index] == true
end

function NewGameBoardLogic:usePosForPreProp(x, y)
	local index = x*10+y
	self.pre_prop_pos[index] = true
end

function NewGameBoardLogic:preGameProp(propID, animCallback)
	...
	elseif propID == GamePropsType.kAdd2_b then
		self.gameMode:getAddSteps(2)
	end
end

function NewGameBoardLogic:isPosValid(r, c)
	return r > 0 and r <= #self.gameItemMap and c > 0 and c <= #self.gameItemMap[r] 
end

function NewGameBoardLogic:getGameItemAt(r, c)
	if self:isPosValid(r, c) then
		return self.gameItemMap[r][c]
	end
	return nil
...
function NewGameBoardLogic:getIngredientsOnScreen( ... )
	-- body
	local result = 0
	for r = 1, #self.gameItemMap do 
		for c = 1, #self.gameItemMap[r] do 
...
function NewGameBoardLogic:getItemAmountByItemType(itemType, specialType, otherInfo, bIncludeDoubleSide)
	local function getCount(item, itemType, specialType, otherInfo)
		if item.ItemType == itemType then 
			if otherInfo then
				...
				elseif item.ItemType == GameItemType.kWater then
					if tostring(item.waterLevel) == tostring(otherInfo) then
						return 1
					end
				end
...
function NewGameBoardLogic:getFurballAmout( furballType )
	-- body
	local result = 0
	for r = 1, #self.gameItemMap do 
		for c = 1, #self.gameItemMap[r] do 
...
function NewGameBoardLogic:testEmpty()
	if true then return end
	-- printx( 1 , "   ======================================================")
	-- printx( 1 , "   ======================================================")
	-- printx( 1 , "   ============   NewGameBoardLogic:testEmpty   ============")
	-- printx( 1 , "   ======================================================")
	-- printx( 1 , "   ======================================================")
...
function NewGameBoardLogic:testItem()
	local r = 3
	local c = 2
	local item = self.gameItemMap[r][c]
...
function NewGameBoardLogic:quitLevel()
	local replayData = self:getReplayRecordsData()
	if replayData then
		replayData.sectionData = nil
		replayData.lastSectionData = nil
	end
	local opLog = replayData--table.serialize(replayData) 
...
function NewGameBoardLogic:getForbiddenOctopus()
	local result = {}
	for r = 1, #self.gameItemMap do
		for c = 1, #self.gameItemMap[r] do 
			local item = self.gameItemMap[r][c]
...
function NewGameBoardLogic:hasItemOfType(itemType)
	for r = 1, #self.gameItemMap do
		for c = 1, #self.gameItemMap[r] do 
			local item = self.gameItemMap[r][c]
			if item.ItemType == itemType then
...
function NewGameBoardLogic:hasItemOfTypeFromDigItemMap( itemType )
	if self.digItemMap and #self.digItemMap > 0 then
		for r = 1, #self.digItemMap do
			for c = 1, #self.digItemMap[r] do 
				local item = self.digItemMap[r][c]
				if item.ItemType == itemType then
					return true
				end
			end
...
function NewGameBoardLogic:getItemAmountOfType(itemType, isFurball, isGhost)
	local itemAmount = 0
	local item
	for r = 1, #self.gameItemMap do
		for c = 1, #self.gameItemMap[r] do 
...
function NewGameBoardLogic:areaDevidedByRope(x, y, xEnd, yEnd)
...
function NewGameBoardLogic:getPositionForRandomBird()
	local list = {}
...
function NewGameBoardLogic:getMoleWeeklyBossData()
	return self.moleBossData
end

function NewGameBoardLogic:getLevelTargetGlobalPosition(id)
	local position = nil
...
function NewGameBoardLogic:getHalloweenBossLevelTargetPosition()
...
function NewGameBoardLogic:onMoleWeeklyBossDie(diePosition)
...
function NewGameBoardLogic:addDigJewelCountWhenBossDie(addCount , fromPosition)
...
function NewGameBoardLogic:initMoleWeeklyBoss(bossConfig)
...
function NewGameBoardLogic:updateAllMagicTiles(boardmap)
...
function NewGameBoardLogic:useMegaPropSkill(notUseEnergy, noReplayRecord, updateEnergyView, noSectionResumeRecord)
	if not notUseEnergy then 
		...
		-- 道具大招消耗能量在useProp之前处理，新的能量在下一个section中才会被存储，
		-- 若是在【点击使用】--【在下一个waitingState】之间闪退，则道具大招的能量无法被清除
		-- 故在清除能量后主动存储一次断面

		-- 断面恢复之父建议，直接存储一个完整断面，而不是修改上一个断面中的数值
		-- “理论上，存储任何一个稳定状态的断面都是安全合法的。” 他扶了扶口罩，说道。
		SectionResumeManager:addSection()
		ReplayDataManager:updateCurrSectionDataToReplay()

		-- local sectionData = SectionResumeManager:getCurrSectionData()
		-- if sectionData then
		-- 	sectionData.fireworkEnergy = self.fireworkEnergy
		-- 	ReplayDataManager:updateCurrSectionDataToReplay()
		-- end
	end
	...
	elseif self.levelType == GameLevelType.kWeeklyRace2020 then
		return self:useProps(GamePropsType.kWeeklyRace2020SPProp, nil, nil, nil, nil, nil, nil, noReplayRecord, noSectionResumeRecord)
...
function NewGameBoardLogic:chargeFirework(count, r, c)
...
function NewGameBoardLogic:onProduceQuestionMark(r, c)
    self.firstProduceQuestionMark = true
end

-- for new dc use
function NewGameBoardLogic:getStageIndex()
	return self.gameMode:getStageIndex()
end

-- for new dc use
function NewGameBoardLogic:getStageMoveLimit()
	return self.gameMode:getStageMoveLimit()
end

function NewGameBoardLogic:hasChainInNeighbors(r1, c1, r2, c2)
...
-- 两格间的冰柱削减一层
function NewGameBoardLogic:decChainBetween(r1, c1, r2, c2)
	if not self:isPosValid(r1, c1) or not self:isPosValid(r2, c2) then
		return
	end
	...
	local deltaR = r2 - r1
	local deltaC = c2 - c1
	local board1 = self.boardmap[r1][c1]
	local board2 = self.boardmap[r2][c2]
	if deltaC == 1 then
		MatchCoverLogic:doEffectChainsAtPosWithDirs(self, r1, c1, {ChainDirConfig.kRight})
		MatchCoverLogic:doEffectChainsAtPosWithDirs(self, r2, c2, {ChainDirConfig.kLeft})
	elseif deltaC == -1 then
		MatchCoverLogic:doEffectChainsAtPosWithDirs(self, r1, c1, {ChainDirConfig.kLeft})
		MatchCoverLogic:doEffectChainsAtPosWithDirs(self, r2, c2, {ChainDirConfig.kRight})
	elseif deltaR == 1 then
		MatchCoverLogic:doEffectChainsAtPosWithDirs(self, r1, c1, {ChainDirConfig.kDown})
		MatchCoverLogic:doEffectChainsAtPosWithDirs(self, r2, c2, {ChainDirConfig.kUp})
	elseif deltaR == -1 then
		MatchCoverLogic:doEffectChainsAtPosWithDirs(self, r1, c1, {ChainDirConfig.kUp})
		MatchCoverLogic:doEffectChainsAtPosWithDirs(self, r2, c2, {ChainDirConfig.kDown})
	end
end

function NewGameBoardLogic:isTheSameMatchData(r1, c1, r2, c2)
...
function NewGameBoardLogic:hasRopeInNeighbors(r1, c1, r2, c2, isSwapCheck)
	if not self:isPosValid(r1, c1) or not self:isPosValid(r2, c2) then
		return false
	end
	...
	local borad2 = self.boardmap[r2][c2]
	if deltaC == 1 then
		return borad1:hasRightRope(isSwapCheck) or borad2:hasLeftRope(isSwapCheck)
	elseif deltaC == -1 then
		return borad1:hasLeftRope(isSwapCheck) or borad2:hasRightRope(isSwapCheck)
	elseif deltaR == 1 then
		return borad1:hasBottomRope(nil, isSwapCheck) or borad2:hasTopRope(nil, isSwapCheck)
	elseif deltaR == -1 then
		return borad1:hasTopRope(nil, isSwapCheck) or borad2:hasBottomRope(nil, isSwapCheck)
	end
	return false
end

function NewGameBoardLogic:isCanEffectLikeProp( r, c )
	-- body
...
function NewGameBoardLogic:isHedgehogCrazyBuffInBonusTime( ... )
	-- body
	if self.theGamePlayType == GameModeTypeId.HEDGEHOG_DIG_ENDLESS_ID then
		local r, c = self.gameMode:findHedgehogRC()
...
-- colortype 染色宝宝自身的颜色
function NewGameBoardLogic:findCrystalStoneEffectColor(colortype)
	local colorNums = {}
	for r = 1, #self.gameItemMap do
		for c = 1, #self.gameItemMap[r] do
			local item = self.gameItemMap[r][c]
...
function NewGameBoardLogic:tryBombCrystalStones(isSpecial)
	local num = 0
	for r = 1, #self.gameItemMap do
		for c = 1, #self.gameItemMap[r] do
			local item = self.gameItemMap[r][c]
...
function NewGameBoardLogic:randomSingleDropColor( itemId , r , c )
	local singleDropCfg = ProductItemLogic:getCurrSingleDropCfg( self , r , c )

	local limitedColors = nil
	local colourWeightList = nil
	if itemId and singleDropCfg then
		limitedColors = singleDropCfg[itemId]

		if singleDropCfg["weightList"] then
			colourWeightList = singleDropCfg["weightList"][itemId]
		end
	end

	return self:_randomColorByWeight(colourWeightList, limitedColors, true)
end

function NewGameBoardLogic:getSingleDropLimitedColors(itemId , r , c)
...
function NewGameBoardLogic:getSingleDropConfig( r , c )
	local singleDropCfg = ProductItemLogic:getCurrSingleDropCfg( self , r , c )
	-- local singleDropCfg = self.singleDropCfg
	return singleDropCfg or {}
end

function NewGameBoardLogic:randomCrystalStoneColor()
...
function NewGameBoardLogic:addAllItemsForMatchCheck()
...
function NewGameBoardLogic:getPositionCoverByCrystalStone(color1, color2, specialType)
	local posList = {}
...
function NewGameBoardLogic:addReplayReordPreviewBlock()
	self.blockReplayReord = self.blockReplayReord + 1
end

function NewGameBoardLogic:releasReplayReordPreviewBlock()
	self.blockReplayReord = self.blockReplayReord - 1
...
function NewGameBoardLogic:addNewSuperTotemPos(totemPos)
...
function NewGameBoardLogic:tryBombSuperTotemsByForce()
	for r = 1, #self.gameItemMap do
		for c = 1, #self.gameItemMap[r] do
			local item = self.gameItemMap[r][c]
...
function NewGameBoardLogic:tryBombSuperTotems()
...
function NewGameBoardLogic:sortProductPortals(posList)
...
function NewGameBoardLogic:addScoreToTotal(r, c, addScore, colorType, posType, isBuffScore , oringinAddScore)
	if not oringinAddScore then oringinAddScore = addScore end

	-- printx(11, "*** NewGameBoardLogic:addScoreToTotal, val:", addScore, self.globalScoreBuffVal)
	if self.globalScoreBuffVal and self.globalScoreBuffVal > 0 then
		addScore = math.ceil(addScore * self.globalScoreBuffVal)
		-- printx(11, "new score with buff:", addScore)
	end
	...
	-- 添加大招能量
	if (self.theGamePlayType == GameModeTypeId.MOLE_WEEKLY_RACE_ID or self.theGamePlayType == GameModeTypeId.WEEKLY_RACE_2020_MODE_ID)
		and not self.isBonusTime 
		then
...
function NewGameBoardLogic:addScoreAction(r, c, addScore, colorType, posType, isBuffScore)
...
function NewGameBoardLogic:playEliminateMusic(music)
...
function NewGameBoardLogic:setInitScrollCannon(initCannon, initSpawnPrepareData)
	self.initScrollCannonData = initCannon
	self.scrollSpawnQueuePrepareData = initSpawnPrepareData
	-- printx(11, "===== ### initSpawnPrepareData", table.tostring(initSpawnPrepareData))
end

function NewGameBoardLogic:setInitScrollSpawnData()
	if self.boardmap and self.scrollSpawnQueuePrepareData and #self.scrollSpawnQueuePrepareData > 0 then
		self.initScrollSpawnQueueData = {}

		for _, needUpdateGrid in pairs(self.scrollSpawnQueuePrepareData) do
			local boardData = self:safeGetBoardData(needUpdateGrid.r, needUpdateGrid.c)
			if boardData then
				self.initScrollSpawnQueueData[needUpdateGrid.c] = boardData.spawnQueueOfGrid
			end
		end
	end
	-- printx(11, "===== ### initSpawnQueueData", table.tostring(self.initScrollSpawnQueueData))
end

function NewGameBoardLogic:updateScrollCannon()
			...
			if item and item.isUsed then 
				...
				if not preRow or not preRow[c].isUsed then
					if not item.theGameBoardFallType or #item.theGameBoardFallType <= 0 then 
						...
						if self.initScrollSpawnQueueData[c] then
							item.spawnQueueOfGrid = self.initScrollSpawnQueueData[c]
						end
...
function NewGameBoardLogic:difficultyAdjustActivated()
...
function NewGameBoardLogic:onTouchInWrongState(x, y)
...
function NewGameBoardLogic:isHorizontalEndless()
	return self.theGamePlayType == GameModeTypeId.SPRING_HORIZONTAL_ENDLESS_ID
end

function NewGameBoardLogic:addJamSrcSpecial(r, c, bForceAdd )
    ...
    if bIsSafePos then
        ...
        PosInfo.isJamSperad = boardData:hasJamSperad()
        if bForceAdd then
...
--把特效的原点 数据记录列表
function NewGameBoardLogic:addSrcSpecialCoverToList( Item1Pos, Item2Pos, Item3Pos, Item4Pos )
...
--把特效的原点 数据记录列表
function NewGameBoardLogic:addSrcSpecialCoverToListEx( PosList, bForceAdd )
    if self.theGamePlayType ~= GameModeTypeId.JAMSPREAD_ID then
        return 0
    end
...
--清空特效原点数据
function NewGameBoardLogic:clearSrcSpecialCoverList()
    if self.theGamePlayType ~= GameModeTypeId.JAMSPREAD_ID then
        return
    end
...
--特效原点数据 是否有果酱
function NewGameBoardLogic:checkSrcSpecialCoverListIsHaveJamSperad( SrcSpecialCoverListID )
    if self.theGamePlayType ~= GameModeTypeId.JAMSPREAD_ID then
        return false
    end
...
--检查当前位置是否可以阻挡果酱
function NewGameBoardLogic:CheckPosCanStopJamSperad( Pos, Pos2, justRemove )
    if self.theGamePlayType ~= GameModeTypeId.JAMSPREAD_ID then
        return false
    end
    ...
    if itemData.isBlock then
        ...
        if itemData.ItemType == GameItemType.kMagicLamp then 
             --大眼仔
            bBlock = false
        elseif itemData.ItemType == GameItemType.kShellGift then 
             -- 贝壳礼包
            bBlock = false
        ...
        elseif itemData.ItemType == GameItemType.kCanevine then
        	if CanevineLogic:can_bomb(itemData.canevine_data) then
	            bBlock = false
        	end
        elseif CanevineLogic:is_occupy(itemData) then
        	local canevine_root_rc = CanevineLogic:get_canevine_root(itemData)
			local canevine_root_item = self.gameItemMap[canevine_root_rc.r][canevine_root_rc.c] 
        	if CanevineLogic:can_bomb(canevine_root_item.canevine_data) then
	            bBlock = false
        	end
        elseif CatteryLogic:checkCatteryRolling(self, itemData) or itemData:hasMeowLock() then
        	bBlock = false
        end
    end
    ...
    if itemData.ItemType ==  GameItemType.kChameleon then --染色蛋，变色龙
    	return true
    end
    ...
    if WaterBucketLogic:hasBucket(itemData) then
    	return true
    end

    if itemData.ItemType == GameItemType.kWater then -- 魔法药水（水滴）
        return true
    end
    ...
    if itemData.ItemType == GameItemType.kDynamiteCrate then --愤怒的炸药箱
        return true
    end
    ...
    if itemData.ItemType == GameItemType.kPlane then --飞机
        return true
    end

    if itemData.ItemType == GameItemType.kpuffedRice then --爆米花
        return true
    end

    if itemData.ItemType == GameItemType.kBattery then
    	return true
    end
...
function NewGameBoardLogic:getCurScoreAndStar( ... )
...
function NewGameBoardLogic:revertSpringSkillUseType( ... )
...
function NewGameBoardLogic:getTotalMilksCountAndTarget( ... )
...
function NewGameBoardLogic:updateGlobalScoreBuffView()
	-- printx(11, "NewGameBoardLogic:updateGlobalScoreBuffView", self.globalScoreBuffVal)
	if self.globalScoreBuffVal and self.globalScoreBuffVal > 0 and self.PlayUIDelegate then
		self.PlayUIDelegate:onUpdateGlobalScoreBuffView(self.globalScoreBuffVal)
	end
end

function NewGameBoardLogic:addExtraGainedActCollection(collectionID, addAmount)
	if not collectionID or not addAmount then return end

	if not self.extraGainedActCollections then
		self.extraGainedActCollections = {}
	end

	local oldAmount = 0
	if self.extraGainedActCollections[collectionID] then
		oldAmount = self.extraGainedActCollections[collectionID]
	end
	self.extraGainedActCollections[collectionID] = oldAmount + addAmount
end

function NewGameBoardLogic:setExtraGainedActCollectionFromRevertData(revertData)
	if not revertData then return end

	self.extraGainedActCollections = {}
	for dataKey, dataVal in pairs(revertData) do
		local collectionKey = dataKey
		if type(dataKey) == "string" then
			collectionKey = tonumber(dataKey)
		end
		self.extraGainedActCollections[collectionKey] = dataVal
	end
end

function NewGameBoardLogic:checkReachTarget()
	if not self.gameReachTarget then
		local reachEnd = self.gameMode:reachEndCondition(true)
		local reachTarget = self.gameMode:reachTarget() 
		self.gameReachTarget = reachTarget
		if self.gameReachTarget then
			Notify:dispatch("GameReachTarget")
		end
	end
end
