...
function GameBoardActionRunner:runningGameItemActionBonusStepToLine(mainLogic, theAction, actid, actByView)
	if theAction.addInfo == "start" then
		...
		local function onFlyEndCallback()
			...
			local bonusTimeScore = GamePlayConfigScore.BonusStepScores[math.clamp(theAction.addInt2, 1, #(GamePlayConfigScore.BonusStepScores))]
            ...
            --闯关晋级赛 bonus给道具
            if CloverRaceManager.getInstance():getCurLevelIsActCollect() then
            	ActCollectionLogic.genCollectMoveLimit = 0 --bons 走一步掉一个上限为止
            	ActCollectionLogic:addUseMove()
	            if ActCollectionLogic:checkGenLimit() then 
	            	local pos = {}
	            	pos.r = theAction.ItemPos1.x
	            	pos.c = theAction.ItemPos1.y
	        	    local turnSuc = ActCollectionLogic:handleTurnByPos(nil,true,pos)
	                if turnSuc then 
...
function GameBoardActionRunner:runningGameItemActionMagicLampCasting(mainLogic, theAction, actid, actByView)
	...
	if not theAction.specialTypes then
		...
		local chosenDir = 0
		if lamp.blocker83Dirs and #lamp.blocker83Dirs == 1 then  --旧数据，没有选的都是0
			chosenDir = tonumber(lamp.blocker83Dirs[1])
		end
		for k, v in pairs(theAction.speicalItemPos) do
			...
			if chosenDir == 0 then
				...
				table.insert( theAction.specialTypes , list[chosenDir] )
			end
		end
	end
	...
	if theAction.addInfo == 'over' then
		...
		local isJamSperad = boardData:hasJamSperad()
...
function GameBoardActionRunner:runningGameItemActionMonsterDestroyItem(mainLogic, theAction, actid, actByView, SpecialID)
	...
	if theAction.addInfo == "over" then
		...
		-- if _G.isLocalDevelopMode then printx(0, "TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT") end
		-- if _G.isLocalDevelopMode then printx(0, "TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT") end
		-- HapticsKitUtil:tiggerWithGrade(5, "haptic.control.search_long_press")
		GamePlayHapticsPlayer.getInstance():playEffect("monster_destroy")
...
function GameBoardActionRunner:runningGameItem_Roost_Replace(mainLogic, theAction, actid, actByView)
	if theAction.addInfo == "replace" then
		...
		local SpecialID = mainLogic:addSrcSpecialCoverToList(IntCoord:create(roostR, roostC))
		local hasJam = SpecialID and mainLogic:checkSrcSpecialCoverListIsHaveJamSperad(SpecialID)
...
--                table.insert( theAction.UpdateData, { ItemType = ItemType, r=v.item.x, c=v.item.y} )
            ...
            elseif ItemType == TileConst.kAnimal then 
                ...
                bCanNeedCheckMatch = true
            elseif ItemType == TileConst.kGyro then
            	--陀螺
            	local arrow = tonumber(attr)
                tItem:changeToGyro( arrow )
            elseif ItemType == TileConst.kWater then
            	local waterLevel = tonumber(attr)
            	tItem:changeToWater(waterLevel)
                bCanNeedCheckMatch = true
            elseif ItemType == TileConst.kPlane then
                local planeType = tonumber(attr)
                tItem:changeToPlane(planeType)
			elseif ItemType == TileConst.kCuckooWindupKey then
				local keyColour = tonumber(attr)
				tItem:changeToCuckooWindupKey(keyColour)
				bCanNeedCheckMatch = true
			elseif ItemType == TileConst.kBattery then
				tItem:changeToBattery()
			elseif ItemType == TileConst.kpuffedRice then
				tItem:changeToPuffedRice()
            end
            ...
            if theAction.SpecialID and mainLogic:checkSrcSpecialCoverListIsHaveJamSperad( theAction.SpecialID ) then
            	GameExtandPlayLogic:addJamSperadFlag(mainLogic, v.item.x,v.item.y, true )
            end
		end
--		ObstacleFootprintManager:addRecord(ObstacleFootprintType.k_HoneyBottle, ObstacleFootprintAction.k_GenerateSubItem, #theAction.infectList)
