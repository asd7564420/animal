...
function SaveRevertData:create(mainLogic , justCreate )
	...
	if mainLogic.productRuleConfig then
		ret.productRuleConfig = table.clone( mainLogic.productRuleConfig )
	end
	
	if mainLogic.productRuleGlobalConfig then
		ret.productRuleGlobalConfig = table.clone( mainLogic.productRuleGlobalConfig )
	end

	if mainLogic.defaultProductRuleGroupId then
		ret.defaultProductRuleGroupId = mainLogic.defaultProductRuleGroupId
	end
	...
	ret.level = mainLogic.level
	ret.entranceLevel = mainLogic.entranceLevel
	...
	ret.curWaterBucketChargingGroupId = mainLogic.curWaterBucketChargingGroupId
	...
	ret.generateFirecrackerTemp = mainLogic.generateFirecrackerTemp
	...
	ret.globalScoreBuffVal = mainLogic.globalScoreBuffVal
	ret.hadAutoSnowMonsterBuff = mainLogic.hadAutoSnowMonsterBuff
	ret.hadAutoBuffActived = mainLogic.hadAutoBuffActived
	ret.lastSlyBunnyFleeingMoves = mainLogic.lastSlyBunnyFleeingMoves
	...
	if mainLogic.collectTargetNums then
		ret.collectTargetNums = table.clone(mainLogic.collectTargetNums)
	end
	if mainLogic.extraGainedActCollections then
		ret.extraGainedActCollections = table.clone(mainLogic.extraGainedActCollections)
	end
	ret.moveBonusHarvested = mainLogic.moveBonusHarvested
	ret.inObservingMode = mainLogic.inObservingMode
    ...
    ret.CloverRaceData = CloverRaceManager:getInstance():getDataForRevert()
	ret.WeeklyRace2020Data = WeeklyRace2020Mgr:getInstance():getDataForRevert()
	
	ret.shellGiftData = mainLogic.shellGiftData and table.clone(mainLogic.shellGiftData)

	LocalActCoreModel.getOrCreateInstance():notify(ActInterface.kWriteSectionData, ret)
	...
	if StageInfoLocalLogic:getStageInfo( UserManager:getInstance().uid) and StageInfoLocalLogic:getStageInfo( UserManager:getInstance().uid).propsUsedInLevel then
		ret.stagePropsUsedInfo = table.clone(StageInfoLocalLogic:getStageInfo( UserManager:getInstance().uid).propsUsedInLevel )
	end
	...
	-- ret.redCrossData = RedCross:getOrCreateInstance():getRevertData()
	Notify:dispatch("createRevertData", mainLogic , ret  )
	return ret
end
