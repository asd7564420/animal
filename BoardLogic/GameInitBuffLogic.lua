...
InitBuffType = {
	...
	GLOBAL_SCORE_BUFF_120 = 9,		  -- 分数1.2倍
	GLOBAL_SCORE_BUFF_105 = 10,		  -- 分数1.05倍
	ADD_2_STEP = 11,    --增加2步
	ADD_STEP_ANIMAL = 12,    --加5步 小动物
}
...
InitBuffCreateType = {
	...
	GLOBAL_SCORE_BUFF = 7,  -- 全局分数加成
	COMMON_BUFF_ACTIVITY = 8, --通用的活动buff
	WINNINGSTREAK_BUFF_ACTIVITY = 9, --连胜排行buff，有些逻辑和8不同
}
...
Prop2BuffMapping = {
	...
	[GamePropsType.kAdd2_b] 		= InitBuffType.ADD_2_STEP,
	[GamePropsType.kFirecracker_b] 	= InitBuffType.FIRECRACKER,		-- 其实所有引用都已失效…………
	[ItemType.ADD_FIVE_STEP_BY_MATCH_FESTIVAL] 	= InitBuffType.ADD_STEP_ANIMAL,
}
...
local InitBuffTypeSort = {
	...
	InitBuffType.ADD_2_STEP,
	InitBuffType.REFRESH,
	InitBuffType.FIRECRACKER,
	InitBuffType.GLOBAL_SCORE_BUFF_120,
	InitBuffType.GLOBAL_SCORE_BUFF_105,
	InitBuffType.ADD_STEP_ANIMAL,
...
local InitBuffCreateTypeSort = {
	...
	InitBuffCreateType.COMMON_BUFF_ACTIVITY,
	InitBuffCreateType.WINNINGSTREAK_BUFF_ACTIVITY,
}
MACRO_DEV_START()
function GameInitBuffLogic:addBuffByTestFlag()

	-- printx( 1 , "GameInitBuffLogic:addBuffByTestFlag --------- " , debug.traceback() )
	...
	elseif self.gameInitBuffLogicTest == 17 then
		...
		GameInitBuffLogic:addInitBuff( {buffType = 4 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
		--printx(1,"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~1")
	elseif self.gameInitBuffLogicTest == 101 then
		GameInitBuffLogic:clearInitBuff()
		GameInitBuffLogic:addInitBuff( {buffType = 1 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
	elseif self.gameInitBuffLogicTest == 102 then
		GameInitBuffLogic:clearInitBuff()
		GameInitBuffLogic:addInitBuff( {buffType = 2 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
		GameInitBuffLogic:addInitBuff( {buffType = 3 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
	elseif self.gameInitBuffLogicTest == 103 then
		GameInitBuffLogic:clearInitBuff()
		GameInitBuffLogic:addInitBuff( {buffType = 1 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
		GameInitBuffLogic:addInitBuff( {buffType = 2 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
	elseif self.gameInitBuffLogicTest == 104 then
		GameInitBuffLogic:clearInitBuff()
		GameInitBuffLogic:addInitBuff( {buffType = 1 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
		GameInitBuffLogic:addInitBuff( {buffType = 2 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
		GameInitBuffLogic:addInitBuff( {buffType = 3 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
	elseif self.gameInitBuffLogicTest == 105 then
		GameInitBuffLogic:clearInitBuff()
		GameInitBuffLogic:addInitBuff( {buffType = 1 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
		GameInitBuffLogic:addInitBuff( {buffType = 1 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
		GameInitBuffLogic:addInitBuff( {buffType = 2 , createType = InitBuffCreateType.BUFF_ACTIVITY } )
...
function GameInitBuffLogic:testSongSongBuff()
	if not self.gameInitBuffLogicTest then self.gameInitBuffLogicTest = 0 end
	if self.gameInitBuffLogicTest == 0 then self.gameInitBuffLogicTest = 100 end
	--printx(1,"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!~~ 222~~~~~~~~~~~~~~~~~~~~~~~~~~~")

	self.gameInitBuffLogicTest = self.gameInitBuffLogicTest + 1
	--printx(1,"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!~~~3333~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")

	if self.gameInitBuffLogicTest > 105 then self.gameInitBuffLogicTest = 0 end
	--printx(1,"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!~~~self.gameInitBuffLogicTest",self.gameInitBuffLogicTest)
	if self.gameInitBuffLogicTest == 101 then
		CommonTip:showTip("松松Buff：魔力鸟" , "negative", nil , 1.5)
		self:addBuffByTestFlag()
	elseif self.gameInitBuffLogicTest == 102 then
		CommonTip:showTip("松松Buff：直线+爆炸" , "negative", nil , 1.5)
		self:addBuffByTestFlag()
	elseif self.gameInitBuffLogicTest == 103 then
		CommonTip:showTip("松松Buff：直线+魔力鸟" , "negative", nil , 1.5)
		self:addBuffByTestFlag()
	elseif self.gameInitBuffLogicTest == 104 then
		CommonTip:showTip("松松Buff：直线+爆炸+魔力鸟" , "negative", nil , 1.5)
		self:addBuffByTestFlag()
	elseif self.gameInitBuffLogicTest == 105 then
		CommonTip:showTip("松松Buff：魔力鸟+魔力鸟+直线" , "negative", nil , 1.5)
		self:addBuffByTestFlag()
	else
		CommonTip:showTip("松松Buff 关闭" , "negative", nil , 3)
	end
...
function GameInitBuffLogic:hasInitBuffFromPreBuffAct( ... )
	...
	if mainLogic.replayMode and
		...
		and (mainLogic.replayMode ~= ReplayMode.kMcts or (mainLogic.replayMode == ReplayMode.kMcts and _G.AIAutoCheckReplayCheck) ) 
		then
...
function GameInitBuffLogic:__checkMapAndFindCreatePosition( priorityTable , planList , resultList, isInGame )
	-- printx(11, "=== GameInitBuffLogic:__checkMapAndFindCreatePosition", table.tostring(planList))
	...
	local candidateIndex_addStepAnimal = {} 	-- 可投放 加步数动物
		...
		for c = 1, #mainLogic.gameItemMap[r] do
			...
			if item and board then
				...
				if (item.ItemType == GameItemType.kAnimal  ) 
					...
					if not item.hasActCollection
						and not board.preAndBuffLineWrapPassSelect
						and not board.preAndBuffFirecrackerPassSelect
						and not board.buffBoomPassSelect
						and not board.preAndBuffMagicBirdPassSelect
					then
						insertWithSwapIndex(currSwapIndex, candidateIndex_addStepAnimal, currPos)
					end
	...
	for k,v in ipairs( planList ) do
		...
		elseif v.buffType == InitBuffType.ADD_STEP_ANIMAL then
			tarItemType = GameItemType.kAddMove
			tarItemSpecialType = 0
		...
		elseif v.buffType == InitBuffType.GLOBAL_SCORE_BUFF_120 or v.buffType == InitBuffType.GLOBAL_SCORE_BUFF_105 then
			if mainLogic then
				if v.buffType == InitBuffType.GLOBAL_SCORE_BUFF_120 then
					mainLogic.globalScoreBuffVal = ScoreBuffPropIDToValueMap[ItemType.INFINITE_SCORE_BUFF_120_ONE_MINUTE]
				elseif v.buffType == InitBuffType.GLOBAL_SCORE_BUFF_105 then
					mainLogic.globalScoreBuffVal = 1.05
				end
				-- printx(11, "+++++++++++++ set globalScoreBuffVal!", debug.traceback())
				mainLogic:updateGlobalScoreBuffView()
			end
			needContinue = true --不改变棋盘，不加入initBuffResult
		end
		...
		if not needContinue then
			...
			elseif v.buffType == InitBuffType.ADD_STEP_ANIMAL then
				randomFind(candidateIndex_addStepAnimal)
...
function GameInitBuffLogic:canAddLINEWRAP( buffType )
		...
		for c = 1, #mainLogic.gameItemMap[r] do
			...
			if item and board then
				if (item.ItemType == GameItemType.kAnimal  ) 
					...
					if not board.preAndBuffLineWrapPassSelect then
						table.insert(candidateIndex_lineAndWrap, currPos)
					end
				end
			end
		end
	...
	if  #candidateIndex_lineAndWrap > 1 then
		return true
	end
...
function GameInitBuffLogic:canAddBuffByBuffType( buff )
	-- if buffType ~= InitBuffType.LINE_WRAP then
	-- 	return false
	-- end
	...
	self:__checkMapAndFindCreatePosition(nil,{buff},ret)
...
function GameInitBuffLogic:tryCreateBuffPositonByInitBuffPassedPlanList()
	...
	if mainLogic and mainLogic.theGamePlayType == GameModeType.CLASSIC_MOVES then
		...
		return nil --时间关只尝试创建一次，之后不再创建		--CLASSIC_MOVES不是时间关啊大哥，是步数关...时间关是CLASSIC
	end
...
function GameInitBuffLogic:doChangeBoardByGameInitBuff( callback )
	...
	if notPrePropResult and #notPrePropResult> 0 then
		...
		for k,v in pairs( buffPlan ) do
			...
			if v.createType == InitBuffCreateType.BUFF_ACTIVITY 
				or v.createType == InitBuffCreateType.PRE_BUFF_ACTIVITY
				or v.createType == InitBuffCreateType.COMMON_BUFF_ACTIVITY
				or v.createType == InitBuffCreateType.WINNINGSTREAK_BUFF_ACTIVITY  then

				local check = false
				for k1 , v1 in pairs(resultList) do
					if v.buffType == v1.buffType 
						and ( v1.createType == InitBuffCreateType.BUFF_ACTIVITY 
							or v1.createType == InitBuffCreateType.PRE_BUFF_ACTIVITY
							or v.createType == InitBuffCreateType.COMMON_BUFF_ACTIVITY
							or v.createType == InitBuffCreateType.WINNINGSTREAK_BUFF_ACTIVITY ) 
						then
						resultList[k1] = nil
						check = true
						break
			...
			if data.shouldGiveBack then 	--此分支应已关闭
				...
				if data.buffType == InitBuffType.ADD_3_STEP then
					action =  GameBoardActionDataSet:createAs(
						...
						nil,
						GamePlayConfig_MaxAction_time)
				elseif data.buffType == InitBuffType.ADD_2_STEP then
					action =  GameBoardActionDataSet:createAs(
						GameActionTargetType.kPropsAction,
						GameItemActionType.kAddBuffAdd2Step,
						nil,
						nil,
						GamePlayConfig_MaxAction_time)
...
function GameInitBuffLogic:doUsePreProps(selectedItemsData, callback)
	...
	local function appearsInPreItemResult(id)
		for _, v in ipairs(prePropResult) do
			if v.propId == id then
				-- +3步可以不管有无引导，直接释放
				if not passLogic or v.buffType == InitBuffType.ADD_3_STEP or v.buffType == InitBuffType.ADD_2_STEP then
					return v
				end
			end
		end
		...
		if not targetBuff.isPrivilegeFree then 
			...
			for _, origPlan in pairs(initAllBuffPlan) do
				if origPlan.createType == InitBuffCreateType.PRE_PROP 
					...
					then
					if origPlan.propId == targetBuff.propId 
						and origPlan.buffType == targetBuff.buffType 
						and origPlan.createType == targetBuff.createType 
						then
						table.insert(passedPrePropPlanList , origPlan)
						-- printx(11, "+ + + add to prePropsPassedPlanList! + + + ")
						-- 为了应对initBuff中有重复对象
						-- 说实话，我有点不明白为什么要用initBuff的属性来再判断一遍，不过当时这么写应该是有理由的吧……失忆的人还是不要随便乱改了
						return
					end
				end
			end
...
function GameInitBuffLogic:addInitBuffByItems(items, createType)
	...
	for k,v in pairs(items) do
		...
		-- 特殊前置道具  雪怪技能道具
		if realPropId == ItemType.SNOW_MONSTER_SKILL then
			self:setSnowMonsterBuff(true)
		end
	end
	return buffs
end
...
-- 是否有雪怪技能道具触发的特效
function GameInitBuffLogic:isSnowMonsterBuff()
	return self._isSnowMonsterBuff
end

-- 设定是否有雪怪技能道具触发的特效
function GameInitBuffLogic:setSnowMonsterBuff(value)
	self._isSnowMonsterBuff = value
end
