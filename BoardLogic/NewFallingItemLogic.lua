...
function NewFallingItemLogic:hasTopRope( board )
	return board:hasTopRopeProperty() or board:isWindTunnelWallActive(1) --只判断绳子。新增：风区边缘视为可交换的隐形绳子
end

function NewFallingItemLogic:hasBottomRope( board )
	return board:hasBottomRopeProperty() or board:isWindTunnelWallActive(3) --只判断绳子。新增：风区边缘视为可交换的隐形绳子
end

function NewFallingItemLogic:hasLeftRope( board )
	return board:hasLeftRopeProperty() or board:isWindTunnelWallActive(4) --只判断绳子。新增：风区边缘视为可交换的隐形绳子
end

function NewFallingItemLogic:hasRightRope( board )
	return board:hasRightRopeProperty() or board:isWindTunnelWallActive(2) --只判断绳子。新增：风区边缘视为可交换的隐形绳子
end
...
function NewFallingItemLogic:checkFallingByPortal(mainLogic , r , c)
	...
	-- printx( 1 , "NewFallingItemLogic:checkFallingByPortal  " ,  r , c , ctx.endlessLoopData.deathLoop)
	if ctx.endlessLoopData and ctx.endlessLoopData.deathLoop 
		and ctx.endlessLoopData.deathLoop.loopType == DeathLoopType.kPortal 
		and ctx.endlessLoopData.deathLoop.r == r 
		and ctx.endlessLoopData.deathLoop.c == c 
		then
		ctx.endlessLoopData.deathLoop.fixCount =ctx.endlessLoopData.deathLoop.fixCount + 1
		if ctx.endlessLoopData.deathLoop.fixCount > 30 then
			ctx.endlessLoopData = {}
		end
		-- printx( 1 , "NewFallingItemLogic:checkFallingByPortal  endlessLoopData" ,  ctx.endlessLoopData.deathLoop.fixCount)
		return false
	end
...
function NewFallingItemLogic:doFallingByPortal(mainLogic , r , c)
	...
	if ctx and ctx.endlessLoopData then
		...
		if not ctx.endlessLoopData.deathLoop then
			if not ctx.endlessLoopData.portalCountMap then
				ctx.endlessLoopData.portalCountMap = {}
			end

			if not ctx.endlessLoopData.portalCountMap[ tostring(r) .. "_" .. tostring(c) ] then
				ctx.endlessLoopData.portalCountMap[ tostring(r) .. "_" .. tostring(c) ] = {
					score = 0 ,
					pCount = 0 ,
					list = {} ,
					loopCount = 0 ,
			...
			local dataObj = ctx.endlessLoopData.portalCountMap[ tostring(r) .. "_" .. tostring(c) ]

			if not dataObj.pCount then
				dataObj.pCount = 0
			end

			if dataObj.score ~= mainLogic.totalScore then
				dataObj.score = mainLogic.totalScore
				dataObj.pCount = 0
				dataObj.list = {}
				dataObj.loopCount = 0
				dataObj.loopLength = 0
			end
			
			dataObj.pCount = dataObj.pCount + 1

			local function checkLoop( list , needPrint )
				local totalLength = #list
				local lastV = list[ totalLength ]
				local keyIndex = totalLength
					
				local function addLoopCount()
					dataObj.loopCount = dataObj.loopCount + 1
					if dataObj.loopCount + dataObj.loopLength > 25 then
						return true
					end
					return false
				end
				...
				local function __check()
					...
					if dataObj.loopLength and dataObj.loopLength > 1 then
						if list[totalLength] == list[totalLength - dataObj.loopLength] then
							return addLoopCount()
						else
							dataObj.loopCount = 0
							dataObj.loopLength = 0
					...
					for i = totalLength - 1 , 1 , -1 do
						local v = list[i]
						
						if totalLength - i > 25 then
							break
						else
							if v == lastV and i < keyIndex then
								keyIndex = i
								found = true
								break
							end
					...
					-- if needPrint then
						-- printx( 1 , "list = " , table.tostring(list) , "keyIndex:" , keyIndex , "totalLength:" , totalLength )
					-- end

					local loop = true
					for i = totalLength - 1 , keyIndex + 1 , -1 do
						if list[i] ~= list[keyIndex - (totalLength - i)] then
							-- if needPrint then
								-- printx( 1 , i , "->" , list[i] , keyIndex - (totalLength - i) , "->" , list[keyIndex - (totalLength - i)] )
							-- end
							loop = false
							break
						end
					...
					-- if needPrint then
						-- printx( 1 , "loop" , loop , dataObj.loopCount)
					-- end

					if loop then 
						dataObj.loopLength = totalLength - keyIndex
						return addLoopCount()
					else
						dataObj.loopCount = 0
						return __check()
			...
			local colorIndex = AnimalTypeConfig.convertColorTypeToIndex( item2._encrypt.ItemColorType ) 
			if not colorIndex then colorIndex = 0 end

			table.insert( dataObj.list , colorIndex )

			-- printx( 1 , "deathLoop check ~~~ " , table.tostring(dataObj.list) )
			-- local needPrint = false
			-- if r == 9 and c == 5 then
			-- 	needPrint = true
			-- end
			local isLoop = checkLoop( dataObj.list , needPrint )

			-- if needPrint then
			-- 	printx( 1 , "[" .. tostring(r) .. "_" .. tostring(c) .. "]deathLoop check ~~~ " , colorIndex , isLoop  )
			-- end
			

			if isLoop then
				ctx.endlessLoopData.deathLoop = {
					loopType = DeathLoopType.kPortal ,
					fixCount = 0 ,
					r = r ,
					c = c ,
					costMove = mainLogic.realCostMove ,
				}
				table.insert( ctx.endlessLoopLog , ctx.endlessLoopData.deathLoop )
				-- printx( 1 , "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  ctx.endlessLoopData.deathLoop")
				-- debug.debug()
			end
		end
	end
...
function NewFallingItemLogic:getVerticalFallingPlanMap()
	if not self.fallingPlanMap then self.fallingPlanMap = {} end
	if not self.fallingPlanMap.verticalFallingPlan then self.fallingPlanMap.verticalFallingPlan = {} end

	return self.fallingPlanMap.verticalFallingPlan
end

function NewFallingItemLogic:clearVerticalFallingPlan()
	if not self.fallingPlanMap then self.fallingPlanMap = {} end
...
function NewFallingItemLogic:FallingGameItemCheck(mainLogic)----全局检测掉落-----to do-----------------------------------------------
	...
	local randomColArr = {}		--每次都重置随机队列
	local randomRowArr = {}		--每次都重置随机队列	
	local rowAmount, colAmount = GameBoardUtil:getRowAndColAmountOfBoard(mainLogic)

	local arr = {}
	for idx = 1, colAmount do
		table.insert(arr, idx)
	end
	for i = 1, colAmount do
		local cindex = mainLogic.fallingLogicRandFactory:rand( 1 , #arr )
		table.insert( randomColArr , arr[cindex] )
		table.remove( arr , cindex )
	end

	if rowAmount == colAmount then
		randomRowArr = randomColArr	--不影响老逻辑：如行列相同，则同一队列公用
	else
		local arr2 = {}
		for idx = 1, rowAmount do
			table.insert(arr2, idx)
		end
		for i = 1, rowAmount do
			local rindex = mainLogic.fallingLogicRandFactory:rand( 1 , #arr2 )
			table.insert( randomRowArr , arr2[rindex] )
			table.remove( arr2 , rindex )
		end
	end
	...
	for r = 1, rowAmount do			---- ↓：由下至上； ↑：由上至下
		for _k , c in ipairs(randomColArr) do              ----横排随机取
			local board = mainLogic.boardmap[r][c]
			if board:isGravityDown() then
	...
	for c = 1 , colAmount do			---- ←：由左至右； →：由右至左
		for _k , r in ipairs(randomRowArr) do              ----竖列随机取
			local board = mainLogic.boardmap[r][c]
