...
function BombItemLogic:BombByMatch(mainLogic)
	...
	if mainLogic.theGamePlayType == GameModeTypeId.LIGHT_UP_ID 
		...
		or mainLogic.theGamePlayType == GameModeTypeId.TREASURE_HUNT_MODE_ID 
		or ChannelWaterLogic.isEnable()
		then 
		for r = 1, #mainLogic.swapHelpMap do
			for c = 1, #mainLogic.swapHelpMap[r] do
...
function BombItemLogic:tryCoverByBomb(mainLogic, r, c, coverFalling, scoreScale, bombNoReach, noScore, footprintType)
	...
	elseif item.ItemType == GameItemType.kPuffer and item:isVisibleAndFree() then
		return 2
	elseif WaterBucketLogic:hasBucket(item) then
		return 2
	elseif CanevineLogic:is_occupy(item) and item:isAvailable() and (not item:hasLock()) then
		return 2
...
function BombItemLogic:setSignOfBombResWithBirdFlying(mainLogic, r1, c1, theColor, newType,SpecialID)
	...
	for i,v in ipairs(PosList) do
		...
		elseif item:isAvailable() and ((item.ItemType == GameItemType.kMagicLamp and item.lampLevel > 0)
...
function BombItemLogic:tryBombWithBombRes(mainLogic, r1, c1, isBombScore, scoreScale)
	for r = 1, #mainLogic.gameItemMap do
		for c = 1, #mainLogic.gameItemMap[r] do
			local item = mainLogic.gameItemMap[r][c];
			if item.ItemType ~= GameItemType.kNone and item.bombRes ~= nil and item.bombRes.x == r1 and item.bombRes.y == c1 then
		...
		for c = 1, #mainLogic.gameItemMap[r] do
			...
			if item:isItemCanBeCoverByBirdBird() then
				if item:isItemCanBeEliminateByBirdBird() then
					...
					elseif mainLogic.gameItemMap[r][c].ItemType == GameItemType.kTravelEnergyBag 
						or mainLogic.gameItemMap[r][c].ItemType == GameItemType.kCuckooWindupKey
						then
						addScore = 0	--自己单独加分
					end
...
-------鸟鸟交换清屏-----
function BombItemLogic:tryCoverByBirdBird(mainLogic, birdBirdPos, isBombScore, scoreScale, collectPos, birdBirdPos2, SpecialID)
		...
		for c = 1, #mainLogic.gameItemMap[r] do
			...
			if item:isItemCanBeCoverByBirdBird() then
				if item:isItemCanBeEliminateByBirdBird() then
					...
					elseif item.ItemType == GameItemType.kRocket then
						addScore = GamePlayConfigScore.Rocket * scoreScale
					elseif item.ItemType == GameItemType.kTravelEnergyBag 
						or item.ItemType == GameItemType.kCuckooWindupKey 
						then
						addScore = 0	--自己单独加分
					end
				...
				elseif item.ItemType == GameItemType.kCrystalStone 
					...
					or item.ItemType == GameItemType.kTravelEnergyBag
					or item.ItemType == GameItemType.kCanevine
					or item.ItemType == GameItemType.kCuckooWindupKey
					then
					SpecialCoverLogic:SpecialCoverAtPos(mainLogic, r, c, 0, scoreScale,nil,nil,nil,nil,SpecialID)
				end
