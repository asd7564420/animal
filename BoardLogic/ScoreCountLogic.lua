...
function ScoreCountLogic:endCombo(mainLogic)
	...
	local sound
	local image
	local musictype = GameMusicType.kContinueMatch
	if mainLogic.levelType == GameLevelType.kMatchFestival then
		musictype = "tempFunctionResInLevel/MatchFestival/ingame/match%d.mp3"
	end
 	if mainLogic.comboCount >= 11 then
 		image = 5
 		sound = string.format(musictype, 11)
 	elseif mainLogic.comboCount >= 9 then
 		image = 4
 		sound = string.format(musictype, 9)
 	elseif mainLogic.comboCount >= 7 then
 		image = 3
 		sound = string.format(musictype, 7)
 	elseif mainLogic.comboCount >= 5 then
 		image = 2
 		sound = string.format(musictype, 5) 
 	elseif mainLogic.comboCount >= 3 then
 		image = 1
 		sound = string.format(musictype, 3)
 	end 
...
function ScoreCountLogic:addScoreToTotal(mainLogic, score , oringinAddScore)
	...
	if _G.isLocalDevelopMode then
		if score % 5 ~= 0 
			and not (mainLogic.globalScoreBuffVal and mainLogic.globalScoreBuffVal > 0 )
			then
			RemoteDebug:uploadLogWithTag("score_error", score, debug.traceback())
		end
	end

	Notify:dispatch("afterAddScore")
end
...
function ScoreCountLogic:getItemDestroyBaseScore(itemType)
	...
	-- elseif itemType == GameItemType.kTravelEnergyBag then
	-- 	scoreBase = GamePlayConfigScore.TravelEnergyBag
	end
