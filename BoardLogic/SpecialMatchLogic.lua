...
function SpecialMatchLogic:MatchBirdBird(mainLogic, r1,c1,r2,c2)
	...
	if birdCount == 2 then
		...
		GamePlayHapticsPlayer.getInstance():playEffect("bird_bird_swap")
...
function SpecialMatchLogic:BirdLineSwapBomb(mainLogic, r1, c1, r2, c2)
	...
	item2.shieldToColorFilter = true --为了保持魔力鸟放招过程中棋盘稳定，屏蔽过滤器的影响（对象最终会被消除，不用维护此字段）

	local keyname = "bird_line_swap"
	GamePlayContext:getInstance():updatePlayInfo( keyname , 1 )
	GamePlayHapticsPlayer.getInstance():playEffect("bird_line_swap")
...
function SpecialMatchLogic:BirdColorSwapBomb(mainLogic, r1, c1, r2, c2)
	...
	elseif item2.ItemType == GameItemType.kShellGift and item2:isAvailable() then
		ShellGiftLogic.hitShellGift(mainLogic,r2,c2,item2)
...
function SpecialMatchLogic:LineLineSwapBomb(mainLogic, r1, c1, r2, c2)
    ...
    if boardData1:hasJamSperad() or boardData2:hasJamSperad() then
	...
	GamePlayContext:getInstance():updatePlayInfo( keyname , 1 )
	GamePlayHapticsPlayer.getInstance():playEffect("line_line_swap")
...
function SpecialMatchLogic:WrapLineSwapBomb(mainLogic, r1, c1, r2, c2)
	...
	GamePlayContext:getInstance():updatePlayInfo( keyname , 1 )
	GamePlayHapticsPlayer.getInstance():playEffect("line_wrap_swap")
...
function SpecialMatchLogic:WrapWrapSwapBomb(mainLogic, r1, c1, r2, c2)
	...
	GamePlayContext:getInstance():updatePlayInfo( keyname , 1 )
	GamePlayHapticsPlayer.getInstance():playEffect("wrap_wrap_swap")
