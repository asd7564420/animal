	...
	for r = 1, #mainLogic.swapHelpMap do 
		for c = 1, #mainLogic.swapHelpMap[r] do
			...
			if spd > 0 then 	--该格子上有匹配
				...
				MatchCoverLogic:doEffectFlyBoard(mainLogic,r,c)

				MatchCoverLogic:doEffectToNDBunnyIce(mainLogic, r, c)
...
function MatchCoverLogic:doEffectFlyBoard( mainLogic, r, c )
	...
	if board.haveBoard then
		...
		if needFly then
			-- printx(15,"要飞，目标格子：",targetR,targetC)
			local function completeCallback( ... )
				-- printx(15,"飞完了",targetR,targetC)
				board.prepareToFly = false
...
function MatchCoverLogic:doEffectToNDBunnyIce(mainLogic, r, c)
	if not mainLogic:isPosValid(r, c) then return false end

	local item = mainLogic.gameItemMap[r][c]
	local board = mainLogic.boardmap[r][c]
	------2.冰层变化------
	if item:canEffectLightUp() and board.NDBunnyIceLevel > 0 and board.blockerCoverMaterialLevel == 0 then
...
function MatchCoverLogic:doEffectAtByMatchAt(mainLogic, r, c, comboCount)
	...
	-- 尝试修复一个bug：JustArrived状态的对象，会因为别人的匹配检测被拉入匹配组合中进行消除
	-- 这种消除可能带来ItemStatus的状态不再被更新（比如闪电鸟参与匹配后变为blocker），从而影响棋盘的stable判定
	-- 若是屏蔽JustArrived状态的对象参与匹配，可能,也许,会引发最终都没有检测到匹配的状况（被“这些同色的怎么不消除啊”类bug支配的恐惧）
	-- 所以两害相权取其轻，不屏蔽参与匹配，改为强行将状态进行一次纠正
	if item.ItemStatus == GameItemStatusType.kJustArrived then
		item.ItemStatus = GameItemStatusType.kItemHalfStable
	end
...
function MatchCoverLogic:doEffectAtByMatchAround(mainLogic, r, c, comboCount)
	...
	if board.colorFilterBLevel > 0 and not item.isReverseSide and not item:hasBlocker206() and not item:hasSquidLock() and (not WaterBucketLogic:hasBucket(item)) then
		local f_count = 1;
	...
	elseif item.ItemType == GameItemType.kDynamiteCrate	 then
		DynamiteCrateLogic:hitDynamiteCrate(mainLogic,item,r,c,false)
	...
	elseif item.ItemType == GameItemType.kWater and WaterBucketLogic:canAttackWater(item) then
		WaterBucketLogic:attackWater(mainLogic, r, c, t_count)
	elseif item.windTunnelSwitchLevel > 0 then
		WindTunnelLogic:decreaseWindTunnelSwitch(mainLogic, r, c, t_count, scoreScale)
	elseif item.ItemType == GameItemType.kActivityCollectionItem then
		ActivityClollectionItemLogic:destoryCollectionItem(mainLogic, r, c, false)
	elseif CanevineLogic:is_occupy(item) then
		...
		if canevine_root_item.can_attack_canevine then
			if CanevineLogic:attack_canevine(mainLogic, canevine_root_item, scoreScale) then
				canevine_root_item.can_attack_canevine = false
			end
		end
	elseif item.ItemType == GameItemType.kPlane and item.planeCountDown > 0 and not hashoney then
		PlaneLogic:onPlaneBeingHit(mainLogic, r, c, t_count, scoreScale)
	...
	elseif item.ItemType == GameItemType.kCattery or item.ItemType == GameItemType.kCatteryEmpty then
		CatteryLogic:tryToDecrease(mainLogic, item, t_count)
	elseif item.ItemType == GameItemType.kMeow and item.meowLevel > 0 then
		CatteryLogic:decreaseMeow(mainLogic, r, c, t_count, scoreScale)
	elseif item.ItemType == GameItemType.kAngryBird then
		AngryBirdLogic:addBirdEnergy(mainLogic,r,c,item)
	elseif item.bunnyLaneLevel > 0 then
		SlyBunnyLogic:decreaseSlyBunnyLane(mainLogic, r, c, t_count, scoreScale)
	elseif item.ItemType == GameItemType.kFirework then
		FireworkLogic:triggerOneFirework(mainLogic,item)
	elseif item.ItemType == GameItemType.kBattery and not hashoney then
		BatteryLogic:batteryCharge(mainLogic,item)
	elseif item.ItemType == GameItemType.kNDBunnySnow and item.NDBunnySnowLevel > 0 then
		NationalDayBunnyLogic:decreaseBunnySnow(mainLogic, r, c, t_count, scoreScale)
	elseif item.ItemType == GameItemType.kNationalDayBunny and item.NDBunnyHp > 0 then
		NationalDayBunnyLogic:decreaseBunny(mainLogic, r, c, t_count, scoreScale)
	elseif item.ItemType == GameItemType.kpuffedRice and item.puffedRiceHp > 0 and not hashoney then
		PuffedRiceLogic:onPuffedRiceBeingHit(mainLogic, r, c, t_count, scoreScale)
	end
end
