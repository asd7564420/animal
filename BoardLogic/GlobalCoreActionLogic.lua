...
function GlobalCoreActionLogic:runViewAction(boardView, theAction)
	...
	elseif theAction.actionType == GameItemActionType.kAddBuffAdd2Step then
		GlobalCoreActionLogic:runView_AddBuffAdd2Step( boardView , theAction )
	...
	elseif theAction.actionTarget == GameActionTargetType.kTopPartAction and theAction.actionType == GameBoardTopPartActionType.kDHCTip then
		GlobalCoreActionLogic:runView_DHCLevelTip(boardView, theAction) 
	elseif theAction.actionType == GameItemActionType.kActivityCollectionItemShow then
		GlobalCoreActionLogic:runView_ActivityCollectionItemShow(boardView, theAction)
	elseif theAction.actionType == GameItemActionType.kItem_Travel_Hero_Reach_End then
		GlobalCoreActionLogic:runView_TravelHeroReachEnd(boardView, theAction)
	elseif theAction.actionType == GameItemActionType.kAct_CollectionDouble then
		GlobalCoreActionLogic:runView_Act_CollectionDouble(boardView, theAction) 
	elseif theAction.actionType == GameItemActionType.kItem_SlyBunny_Fleeing then
		GlobalCoreActionLogic:runView_SlyBunnyFleeing(boardView, theAction) 
	elseif theAction.actionType == GameItemActionType.kItem_Walk_Chick_Reach_End then
		GlobalCoreActionLogic:runView_WalkChickReachEnd(boardView,theAction)
	elseif theAction.actionType == GameItemActionType.kAct_RotaryTablePromp then
		GlobalCoreActionLogic:runView_Act_RotaryTable(boardView, theAction)
	elseif theAction.actionType == GameItemActionType.kAct_Treasure_Hunt_Collect then
		GlobalCoreActionLogic:runView_Act_Treasure_Hunt_Collect(boardView, theAction)
	elseif theAction.actionTarget == GameActionTargetType.kTopPartAction and theAction.actionType == GameBoardTopPartActionType.kMatchFestival then
		GlobalCoreActionLogic:runView_ACT_MatchFestival_Progress(boardView, theAction) 
	elseif theAction.actionType == GameItemActionType.kItem_battle_trophy_generate then
		GlobalCoreActionLogic:runView_BattleTrophyGenerate(boardView, theAction)
	elseif theAction.actionTarget == GameActionTargetType.kTopPartAction and theAction.actionType == GameBoardTopPartActionType.kLuckyBag2020 then
		GlobalCoreActionLogic:runView_ACT_LuckyBag2020_Progress(boardView, theAction) 
	end
end
...
function GlobalCoreActionLogic:runLogicAction(mainLogic, theAction, actid)
	...
	elseif theAction.actionType == GameItemActionType.kAddBuffAdd2Step then
		GlobalCoreActionLogic:runLogic_AddBuffAdd2Step( mainLogic , theAction , actid )
	elseif theAction.actionType == GameItemActionType.kAddBuffSpecialAnimal then
	...
	elseif theAction.actionTarget == GameActionTargetType.kTopPartAction and theAction.actionType == GameBoardTopPartActionType.kDHCTip then
		GlobalCoreActionLogic:runLogic_DHCLevelTip(mainLogic, theAction, actid, actByView)
	elseif theAction.actionType == GameItemActionType.kActivityCollectionItemShow then
		GlobalCoreActionLogic:runLogic_ActivityCollectionItemShow(mainLogic, theAction, actid, actByView)
	elseif theAction.actionType == GameItemActionType.kItem_Travel_Hero_Reach_End then
		GlobalCoreActionLogic:runLogic_TravelHeroReachEnd(mainLogic, theAction, actid, actByView)
	elseif theAction.actionType == GameItemActionType.kAct_CollectionDouble then
		GlobalCoreActionLogic:runLogic_Act_CollectionDouble(mainLogic, theAction, actid)
	elseif theAction.actionType == GameItemActionType.kItem_SlyBunny_Fleeing then
		GlobalCoreActionLogic:runLogic_SlyBunnyFleeing(mainLogic, theAction, actid, actByView)
	elseif theAction.actionType == GameItemActionType.kItem_Walk_Chick_Reach_End then
		GlobalCoreActionLogic:runLogic_WalkChickReachEnd(mainLogic, theAction, actid, actByView)
	elseif theAction.actionType == GameItemActionType.kAct_RotaryTablePromp then 
		GlobalCoreActionLogic:runLogic_Act_RotaryTable(mainLogic, theAction, actid, actByView)
	elseif theAction.actionType == GameItemActionType.kAct_Treasure_Hunt_Collect then
		GlobalCoreActionLogic:runLogic_Act_Treasure_Hunt_Collect(mainLogic, theAction, actid, actByView)
	elseif theAction.actionTarget == GameActionTargetType.kTopPartAction and theAction.actionType == GameBoardTopPartActionType.kMatchFestival then
		GlobalCoreActionLogic:runLogic_ACT_MatchFestival_Progress(mainLogic, theAction, actid, actByView)
	elseif theAction.actionType == GameItemActionType.kItem_battle_trophy_generate then
		GlobalCoreActionLogic:runLogic_BattleTrophyGenerate(mainLogic, theAction, actid, actByView)
	elseif theAction.actionTarget == GameActionTargetType.kTopPartAction and theAction.actionType == GameBoardTopPartActionType.kLuckyBag2020 then
		GlobalCoreActionLogic:runLogic_ACT_LuckyBag2020_Progress(mainLogic, theAction, actid, actByView)
	end
end
-------------------------------------------------------------------------------
	...
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		...
		if theAction.addInfo == "startPlayAni" then 
			...
			for i,v in ipairs(turnTargets) do
				...
				itemView:playActCollectionShow(cb, theAction.bBonsTimeTurn)
			end
		end
	end 
end
...
function GlobalCoreActionLogic:runView_AddBuffItemToBoard(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		...
		local function playWinningBoomEff(targetPos)
			local boomEff = ArmatureNode:create("winningstreakrankingbuff/boomEff")
			boomEff:playByIndex(0)
			boomEff:update(0.001) -- 此处的参数含义为时间
        ...
        elseif animeType == AddGameInitBuffAnimeType.kDragonBuff2018 then
			...
			animeDelay = 102 + 1 --已废弃
		elseif animeType == AddGameInitBuffAnimeType.kTsumBuff2019 then
            TsumBuffManager.getInstance():playTsumBuffAnim( theAction, boardView )
        elseif animeType == AddGameInitBuffAnimeType.kWinningStreakRankingBuff then			
            FrameLoader:loadArmature('skeleton/winningstreakrankingbuff')
			local skeletonAnimeName = "winningstreakrankingbuff/AddBuff_1"
			local skeletonAnime = ArmatureNode:create( skeletonAnimeName )
			skeletonAnime:playByIndex(0)
			skeletonAnime:update(0.001) -- 此处的参数含义为时间
			--skeletonAnime:playByIndex(0)
			local buffIcon = skeletonAnime:getCon({"icon"})
			local buffId = WinningStreakRankingManager.getInstance().data.buff.id
			local groupId = WinningStreakRankingManager.getInstance():getActGroup()
	    	if groupId ~= 3 and buffId == 5 then
	    		buffId = 4
	    	end
	    	local prefix = ""
	    	...
	    	local buffImage = UIHelper:createUI('tempFunctionRes/WinningStreakRanking/WinningStreakingRankingRes.json', 'WinningStreakingRanking/buff'.. prefix ..buffId)
	    	buffImage:setScale(1.3)
	    	local posX = {-80, -50, -80, -100, -100, -80, -100, -100, -80, -100, -100, -80, -100, -100, -100, -100}
	    	local posY = { 50,  50,  40,   40,   40,  50,  40,    40,  50,   40,   40,  50,   40,   40,  40,   40}
	    	buffImage:setPositionX(posX[buffId])
	    	buffImage:setPositionY(posY[buffId])
	    	buffIcon:addChild(buffImage.refCocosObj)
	    	buffImage:dispose()
			...
			local function onBubbleBreak()
				...
				for k,v in pairs( initBuffResult ) do
					...
					if itemView --[[ and buffType == 4 ]] then
						...
						local flyEff = ArmatureNode:create("winningstreakrankingbuff/FlyEff")
						flyEff:playByIndex(0)
						flyEff:update(0.001) -- 此处的参数含义为时间
					    ...
					    actArr:addObject(CCCallFunc:create(function ()
					        ...
					        playWinningBoomEff(globalTagPos)
					        theAction.addInfo = "doAddBuff"
					    end))
...
function GlobalCoreActionLogic:runView_AddBuffAdd2Step(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		theAction.actionStatus = GameActionStatus.kRunning
		local function allPrePropFinishCallback()
			theAction.addInfo = "over"
		end
		boardView.PlayUIDelegate:playPreGamePropAddStep2Anim(theAction.data, allPrePropFinishCallback)
		setTimeOut( function () GamePlayMusicPlayer:playEffect( GameMusicType.kPropAdd3stepFlyon ) end , 1 )
	end
end

function GlobalCoreActionLogic:runLogic_AddBuffAdd2Step(mainLogic, theAction, actid, actByView)
	if theAction.addInfo == "over" then
		if theAction.completeCallback then
			theAction.completeCallback( theAction.fromGuide )
		end
...
function GlobalCoreActionLogic:runView_AddBuffSpecialAnimal(boardView, theAction)
	...
	if theAction.actionStatus == GameActionStatus.kRunning then
		theAction.jsq = theAction.jsq + 1
		if theAction.jsq == theAction.frameAdd then
			animFinishCallback()
		elseif theAction.jsq == theAction.frameFinish then
	...
	elseif theAction.actionStatus == GameActionStatus.kWaitingForStart then
		...
		if theAction.itemId == ItemType.ADD_FIVE_STEP_BY_MATCH_FESTIVAL then
			-- 加五小动物，前置道具里没有，特殊动画逻辑
			local sprite = TileAddMove:create(AnimalTypeConfig.kYellow, 5)
			boardView:addChild(sprite)
			local startPos = theAction.startPos or ccp(360,640)
		    local startPos = boardView:convertToNodeSpace(startPos)
			sprite:setPositionXY(startPos.x,startPos.y)
			
			local pos = theAction.pos
			local itemView = boardView.baseMap[pos.r][pos.c]
			local posItem = itemView:getBasePosition(pos.c,pos.r)
			Tween.to(sprite,0.7,{delay = 0,x = posItem.x,y = posItem.y,onComplete = function()
				sprite:removeFromParentAndCleanup(true)
				-- animFinishCallback()
				-- PropsAnimation:playWrapEffect(pos.r, pos.c, boardView, allPrePropFinishCallback)
				PropsAnimation:playWrapEffect(pos.r, pos.c, boardView, nil)
			end})
			theAction.frameAdd = 50
			theAction.frameFinish = 80
			setTimeOut( function () GamePlayMusicPlayer:playEffect( GameMusicType.kPropAdd3stepFlyon ) end , 1 )
			return
		end
		...
		if validData then
			if theAction.startPos then
				data.destXInWorldSpace = theAction.startPos.x
				data.destYInWorldSpace = theAction.startPos.y
			end
			-- boardView.PlayUIDelegate:playCommonAddPrePropEffect(theAction.buffType, data, lineData, wrapData, animFinishCallback, allPrePropFinishCallback,theAction.skipStart)
			boardView.PlayUIDelegate:playCommonAddPrePropEffect(theAction.buffType, data, lineData, wrapData, nil, nil,theAction.skipStart)
			theAction.frameAdd = 50
			theAction.frameFinish = 80
		else
			allPrePropFinishCallback()
...
function GlobalCoreActionLogic:runLogic_AddBuffSpecialAnimal(mainLogic, theAction, actid, actByView)
	...
	elseif theAction.addInfo == "over" then
		-- print("GlobalCoreActionLogic:runLogic_AddBuffSpecialAnimal()over:",theAction.completeCallback)
		mainLogic:setNeedCheckFalling()
		if theAction.completeCallback then
			theAction.completeCallback( theAction.fromGuide )
		end
...
function GlobalCoreActionLogic:runView_SummerFishFlyLogic(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		...
		if node.pieceList then
			FishItemList = node.pieceList
		else
			for i=1, pieceAmount do
				local key = "piece"..i;
				FishItemList[i] = UIHelper:getCon(node,key)
		...
		local function MoveEnd( NoAdd )
            ...
            if bAdd then
                ...
                if MoveEndNum > pieceAmount then
                    MoveEndNum = pieceAmount
                end
            ...
            -- if MoveEndNum == pieceAmount and bNumberAniEnd then
            if MoveEndNum == pieceAmount then
        ...
        local function onFishMoveOut()
            ...
            for i=1, pieceAmount do
				...
				local FishDstPos = nil
				if node.autoDestPos then
					FishDstPos = destPos
				else
					FishDstPos = node:convertToNodeSpace(destPos)
				end

	            b:addObject(CCEaseSineInOut:create(CCMoveTo:create(moveTime, ccp(FishDstPos.x, FishDstPos.y))))
	            b:addObject(CCFadeTo:create(moveTime, 30))
	            b:addObject(CCScaleTo:create(moveTime, 0.3))

                c:addObject(CCDelayTime:create(moveTime+i*0.2))	--2
...
function GlobalCoreActionLogic:runLogic_BlastFirecracker(mainLogic, theAction, actid, actByView)
	...
	elseif theAction.addInfo == "explodeTarget" then
		for _, v in pairs(theAction.targetPositions) do
			...
			rectangleAction.SpecialID = theAction.SpecialID
...
function GlobalCoreActionLogic:runView_kCSYEBranchProgress(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.jsq == 0 then
			StarBuffYEManager.getInstance():doPlayBuffAnim(boardView.PlayUIDelegate, theAction.startPos, theAction.scoreToShow, function ()
				theAction.addInfo = "over"
			end)
		end
		theAction.jsq = theAction.jsq + 1
...
function GlobalCoreActionLogic:runView_DHCLevelTip(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.jsq == 0 then
...
function GlobalCoreActionLogic:runLogic_DHCLevelTip(mainLogic, theAction, actid, actByView)
	local function overCheck()
		if theAction.addInfo == "over" then
			if theAction.completeCallback then
				theAction.completeCallback()
	...
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		if mainLogic.PlayUIDelegate then
			...
			TurnTable2019DHCManager.getInstance():doPlayTipAnim(mainLogic, theAction, function ()
				theAction.addInfo = "over"
			end)
		else
			theAction.addInfo = "over" 
...
function GlobalCoreActionLogic:runView_ActivityCollectionItemShow(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.addInfo == "playItemCreateAnim" then
			local r, c = theAction.r, theAction.c
			local itemView = boardView.baseMap[r][c]
			if itemView then
				local itemId = theAction.collectionItemId
				local toPos = boardView:convertToWorldSpace(itemView:getBasePosition(c, r))

				ActivityClollectionItemLogic:playItemShowAnim(itemId, toPos)
			end

			theAction.addInfo = "waitAnimEnd"
			theAction.jsq = 0
		elseif theAction.addInfo == "waitAnimEnd" then
			if theAction.jsq == 90 then
				theAction.addInfo = "createItem"

				local r, c = theAction.r, theAction.c
				local itemView = boardView.baseMap[r][c]
				itemView:playAnimationAnimalDestroy_DestroyEffect(1)
			end
		end
...
function GlobalCoreActionLogic:runLogic_ActivityCollectionItemShow(mainLogic, theAction, actid, actByView)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		theAction.jsq = 0
		theAction.actionStatus = GameActionStatus.kRunning

		if mainLogic.PlayUIDelegate then
			theAction.addInfo = "playItemCreateAnim"
		else
			theAction.addInfo = "over" 
		end
	end
	...
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.addInfo == "createItem" then
			local r = theAction.r
			local c = theAction.c

			local itemID = theAction.collectionItemId
			local tItem = mainLogic.gameItemMap[r][c]

			tItem:changeToActivityCollectionItem( itemID )
...
-------------------------------------------------------------------------------
function GlobalCoreActionLogic:runView_TravelHeroReachEnd(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.jsq == 0 then

	  		local heroGrid = theAction.heroGrid
	  		local finalBuildingGrid = theAction.finalBuildingGrid
	  		local isFinalMap = theAction.isFinalMap

	  		-- 主人公信仰之跃
	  		if boardView and boardView.baseMap and boardView.baseMap[heroGrid.y] then
		  		local itemView = boardView.baseMap[heroGrid.y][heroGrid.x]
				if itemView then
					local sprite = itemView:getGameItemSprite()
					if sprite then
						local position = UsePropState:getItemPosition(
							IntCoord:create(finalBuildingGrid.y, finalBuildingGrid.x)
							)
						local jumpAction = CCSpawn:createWithTwoActions(CCMoveTo:create(0.2, position), CCScaleTo:create(0.5))
						if isFinalMap then
							jumpAction = CCMoveTo:create(0.2, position)
						end
						...
						itemView:playTravelHeroJumpToEnd()
					end
				end
			end
	  		...
	  		-- 最后一张图，终点建筑也有动画
	  		if isFinalMap then
	  			if boardView and boardView.baseMap and boardView.baseMap[finalBuildingGrid.y] then
	  				local buildingView = boardView.baseMap[finalBuildingGrid.y][finalBuildingGrid.x]
					if buildingView then
						buildingView:playTravelReachEndBuildingAnimation()
					end
	  			end
	  		end
		end
...
function GlobalCoreActionLogic:runView_Act_RotaryTable(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then 
		if theAction.jsq == 0 then
			local scene = Director:sharedDirector():getRunningSceneLua()

			local con = CocosObject:create()
			local bubbleTip = UIHelper:createUI('tempFunctionRes/RotaryTableAct2020/RotaryTableAct2020.json', 'rotaryTableOther/bubbleTip')
			local visibleSize	= CCDirector:sharedDirector():getVisibleSize()
			local visibleOrigin	= CCDirector:sharedDirector():getVisibleOrigin()

			con:addChild(bubbleTip)

			local width = bubbleTip:getGroupBounds().size.width
			local tx = visibleOrigin.x + (visibleSize.width / 2)
			local ty = visibleOrigin.y + (visibleSize.height / 2 )
			con:setPositionXY(tx - width/2, ty + 400)

			if scene and scene.topEffectLayer then 
				scene.topEffectLayer:addChild(con)
			end
		end

		theAction.jsq = theAction.jsq + 1

		if theAction.jsq == 180 then
			theAction.addInfo = "over"
		end
	end
end

function GlobalCoreActionLogic:runLogic_TravelHeroReachEnd(mainLogic, theAction, actid, actByView)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		theAction.actionStatus = GameActionStatus.kRunning
		theAction.jsq = 0
	end

	if theAction.addInfo == "flyTarget" then
...
function GlobalCoreActionLogic:runView_WalkChickReachEnd(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.jsq == 0 then

	  		local walkChick = theAction.walkChick
	  		local walkChickEnd = theAction.walkChickEnd

	  		local function completeCallback( ... )
	  			if boardView and boardView.baseMap and boardView.baseMap[walkChickEnd.y] then
	  				local itemView = boardView.baseMap[walkChickEnd.y][walkChickEnd.x]
					if itemView then
						itemView:playWalkChickEndDisappearAnimation()
					end
	  			end
	  		end

	  		if boardView and boardView.baseMap and boardView.baseMap[walkChick.y] then
		  		local itemView = boardView.baseMap[walkChick.y][walkChick.x]
				if itemView then
					local sprite = itemView:getGameItemSprite()
					if sprite then
						local position = UsePropState:getItemPosition(
							IntCoord:create(walkChickEnd.y, walkChickEnd.x)
							)
						-- local jumpAction = CCSpawn:createWithTwoActions(CCMoveTo:create(0.2, position), CCScaleTo:create(0.5))
						
						local jumpAction = CCMoveTo:create(0.2, position)
						local arr = CCArray:create()
						arr:addObject(CCDelayTime:create(0.05))
						arr:addObject(jumpAction)
						arr:addObject(CCDelayTime:create(0.1))
						...
						itemView:playWalkChickJumpToEnd()
					end
				end
			end
		...
		if theAction.jsq == 36 then
			theAction.addInfo = "flyTarget"
		elseif theAction.jsq == 100 then
			theAction.addInfo = "over"
		end
	end
end

function GlobalCoreActionLogic:runLogic_WalkChickReachEnd(mainLogic, theAction, actid, actByView)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		theAction.actionStatus = GameActionStatus.kRunning
		theAction.jsq = 0
	end
	...
	if theAction.addInfo == "flyTarget" then
	    ...
	    BridgeCrossLogic:cleanEndItemData(mainLogic,theAction.walkChickItem,theAction.bridgeEndItem)
		theAction.addInfo = nil
...
function GlobalCoreActionLogic:runView_Act_CollectionDouble(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.jsq == 0 then
...
function GlobalCoreActionLogic:runLogic_Act_CollectionDouble(mainLogic, theAction, actid, actByView)
	local function overCheck()
		if theAction.addInfo == "over" then
			if theAction.completeCallback then
				theAction.completeCallback()
...
-------------------------------------------------------------------------------
function GlobalCoreActionLogic:runView_SlyBunnyFleeing(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.jsq == 0 then
			for _, targetMovePlan in ipairs(theAction.bunnyMovePlanList) do
				local targetBunny = targetMovePlan.targetBunny
				local nextPos = targetMovePlan.nextPos
				if targetBunny and nextPos then
					local bunnyView = boardView:safeGetItemView(targetBunny.y, targetBunny.x)
					if bunnyView then
						bunnyView:playBunnyLeavesLane()
					end
					local moveDirection = ItemViewUtils:getDirectionByShiftValue(targetBunny.y - nextPos.r, targetBunny.x - nextPos.c)
					local nextGridView = boardView:safeGetItemView(nextPos.r, nextPos.c)
					if nextGridView then
						nextGridView:playSlyBunnyMoveFrom(moveDirection)
					end
				end
			end
		end
...
function GlobalCoreActionLogic:runLogic_SlyBunnyFleeing(mainLogic, theAction, actid, actByView)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		theAction.actionStatus = GameActionStatus.kRunning
		theAction.jsq = 0
	end
	...
	if theAction.addInfo == "over" then
		for _, targetMovePlan in ipairs(theAction.bunnyMovePlanList) do
			local targetBunny = targetMovePlan.targetBunny
			local nextPos = targetMovePlan.nextPos
			SlyBunnyLogic:updateBunnyPos(mainLogic, targetBunny, nextPos)
		end
...
function GlobalCoreActionLogic:runLogic_Act_RotaryTable(mainLogic, theAction, actid, actByView)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		theAction.actionStatus = GameActionStatus.kRunning
		theAction.jsq = 0
	end
...
function GlobalCoreActionLogic:runView_Act_Treasure_Hunt_Collect( boardView, theAction )
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		theAction.actionStatus = GameActionStatus.kRunning
		theAction.jsq = 0

		theAction.addInfo = 'waitingAnim'
		local picture = boardView.showPanel[ItemSpriteType.kTreasureRoadMode]
		if picture then
			local count = picture:getNumOfChildren() or 0
			if count <= 0 then theAction.addInfo = 'playFly' end
			for k,v in pairs(picture:getChildrenList()) do
				if v.bg and not v.bg.isDisposed then
					v.bg:setVisible(true)
					v.bg:setOpacity(255*0.28)
					local arr = CCArray:create()
					arr:addObject(CCFadeTo:create(0.4, 255))
					arr:addObject(CCFadeTo:create(0.4, 255*0.28))
					...
					arr:addObject(CCCallFunc:create(function( ... )
						...
						count = count - 1
						if count == 0 then
							theAction.addInfo = 'playFly'
						end
					end))
					v.bg:runAction(CCSequence:create(arr))
				end
			end
		else
			theAction.addInfo = 'playFly'
...
function GlobalCoreActionLogic:runLogic_Act_Treasure_Hunt_Collect(mainLogic, theAction, actid, actByView)
	if theAction.addInfo == "waitingAnim" then
		theAction.jsq = theAction.jsq + 1
		if theAction.jsq >= 140 then
			theAction.addInfo = "playFly"
		end
	elseif theAction.addInfo == "playFly" then
		theAction.jsq = 0
		mainLogic.PlayUIDelegate:setTargetNumber(0,0, 0, ccp(0,0))
		...
		for r = 1, #gameItemMap do
			for c = 1, #gameItemMap[r] do
				...
				if itemData and itemData.ItemType == GameItemType.kTreasureFish then
					mainLogic:addScoreToTotal(r, c, 1000)
				end
			end
		end
		theAction.addInfo = "playingFly"
	elseif theAction.addInfo == "playingFly" then
		theAction.jsq = theAction.jsq + 1
		if theAction.jsq >= 72 then
			theAction.addInfo = "over"
		end
	elseif theAction.addInfo == "over" then
...
function GlobalCoreActionLogic:runView_ACT_MatchFestival_Progress(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.jsq == 0 then
			if theAction.topArea then
				theAction.topArea:doPlayVipAnim(theAction.startPos, theAction.scoreToShow,theAction.vipType, function ()
					theAction.addInfo = "over"
				end)
			end
		end
...
function GlobalCoreActionLogic:runLogic_ACT_MatchFestival_Progress(mainLogic, theAction, actid, actByView)
	local function overCheck()
		if theAction.addInfo == "over" then
			if theAction.completeCallback then
				theAction.completeCallback()
...
-------------------------------------------------------------------------------
function GlobalCoreActionLogic:runView_ACT_LuckyBag2020_Progress(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.jsq == 0 then
			if theAction.topArea then
...
function GlobalCoreActionLogic:runLogic_ACT_LuckyBag2020_Progress(mainLogic, theAction, actid, actByView)
	local function overCheck()
		if theAction.addInfo == "over" then
			if theAction.completeCallback then
				theAction.completeCallback()
	...
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		...
		-- 	mainLogic.PlayUIDelegate.scoreProgressBar and 
		-- 	mainLogic.PlayUIDelegate.scoreProgressBar.ladyBugAnimation and 
		-- 	mainLogic.PlayUIDelegate.scoreProgressBar.ladyBugAnimation.moveTo then
		-- 		mainLogic.PlayUIDelegate.scoreProgressBar.ladyBugAnimation:moveTo(0,theAction.extraProgress)
		-- 		theAction.actionStatus = GameActionStatus.kRunning
		-- 		theAction.jsq = 0
		if theAction.topArea then
			theAction.topArea:doPlayAddBuffAnim(theAction.startPos,theAction.extraProgress)
			theAction.actionStatus = GameActionStatus.kRunning
			theAction.jsq = 0
		else
			theAction.addInfo = "over" 
...
-------------------------------------------------------------------------------
function GlobalCoreActionLogic:runView_BattleTrophyGenerate(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.jsq == 0 then
			for _, targetItem in ipairs(theAction.pickedTargets) do
				local gridView = boardView.baseMap[targetItem.y][targetItem.x]
				gridView:removeItemSpriteGameItem()
				gridView:cleanFurballView()
				gridView.itemSprite[ItemSpriteType.kItem] = ItemViewUtils:buildBeanpod()
			end
		end
...
function GlobalCoreActionLogic:runLogic_BattleTrophyGenerate(mainLogic, theAction, actid, actByView)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		theAction.actionStatus = GameActionStatus.kRunning
		theAction.jsq = 0
	end
	...
	if theAction.addInfo == "updateData" then
		GoldenPodBattleLogic:replaceGridDataByTrophy(mainLogic, theAction.pickedTargets)
	end
