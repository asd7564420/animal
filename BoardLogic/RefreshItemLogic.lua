...
-- 从 selectionPool 的候选位置中，选出符合 color 颜色的item，从数据源 sourceGameItemMap 制作副本后放入结果表 destGameItemMap 中
-- 之后，更新 selectionPool，去除刚刚被选走的对象相关标记
local function tryAssignItemWithColor(selectionPool, sourceGameItemMap, destGameItemMap, position, color, isSpecial)
	for r, row in pairs(selectionPool.items) do
		for c, col in pairs(row) do
			...
			then
				local newItemData = item:copy()
				newItemData.shieldToColorFilter = true --刷新时是先把数据改了，再播动画，添加标记以避免过滤器提前消除未真正刷新到该位置的对象
				destGameItemMap[position.r][position.c] = newItemData
...
local function tryExchangeItemColor(newGameItemMap, needExchangeMap, position, fixedPositions, refreshAreaReferMap)
	-- printx(11, "======= tryExchangeItemColor ========")
	-- printx(11, "=== fixedPositions", table.tostring(fixedPositions))
	for r = 1, #newGameItemMap do
		for c = 1, #newGameItemMap[r] do
			local item = newGameItemMap[r][c]
			local color = item._encrypt.ItemColorType
			if RefreshItemLogic.isItemRefreshableInRefreshArea(item, refreshAreaReferMap) 
			and not isItemInFixedPositions(r, c, fixedPositions)
			and AnimalTypeConfig.isColorTypeValid(color) --[[or color == AnimalTypeConfig.kDrip]] 
			then
				local banned = false
				-- 检查颜色是否被ban
				-- printx(11, "item(r,c), colour:", item.y, item.x, AnimalTypeConfig.convertColorTypeToIndex(color))
				-- printx(11, "BAN colour:", table.tostring(needExchangeMap[position.r][position.c].banColor))
...
-- 扫描棋盘上当前是否存在能直接三连匹配的地方，选择其中一个标记为需要移走，并记录此位置与别人犯冲的颜色
		...
		for c = 1, #newGameItemMap[r] do
			...
			then
				...
				if not isItemInFixedPositions(r, c, fixedPositions) and RefreshItemLogic.isItemRefreshable(item1) then
					...
					-- printx(11, "Add to ban colour:", r, c, item1, item1._encrypt.ItemColorType)
				elseif not isItemInFixedPositions(r, c+1, fixedPositions) and RefreshItemLogic.isItemRefreshable(item2) then
					...
					-- printx(11, "Add to ban colour:", r, c+1, item2, item2._encrypt.ItemColorType)
				elseif not isItemInFixedPositions(r, c+2, fixedPositions) and RefreshItemLogic.isItemRefreshable(item3) then
					...
					-- printx(11, "Add to ban colour:", r, c+2, item3, item3._encrypt.ItemColorType)
				else
			...
			if newGameItemMap[r+1] and newGameItemMap[r+2] then
				...
				then
					...
					elseif not isItemInFixedPositions(r+1, c, fixedPositions) and RefreshItemLogic.isItemRefreshable(item2) then
						...
						-- printx(11, "Add to ban colour:", r+1, c, item2, item2._encrypt.ItemColorType)
					elseif not isItemInFixedPositions(r+2, c, fixedPositions) and RefreshItemLogic.isItemRefreshable(item3) then
						...
						-- printx(11, "Add to ban colour:", r+2, c, item3, item3._encrypt.ItemColorType)
					else
						return false, needExchangeMap
					end
				end
...
function RefreshItemLogic:checkCrystalStoneNeedRefresh(mainLogic, rowLimit, colLimit)
	local numRow = rowLimit or #mainLogic.gameItemMap
	for r = 1, numRow do
		local numCol = colLimit or #mainLogic.gameItemMap[r]
		for c = 1, numCol do
			...
			if c2 <= #mainLogic.gameItemMap[r] and (SwapItemLogic:canBeSwaped(mainLogic, r1,c1,r2,c2) == 1) then
				if SwapItemLogic:SwapedCrystalStoneAndMatch(mainLogic, r1,c1,r2,c2) then
					return false
				end
			end
			if r3 <= #mainLogic.gameItemMap and (SwapItemLogic:canBeSwaped(mainLogic, r1,c1,r3,c3) == 1) then
				if SwapItemLogic:SwapedCrystalStoneAndMatch(mainLogic, r1,c1,r3,c3) then
					return false
				end
			end
		end
...
function RefreshItemLogic:runRefreshAction(mainLogic, isProp, completeCallback)
	-- printx( 1 , "RefreshItemLogic:runRefreshAction", debug.traceback())
	local numItem = 0
	...
	-- 	end
	-- end
	local function createAction( r , c )
		...
		-- printx( 1 , "RefreshItemLogic:runRefreshAction  FlyingAction       " , isProp , item.y, item.x, "-->", r, c)
	...
	for r=1,#mainLogic.gameItemMap do
		for c=1,#mainLogic.gameItemMap[r] do
			...
			if not (r == item.y and c == item.x) then
				createAction( r , c )
				numItem = numItem + 1
			end
		end
	...
	mainLogic.squidOnBoard = nil	-- 刷新会重做itemData，故需清除老数据
...
-- refreshAreaReferMap 当前刷新区域速查表
function SpecialSwapArea:isValide(mainLogic, refreshAreaReferMap)
	...
	if itemA and itemB then
		...
		then
			...
			if self.direction == 'U' then
				if boardA:hasBottomRope(nil, true) or boardB:hasTopRope(nil, true) then
					return false
				end
			elseif self.direction == 'L' then
				if boardA:hasRightRope(true) or boardB:hasLeftRope(true) then
					return false
				end
			end

			-- 特效组合两格都需要参与交换，所以都必须在刷新组内
			if refreshAreaReferMap then
				if not RefreshItemLogic.isGridInCurrRefreshArea(self.posA.r, self.posA.c, refreshAreaReferMap) then
					return false
				end
				if not RefreshItemLogic.isGridInCurrRefreshArea(self.posB.r, self.posB.c, refreshAreaReferMap) then
					return false
				end
			end
...
-- refreshAreaReferMap 当前刷新区域速查表
-- strictRefreshArea - true： 当有刷新区域时，严格遵守刷新区域，SwapArea中只能包含区域内对象
--                     false： 当有刷新区域时，宽松遵守刷新区域，区域内对象可以与区域外对象组合为SwapArea，但区域外对象视为不可移动
function SwapArea:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea)
	...
	then 
		...
		-- 判断AD之间是否有绳子
		if self.direction == 'U' then
			if boardA:hasBottomRope(nil, true) or boardD:hasTopRope(nil, true) then
				return false
			end
		elseif self.direction == 'D' then
			if boardA:hasTopRope(nil, true) or boardD:hasBottomRope(nil, true) then
				return false
			end
		elseif self.direction == 'L' then
			if boardA:hasRightRope(true) or boardD:hasLeftRope(true) then
				return false
			end
		elseif self.direction == 'R' then
			if boardA:hasLeftRope(true) or boardD:hasRightRope(true) then
				return false
			end
		end

		-- 有刷新组时，根据需求检查所有格子与刷新组的包含关系
		if refreshAreaReferMap then
			-- AD是需要移动的，必须在区域内
			if not RefreshItemLogic.isGridInCurrRefreshArea(self.posA.r, self.posA.c, refreshAreaReferMap) then
				return false
			end
			if not RefreshItemLogic.isGridInCurrRefreshArea(self.posD.r, self.posD.c, refreshAreaReferMap) then
				return false
			end
			-- 严格条件下，BC也必须在区域内；而在宽松条件下，BC可以不在区域内
			if strictRefreshArea then
				if not RefreshItemLogic.isGridInCurrRefreshArea(self.posB.r, self.posB.c, refreshAreaReferMap) then
					return false
				end
				if not RefreshItemLogic.isGridInCurrRefreshArea(self.posC.r, self.posC.c, refreshAreaReferMap) then
					return false
				end
			end
		end
...
function RefreshItemLogic.isItemRefreshable(item)
	if item.isUsed and not item:hasLock() and not item:hasFurball()
		-- and (item.ItemType == GameItemType.kAnimal or item.ItemType == GameItemType.kCuckooWindupKey)
...
-- 对象可刷新且属于目标刷新组或友好邻邦
function RefreshItemLogic.isItemRefreshableInRefreshArea(item, refreshAreaReferMap)
	if RefreshItemLogic.isItemRefreshable(item) and RefreshItemLogic.isGridInCurrRefreshArea(item.y, item.x, refreshAreaReferMap) then
		return true
	end
	return false
end
...
-- swap_areas: 			所有满足【匹配】交换形状、并且当下能进行交换的区域格子组合（4种形状x4个方向），有重叠
-- special_swap_areas: 	所有满足【特效】交换形状、并且当下能进行交换的区域格子组合（1种形状x2个方向），有重叠
-- colorStatistics:		所有颜色中，可参与刷新的item的颜色统计
-- refreshableItems:	.items[r][c]=true:该位置item可参与刷新，colors[colorIndex].count:可参与刷新的item的颜色统计
-- strictRefreshArea  - true： 当有刷新区域时，严格遵守刷新区域，SwapArea中只能包含区域内对象
--                    - false： 当有刷新区域时，宽松遵守刷新区域，区域内对象可以与区域外对象组合为SwapArea，但区域外对象视为不可移动
function RefreshItemLogic.getProcessedGameBoard(mainLogic, refreshAreaReferMap, strictRefreshArea)
	...
	for r = 1, #gameItemMap do
		for c = 1, #gameItemMap[r] do
			...
			if t1:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t1) end
			local t2 = SwapArea:create({r=r, c=c}, 'T', 'R')
			if t2:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t2) end
			local t3 = SwapArea:create({r=r, c=c}, 'T', 'U')
			if t3:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t3) end
			local t4 = SwapArea:create({r=r, c=c}, 'T', 'D')
			if t4:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t4) end
			local t5 = SwapArea:create({r=r, c=c}, 'L', 'L')
			if t5:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t5) end
			local t6 = SwapArea:create({r=r, c=c}, 'L', 'R')
			if t6:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t6) end
			local t7 = SwapArea:create({r=r, c=c}, 'L', 'U')
			if t7:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t7) end
			local t8 = SwapArea:create({r=r, c=c}, 'L', 'D')
			if t8:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t8) end
			local t9 = SwapArea:create({r=r, c=c}, 'RL', 'L')
			if t9:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t9) end
			local t10 = SwapArea:create({r=r, c=c}, 'RL', 'R')
			if t10:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t10) end
			local t11 = SwapArea:create({r=r, c=c}, 'RL', 'U')
			if t11:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t11) end
			local t12 = SwapArea:create({r=r, c=c}, 'RL', 'D')
			if t12:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t12) end
			local t13 = SwapArea:create({r=r, c=c}, 'I', 'L')
			if t13:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t13) end
			local t14 = SwapArea:create({r=r, c=c}, 'I', 'R')
			if t14:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t14) end
			local t15 = SwapArea:create({r=r, c=c}, 'I', 'U')
			if t15:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t15) end
			local t16 = SwapArea:create({r=r, c=c}, 'I', 'D')
			if t16:isValide(mainLogic, refreshAreaReferMap, strictRefreshArea) then table.insert(swap_areas, t16) end

			local s1 = SpecialSwapArea:create({r=r, c=c}, 'U')
			if s1:isValide(mainLogic, refreshAreaReferMap) then table.insert(special_swap_areas, s1) end
			local s2 = SpecialSwapArea:create({r=r, c=c}, 'L')
			if s2:isValide(mainLogic, refreshAreaReferMap) then table.insert(special_swap_areas, s2) end
	...
	for r = 1, #gameItemMap do
		for c = 1, #gameItemMap[r] do
			local item = gameItemMap[r][c]
			if RefreshItemLogic.isItemRefreshableInRefreshArea(item, refreshAreaReferMap) then
...
function RefreshItemLogic.tryNormalRefresh(mainLogic)
	-- printx(11, "=========== RefreshItemLogic.tryNormalRefresh ===============")

	-- 新增：刷新区域
	-- 规则：
	-- 1、本刷新组区域内移动，构建本组区域范围内的可交换
	-- 		若所有刷新组规则1均失败，则
	-- 2、本刷新组区域内移动，构建所有范围内的可交换（即，组内可以和外界组成匹配）
	-- 		若所有刷新组规则2均失败，则
	-- 3、恢复老刷新规则，全局移动刷新，之后判定同老规则

	local hasRefreshGroup = false --当前棋盘上是否有刷新组
	local rawRefreshGroups = {}
	for r = 1, #mainLogic.boardmap do 
		for c = 1, #mainLogic.boardmap[r] do
			local boardData = mainLogic:safeGetBoardData(r, c)
			if boardData then
				local refreshAreaFlag = 0 --没有标记的组别
				if boardData.refreshAreaFlag and boardData.refreshAreaFlag > 0 then
					refreshAreaFlag = boardData.refreshAreaFlag
					hasRefreshGroup = true
				end

				if not rawRefreshGroups[refreshAreaFlag..""] then
					rawRefreshGroups[refreshAreaFlag..""] = {}
				end
				table.insert(rawRefreshGroups[refreshAreaFlag..""], {r=r, c=c})
			end
		end
	end
	-- printx(11, "rawRefreshGroups", hasRefreshGroup, table.tostring(rawRefreshGroups))

	-- 构造刷新组速查表，属于该组的格子标记为true，其它格子为false。节省效率用。
	local function formRefreshAreaReferMap(refreshGroupGrids)
		local gameItemMap = mainLogic.gameItemMap
		local refreshAreaMap = {}

		for r = 1, #gameItemMap do
			refreshAreaMap[r] = {}
			for c = 1, #gameItemMap[r] do
				refreshAreaMap[r][c] = false
			end
		end

		for _, grid in pairs(refreshGroupGrids) do
			if refreshAreaMap[grid.r] then
				refreshAreaMap[grid.r][grid.c] = true
			end
		end
		return refreshAreaMap
	end

	-- printx(11, "=========== REFRESH ==== Round 1 ===============")
	-------------------------------------------------------------------
	-- 第一轮：尝试组内刷新和匹配
	local refreshAndMatchInGroupSucceed = false
	if hasRefreshGroup and rawRefreshGroups then
		for areaFlagStr, groupGrids in pairs(rawRefreshGroups) do
			local refreshAreaReferMap = formRefreshAreaReferMap(groupGrids)
			-- printx(11, "=== Try refresh area "..areaFlagStr.." !")
			-- printx(11, "refreshAreaReferMap", areaFlagStr, table.tostring(refreshAreaReferMap))
			local resultFlag = RefreshItemLogic.tryNormalRefreshByGroup(mainLogic, refreshAreaReferMap, true)
			if resultFlag then
				-- printx(11, "=== !!! refresh in area "..areaFlagStr.." succeed !!!")
				refreshAndMatchInGroupSucceed = true
			end
		end
	end

	if refreshAndMatchInGroupSucceed then
		return true
	end

	-- printx(11, "=========== REFRESH ==== Round 2 ===============")
	-------------------------------------------------------------------
	-- 第二轮：尝试组内刷新，可以和组外匹配
	local refreshInGroupSucceed = false
	if hasRefreshGroup and rawRefreshGroups then
		for areaFlagStr, groupGrids in pairs(rawRefreshGroups) do
			local refreshAreaReferMap = formRefreshAreaReferMap(groupGrids)
			-- printx(11, "refreshAreaReferMap", areaFlagStr, table.tostring(refreshAreaReferMap))
			local resultFlag = RefreshItemLogic.tryNormalRefreshByGroup(mainLogic, refreshAreaReferMap, false)
			if resultFlag then
				refreshInGroupSucceed = true
			end
		end
	end

	if refreshInGroupSucceed then
		return true
	end

	-- printx(11, "=========== REFRESH ==== Round End ===============")
	-------------------------------------------------------------------
	-- 第三轮：回归老刷新逻辑，全盘刷新
	return RefreshItemLogic.tryNormalRefreshByGroup(mainLogic)
end

function RefreshItemLogic.isGridInCurrRefreshArea(r, c, refreshAreaReferMap)
	if not refreshAreaReferMap or not refreshAreaReferMap[r] then 
		-- 没有刷新区域或者是什么天外格子，统一放行
		return true 
	end
	if refreshAreaReferMap[r][c] then
		return true
	end
	return false
end

function RefreshItemLogic.tryNormalRefreshByGroup(mainLogic, refreshAreaReferMap, strictRefreshArea)
	-- swap_areas: 			所有满足【匹配】交换形状、并且当下能进行交换的区域格子组合（4种形状x4个方向），有重叠
	-- special_swap_areas: 	所有满足【特效】交换形状、并且当下能进行交换的区域格子组合（1种形状x2个方向），有重叠
	-- colorStatistics:		所有颜色中，可参与刷新的item的颜色统计
	-- refreshableItems:	.items[r][c]=true:该位置item可参与刷新，colors[colorIndex].count:可参与刷新的item的颜色统计
	local swap_areas, colorStatistics, refreshableItems, special_swap_areas 
		= RefreshItemLogic.getProcessedGameBoard(mainLogic, refreshAreaReferMap, strictRefreshArea)
	-- printx(11, "------ 000 swap_areas", table.tostring(swap_areas))
	-- printx(11, "------ 000 colorStatistics", table.tostring(colorStatistics))
	-- printx(11, "------ 000 refreshableItems", table.tostring(refreshableItems))
	-- printx(11, "------ 000 special_swap_areas", table.tostring(special_swap_areas))
	...
	if #swap_areas == 0 then
		...
		for k, v in ipairs(swap_areas) do
			...
			local result = RefreshItemLogic.tryWithSwapArea(mainLogic, gameItemMapCopy, v, colorStatistics, refreshableItems, nil, refreshAreaReferMap)
			if result then
	...
	if not hasBird and refreshableSpeicalCount < 2 or #special_swap_areas == 0 then
		...
		for k, v in ipairs(special_swap_areas) do
			...
			local result = RefreshItemLogic.trySpecialSwapArea(mainLogic, gameItemMapCopy, v, colorStatistics, refreshableItems, refreshAreaReferMap)
			if result then
...
-- 对于随机的某个满足匹配交换手势（但是现在无匹配）的区域，尝试把区域外棋盘上的可刷新item换进来，以形成交换后可匹配的局面
function RefreshItemLogic.tryWithSwapArea(mainLogic, gameItemMapCopy, swap_area, colorStatistics, refreshableItems, retryBanColor, refreshAreaReferMap)
	-- printx(11, "============= tryWithSwapArea ==================")
	...
	if not RefreshItemLogic.isItemRefreshableInRefreshArea(itemA, refreshAreaReferMap) then
		table.insert(unrefreshableItems, itemA)
	end
	if not RefreshItemLogic.isItemRefreshableInRefreshArea(itemB, refreshAreaReferMap) then
		table.insert(unrefreshableItems, itemB)
	end
	if not RefreshItemLogic.isItemRefreshableInRefreshArea(itemC, refreshAreaReferMap) then
	...
	if #unrefreshableItems > 0 then
		...
		for k, v in pairs(unrefreshableItems) do
			if v._encrypt.ItemColorType ~= fixedColor then
				-- printx(11, "============= tryWithSwapArea, fail 1: Different fixed colours")
				return false
			end
		end
	end
	...
	if fixedColor and table.exist(retryBanColor, fixedColor) then
		-- printx(11, "============= tryWithSwapArea, fail 2: Fixed colours banned")
		return false
	end
	...
	if fixedColor then
		-- 如果BC位(edit:是ABC吧？)有锁定的颜色，那么这个颜色不需要3个，而只需要3-#unrefreshableItems个就行
		...
		if not countObj then
			...
			-- printx(11, "============= tryWithSwapArea, fail 3: 'not science' in chinglish")
			return false
		end
		if countObj.count < 3 - #unrefreshableItems then
			-- printx(11, "============= tryWithSwapArea, fail 4: Not enough target fixed colour in valid grids")
			return false
		end
	elseif mostColor then 
		...
		if countObj.count < 3 then
			-- printx(11, "============= tryWithSwapArea, fail 5: Not enough target colour in valid grids")
			return false
		end
	elseif not mostColor then 
		-- 如果没有mostColor，应该是因为所有颜色都在retryBanColor里面
		-- 那么对于这个t_area已经失败，应该换下一个t_area再试
		-- printx(11, "============= tryWithSwapArea, fail 6: Couldn't pick out valid colour")
		return false
	end

	-- 经上面各种筛选后，为本轮尝试选择一个作为匹配目标的颜色
	if not fixedColor then
	...
	-- 首先给ABC号位赋颜色
	if RefreshItemLogic.isItemRefreshableInRefreshArea(itemA, refreshAreaReferMap) then
		if not tryAssignItemWithColor(selectionPool, gameItemMap, newGameItemMap, swap_area.posA, fixedColor) then 
			-- printx(11, "============= tryWithSwapArea, fail 7-A: Couldn't set colour to match group items")
			return false
		end
	end
	if RefreshItemLogic.isItemRefreshableInRefreshArea(itemB, refreshAreaReferMap) then
		if not tryAssignItemWithColor(selectionPool, gameItemMap, newGameItemMap, swap_area.posB, fixedColor) then 
			-- printx(11, "============= tryWithSwapArea, fail 7-B: Couldn't set colour to match group items")
			return false
		end
	end
	if RefreshItemLogic.isItemRefreshableInRefreshArea(itemC, refreshAreaReferMap) then
		if not tryAssignItemWithColor(selectionPool, gameItemMap, newGameItemMap, swap_area.posC, fixedColor) then 
			-- printx(11, "============= tryWithSwapArea, fail 7-C: Couldn't set colour to match group items")
			return false
		end
	end
	...
	for k, v in pairs(randomizedDestinationItems) do
		local color = nil
		for colorIndex, poolColor in pairs(selectionPool.colors) do
			if poolColor.count > 0 then
				color = colorListForSwap[colorIndex]
			end
		end
		if not tryAssignItemWithColor(selectionPool, gameItemMap, newGameItemMap, {r = v.r, c = v.c}, color) then
			-- printx(11, "============= tryWithSwapArea, fail 8: Couldn't set colour to other item")
			return false
		end
	end
	...
	-- printx(11, "!!!! needExchangeMap", success, table.tostring(needExchangeMap))
	...
	-- 交换已标记的item
	-- 尝试用 newGameItemMap 中其它地方的对象与记录中目前会组成消除的对象进行交换
	-- 为什么这里在前面 success = false 的情况下还要走一遍，无法理解
	for r = 1, #needExchangeMap do
		for c = 1, #needExchangeMap[r] do
			if needExchangeMap[r][c].item ~= nil then
				local result = tryExchangeItemColor(newGameItemMap, needExchangeMap, {r = r, c = c}, fixedPositions, refreshAreaReferMap)
				if not result then
					-- 只是本次选定的颜色无法形成满意的局面，不知道为什么直接返回失败而不Retry？
					-- printx(11, "============= tryWithSwapArea, fail 9: ??? Why not just retry?")
					return false
				end
			end
		end
	...
	if not retry then
		...
		return RefreshItemLogic.tryWithSwapArea(mainLogic, gameItemMapCopy, swap_area, colorStatistics, refreshableItems, retryBanColor, refreshAreaReferMap)
	end
end

function RefreshItemLogic.trySpecialSwapArea(mainLogic, gameItemMapCopy, special_swap_area, colorStatistics, refreshableItems, refreshAreaReferMap)
	local gameItemMap = gameItemMapCopy
	local selectionPool = table.clone(refreshableItems, true)
	local destination = table.clone(selectionPool, true)
