...
function GameInitDiffChangeLogic:tryChangeBoardByVirtualSeed(datas)
	...
	-- printx( 1 ,"GameInitDiffChangeLogic:tryChangeBoardByVirtualSeed !!!  self.mode = " , self.mode)
	...
	-- RemoteDebug:uploadLogWithTag( "VirtualSeed" , "tryChangeBoardByVirtualSeed  self.mode" , self.mode )
	...
	if not mode or mode < 1 or mode > modeCount then 
		...
		-- RemoteDebug:uploadLogWithTag( "VirtualSeed" , "tryChangeBoardByVirtualSeed  return 1" )
		return 
	end
						...
						if itemMap[r] and itemMap[r][c] and itemMap[centerR] and itemMap[centerR][centerC] then
							...
							if itemB:canBeSwap() then
								...
								if centerR > r and centerC == c then
									if boardA:hasTopRope(nil, true) or boardB:hasBottomRope(nil, true) then
								...
								elseif centerR == r and centerC > c then
									if boardA:hasLeftRope(true) or boardB:hasRightRope(true) then
