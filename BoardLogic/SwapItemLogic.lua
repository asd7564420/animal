	...
	if (r1 == r2 and c1 == c2 + 1)
		...
		if item1:canBeSwap() and item2:canBeSwap() then
			...
			if (r1 == r2 and c1 == c2 + 1) then ----2左1右
				if board1:hasLeftRope(true) or board2:hasRightRope(true) then
					return 2;
				end
			end
			if (r1 == r2 and c1 == c2 - 1) then ----1左2右
				if board1:hasRightRope(true) or board2:hasLeftRope(true) then
					return 2;
				end
			end
			if (r1 == r2 + 1 and c1 == c2) then ----2上1下
				if board1:hasTopRope(nil, true) or board2:hasBottomRope(nil, true) then
					return 2;
				end
			end
			if (r1 == r2 - 1 and c1 == c2) then ----1上2下
				if board1:hasBottomRope(nil, true) or board2:hasTopRope(nil, true) then
					return 2;
				end
			end
...
local function checkItemBlock(mainLogic, item1, item2, r1, c1, r2, c2)--重新计算格子的block状态
	if item1.ItemType == GameItemType.kMagicLamp or item2.ItemType == GameItemType.kMagicLamp 
		...
		or item1.ItemType == GameItemType.kNationalDayBunny or item2.ItemType == GameItemType.kNationalDayBunny
...
function SwapItemLogic:SwapedCrystalStoneAndMatch(mainLogic, r1, c1, r2, c2)
	...
	local function isCrystalStoneFullEnergy(item)
		return item:getCrystalStoneEnergy() >= GamePlayConfig_CrystalStone_Energy
	end
	...
	if item1.ItemType == GameItemType.kCrystalStone or item2.ItemType == GameItemType.kCrystalStone then
		...
		elseif item1.ItemType == GameItemType.kCrystalStone and isCrystalStoneFullEnergy(item1) and item2:canBeCoverByCrystalStone() then
		elseif item2.ItemType == GameItemType.kCrystalStone and isCrystalStoneFullEnergy(item2) and item1:canBeCoverByCrystalStone() then
		elseif item2.ItemType == GameItemType.kCrystalStone and isCrystalStoneFullEnergy(item2)
			and item1.ItemType == GameItemType.kCrystalStone and isCrystalStoneFullEnergy(item1) then
		else
			return false
		end
		return true
...
function SwapItemLogic:_trySwapedBlocker195(mainLogic, r1, c1, r2, c2, doSwap)
	...
	if item1:isBlocker195Available() or item2:isBlocker195Available() then
		...
		elseif table.exist({
					...
					GameItemType.kBalloon,
					GameItemType.kMagicLamp,
					GameItemType.kScoreBuffBottle,
					GameItemType.kFirecracker,
		...
		elseif item2.ItemType == GameItemType.kDynamiteCrate then
			if doSwap then
				SpecialMatchLogic:Blocker195WithSwap(mainLogic, GameItemActionType.kItemSpecial_Blocker195_Dynamite, r1, c1, r2, c2)
			end
		...
		elseif item2.ItemType == GameItemType.kWater then
			if doSwap then
				SpecialMatchLogic:Blocker195WithSwap(mainLogic, GameItemActionType.kItemSpecial_Blocker195_Water, r1, c1, r2, c2)
			end
		elseif item2.ItemType == GameItemType.kPlane then
			if doSwap then
				SpecialMatchLogic:Blocker195WithSwap(mainLogic, GameItemActionType.kItemSpecial_Blocker195_Plane, r1, c1, r2, c2)
			end
		elseif item2.ItemType == GameItemType.kpuffedRice then
			if doSwap then
				SpecialMatchLogic:Blocker195WithSwap(mainLogic, GameItemActionType.kItemSpecial_Blocker195_PuffedRice, r1, c1, r2, c2)
			end
		elseif item2.ItemType == GameItemType.kBattery then
			if doSwap then
				SpecialMatchLogic:Blocker195WithSwap(mainLogic, GameItemActionType.kItemSpecial_Blocker195_Battery, r1, c1, r2, c2)
			end
		else
			return false
		end
...
function SwapItemLogic:calculatePossibleSwap(mainLogic, priorityTable, needAll)
	...
	local function insertPossibleMoves(priority, possbileMoves)
		local checkNewPriority = BeginnerPlugin.checkAdvise(mainLogic, needAll , priority, possbileMoves)
		if checkNewPriority then
			priority = checkNewPriority
		else
