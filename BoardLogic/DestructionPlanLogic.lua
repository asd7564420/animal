...
function DestructionPlanLogic:runViewAction(boardView, theAction)
	...
	elseif theAction.actionType == GameItemActionType.kItemSpecial_Blocker195_Dynamite then
		DestructionPlanLogic:runGameItemBlocker195DynamiteCrateAction(boardView, theAction)
	...
	elseif theAction.actionType == GameItemActionType.kItemSpecial_Blocker195_Water then
		DestructionPlanLogic:runGameItemBlocker195WaterAction(boardView, theAction)
	elseif theAction.actionType == GameItemActionType.kItemSpecial_Blocker195_Plane then
		DestructionPlanLogic:runGameItemBlocker195PlaneAction(boardView, theAction)
	elseif theAction.actionType == GameItemActionType.kItem_WeeklyRace2020_Add_Five_Effect then
		DestructionPlanLogic:runWeeklyRace2020AddFiveEffectView(boardView, theAction)
	elseif theAction.actionType == GameItemActionType.kItemSpecial_Blocker195_PuffedRice then
		DestructionPlanLogic:runGameItemBlocker195PuffedRiceAction(boardView, theAction)
	elseif theAction.actionType == GameItemActionType.kItemSpecial_Blocker195_Battery then
		DestructionPlanLogic:runGameItemBlocker195BatteryAction(boardView, theAction)
	else
		local actionMap = {
			[GameItemActionType.kItem_CanevineBomb] = DestructionPlanLogic.runGameItemCanevineBombAction,
		}
		with(actionMap[theAction.actionType], function ( ... )
			it(DestructionPlanLogic, boardView, theAction)
		end)

	end
end

function DestructionPlanLogic:runGameItemCanevineBombAction( boardView, theAction )
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		theAction.actionStatus = GameActionStatus.kRunning 
		theAction.addInfo = 'play_anim_1'
	end

	if theAction.addInfo == 'play_anim_1' then

		local c1 = theAction.ItemPos1.x
		local r1 = theAction.ItemPos1.y

		local targetItemView = boardView.baseMap[r1][c1]
		targetItemView:play_canvine_withered_anim()

		theAction.addInfo = 'wait_play_anim_1'
		theAction.jsq = 0
	end
...
function DestructionPlanLogic:runningGameItemActionCanevineBomb( mainLogic, theAction, actid )

	if theAction.addInfo == 'wait_play_anim_1' then
	...
	if theAction.addInfo == 'wait_over' then
		theAction.jsq = theAction.jsq + 1
		if theAction.jsq >= theAction.wait_over_frames then
	...
	if theAction.addInfo == 'bomb' then
		...
		mainLogic:addScoreToTotal(theAction.ItemPos1.y, theAction.ItemPos1.x, GamePlayConfigScore.CanevineBomb)

		theAction.pos_list = {}
		theAction.step = 0
		theAction.jsq = 0
		theAction.jsq_counter = 0
	end

	if theAction.addInfo == 'bombing' then

		if theAction.jsq == 0 then

			local something_bomb = false

			if theAction.addInt == DefaultDirConfig.kUp or theAction.addInt == DefaultDirConfig.kDown then

				local c = theAction.ItemPos1.x
				for r = theAction.ItemPos1.y + theAction.step, theAction.ItemPos2.y, math.sign(theAction.ItemPos2.y - theAction.ItemPos1.y) do
					...
					lineAction.addInt2 = 1;
					lineAction.animInfo = 'canevine'
					mainLogic:addDestructionPlanAction(lineAction)

					table.insert(theAction.pos_list, {r=r, c=c})

					something_bomb = math.sign(theAction.ItemPos2.y - theAction.ItemPos1.y) ~= 0
					theAction.step = theAction.step + math.sign(theAction.ItemPos2.y - theAction.ItemPos1.y)

					break
				end
			end

			if theAction.addInt == DefaultDirConfig.kLeft or theAction.addInt == DefaultDirConfig.kRight then
				local r = theAction.ItemPos1.y
				for c = theAction.ItemPos1.x + theAction.step, theAction.ItemPos2.x, math.sign(theAction.ItemPos2.x - theAction.ItemPos1.x) do
					...
					table.insert(theAction.pos_list, {r=r, c=c})
					something_bomb = math.sign(theAction.ItemPos2.x - theAction.ItemPos1.x) ~= 0
					theAction.step = theAction.step + math.sign(theAction.ItemPos2.x - theAction.ItemPos1.x)
					break
				end
			end

			if not something_bomb then
				theAction.addInfo = 'wait_over'
				theAction.wait_over_frames = 48 - theAction.jsq_counter
				theAction.jsq = 0
			end
		...
		theAction.jsq = theAction.jsq + 1
		theAction.jsq_counter = theAction.jsq_counter + 1
	...
	if theAction.addInfo == "over" then
		CanevineLogic:clear(mainLogic, mainLogic.gameItemMap[theAction.ItemPos1.y][theAction.ItemPos1.x])
		for _, v in ipairs(theAction.pos_list or {}) do
			SnailLogic:SpecialCoverSnailRoadAtPos(mainLogic, v.r, v.c)
		end
...
function DestructionPlanLogic:runningGameItemAction(mainLogic, theAction, actid)
	...
	elseif theAction.actionType == GameItemActionType.kItemSpecial_Column then 
		if SpecialCoverLogicVer == 1 then
			DestructionPlanLogic:runingGameItemSpecialBombColumnAction(mainLogic, theAction, actid)
		else
			DestructionPlanLogic:runingGameItemSpecialBombColumnAction_old(mainLogic, theAction, actid)
		end
	elseif theAction.actionType == GameItemActionType.kItemSpecial_Wrap then 
		if SpecialCoverLogicVer == 1 then
			...
			DestructionPlanLogic:runingGameItemSpecialBombWrapAction_old(mainLogic, theAction, actid)
		end
	elseif theAction.actionType == GameItemActionType.kItemSpecial_WrapWrap then
		if SpecialCoverLogicVer == 1 then
			DestructionPlanLogic:runingGameItemSpecialBombWrapWrapAction(mainLogic, theAction, actid)
		else
			DestructionPlanLogic:runingGameItemSpecialBombWrapWrapAction_old(mainLogic, theAction, actid)
		end
	...
	elseif 
		...
		or theAction.actionType == GameItemActionType.kItemSpecial_Blocker195_Dynamite 
		...
		or theAction.actionType == GameItemActionType.kItemSpecial_Blocker195_Water 
		or theAction.actionType == GameItemActionType.kItemSpecial_Blocker195_Plane 
		or theAction.actionType == GameItemActionType.kItemSpecial_Blocker195_PuffedRice
		or theAction.actionType == GameItemActionType.kItemSpecial_Blocker195_Battery
		then
	...
	elseif theAction.actionType == GameItemActionType.kItem_WeeklyRace2020_Add_Five_Effect then
		DestructionPlanLogic:runWeeklyRace2020AddFiveEffectLogic(mainLogic, theAction, actid)
	else

		local actionMap = {
			[GameItemActionType.kItem_CanevineBomb] = DestructionPlanLogic.runningGameItemActionCanevineBomb,
		}
		with(actionMap[theAction.actionType], function ( ... )
			it(DestructionPlanLogic, mainLogic, theAction, actid)
		end)
	end
end
...
function DestructionPlanLogic:runningGameItemBlocker211CollectAction(mainLogic, theAction, actid)
	if theAction.actionStatus == GameActionStatus.kRunning then
		...
		if theAction.addInfo == 'collect' then
			local maxJsq = 35
			if theAction.isActive then maxJsq = 40 end
		...
		elseif theAction.addInfo == 'explode' then
			if theAction.jsq == 33 then
				...
				--寄居蟹，果酱兼容
				action.SpecialID = mainLogic:addSrcSpecialCoverToList(IntCoord:create(r, c))
		...
		local function checkGrid(keyItem)
			...
			for i = 1 , #castingList do
				...
				if filterMap[ tostring(checkItem.x) .. "_" .. tostring(checkItem.y) ] then
					...
					if checkItem.x <= colAmount and checkItem.y <= rowAmount then
						available = true
					end
				end
			end
		...
		local function getCastingTarget( needCount )
			...
			local findTarget = function ()
				...
				for r = 1, #mainLogic.gameItemMap do--屏幕最下排不用检测
	   		 	...
	   		 	elseif #blockList > 0 then
   		 			...
   		 			--if topBlockY == rowAmount then topBlockY = rowAmount - 1 end
			...
			for i = 1 , needCount do
				...
				if rst then
					...
					if rst.y == rowAmount then
						local newrst = mainLogic.gameItemMap[rowAmount - 1][rst.x]
						if newrst then
							rst = newrst
						else
							break
					...
					if rst.x == colAmount then
						local newrst = mainLogic.gameItemMap[rst.y][colAmount - 1]
						if newrst then
							rst = newrst
						else
							break
...
function DestructionPlanLogic:runGameItemSuperTotemsExplodePart1Action(boardView, theAction)
	...
	elseif theAction.addInfo == "startBomb" then
		...
		if theAction.isLongWaitTime then
			local layer = LayerColor:create()
			local size = Director:sharedDirector():getWinSize()
			layer:setFullScreenModeForExpandMode(true)
			layer:setContentSize(size)
			layer:setOpacity(180)
			local scene = Director:sharedDirector():getRunningScene()
			scene:addChild(layer)
...
function DestructionPlanLogic:runingGameItemSuperTotemsExplodePart2Action(mainLogic, theAction, actid)
	...
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.addInfo == "start" then -- 等待棋盘稳定后爆掉区域内的动物
			...
			for r = 1, #mainLogic.gameItemMap do
				for c = 1, #mainLogic.gameItemMap[r] do
					...
					if item and item.ItemType == GameItemType.kTotems and ( item:isActiveTotems() or item.totemsState == GameItemTotemsState.kWattingBomb ) and item:isAvailable() then
						passPosMap[tostring(r) .. "_" .. tostring(c)] = true
					end
				end
			end	
			]]

			-- local passItemFilter = function (item)
			-- 	if item and item.ItemType == GameItemType.kTotems and ( item:isActiveTotems() or item.totemsState == GameItemTotemsState.kWattingBomb ) and item:isAvailable() then
			-- 		return true
			-- 	end
			-- 	return false
			...
			if mainLogic:isItemAllStableExceptSuperTotems( passPosMap ) then
			-- if mainLogic:isItemAllStable( passPosMap ) then
				-- printx(15,"--------棋盘稳定，闪电鸟连线消除----------------")
				theAction.addInfo = "waiting3"
				theAction.actionTick = 1
			end
		elseif theAction.addInfo == "waiting3" then
		...
		elseif theAction.addInfo == "bombAll" then
            ...
            --闪电鸟本身感染果酱
            local isJamSperad1 = mainLogic.boardmap[r1][c1]:hasJamSperad()
            local isJamSperad2 = mainLogic.boardmap[r2][c2]:hasJamSperad()
...
function DestructionPlanLogic:runingGameItemActionWukongMonkeyBar(mainLogic, theAction, actid, actByView)
	if theAction.actionStatus == GameActionStatus.kRunning then
		...
		if not theAction.currCol then theAction.currCol = 9 end		--棋盘大小不要写死数字，已下线活动不重构，复用时请注意修改
		...
		if theAction.addInfo == "start" then
			...
			if theAction.jsq >= 5 then
				...
				local dr = r
				local dc = 9		--棋盘大小不要写死数字，已下线活动不重构，复用时请注意修改
...
function DestructionPlanLogic:runingGameItemSpecialCrystalStoneBirdPart2Action(mainLogic, theAction, actid)
	if theAction.actionStatus == GameActionStatus.kRunning then
		...
		if theAction.addInfo == "start" then
			-- if mainLogic:isItemAllStable()
			-- 	and mainLogic:numDestrunctionPlan() == 1 -- 只剩下自己了
			if mainLogic:segmentedActionCanMoveOn()
				then
				------停止掉落了-----
				----进入下一个阶段----
				if not theAction.stableCount then
					theAction.stableCount = 1
				else
					theAction.stableCount = theAction.stableCount + 1
				end

				if theAction.stableCount == 10 then
					theAction.addInfo = "bombSpecials"
					local result = {}
					for r = 1, #mainLogic.gameItemMap do
						for c = 1, #mainLogic.gameItemMap[r] do
				...
				if theAction.stableCount then
					theAction.stableCount = 0
				end
			end
		elseif theAction.addInfo == "bombSpecials" then
			theAction.actionTick = theAction.actionTick + 1
...
function DestructionPlanLogic:runingGameItemSpecialCrystalStoneCrystalStoneAction(mainLogic, theAction, actid)
	if theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.addInfo == "start" then
			...
			local isJamSperad1 = boardData1:hasJamSperad()
			local isJamSperad2 = boardData2:hasJamSperad()
...
function DestructionPlanLogic:runGameItemBlocker195CollectAction(boardView, theAction)
	...
	elseif theAction.actionStatus == GameActionStatus.kRunning then
		...
		if theAction.jsq >= theAction.maxJsq then
			-- 鱿鱼用：鱿鱼为星星瓶充能时，需要暂时先屏蔽通用的试图更新逻辑....此时充能动画播放完毕，解锁
			local r, c = theAction.ItemPos2.x, theAction.ItemPos2.y
...
function DestructionPlanLogic:runGameItemBlocker195BlockerAction(boardView, theAction)
	local mainLogic = boardView.gameBoardLogic
	local itemType = theAction.targetItemType
	local posList = mainLogic:getPosListOfItemType(itemType)
	local function callback(mainLogic, position)
		local r, c = position.x, position.y
...
function DestructionPlanLogic:runGameItemBlocker195DynamiteCrateAction(boardView, theAction)
	local mainLogic = boardView.gameBoardLogic
	local posList = mainLogic:getPosListOfItemType(GameItemType.kDynamiteCrate)
	local function callback(mainLogic, position)
		local r, c = position.x, position.y
		if SpecialCoverLogic:canBeEffectBySpecialAt(mainLogic, r, c) then
			SpecialCoverLogic:effectBlockerAt(mainLogic, r, c, 1, nil, false, false)
...
function DestructionPlanLogic:runGameItemBlocker195WaterAction(boardView, theAction)
	local mainLogic = boardView.gameBoardLogic
	local posList = mainLogic:getPosListOfItemType(GameItemType.kWater)
	local function callback(mainLogic, position)
		local r, c = position.x, position.y
		if SpecialCoverLogic:canBeEffectBySpecialAt(mainLogic, r, c) then
			SpecialCoverLogic:effectBlockerAt(mainLogic, r, c, 1, nil, false, false)
...
function DestructionPlanLogic:runGameItemBlocker195PlaneAction(boardView, theAction)
	local mainLogic = boardView.gameBoardLogic
	local posList = mainLogic:getPosListOfItemType(GameItemType.kPlane)
	local function callback(mainLogic, position)
		local r, c = position.x, position.y
		if SpecialCoverLogic:canBeEffectBySpecialAt(mainLogic, r, c) then
			SpecialCoverLogic:effectBlockerAt(mainLogic, r, c, 1, nil, false, false)
...
function DestructionPlanLogic:runGameItemBlocker195PuffedRiceAction(boardView, theAction)
	local mainLogic = boardView.gameBoardLogic
	local posList = mainLogic:getPosListOfItemType(GameItemType.kpuffedRice)
	local function callback(mainLogic, position)
		local r, c = position.x, position.y
		if SpecialCoverLogic:canBeEffectBySpecialAt(mainLogic, r, c) then
			SpecialCoverLogic:effectBlockerAt(mainLogic, r, c, 1, nil, false, false)
...
function DestructionPlanLogic:runGameItemBlocker195BatteryAction(boardView, theAction)
	local mainLogic = boardView.gameBoardLogic
	local posList = mainLogic:getPosListOfItemType(GameItemType.kBattery)
	local function callback(mainLogic, position)
		local r, c = position.x, position.y
		if SpecialCoverLogic:canBeEffectBySpecialAt(mainLogic, r, c) then
			SpecialCoverLogic:effectBlockerAt(mainLogic, r, c, 1, nil, false, false)
...
--        local SpecialID = mainLogic:addSrcSpecialCoverToList( theAction.ItemPos1 )
		...
		if theAction.addInfo == '' then
			...
			if theAction.ItemPos2.x == 1 then
				for i = c1 + 1, colAmount do

                    local CurBoard = mainLogic.boardmap[r1][i]
                    if CurBoard:hasJamSperad() and theAction.LeftStop then
                        theAction.LeftStop = false
                        theAction.LeftStopNum = 0
                    end
...
function DestructionPlanLogic:runingGameItemSpecialBombLineAction(mainLogic, theAction, actid)
	...
	-- printx( 1 , "runingGameItemSpecialBombLineAction  ================================\n"  , TimerUtil.frameId ,  table.tostring(theAction) ) 
	local r1 = theAction.ItemPos1.x
	local c1 = theAction.ItemPos1.y
	...
	-- printx( 1 , "addInt :" , theAction.addInt , "maxRange :" , maxRange , "extendOffset :" , theAction.extendOffset)
	...
	-- printx( 1 , "AAA  1   offset :" , offset , "maxRange :" , maxRange)
	...
	while offset <= maxRange do
		...
		-- printx( 1 , "BBB  1   theAction.addInfo =" , theAction.addInfo)
		-- printx( 1 , "BBB  2   destroy items >>  left :" , tostring(r1) .. "_" .. tostring(c2) ,  "right :" , tostring(r1) .. "_" .. tostring(c3) )
		...
		if theAction.addInfo ~= "left" then
			if c1 ~= c2 and mainLogic:isPosValid(r1, c2) and mainLogic.gameItemMap[r1][c2].dataReach then
				...
				-- printx( 1 , "BBB  3   s1 =" , s1 )

				if s1 == 2 then   ----遇到银币

					if theAction.leftKillAnim == nil then
						theAction.leftKillAnim = math.abs(c2 - c1) + 1
					end
		...
		if theAction.addInfo ~= "right" then
			if c1 ~= c3 and mainLogic:isPosValid(r1, c3) and mainLogic.gameItemMap[r1][c3].dataReach then
                ...
                if CurBoard:hasJamSperad() and theAction.RightStop then
                    theAction.RightStop = false
                    theAction.RightStopNum = 0
                end
				...
				if s2 == 2 then 
					...
					if theAction.rightKillAnim == nil then
						theAction.rightKillAnim = math.abs(c3 - c1) + 1
					end
					if theAction.addInfo == "" then
...
function DestructionPlanLogic:runingGameItemSpecialBombColumnAction(mainLogic, theAction, actid)
	...
	local scoreScale = theAction.addInt2
	local item = mainLogic.gameItemMap[r1][c1]
	local vfPlan = NewFallingItemLogic:getVerticalFallingPlanMap()

	local function updateVerticalFallingPlanMap( r , c , value )
		vfPlan[tostring(r) .. "_" .. tostring(c) ] = value
	end
    ...
    if theAction.addInt == 0 then
		...
		theAction.topCoverCount = r1 - 1
		theAction.bottomCoverCount = #mainLogic.gameItemMap - r1
		theAction.coverLogList = {}
		table.insert( theAction.coverLogList , { r = r1 , c = c1 } )
	end

	-- theAction.addInfo = "over"
	theAction.addInt = theAction.addInt + 1

	local offsetTopR = r1 - theAction.addInt
	local offsetBottomR = r1 + theAction.addInt

	if offsetBottomR <= #mainLogic.gameItemMap and (not theAction.stopOnBottom) then
		local CurBoard = mainLogic.boardmap[offsetBottomR][c1]
        if CurBoard:hasJamSperad() and theAction.downStop then
            theAction.downStop = false
            theAction.downStopNum = 0
        end

        local SpecialID = nil
        local nextPos = ccp(offsetBottomR+1,c1)
        SpecialID, theAction.downStopNum, theAction.downStop, theAction.downSpecialID = JamSperadSpecialIDCheck( 
            mainLogic, 
            ccp(offsetBottomR,c1), 
            nextPos,
            theAction.downStop, 
            theAction.downStopNum, 
            theAction.downSpecialID )

		if not DestructionPlanLogic:existInSpecialBombLightUpPos(mainLogic, offsetBottomR, c1, theAction.lightUpBombMatchPosList) then
			SpecialCoverLogic:SpecialCoverLightUpAtPos(mainLogic, offsetBottomR, c1, scoreScale)
		end
		local s1 = BombItemLogic:tryCoverByBomb(mainLogic, offsetBottomR, c1, true, scoreScale, true) 		----空中的也炸掉
		SpecialCoverLogic:SpecialCoverAtPos(mainLogic, offsetBottomR, c1, 2, scoreScale, actid, nil,nil,nil,SpecialID)
		if mainLogic:isPosValid(offsetBottomR, c1) then 
			GameExtandPlayLogic:doABlocker211Collect(mainLogic, r1, c1, offsetBottomR, c1, item._encrypt.ItemColorType, true, 3)
		end
		if s1 == 2 then 
			----遇到银币
			...
			SpecialCoverLogic:specialCoverChainsAtPos(mainLogic, offsetBottomR, c1, {ChainDirConfig.kUp})
			ObstacleFootprintManager:addBlockEffectRecord(mainLogic.gameItemMap[offsetBottomR][c1])
			theAction.stopOnBottom = true 



			if theAction.bottomKillAnim == nil then
				theAction.bottomKillAnim = math.abs(offsetBottomR - r1) + 1
			end


		else
			SpecialCoverLogic:specialCoverChainsAtPos(mainLogic, offsetBottomR, c1, {ChainDirConfig.kUp, ChainDirConfig.kDown})
		end

		table.insert( theAction.coverLogList , { r = offsetBottomR , c = c1 } )
	end

	if offsetTopR >= 1 and (not theAction.stopOnTop) then
		local CurBoard = mainLogic.boardmap[offsetTopR][c1]
        if CurBoard:hasJamSperad() and theAction.upStop then
            theAction.upStop = false
            theAction.upStopNum = 0
        end

        local SpecialID = nil
        local nextPos = ccp(offsetTopR-1,c1)
        SpecialID, theAction.upStopNum, theAction.upStop, theAction.upSpecialID = JamSperadSpecialIDCheck( 
            mainLogic, 
            ccp(offsetTopR,c1), 
            nextPos,
		...
		if not DestructionPlanLogic:existInSpecialBombLightUpPos(mainLogic, offsetTopR, c1, theAction.lightUpBombMatchPosList) then
			SpecialCoverLogic:SpecialCoverLightUpAtPos(mainLogic, offsetTopR, c1, scoreScale)
		end
		local s1 = BombItemLogic:tryCoverByBomb(mainLogic, offsetTopR, c1, true, scoreScale, true) 		----空中的也炸掉
		SpecialCoverLogic:SpecialCoverAtPos(mainLogic, offsetTopR, c1, 2, scoreScale, actid, nil,nil,nil,SpecialID )
		if mainLogic:isPosValid(offsetTopR, c1) then 
			GameExtandPlayLogic:doABlocker211Collect(mainLogic, r1, c1, offsetTopR, c1, item._encrypt.ItemColorType, true, 3)
		end
		if s1 == 2 then 
			----遇到银币
			SpecialCoverLogic:specialCoverChainsAtPos(mainLogic, offsetTopR, c1, {ChainDirConfig.kDown})
			ObstacleFootprintManager:addBlockEffectRecord(mainLogic.gameItemMap[offsetTopR][c1])
			theAction.stopOnTop = true  


			if theAction.topKillAnim == nil then
				theAction.topKillAnim = math.abs(offsetTopR - r1) + 1
			end

		else
			SpecialCoverLogic:specialCoverChainsAtPos(mainLogic, offsetTopR, c1, {ChainDirConfig.kUp, ChainDirConfig.kDown})
		end

		table.insert( theAction.coverLogList , { r = offsetTopR , c = c1 } )
	end

	local function endAtction()
		theAction.addInfo = "over"
	end

	if theAction.stopOnTop and theAction.stopOnBottom then
		endAtction()
		return
	end

	if theAction.addInt >= theAction.topCoverCount and theAction.addInt >= theAction.bottomCoverCount then
		endAtction()
		return
	end

	for k,v in ipairs( theAction.coverLogList ) do
		updateVerticalFallingPlanMap( v.r , v.c , true )
	end
	
end

----直线（竖排）消除特效执行中逻辑(旧版算法)
function DestructionPlanLogic:runingGameItemSpecialBombColumnAction_old(mainLogic, theAction, actid)
	if theAction.addInfo == "over" then
		return 
	end
	----执行一次就over----
	...
	for i=r1 + 1, #mainLogic.gameItemMap do
		...
		if s1 == 2 then 
			...
			if theAction.bottomKillAnim == nil then
				theAction.bottomKillAnim = math.abs(i - r1) + 1
			end
	...
	for i=r1 - 1, 1, -1 do
		...
		if s1 == 2 then 
			...
			if theAction.topKillAnim == nil then
				theAction.topKillAnim = math.abs(i - r1) + 1
			end
...
local function getRectBombList(radius, r, c , fx)
	local result = {}
	...
	for len = 1 , radius do
		...
		for rf = 0 , len do
			...
			if rf == 0 then
				lastVertexPos = arr
				table.insert( arr, { r = r-rf , c = c-cf } )
				table.insert( arr, { r = r+rf , c = c+cf } )
			elseif rf == len then
				arr = lastVertexPos
				lastVertexPos = nil
			...
			if fx and len == radius then
				if fx == "up" then
					table.insert( arr, { r = r-rf-1 , c = c-cf } )
					table.insert( arr, { r = r-rf-1 , c = c+cf } )
				elseif fx == "down" then
					table.insert( arr, { r = r+rf+1 , c = c-cf } )
					table.insert( arr, { r = r+rf+1 , c = c+cf } )
				elseif fx == "left" then
					table.insert( arr, { r = r-rf , c = c-cf-1 } )
					table.insert( arr, { r = r+rf , c = c-cf-1 } )
	...
	table.insert ( result[1] , 1 , { r = r , c = c } )
	
	-- printx( 1 , "WARP !!!!!!!!!!!!!!!!!!!!" , table.tostring(result))
	return result
end
...
function DestructionPlanLogic:runingGameItemSpecialBombWrapAction(mainLogic, theAction, actid)
	...
	if theAction.addInt == 0 then
		...
		theAction.bombPosList = getRectBombList(2, r1, c1)
		theAction.coverLogList = {}
	end
	...
	if theAction.addInt <= #theAction.bombPosList then
		local list = theAction.bombPosList[theAction.addInt]
		...
		for k,v in pairs(list) do
			if mainLogic:isPosValid(v.r, v.c) then
				...
				if mainLogic.gameItemMap[v.r][v.c].dataReach then
					...
					SpecialCoverLogic:SpecialCoverAtPos(mainLogic, v.r, v.c, 3, scoreScale, actid, nil,nil,nil,theAction.SpecialID)
					...
					table.insert( theAction.coverLogList , { r = v.r , c = v.c } )
				end
			end
		end
...
function DestructionPlanLogic:runingGameItemSpecialBombWrapAction_old(mainLogic, theAction, actid)
...
function DestructionPlanLogic:runingGameItemSpecialBombWrapWrapAction(mainLogic, theAction, actid)
	...
	if theAction.addInt == 0 then
		...
		theAction.bombPosList = getRectBombList(4, r1, c1 , fx)
		theAction.coverLogList = {}
		GamePlayMusicPlayer:playEffect(GameMusicType.kSwapWrapWrap);
	end

	if not theAction.SpecialID then
		theAction.SpecialID = mainLogic:addSrcSpecialCoverToList( theAction.ItemPos1, theAction.ItemPos2 )
	end
...
function DestructionPlanLogic:runingGameItemSpecialBombWrapWrapAction_old(mainLogic, theAction, actid)
	local function getRectBombPos(length, r, c)
		local result = {}
		local rowOffset = r - math.floor(length / 2) - 1
		local colOffset = c - math.floor(length / 2)
...
function DestructionPlanLogic:runingGameItemSpecialBombColorAction(mainLogic, theAction, actid)
	...
	elseif theAction.addInfo == "waiting" then
		...
		if theAction.actionDuring <= GamePlayConfig_SpecialBomb_BirdAnimal_Time3 then
			...
			GamePlayHapticsPlayer.getInstance():playEffect("bird_animal_shake")
	...
	if theAction.addInfo == "bomb" then
		...
		for k,v in pairs(posList) do
			...
			elseif itemBomb.ItemType == GameItemType.kTotems then--闪电鸟集中引爆的时候没有处理附着在上面的障碍
				if not WaterBucketLogic:hasBucket(itemBomb) then
...
function DestructionPlanLogic:runingGameItemSpecialBombColorLineAction(mainLogic, theAction, actid, sptype)
	...
	elseif theAction.addInfo == "waiting1" then
		if theAction.actionDuring == GamePlayConfig_SpecialBomb_BirdLine_Time1 - GamePlayConfig_SpecialBomb_BirdLine_Time2 then
		    ...
		    if mathBoard2:hasJamSperad() then
	...
	elseif theAction.addInfo == "flyingBird" then
		...
		if theAction.sptype == AnimalTypeConfig.kWrap then
			GamePlayHapticsPlayer.getInstance():playEffect("bird_wrap_change")
		else
			GamePlayHapticsPlayer.getInstance():playEffect("bird_line_change")
		end
	elseif theAction.addInfo == "waiting2" then
	...
	elseif theAction.addInfo == "BombTime" then
		...
		if xtime > 0 and xtime % math.ceil(GamePlayConfig_SpecialBomb_BirdLine_Time5) == 0 then ----每隔一段时间
			if (BombItemLogic:tryBombWithBombRes(mainLogic, r1, c1, 1, scoreScale)) then
				----引爆某一个
				-- if theAction.sptype == AnimalTypeConfig.kWrap then
				-- 	GamePlayHapticsPlayer.getInstance():playEffect("bird_wrap_bomb")
				-- else
				-- 	GamePlayHapticsPlayer.getInstance():playEffect("bird_line_bomb")
				-- end
			else
				----失败了----没有可以引爆的了，----爆炸结束----
				theAction.addInfo = "Over"
...
function DestructionPlanLogic:runingGameItemSpecialBombColorColorPart1Action(mainLogic, theAction, actid)
	...
	elseif theAction.addInfo == "start" then
		...
		local part2Action = GameBoardActionDataSet:createAs(
			...
			part2Action.SpecialID = theAction.SpecialID
			mainLogic:addStableDestructionPlanAction(part2Action)
	elseif theAction.addInfo == "Over" then
		mainLogic.destructionPlanList[actid] = nil
	end
...
function DestructionPlanLogic:runingGameItemSpecialBombColorColorPart2Action(mainLogic, theAction, actid)
	...
	local SpecialID = theAction.SpecialID--mainLogic:addSrcSpecialCoverToList( theAction.ItemPos1, theAction.ItemPos2 )

	if theAction.addInfo == "" or theAction.addInfo == "Pass" then
		--------第一次等待-------
		-- if mainLogic:isItemAllStable()
		-- 	and mainLogic:numDestrunctionPlan() == 1 
		-- 	and mainLogic:numDestroyAction() + mainLogic:numFallingAction() + mainLogic:getGlobalCoreActionListCount() == 0
		if mainLogic:segmentedActionCanMoveOn()
			then
				-- printx(15,"==================鸟鸟交换part2======================")
			if not theAction.stableCount then
			...
			------停止掉落了-----
			----进入下一个阶段----
			-- printx(15,"==================鸟鸟交换part2检测======================",theAction.stableCount)
			if theAction.stableCount == 10 then
				-- printx(15,"鸟鸟特效消除")
				theAction.addInfo = "BombAll"
				theAction.bombList = getSpecialBombPosList()
	...
	elseif theAction.addInfo == "BombAll" then
		if theAction.addInt >= GamePlayConfig_SpecialBomb_BirdBird_Time3 then
			...
			part3Action.SpecialID = theAction.SpecialID
			mainLogic:addStableDestructionPlanAction(part3Action)
		end
		theAction.addInt = theAction.addInt + 1
	elseif theAction.addInfo == "Over" then
...
function DestructionPlanLogic:runingGameItemSpecialBombColorColorPart3Action(mainLogic, theAction, actid)
	...
	if theAction.addInfo == "" or theAction.addInfo == "Pass" then
		...
		if mainLogic:segmentedActionCanMoveOn()
			...
			-- printx(15,"==================鸟鸟交换part3检测======================",theAction.stableCount)
			if theAction.stableCount == 10 then
				-- printx(15,"鸟鸟开始全屏消除")
				theAction.addInfo = "shake"
			end
	...
	elseif theAction.addInfo == "CleanAll" then
		...
		if theAction.addInt >= 1 then
			...
			local collectPos = IntCoord:create((r1 + r2)/2, (c1+c2)/2)
	 		BombItemLogic:tryCoverByBirdBird(mainLogic, birdBirdPos, 1, scoreScale, collectPos, birdBirdPos2, theAction.SpecialID)
			theAction.addInfo = "waiting3"

			GamePlayHapticsPlayer.getInstance():playEffect("bird_bird_bomb")
			GamePlayMusicPlayer:playEffect(GameMusicType.kSwapColorColorCleanAll)
		end
	elseif theAction.addInfo == "waiting3" then
		theAction.addInt = theAction.addInt + 1
...
function DestructionPlanLogic:runGameItemActionBombSpecialLine(boardView, theAction) 		----播放横排直线爆炸特效
	...
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		...
		-- GamePlayHapticsPlayer.getInstance():playEffect("line_bomb")
		if theAction.animInfo == 'canevine' then
			...
			local c1 = theAction.ItemPos1.y
			local position1 = DestructionPlanLogic:getItemPosition(theAction.ItemPos1)
			local node = SpriteBatchNode:createWithTexture(CCSprite:createWithSpriteFrameName(resPath.."/CDBZ_00.png"):getTexture())
			local clipping = SimpleClippingNode:create()
			clipping:setAnchorPoint(ccp(0.5, 0.5))
			clipping:setContentSize(CCSizeMake(GamePlayConfig_Tile_Width*18, 150))
		    clipping:ignoreAnchorPointForPosition(false)
		    clipping:setPosition(ccp(position1.x, position1.y))


			local sp1
			sp1 = CanevineLogic:create_line_effect(node, function ( ... )
				if not sp1.isDisposed  then sp1:setVisible(false) end
			end)

			sp1:setFlipX(true)

			local sp2
			sp2 = CanevineLogic:create_line_effect(node, function ( ... )
				if not sp2.isDisposed  then sp2:setVisible(false) end
			end)

			sp1:setAnchorPoint(ccp(1, 0.5))
			sp2:setAnchorPoint(ccp(0, 0.5))

			sp1:setPosition(ccp(GamePlayConfig_Tile_Width*18/2, 150/2))
			sp2:setPosition(ccp(GamePlayConfig_Tile_Width*18/2, 150/2))


			boardView:addChild(clipping)
			clipping:addChild(node)

			theAction.animNodeLeft = sp1
			theAction.animNodeRight = sp2

			theAction.clipping = clipping


			setTimeOut(function ( ... )
				if not clipping.isDisposed  then clipping:removeFromParentAndCleanup(true) end
			end, 36 / 30)
	...
	elseif theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.rightKillAnim then
			if theAction.animNodeRight and (not theAction.animNodeRight.isDisposed) then

				local ow = theAction.clipping:getContentSize().width
				local orw = ow * (1-theAction.clipping:getAnchorPoint().x)
				local nrw = (theAction.rightKillAnim-0.5) * GamePlayConfig_Tile_Width
				local nw = ow - (orw - nrw)
				
				theAction.clipping:setContentSize(CCSizeMake(nw, 150))
				theAction.clipping:setAnchorPoint(ccp( (nw-nrw)/(nw) , 0.5))

			end
			theAction.rightKillAnim = false
		end
		if theAction.leftKillAnim then
			if theAction.animNodeLeft and (not theAction.animNodeLeft.isDisposed) then

				local ow = theAction.clipping:getContentSize().width
				local olw = ow * (theAction.clipping:getAnchorPoint().x)
				local nlw = (theAction.leftKillAnim-0.5) * GamePlayConfig_Tile_Width
				local nw = ow - (olw - nlw)
				
				theAction.clipping:setContentSize(CCSizeMake(nw, 150))
				theAction.clipping:setAnchorPoint(ccp( (nlw)/(nw) , 0.5))

				theAction.animNodeLeft:setPositionX(theAction.animNodeLeft:getPositionX() - (olw - nlw))
				theAction.animNodeRight:setPositionX(theAction.animNodeRight:getPositionX() - (olw - nlw))

			end
			theAction.leftKillAnim = false
		end
	end
end 
...
function DestructionPlanLogic:runGameItemActionBombSpecialColumn(boardView, theAction)		----播放竖排直线爆炸特效
	...
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		...
		if theAction.animInfo == 'canevine' then
			...
			local clipping = SimpleClippingNode:create()
			clipping:setAnchorPoint(ccp(0.5, 0.5))
			clipping:setContentSize(CCSizeMake(GamePlayConfig_Tile_Height*18, 150))
		    clipping:ignoreAnchorPointForPosition(false)
		    clipping:setPosition(ccp(position1.x, position1.y))
			clipping:setRotation(90)
			...
			sp1:setPosition(ccp(GamePlayConfig_Tile_Height*18/2, 150/2))
			sp2:setPosition(ccp(GamePlayConfig_Tile_Height*18/2, 150/2))

			boardView:addChild(clipping)
			clipping:addChild(node)

			theAction.animNodeTop = sp1
			theAction.animNodeBottom = sp2
	...
	elseif theAction.actionStatus == GameActionStatus.kRunning then
		if theAction.bottomKillAnim then
			if theAction.animNodeBottom and (not theAction.animNodeBottom.isDisposed) then

				local ow = theAction.clipping:getContentSize().width
				local orw = ow * (1-theAction.clipping:getAnchorPoint().x)
				local nrw = (theAction.bottomKillAnim-0.5) * GamePlayConfig_Tile_Height
				local nw = ow - (orw - nrw)
			...
			theAction.bottomKillAnim = false
		end
		if theAction.topKillAnim then
			if theAction.animNodeTop and (not theAction.animNodeTop.isDisposed) then

				local ow = theAction.clipping:getContentSize().width
				local olw = ow * (theAction.clipping:getAnchorPoint().x)
				local nlw = (theAction.topKillAnim-0.5) * GamePlayConfig_Tile_Height
				...
				theAction.animNodeTop:setPositionX(theAction.animNodeTop:getPositionX() - (olw - nlw))
				theAction.animNodeBottom:setPositionX(theAction.animNodeBottom:getPositionX() - (olw - nlw))

			end
			theAction.topKillAnim = false
		end
	end
end
...
function DestructionPlanLogic:runGameItemActionBombSpecialWrap(boardView, theAction)			----播放区域特效爆炸效果
	...
	GamePlayHapticsPlayer.getInstance():playEffect("wrap_bomb")
...
------------------------------ 周赛+5步附带的棋盘特效 -------------------------------
function DestructionPlanLogic:runWeeklyRace2020AddFiveEffectLogic(mainLogic, theAction, actid)
	if theAction.addInfo == "over" then
		mainLogic.destructionPlanList[actid] = nil
		return 
	end
	...
	if theAction.addInfo == "startAnimation" then
		theAction.jsq = theAction.jsq + 1
		if theAction.jsq == 2 then
			WeeklyRace2020Logic:playWeeklyRace2020AddFiveBombAnimation(mainLogic)

			theAction.jsq = 0
			theAction.addInfo = "startBombBoard"
		end
	end

	if theAction.addInfo == "startBombBoard" then
		theAction.jsq = theAction.jsq + 1
		if theAction.jsq == 50 then
			WeeklyRace2020Logic:weeklyRace2020AddFiveBombBoard(mainLogic)
...
function DestructionPlanLogic:runWeeklyRace2020AddFiveEffectView(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		theAction.actionStatus = GameActionStatus.kRunning
