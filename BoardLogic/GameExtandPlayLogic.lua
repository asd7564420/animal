	    ...
	    local callback = function()
	        for i,missileItemData in ipairs(missiles) do
	            if missileItemData then 
	            	...
	            	--[[
	            	--这个MissileBugFix的分组开关已经废弃了
	            	local uid = UserManager:getInstance().uid or 12345

	            	local aiLocalData = HEAICore:getLocalData()
	            	local levelId = nil
	            	if GamePlayContext:getInstance() and GamePlayContext:getInstance().levelInfo then
	            		levelId = GamePlayContext:getInstance().levelInfo.levelId
	            	end
	            	
	            	if levelId and aiLocalData and aiLocalData.startLevel and  aiLocalData.endLevel and 
	            		levelId >= aiLocalData.startLevel and levelId <= aiLocalData.endLevel then
...
function GameExtandPlayLogic:tryWarp( source , count )
	...
	elseif source == GameItemType.kWeeklyRace2020Chest then
		pid = ItemType.WEEKLY_RACE_2020_FINAL_CHEST_ADD_STEP
		datas.addMovesCount = count
	end
...
function GameExtandPlayLogic:findMissileTarget(mainLogic,missile, pickAmount)
	if not pickAmount then pickAmount = 0 end
...
function GameExtandPlayLogic:hitMissile(mainLogic, item, r, c, noScore, hitAll)
	...
	if item.missileLevel > 0 then 
		...
		if mainLogic.boardView then
			...
			if itemView then
				itemView:playMissileDecAnimation(item.missileLevel)
			end
		end
...
function GameExtandPlayLogic:checkCrystalChangeColor(mainLogic, completeCallback)
	local useNewChangeColorLogic = false
	local levelId = mainLogic.level
	if levelId >= GamePlayGlobalParams.KNewCrystalLogicActLevel then --新的活动关
		useNewChangeColorLogic = true
	elseif LevelType:isHideLevel(levelId) then -- 精英关，两段
		if levelId >= GamePlayGlobalParams.KNewCrystalLogicHiddenLevel or ( levelId >= 10274 and levelId <= 10477 ) then
			useNewChangeColorLogic = true
		end
	elseif LevelType:isMainLevel(levelId) and levelId >= GamePlayGlobalParams.KNewCrystalLogicMainLevel then -- 主线关
		useNewChangeColorLogic = true
	end

	local ret = 0
	local hasSpecialHandle = false
	...
	local function getPossibleChangeColor(item)
		...
		-- end
		if colorFilterColor then
			if not hasSpecialHandle and colorFilterColor ~= oriColor then
				table.insert(banColorTable, colorFilterColor)
			elseif hasSpecialHandle then
				table.insert(banColorTable, colorFilterColor)
			end
		end
	...
	--	printx(15,"getPossibleChangeColor_result",#result)
		return result
	end

	local function checkHasMatchQuick()--变色后会不会直接造成三消
...
--	local success = -1 
	...
	local reservedIndex = 0
	local needToCheck = false
	local retryLimit = 501
	...
	local function getTotalPossibleComposition()
		if not numTotalPosssibleComposition then
			...
			--	printx(15,"possibleColor_infunction",i,#possibleColor)
				numTotalPosssibleComposition = numTotalPosssibleComposition * (#possibleColor > 0 and #possibleColor or 1)
			end
		end
	--	printx(15,"getTotalPossibleComposition",numTotalPosssibleComposition)
	...
	local function tryHandleCrystalColor()
		if reservedIndex ~= 0 then--直接三消
			local composition = possibleComposition[reservedIndex]
			for i, item in ipairs(crystalBalls) do
				item._encrypt.ItemColorType = composition[i]
			end
			needToCheck = true
			return
		end	
		...
		--既不能满足走一步就三消，也不能直接三消
		local randomIndex = mainLogic.randFactory:rand(1, #possibleComposition)
		local randomComposition = possibleComposition[randomIndex]
		for i, item in ipairs(crystalBalls) do
			item._encrypt.ItemColorType = randomComposition[i]
		end
		return 
	end
	...
	while retryTimes < retryLimit do
		if #possibleComposition >= getTotalPossibleComposition() or retryTimes == retryLimit-1 then
			if not hasSpecialHandle and useNewChangeColorLogic then
				hasSpecialHandle = true
				numTotalPosssibleComposition = nil
				retryTimes = 0
				retryLimit = 201
			else
				tryHandleCrystalColor()
				break
			end
		end
		retryTimes = retryTimes + 1
	--	printx(15,"retryTimes",retryTimes)
		-- 对选择的颜色结果进行查重，重复的组合不再重复计算
		repeat
			local composition = {}
			for i, item in ipairs(crystalBalls) do
				local possibleColor = getPossibleChangeColor(item)
		--		printx(15,"#possibleColor_inrepeat",#possibleColor)
		...
		--		printx(15,"NoNeedRefresh2")
				...
				--	success = 0
		--		printx(15,"NoNeedRefresh22222")
					break
				end
			end
		else --记录可以直接三消的颜色组合
		--	printx(15,"sanxiao!!!!!!!!!!!!!")
			if reservedIndex == 0 then
				reservedIndex = #possibleComposition
			end
		end
	end
...
function GameExtandPlayLogic:filterItemByConditions(mainLogic, conditions)
	...
	-- 目前使用者：GameItemType.kCrystal, lotusLevelLess = 2 
	local function checkLotusLevelLess(item , lotusLevel)
		if not lotusLevel or not item.lotusLevel then
			return true
		end
		-- 虽然刷新会搞乱item的1级lotusLevel，然而因为已有的判断是判断<2，所以虽然乱了但是结果没区别。
	...
	for i, coord in ipairs(aroundDirections) do 
		...
		if tempItem 
			and (tempItem.ItemType == GameItemType.kAnimal 
				...
				or tempItem.ItemType == GameItemType.kAddMove
				...
				or tempItem.ItemType == GameItemType.kTravelEnergyBag
				or tempItem.ItemType == GameItemType.kCuckooWindupKey
				)
...
function GameExtandPlayLogic:checkRoostReplace(mainLogic, completeCallback)
	...
	local possibleItemSubList = {}
	possibleItemSubList[1] = {}
	possibleItemSubList[2] = {}
	for i = 1 , #possibleItemList do
		local item = possibleItemList[i]
		local r = item.y
		local c = item.x
		local board = mainLogic.boardmap[r][c]
		if board.roostSelect then
			table.insert(possibleItemSubList[2],item)
		else
			table.insert(possibleItemSubList[1],item)
		end
	end

	local chosenIndex = 1
	if #possibleItemSubList[chosenIndex] == 0 then
		chosenIndex = chosenIndex + 1
	end

	local chosenItemList = possibleItemSubList[chosenIndex]
	...
	for i, roost in ipairs(roostList) do
		if roost.roostLevel == 4 then
			...
			local itemCount = #possibleItemSubList[1] + #possibleItemSubList[2]
			if itemCount > 0 then

				local needNum = maxReplaceNumFromConfig
				if itemCount < maxReplaceNumFromConfig then
					needNum = itemCount
				end

				for counter = 1, needNum do
					local idx = mainLogic.randFactory:rand(1, #chosenItemList)
					local item = chosenItemList[idx]
					local to = IntCoord:create(item.y, item.x)
					table.insert(replaceList, to)
					table.remove(chosenItemList, idx)
					-- item.ItemType = GameItemType.kAnimal
					item._encrypt.ItemColorType = AnimalTypeConfig.kYellow
					if #chosenItemList == 0 then
						chosenIndex = chosenIndex + 1
						chosenItemList = possibleItemSubList[chosenIndex]
					end
				end
...
function GameExtandPlayLogic:itemDestroyHandler(mainLogic, r, c)
	...
	-- elseif gameItem.ItemType == GameItemType.kShellGift then
	-- 	print("GameExtandPlayLogic:itemDestroyHandler",debug.traceback())
	-- 	ShellGiftLogic.onItemDestroy(mainLogic, r, c , gameItem)
	-- 	mainLogic:addScoreToTotal(r, c, GamePlayConfigScore.ShellGift, gameItem._encrypt.ItemColorType)
	elseif gameItem.ItemType == GameItemType.kTravelEnergyBag then
		-- printx(11, ".*.*.*.*.*.* kTravelEnergyBag .*.*.*.*.*.* GameExtandPlayLogic:itemDestroyHandler")
		TravelLogic:onEnergyBagDemolished(gameItem)
		mainLogic:addScoreToTotal(r, c, GamePlayConfigScore.TravelEnergyBag, gameItem._encrypt.ItemColorType)
	elseif gameItem.ItemType == GameItemType.kCuckooWindupKey then
		-- printx(11, ".*.*.*.*.*.* kCuckooWindupKey .*.*.*.*.*.* GameExtandPlayLogic:itemDestroyHandler")
		CuckooLogic:onCuckooWindUpKeyDemolished(gameItem)
		mainLogic:addScoreToTotal(r, c, GamePlayConfigScore.CuckooWindupKey, gameItem._encrypt.ItemColorType)
	...
	elseif gameItem.ItemType  == GameItemType.kBalloon then
		if mainLogic.theGamePlayStatus == GamePlayStatus.kNormal  then           ----只在bonus前作用有效果
			...
			GameExtandPlayLogic:tryWarp( GameItemType.kBalloon , 5 )
		end
	elseif gameItem.ItemType == GameItemType.kDigJewel then
		...
		elseif mainLogic.theGamePlayType == GameModeTypeId.WEEKLY_RACE_2020_MODE_ID then
			WeeklyRace2020Logic:addJewelAmount(mainLogic, 1, r, c)
	...
	elseif gameItem.ItemType == GameItemType.kAddMove then
		if mainLogic.theGamePlayStatus == GamePlayStatus.kNormal then           ----只在bonus前作用有效果
			...
			if mainLogic.PlayUIDelegate then
				local useCommonEffect = gameItem.numAddMove==5
				useCommonEffect = false
				if useCommonEffect then
					mainLogic.PlayUIDelegate:useStepPropComplete(mainLogic.theCurMoves)
			...
			mainLogic:addScoreToTotal(r, c, GamePlayConfigScore.AddStepAnimal)
			GameExtandPlayLogic:tryWarp( GameItemType.kAddMove , gameItem.numAddMove )
		end
	...
	if gameItem.hasActCollection then 
		...
		local collectionBar = ActCollectionLogic:getCollectionBar("clover_race")
	    if not collectionBar then return end
	    collectionBar:playGetCollectionAnim(r, c, mainLogic)
	end
end
...
function GameExtandPlayLogic:decreaseDigJewel( mainLogic, r, c, scoreScale, noScore, isFromSpringBomb )
	...
	if not noScore then
		...
		elseif mainLogic.levelType == GameLevelType.kWeeklyRace2020 and item.digJewelLevel == 0 then
			baseScore = 20000
		end
	...
	local extraActionCD = 0
	if mainLogic.levelType == GameLevelType.kWeeklyRace2020 then
		extraActionCD = 7
	end
	...
	local digJewelDecAction = GameBoardActionDataSet:createAs(
	 		...
	 		IntCoord:create(r,c),
	 		nil,
	 		GamePlayConfig_GameItemDigJewelDeleteAction_CD + extraActionCD)
...
function GameExtandPlayLogic:choseItemToIngredient( mainLogic, r, c )
	...
	local max = 0
	local item
	for k = 1, #mainLogic.gameItemMap do 
...
function GameExtandPlayLogic:getMimosaToHoldGrid(itemType, mainLogic, r, c )
	...
	if mainLogic.gameItemMap[grid.x] and mainLogic.gameItemMap[grid.x][grid.y] then
		...
		if item_select and item_select.isUsed
			...
			and item_select.ItemType ~= GameItemType.kPlane
			and not CatteryLogic:isCatteryOrMeow(item_select.ItemType)
			and item_select.ItemType ~= GameItemType.kBattery
			then
			result = grid
		end
	end
...
function GameExtandPlayLogic:showUFOReveivePanel(gameMode, isWin)
    ...
    local function confirmReviev( isTryAgain )   ----确认使用兔兔导弹后，修改数据
        ...
        if isTryAgain then
            ...
            local icon = Sprite:createWithSpriteFrameName("propAssets/RabbitMissile")
            local scene = Director:sharedDirector():getRunningScene()
...
function GameExtandPlayLogic:tryShowNDBunnyResurrectPanel(gameMode)
	local mainLogic = gameMode.mainLogic
    local function confirmResurrect( isTryAgain ) 
        if isTryAgain then
        	mainLogic.bunnyLeaveFlag = false
        	mainLogic:setNeedCheckFalling()
        	mainLogic:setGamePlayStatus(GamePlayStatus.kNormal)
        	mainLogic.fsm:changeState(mainLogic.fsm.fallingMatchState)
	    ...
	    if not mainLogic.replaying then
	      	local replaydata = {prop = ItemType.Autumn2020_Resurrect, pt=UsePropsType.TEMP, x1 = r1, y1 = c1, x2 = r2, y2 = c2} --标识为临时，则不会开局当作虚假道具新增
    ...
    mainLogic.PlayUIDelegate:showNDBunnyResurrectPanel(mainLogic.level, mainLogic.totalScore, gameMode:getScoreStarLevel(mainLogic), gameMode:reachTarget(), confirmResurrect)
end

function GameExtandPlayLogic:showAddStepPanel(gameMode)
	-- printx( 1 , "GameExtandPlayLogic:showAddStepPanel " ,  gameMode )
	local mainLogic = gameMode.mainLogic;
	local function tryAgainWhenFailed(isTryAgain, propId, deltaStep, aidPropId)	----确认加5步之后，修改数据
		-- printx( 1 , "GameExtandPlayLogic:showAddStepPanel  tryAgainWhenFailed" ,  isTryAgain , debug.traceback() , "\nmainLogic.needRefreshAfterAddSteps" , mainLogic.needRefreshAfterAddSteps )

	    if isTryAgain then
	      SnapshotManager:stop()
	      gameMode:getAddSteps(deltaStep or 5)

	      if mainLogic.needRefreshAfterAddSteps then
	      	-- printx( 1 , "mainLogic.fsm.currentState = " , mainLogic.fsm.currentState)
	      	-- printx( 1 , "mainLogic.fsm.waitingState = " , mainLogic.fsm.waitingState)
	      	-- printx( 1 , "mainLogic.fsm.fallingMatchState = " , mainLogic.fsm.fallingMatchState)
	      	-- printx( 1 , "mainLogic.fsm.swapState = " , mainLogic.fsm.swapState)
	      	-- printx( 1 , "mainLogic.fsm.usePropState = " , mainLogic.fsm.usePropState)

	      	mainLogic.needRefreshAfterAddSteps = false
	      	mainLogic.refreshAfterAddSteps = true

	      	mainLogic:setGamePlayStatus(GamePlayStatus.kNormal)
	      	-- mainLogic.fsm:changeState(mainLogic.fsm.fallingMatchState)
	      	mainLogic:setNeedCheckFalling()
	      	-- mainLogic:refreshComplete()
	      ...
	      if not mainLogic.replaying then
	      	...
	      	-- if aidPropId then
	      	-- 	local useGetTempProp = {prop = GamePropsType.kGetTempProp, pt = UsePropsType.TEMP, x1 = aidPropId, y1 = 0, x2 = 0, y2 = 0} 
	      	-- 	ReplayDataManager:addReplayStep( useGetTempProp )
	      	-- end
	      ...
	      if aidPropId then
	      	mainLogic:useProps(GamePropsType.kGetTempProp, aidPropId, 0, 0, 0, UsePropsType.TEMP)
	      end


		  if propId == ItemType.ADD_FIVE_STEP_BY_TRAVEL_MODE then

	      	mainLogic:useProps( propId , aidPropId, 0, 0, 0, UsePropsType.TEMP)
	      end

			if propId == ItemType.ADD_FIVE_STEP_BY_VALENTINE_2020 
				or propId == ItemType.ADD_FIVE_STEP_BY_ANGRY_BIRDS 
				or propId == ItemType.ADD_TWO_STEP_BY_ANGRY_BIRDS 
				then		
				--确保在使用中途闪退，恢复后能加上
				mainLogic:useProps(propId, 0, 0, 0, 0, UsePropsType.TEMP, false, true)  --前面道具使用记录已随加五步加上，无需再加
			end

			-- if propId == ItemType.ADD_FIVE_STEP_BY_SAKURA_2020 then
			-- 	mainLogic:useProps(propId, 0, 0, 0, 0, UsePropsType.TEMP, false, true)
			-- end
...
function GameExtandPlayLogic:getNormalPositionsForBoss(mainLogic, count, fromRow, toRow, banList, useOtherRows)
	...
	if not toRow then toRow = 9	end 	--棋盘大小不要写死数字，已下线活动不重构，复用时请注意修改
...
function GameExtandPlayLogic:onEnterWaitingState(mainLogic)
	...
	if mainLogic.gameMode:is(OlympicHorizontalEndlessMode) or 
		...
		mainLogic.gameMode:is(MoleWeeklyRaceMode) 
		or mainLogic.theGamePlayType == GameModeTypeId.WEEKLY_RACE_2020_MODE_ID
		then
		mainLogic.gameMode:onEnterWaitingState()
	end
	
	SmartGuideManager.getInstance():onEnterWaitingState()
	NationalDayBunnyLogic:checkNDBunnyGuide(mainLogic)

	GameExtandPlayLogic:handleJustThrowProp(mainLogic)

	if mainLogic.gameMode:is(MoleWeeklyRaceMode) 
		or mainLogic.theGamePlayType == GameModeTypeId.WEEKLY_RACE_2020_MODE_ID
		then 
...
function GameExtandPlayLogic:handleJustThrowProp( mainLogic )
	-- body

	if mainLogic.justThrowABird then
		mainLogic.justThrowABird = mainLogic.justThrowABird - 1
		if mainLogic.justThrowABird <= 0 then
			mainLogic.justThrowABird = nil
		end
	end

	if mainLogic.justThrowLineWrap then
		mainLogic.justThrowLineWrap = mainLogic.justThrowLineWrap - 1
		if mainLogic.justThrowLineWrap <= 0 then
			mainLogic.justThrowLineWrap = nil
		end
	end
		...
		for _, dir in ipairs(directions) do
			...
			if mainLogic:isPosValid(r1, c1) then
				...
				if item and item.ItemType == GameItemType.kCrystalStone and item:isActiveCrystalStoneAvailble() 
						and not mainLogic:hasRopeInNeighbors(r, c, r1, c1, true) then
					return ccp(r1, c1)
				end
			end
		end
...
function GameExtandPlayLogic:checkSuperCuteBallRecover(mainLogic, callback)
	...
	for r = 1, #boardmap do
		for c = 1, #boardmap[r] do
			local board = boardmap[r][c]
			local item = mainLogic.gameItemMap[r][c]
			if item and not item:hasBlocker206() and not item:hasSquidLock() and (not WaterBucketLogic:hasBucket(item)) then
...
function GameExtandPlayLogic:getSuperCuteValidAroundItems(mainLogic, centerItem)
	...
	for i, coord in ipairs(aroundDirections) do 
		...
		if tempItem 
			...
			and tempItem.ItemType ~= GameItemType.kTreasureFish
			...
			and tempItem.ItemType ~= GameItemType.kTravelHero
			and tempItem.ItemType ~= GameItemType.kTravelFinishBuilding
			and tempItem.ItemType ~= GameItemType.kTravelFinishBuildingEmpty
			and (not CanevineLogic:is_occupy(tempItem))
			and tempItem.ItemType ~= GameItemType.kPlane
			and not CatteryLogic:isCattery(tempItem.ItemType)
			and tempItem.ItemType ~= GameItemType.kAngryBird
			and tempItem.ItemType ~= GameItemType.kSling
			and tempItem.ItemType ~= GameItemType.kCuckooBird
			and tempItem.ItemType ~= GameItemType.kCuckooClock
			and not CuckooLogic:gridBelongsToTravelRoad(tempBoard)
			and tempItem.ItemType ~= GameItemType.kFirework
			then
			table.insert(result, tempItem)
		end
	end
...
function GameExtandPlayLogic:checkSuperCuteBallTransfer(mainLogic, callback)
	...
	if #superCuteBoardList > 0 then
		for _, board in pairs(superCuteBoardList) do
			...
			if not item:hasBlocker206() and not item:hasSquidLock() and #validAroundItems > 0 and (not WaterBucketLogic:hasBucket(item)) then
...
function GameExtandPlayLogic:decIceLevelAt(mainLogic, r, c)
	...
	ChannelWaterLogic.afterDecIceLevelAt(mainLogic, r, c)
	TreasureHuntLogic:checkLevelTarget(mainLogic, board, oldLevel)
end
...
function GameExtandPlayLogic:decreaseBlockerCover(mainLogic, r, c)
	...
	if item.blockerCoverLevel == 0 then
		if item.ItemType == GameItemType.kNone and item.ItemStatus == GameItemStatusType.kNone then
			item.isEmpty = true
		end
...
function GameExtandPlayLogic:destroyBlocker207(mainLogic , br , bc)
	if mainLogic.blocker206Cfg and table.size(mainLogic.blocker206Cfg) > 0 then
		...
		if lockGroupKey then 
			...
			-- printx( 1 , "GameItemActionType.kItem_Blocker207_Dec  !!!!!!!!!  lockGroupKey = " , lockGroupKey , br , bc )
        ...
        for r1 = 1, #mainLogic.swapHelpMap do
			for c1 = 1,#mainLogic.swapHelpMap[r1] do
                ...
                if not table.exist( MachIDList, mathID ) then
                    ...
                    if mathBoard:hasJamSperad() and math.abs(mathID) > 0 and not ItemData:hasLock() then
    ...
    if mainLogic.swapHelpMap then
		...
		for r1 = 1, #mainLogic.swapHelpMap do
			for c1 = 1,#mainLogic.swapHelpMap[r1] do
				...
				if mathID > 0 and mathID == moneyMathID and not(r1 == r and c1 == c) and not mathBoard:hasJamSperad() then
                    self:addJamSperadFlag( mainLogic, r1, c1 )
				end
			end
		end
...
function GameExtandPlayLogic:addJamSperadFlag(mainLogic, r, c, bForceAdd )
    ...
    if mainLogic:isPosValid(r,c) then
        ...
        if itemInfo and mathBoard then
            ...
            if mathBoard:hasJamSperad() then
                return
            end
...
function GameExtandPlayLogic:findBiscuitBoardDataCoveredMe(mainLogic, r, c)
	for rr = math.max(r - 3, 1), r do
		for cc = math.max(c - 3, 1), #mainLogic.boardmap[rr] do
...
function GameExtandPlayLogic:findItem( mainLogic, conditionFunc , startCounter)
	local counter = 1
	startCounter = startCounter or 1
	for r = 1, #mainLogic.boardmap do 
		for c = 1, #mainLogic.boardmap[r] do
			local boardData = mainLogic.boardmap[r][c]
			local itemData = mainLogic.gameItemMap[r][c]
			if counter >= startCounter and conditionFunc(itemData, boardData) then
				return r, c, counter
			end
			counter = counter + 1
		end 
...
function GameExtandPlayLogic:existBird( mainLogic )
	return not not self:findItem(mainLogic, function ( itemData, boardData )
		return itemData and itemData.ItemType == GameItemType.kAnimal and itemData.ItemSpecialType == AnimalTypeConfig.kColor
	end)
end

--  关卡内往棋盘上扔出类似前置道具的特效，多个，前置道具逻辑检查位置，不支持扔的会没有这个数据，或者 r\c=0
function GameExtandPlayLogic:checkAddBuffsOnBoard( mainLogic,itemIds )
	GameInitBuffLogic.mainLogic = mainLogic
	local buffs = {}
	for i=1,#itemIds do
		local buffType = Prop2BuffMapping[itemIds[i]]
		-- print("GameExtandPlayLogic:checkAddBuffsOnBoard",i,itemIds[i],buffType)
		table.insert(buffs,GameInitBuffLogic:createBuffData(buffType, InitBuffCreateType.INGAME_STATE))
	end
	local resultList = {}
	local initBuffInfo = GameInitBuffLogic:__checkMapAndFindCreatePosition( nil , buffs , resultList, true )
	-- 不支持扔的会没有这个数据，或者 r\c=0，这里需要根据来源从新排序，方便来源处检查扔的是哪几个
	local newResult = {}
	for i,v in ipairs(resultList) do
		-- print("GameExtandPlayLogic:checkAddBuffsOnBoard",i,v and v.r,v and v.sourceIndex)
		if v and v.r~=0 and v.sourceIndex then
			newResult[v.sourceIndex] = v
		end
	end
	return newResult,initBuffInfo
end

--  关卡内往棋盘上扔出类似前置道具的特效，多个。 targetItemPosList 每个 node 需要至少有 r\c\tarItemSpecialType
function GameExtandPlayLogic:addBuffsOnBoard( mainLogic,itemIds,startPosList,targetItemPosList,callback )
	if not targetItemPosList then
		targetItemPosList = GameExtandPlayLogic:checkAddBuffsOnBoard(mainLogic,itemIds)
	end
	print("GameExtandPlayLogic:addBuffsOnBoard()targetItemPosList-",#targetItemPosList)
	if not targetItemPosList then
	...
	local function callbackOne()
		now = now + 1
		-- print("GameExtandPlayLogic:addBuffsOnBoard()callbackOne",now,total)
		if now >= total then
			local _ = callback and callback()
		end
	end

	for i,v in ipairs(targetItemPosList) do
		-- print("GameExtandPlayLogic:addBuffsOnBoard",i,v and v.r,v and v.c)
		if v and v.r ~=0 then
			total = total+1
			local tarItemSpecialTypes = {v.tarItemSpecialType,v.tarItemSpecialType2}
			GameExtandPlayLogic:addBuffOnBoard(mainLogic,itemIds[i],startPosList and startPosList[i],targetItemPosList[i],tarItemSpecialTypes,callbackOne)
		end
	end
end

--  关卡内往棋盘上扔出类似前置道具的特效，
function GameExtandPlayLogic:addBuffOnBoard( mainLogic,itemId,startPos,targetItemPos,tarItemSpecialTypes,callback )
	-- print("GameExtandPlayLogic:addBuffOnBoard()",mainLogic,itemId,startPos,targetItemPos,callback)
	local realId = ItemType:getRealId(itemId)
	local levelId = mainLogic.level

	-- local actionType
	-- if itemId == ItemType.PRE_RANDOM_BIRD then
	-- 	-- 10087 智能魔力鸟
	-- 	actionType = GamePropsActionType.kThrowBird
	-- 	local action = GameBoardActionDataSet:createAs(
	-- 			GameActionTargetType.kPropsAction, 
	-- 			actionType,
	-- 			theAction.ItemPos1, 
	...
	-- 	action.fixPos = theAction.fixPos
	-- 	mainLogic:addPropAction(action)
	-- end

	local tarItemSpecialType,tarItemSpecialType2 = nil,nil
	if tarItemSpecialTypes then
		tarItemSpecialType = tarItemSpecialTypes[1]
		tarItemSpecialType2 = tarItemSpecialTypes[2]
	end
	-- 没有指定位置，按前置道具规则选一个位置
	if not targetItemPos then
		local targetItemPosList = GameExtandPlayLogic:checkAddBuffsOnBoard(mainLogic,{itemId})
		local datas = targetItemPosList and targetItemPosList[1]
		if datas then
			targetItemPos = datas
			tarItemSpecialType = datas.tarItemSpecialType
			tarItemSpecialType2 = datas.tarItemSpecialType2
		end
	end

	-- print("GameExtandPlayLogic:addBuffOnBoard()targetItemPos--",targetItemPos.c,targetItemPos.r,targetItemPos.c2,targetItemPos.r2,tarItemSpecialType,tarItemSpecialType2)

	if not targetItemPos or targetItemPos.c==0 then
		local _ = callback and callback(false)
		return
	end
	...
	local buffType = Prop2BuffMapping[itemId]
	local action =  GameBoardActionDataSet:createAs(
		GameActionTargetType.kPropsAction,
		...
		nil,
		GamePlayConfig_MaxAction_time)
	action.pos = {r = targetItemPos.r, c = targetItemPos.c}
	local item = mainLogic.gameItemMap[action.pos.r][action.pos.c]
	action.tarItemColorType = item._encrypt.ItemColorType
	action.tarItemSpecialType = tarItemSpecialType

	if action.tarItemSpecialType == AnimalTypeConfig.kColor then
		action.tarItemColorType = 0
	end

	if buffType == InitBuffType.LINE_WRAP  then
		action.pos2 = {r = targetItemPos.r2, c = targetItemPos.c2}
		local item = mainLogic.gameItemMap[action.pos2.r][action.pos2.c]
		action.tarItemColorType2 = item._encrypt.ItemColorType
		action.tarItemSpecialType2 = tarItemSpecialType2
	end
	...
	action.skipStart = true
	-- print("GameExtandPlayLogic:addBuffOnBoard()action.startPos",table.tostring(action.startPos),action.startPos and action.startPos.x,action.startPos and action.startPos.r)
	mainLogic:addGlobalCoreAction(action)


	if startPos then
		local name = "wrapEffect/anim"
		-- local sprite = UIHelper:createArmature3("skeleton/props_effect_animation", name, name, name)
		FrameLoader:loadArmature("skeleton/props_effect_animation")
		local sprite = ArmatureNode:create(name)
		sprite:addEventListener(ArmatureEvents.COMPLETE, function()
			sprite:removeFromParentAndCleanup(true)
		end)
		sprite:playByIndex(0, count)
		sprite:update(0.001)
		sprite:setPositionXY(startPos.x,startPos.y)
		mainLogic.PlayUIDelegate:addChild(sprite)
	end
end

--  棋盘上变成类似前置道具的特效，注意这里设置完数据，需要下一帧才能刷新 itemView
function GameExtandPlayLogic:changeToBuff( mainLogic,targetItemPos)
	-- print("GameExtandPlayLogic:changeToBuff",table.tostring(targetItemPos))
	-- targetItemPos.item = pos.item
    -- targetItemPos.r = pos.r
    -- targetItemPos.c = pos.c
    -- targetItemPos.tarItemSpecialType = tarItemSpecialType
    -- targetItemPos.tarItemType = tarItemType
    -- targetItemPos.buffType = v.buffType
    -- targetItemPos.createType = v.createType
    -- targetItemPos.propId = v.propId
	-- print("GameExtandPlayLogic:changeToBuff - 0")

	local tarR, tarC = targetItemPos.r, targetItemPos.c
	print("GameExtandPlayLogic:changeToBuff",tarR, tarC,targetItemPos.buffType)
	local item = mainLogic.gameItemMap[tarR][tarC]
	if targetItemPos.buffType == InitBuffType.BUFF_BOOM then
		...
		local tarColor = item._encrypt.ItemColorType
		if targetItemPos.tarItemType then
			item.ItemType = targetItemPos.tarItemType

			if targetItemPos.buffType ==  InitBuffType.FIRECRACKER then
				mainLogic.generateFirecrackerTemp = true
			end
		else
			local tarItemSpecialType = targetItemPos.tarItemSpecialType
			item.ItemType = GameItemType.kAnimal
			item:changeItemType(tarColor, tarItemSpecialType )
		end
		if targetItemPos.itemId and targetItemPos.itemId == ItemType.ADD_FIVE_STEP_BY_MATCH_FESTIVAL then
			item.ItemType = GameItemType.kAddMove
			item:initAddMoveConfig(5)
		end
	end
	item.isNeedUpdate = true

	if not skipBoardAnim then
		if targetItemPos.buffType == InitBuffType.LINE then
			PropsAnimation:playLineEffect(targetItemPos.tarItemSpecialType,tarR, tarC, mainLogic.boardView, nil)
		else
			PropsAnimation:playWrapEffect(tarR, tarC, mainLogic.boardView, nil)
		end
	end

	----
	if targetItemPos.buffType == InitBuffType.LINE_WRAP  then
		local tarR, tarC = targetItemPos.r2, targetItemPos.c2
		local item = mainLogic.gameItemMap[tarR][tarC]
		item.ItemType = GameItemType.kAnimal
		item:changeItemType(targetItemPos.tarItemColorType2, targetItemPos.tarItemSpecialType2 )
		item.isNeedUpdate = true

		if not skipBoardAnim then
			PropsAnimation:playLineEffect(targetItemPos.tarItemSpecialType2,tarR, tarC, mainLogic.boardView, nil)
		end
	end
