...
function CommonMultipleHittingPriorityLogic:getTargetGridPrior(mainLogic, r, c, selfItem, exceptGrids)
	...
	if CommonMultipleHittingPriorityLogic._isTargetInvalid(itemData, boardData) then
		...
		--冰封导弹,吃豆人,前置炸弹 && 底下有果酱
		local selfBoardData = selfItem and mainLogic.boardmap[selfItem.y][selfItem.x]
		local missileHasJam = selfBoardData and selfBoardData:hasJamSperad() and 
								(selfItem.ItemType == GameItemType.kMissile
								or selfItem.ItemType == GameItemType.kDynamiteCrate
								or selfItem.ItemType == GameItemType.kPacman
								or selfItem.ItemType == GameItemType.kFirecracker)
		...
		for i = 1, #priorityFuncList do
			...
			if currFunc(mainLogic, itemData, boardData, missileHasJam) then
...
function CommonMultipleHittingPriorityLogic._isTargetInvalid(itemData, boardData)
	...
	if itemData and (itemData.ItemType == GameItemType.kBlocker195 or itemData.ItemType == GameItemType.kCrystalStone) then
		if boardData and boardData.iceLevel > 0 then
			return true
		end
	end

	if itemData and itemData.ItemType == GameItemType.kCrystalStone then
		if itemData:getCrystalStoneEnergy() >= GamePlayConfig_CrystalStone_Energy then
			return true
		end
	end
	
	-- 真正的一票否决制
	if itemData.isReverseSide 
		or itemData:hasBlocker206() 
		or itemData:hasSquidLock() 
		or itemData:hasLockedWaterBucket()
		then
		return true
	end

	-- 有覆盖物的情况下走覆盖物的优先级
    if not itemData:hasLock() 
    	and not itemData:isFreeGhost()
    	and not itemData:hasActiveWaterBucket() 
    	then
    	-- 无覆盖物时的否决
...
function CommonMultipleHittingPriorityLogic._isTargetPrior1(mainLogic, itemData, boardData)
	...
	if not itemData.colorFilterBLock 
		...
		and (itemData.cageLevel > 0
				...
				or itemData.ItemType == GameItemType.kAngryBird
				or SlyBunnyLogic:laneIsCommonMultiplePrior1(itemData)
		then
		return true
	end
...
function CommonMultipleHittingPriorityLogic._isTargetPrior1Point5(mainLogic, itemData, boardData)
	...
	if GameExtandPlayLogic:isEmptyBiscuit(mainLogic, boardData.y, boardData.x) 
		and table.includes({
			...
			GameItemType.kTravelEnergyBag,
		}, itemData.ItemType) then
		return true
	end

	if itemData:hasActiveWaterBucket() then
		return true
	end

	if WaterBucketLogic:canAttackWater(itemData) then
		return true
	end
...
function CommonMultipleHittingPriorityLogic._isTargetPrior2(mainLogic, itemData, boardData)
	if  itemData.snowLevel > 0 
        ...
        or CanevineLogic:is_canevine_root(itemData)
        or itemData.ItemType == GameItemType.kWanSheng
        or itemData.ItemType == GameItemType.kGyro
        or itemData.ItemType == GameItemType.kTravelEnergyBag
		or itemData.ItemType == GameItemType.kPlane
		or itemData.ItemType == GameItemType.kCattery
		or itemData.ItemType == GameItemType.kMeow
		or WeeklyRace2020Logic:isHittableWeeklyRace2020Chest(itemData)
		or SlyBunnyLogic:laneIsCommonMultiplePrior2(itemData)
		or itemData.ItemType == GameItemType.kCuckooWindupKey
		or (itemData.ItemType == GameItemType.kFirework and itemData.fireworkLevel > 0 )
		or itemData.ItemType == GameItemType.kBattery
		then
		return true
	end
...
function CommonMultipleHittingPriorityLogic._isTargetPrior3(mainLogic, itemData, boardData, missileHasJam)
	if itemData.ItemType == GameItemType.kCoin
		...
		or SlyBunnyLogic:laneIsCommonMultiplePrior3(itemData)
		or itemData.ItemType == GameItemType.kpuffedRice
		then
		return true
	end
	...
	if (boardData.sandLevel > 0 and itemData.ItemType ~= GameItemType.kNone) 
		or (boardData.lotusLevel == 1 and itemData.ItemType ~= GameItemType.kNone) 
		or (boardData.haveBoard and itemData.ItemType ~= GameItemType.kNone and itemData.ItemType ~= GameItemType.kWalkChick) 
		then
		return true
	end

	if not boardData:hasJamSperad() and missileHasJam and itemData:isAnimalLike() then
		return true
	end
...
function CommonMultipleHittingPriorityLogic._isTargetPrior4(mainLogic, itemData, boardData)
	if (itemData.ItemType == GameItemType.kCrystalStone and (not itemData.crystalStoneActive))
		...
		or (itemData.ItemType == GameItemType.kShellGift and not itemData.isHitShellGift)
        ...
        or itemData.ItemType == GameItemType.kWindTunnelSwitch
		then
		return true
	end
	return false
...
function CommonMultipleHittingPriorityLogic._isTargetPrior5(mainLogic, itemData, boardData)
	if (itemData.ItemType == GameItemType.kAnimal and not itemData:isSpecial())
		...
		or (itemData.ItemType == GameItemType.kDynamiteCrate and itemData.missileLevel > 0)
		...
		or itemData.ItemType == GameItemType.kActivityCollectionItem
		then
		return true
	end
	return false
...
function CommonMultipleHittingPriorityLogic._isTargetPrior6(mainLogic, itemData, boardData)
	if itemData.ItemType == GameItemType.kNewGift
		or itemData.ItemType == GameItemType.kAddMove
