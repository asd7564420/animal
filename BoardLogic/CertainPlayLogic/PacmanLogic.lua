...
function PacmanLogic:_addDevouredItemAsRemoved(mainLogic, oldPacman, targetItem)
	...
	elseif targetItem.ItemType == GameItemType.kNewGift then
		addScore = 0
	elseif targetItem.ItemType == GameItemType.kAddMove then
		addScore = GamePlayConfigScore.AddStepAnimal
	elseif targetItem.ItemType == GameItemType.kTravelEnergyBag then
		addScore = GamePlayConfigScore.TravelEnergyBag
	elseif targetItem.ItemType == GameItemType.kBalloon then
		addScore = GamePlayConfigScore.Balloon
	elseif targetItem.ItemType == GameItemType.kCuckooWindupKey then
		addScore = GamePlayConfigScore.CuckooWindupKey
	end
...
function PacmanLogic:onHitTargets(mainLogic, pacman, targetItems)
	...
	for i, targetItem in ipairs(targetItems) do
		...
		if isWrapEffect then
			...
			--果酱兼容
			diagonalSquareAction.SpecialID = mainLogic:addSrcSpecialCoverToList(IntCoord:create(pacman.y, pacman.x))
			...
			--果酱兼容
			rectangleAction.SpecialID = mainLogic:addSrcSpecialCoverToList(IntCoord:create(pacman.y, pacman.x))
