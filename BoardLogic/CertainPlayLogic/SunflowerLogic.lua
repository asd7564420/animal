	...
	if item.sunFlaskLevel == 0 then
		...
		if SunflowerLogic:getSunflowerLackedEnergy(mainLogic) > 0 then
			...
			if targetSunflower then
				action.canCharge = not WaterBucketLogic:hasBucket(targetSunflower)
				if action.canCharge then
...
function SunflowerLogic:onSunFlaskDestroyed(mainLogic, targetFlask, canCharge)
	if targetFlask then
		...
		if mainLogic.sunflowerEnergy and canCharge then
