...
function SquidLogic:isInvalidTargetForSquid(mainLogic, targetItem, targetBoard)
	if not targetItem.isUsed
		...
		or WaterBucketLogic:hasBucketLocked(targetItem)
...
function SquidLogic:onSquidBombGrid(mainLogic, theAction)
	...
	if targetItem:isVisibleAndFree() then
		...
		elseif SquidLogic:isJustRemoveItem(targetItem) then
			...
			elseif targetItemType == GameItemType.kShellGift then
			...
			elseif targetItemType == GameItemType.kFirework and targetItem.fireworkLevel > 0 then
				targetItem.fireworkLevel = 1
				FireworkLogic:triggerOneFirework( mainLogic, targetItem )
			elseif targetItemType == GameItemType.kBattery then
				self:squidRemoveBattery(mainLogic, targetItem)
			elseif targetItemType == GameItemType.kpuffedRice then
				PuffedRiceLogic:onPuffedRiceBeingHit(mainLogic, r, c, 4, 1)
            end
		end
...
function SquidLogic:isCommonBombItem(targetItem)
	...
	if targetItem.ItemType == GameItemType.kAnimal 
		...
		or targetItem.ItemType == GameItemType.kAddMove
		or targetItem.ItemType == GameItemType.kBalloon 
		or targetItem.ItemType == GameItemType.kTravelEnergyBag 
		or targetItem.ItemType == GameItemType.kCuckooWindupKey 
		then
		return true
	end
...
function SquidLogic:dealWithCovers(mainLogic, targetItem, targetItemView, targetBoard)
	...
	if targetBoard then
		...
		if targetBoard.superCuteState == GameItemSuperCuteBallState.kActive then
			SquidLogic:deactivateSuperCuteball(mainLogic, targetItem, targetItemView, targetBoard)
		end
	end

	if targetItem:isFreeGhost() then
		local rowAmount, colAmount = GameBoardUtil:getRowAndColAmountOfBoard(mainLogic)
		GhostLogic:addGhostPace(mainLogic, targetItem, colAmount)
	end
...
-- 蓄电器
function SquidLogic:squidRemoveBattery(mainLogic, targetItem)
	local r, c = targetItem.y, targetItem.x
	targetItem.batteryLevel = 0
    ...
    targetItem.batteryCharged = true
    mainLogic:checkItemBlock(r,c)
    SnailLogic:doEffectSnailRoadAtPos(mainLogic, r,c)

    batteryChargeAction.completeCallback = actionCallback
    batteryChargeAction.battery = targetItem
  
    mainLogic:addDestroyAction(batteryChargeAction)

    mainLogic:addScoreToTotal(targetItem.y, targetItem.x, GamePlayConfigScore.Battery)
...
function SquidLogic:squidRemoveSnow(mainLogic, targetItem)
	...
	SnowAction.collectItemID = targetItem.SakuraIngredientID or targetItem.valentineIngredientID
	if not SnowAction.collectItemID and SnowMatchFestivalLogic:isSnowMatchFestivalLevel() then
		SnowAction.collectItemID = 1
	end
...
function SquidLogic:removeLeafPile(mainLogic, targetItem, targetItemView, targetBoard)
	...
	GameExtandPlayLogic:doAllBlocker195Collect(mainLogic, r, c, Blocker195CollectType.kBlockerCover)
	SquidLogic:checkSquidCollectItem(mainLogic, r, c, TileConst.kBlockerCover)
...
function SquidLogic:deactivateSuperCuteball(mainLogic, targetItem, targetItemView, targetBoard)
	local r, c = targetItem.y, targetItem.x
