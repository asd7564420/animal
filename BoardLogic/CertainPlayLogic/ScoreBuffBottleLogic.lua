...
function ScoreBuffBottleLogic:getScoreBuffBottleInitAmount(levelID)
	...
	MACRO_DEV_START()
	if self.testInitAmount and self.testInitAmount >=0 then
		-- printx(11, "getScoreBuffBottleInitAmount from testVal", self.testInitAmount)
		return self.testInitAmount
	end
	MACRO_DEV_END()
		    ...
		    if item and board and not board.preAndBuffFirecrackerPassSelect 
		    	...
		    	if item:isVisibleAndFree() then
		    		...
		    		if not (
						...
						or WaterBucketLogic:hasBucketLocked(item)
