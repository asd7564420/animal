...
GameMusicType = table.const
{
	...
	kCuckooWalk = "music/sound.cuckoo.walk.mp3",
	kCuckooBonus = "music/sound.cuckoo.bonus.mp3",

	kFireworkGrow = "music/sound.firework.grow.mp3",
	kFireworkBoom = "music/sound.firework.boom.mp3",

	kGoldFly = "music/sound.gold.fly.mp3",

	kRankPopOut = "music/sound.rankPopOut.mp3",
	kRankPopUp = "music/sound.rankPopUp.mp3",
	kRankPopUp10 = "music/sound.rankPopUp10.mp3",
	kRankPopEnd = "music/sound.rankPopEnd.mp3",
}
...
function GamePlayMusicPlayer:ctor()
	...
	self.lockBackgroundMusicPath = nil
end
...
function GamePlayMusicPlayer:appResume()
	...
	if self.IsBackgroundMusicOPen then
		if self.curBgMusicFile then
			local tryTimes = 5
			local function cb()
				if self.isAppPaused or (not self.IsBackgroundMusicOPen) then
					tryTimes = 0
				elseif not SimpleAudioEngine:sharedEngine():isBackgroundMusicPlaying() then

					if __ANDROID and PlatformConfig:isPlatform(PlatformNameEnum.kHuaWei) then
						local Cocos2dxHelper = luajava.bindClass("org.cocos2dx.lib.Cocos2dxHelper")
...
--mp3Path  文件地址
----------------------------------
function GamePlayMusicPlayer:playOtherBgMusic(mp3Path, blockBackgroundMusic )
    if self.isAppPaused then return end

    if blockBackgroundMusic then
    	self:lockBackgroundMusic( mp3Path )
    end
...
function GamePlayMusicPlayer:playWorldSceneBgMusic(Volume, bUnlockBackgroundMusic,...)
	assert(#{...} == 0)
	if self.isAppPaused then return end

	if bUnlockBackgroundMusic then
		self:unLockBackgroundMusic()
	end

	if self.lockBackgroundMusicPath then
		if self.curBgMusicFile ~= self.lockBackgroundMusicPath  then
			self.curBgMusicFile = self.lockBackgroundMusicPath

			if self.IsBackgroundMusicOPen then
				-- Stop Previous Background Music
				SimpleAudioEngine:sharedEngine():stopBackgroundMusic(true)
				SimpleAudioEngine:sharedEngine():playBackgroundMusic(self.lockBackgroundMusicPath, true)
			else
				-- Do Nothing
			end
		end
...
function GamePlayMusicPlayer:stopEffect(soundID,filename)
	if GamePlayMusicPlayer:getInstance().IsMusicOpen then
		SimpleAudioEngine:sharedEngine():stopEffect(soundID)
	end
	if filename and self.effectLifeCycle then
		self.effectLifeCycle[filename] = false
	end
end
		...
		if self.effectLifeCycle[filename] then
			...
			if(insideSfxFullList) then
				if(SimpleAudioEngine:sharedEngine():isEffectPreloaded(filename)) then
					return SimpleAudioEngine:sharedEngine():playEffect(filename)
				end
			else
				return SimpleAudioEngine:sharedEngine():playEffect(filename)
			end
...
function GamePlayMusicPlayer:lockBackgroundMusic(mp3Path)
	self.lockBackgroundMusicPath = mp3Path
end

function GamePlayMusicPlayer:unLockBackgroundMusic()
	self.lockBackgroundMusicPath = nil
end