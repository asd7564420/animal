...
function NormalSwapInteraction:handleTouchBegin(x, y)
    ...
    self.lastBeginX = x
    self.lastBeginY = y
	if self.boardView.isOppositeView then
		self.lastBeginX, self.lastBeginY = GameBoardUtil:convertVisualPosToOppositeBoardViewPos(self.boardView, x, y)
	end
...
function NormalSwapInteraction:handleTouchMove(x, y)
	if self.boardView.isOppositeView then
		x, y = GameBoardUtil:convertVisualPosToOppositeBoardViewPos(self.boardView, x, y)
	end
...
function NormalSwapInteraction:handleTouchEnd(x, y)
    ...
    if self.currentState == self.firstTouchedState then
        if BaseInteraction.isEqualPos(self.item1Pos, touchPos) then
            self.item1Pos = touchPos
            if _G.isLocalDevelopMode then printx(0,"NormalSwapInteraction:handleTouchEnd()", touchPos.x, touchPos.y) end
        end
