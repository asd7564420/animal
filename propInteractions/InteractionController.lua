...
require 'zoo.gamePlay.propInteractions.NewBroomInteraction'
...
function InteractionController:ctor(boardView)
    ...
    self.newBroomInteraction = NewBroomInteraction.new(boardView, self)
...
function InteractionController:onUseProp(propId , usePropType)
    ...
    elseif propId == GamePropsType.kBroom_new then
        self:setInteraction(self.newBroomInteraction)
...
function InteractionController:onInteractionComplete(result)
    if self.boardView.gamePropsType == GamePropsType.kHammer 
        ...
        require('zoo.props.LeftPropListUIDc'):useProp(self.boardView.gamePropsType)
    ...
    elseif self.boardView.gamePropsType == GamePropsType.kBroom 
        or self.boardView.gamePropsType == GamePropsType.kBroom_l 
        or self.boardView.gamePropsType == GamePropsType.kBroom_new
        then
        require('zoo.props.LeftPropListUIDc'):useProp(self.boardView.gamePropsType)
...
function InteractionController:onCancelUsingProp()
    if _G.isLocalDevelopMode then printx(0, 'InteractionController:onCancelUsingProp()') end
    self:setInteraction(self.normalSwapInteraction)
    require('zoo.props.LeftPropListUIDc'):cancel(self.boardView.gamePropsType)
end
