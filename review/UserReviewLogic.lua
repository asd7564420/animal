...
function UserReviewLogic:test( replayData )
	local replay = replayData
	if not replay then
		replay = ReplayDataManager:readLastSuccessReplayCache()
	end
	-- Localhost:safeWriteStringToFile( table.tostring(replayData) , HeResPathUtils:getUserDataPath() .. "/" .. 'debug.rep')
	if replay and replay.level then
		require "zoo.panelBusLogic.NewStartLevelLogic"
		local newStartLevelLogic = NewStartLevelLogic:create(nil, replay.level, {}, false, {})
		newStartLevelLogic:startWithReplay(ReplayMode.kReview, replay)
	else
...
function UserReviewLogic:onWaitingNextStep( mainLogic, nextStepDelegate, errorHandler)
	...
	-- print("mainLogic.replaySteps[mainLogic.replayStep--",mainLogic.replayStep,mainLogic.replaySteps,table.tostring(mainLogic.replaySteps))

	local err, msg = self:checkScoreError(mainLogic)
	if err then
		if _G.isLocalDevelopMode then print("UserReviewLogic:onWaitingNextStep() Error!!! score exception!".. tostring(msg)) end
	...
	if nextStepType == 'swap' then
		...
		if canSwap and (not (MaintenanceManager:getInstance():isEnabled("userreviewerror") and mainLogic.theCurMoves <= 15 )  ) then
			...
			if _G.isLocalDevelopMode then print("UserReviewLogic:onWaitingNextStep() Error!!! can't swap!") end
	...
	elseif nextStepType == 'prop' then
		...
		if canUse then
			-- print("UserReviewLogic:onWaitingNextStep()self:tryDoNextStep!")
			self:tryDoNextStep(nextStepDelegate)
		else
			if _G.isLocalDevelopMode then print("UserReviewLogic:onWaitingNextStep() Error!!! can't use prop!" .. nextStepDetails[1]) end
			if errorHandler then errorHandler("can't use prop " .. nextStepDetails[1]) end
		end
	elseif nextStepType == 'switchPlayer' then
		if GoldenPodBattleLogic:isGoldenPodBattleMode() then
			self:tryDoNextStep(nextStepDelegate)
		else
			if _G.isLocalDevelopMode then print("UserReviewLogic:onWaitingNextStep() Error!!! can't switch player!") end
			if errorHandler then errorHandler("can't switch player!") end
		end
	end
end
...
function UserReviewLogic:doNextStep( nextStepDelegate, finishCallback )
	...
	elseif nextStepType == 'prop' then
		if self.context.showMode then
			setTimeOut(function ( ... )
				self:showUsePropAnimation(nextStepDetails, function ( ... )
					if nextStepDelegate then nextStepDelegate() end
					if finishCallback then finishCallback() end
				end)
			end, 1)
	...
	elseif nextStepType == 'switchPlayer' then
		GoldenPodBattleLogic:switchPlayer()
	end
end
...
function UserReviewLogic:showUsePropAnimation( usePropDetails , finishCallback )

	-- printx( 0 , "UserReviewLogic:showUsePropAnimation()  usePropDetails =" , table.tostring(usePropDetails) )
	if not self.context then
		if _G.isLocalDevelopMode then print("UserReviewLogic:showUsePropAnimation() Error!!! NO self.context") end
		return
	end
	...
	if not playUIDelegate then
		if _G.isLocalDevelopMode then print("UserReviewLogic:showUsePropAnimation() Error!!! NO playUIDelegate") end
		return
	end

	local propId = usePropDetails[1]
	local r1, c1, r2, c2 = usePropDetails[3], usePropDetails[4], usePropDetails[5], usePropDetails[6]

	-- printx(0, "UserReviewLogic:showUsePropAnimation()itemFound-propId:",propId,r1, c1, r2, c2)

	local from = nil
	local to = nil

	if r1 and c1 and r1 > 0 and c1 > 0 then
		from = self:getBoardViewTilePos(playUIDelegate, 9 - (r1 - 0.5), c1 - 0.5)
	end
	
	if r2 and c2 and r2 > 0 and c2 > 0 then
		to = self:getBoardViewTilePos(playUIDelegate, 9 - (r2 - 0.5), c2 - 0.5)
	end

	local handAnim = nil

	local propList = playUIDelegate.propList
	local itemFound, itemIndex = propList:findItemByItemID( propId )

	if not itemFound then
		local mappingItemId = PropsModel.kTempPropMapping[tostring(propId)]

    	if mappingItemId then 
    		itemFound, itemIndex = propList:findItemByItemID( mappingItemId ) 
    	end
	end

	-- printx(0, "UserReviewLogic:showUsePropAnimation()itemFound",itemFound)

	local propPos = nil
    
    if itemFound then
    	propList.leftPropList:ensureItemInSight( itemFound , 0.3 , function() 
			propPos = itemFound:getItemCenterPosition()

			if from and to then
				handAnim = GameGuideAnims:handUsePropAnim( propPos , from , to , 1 )
			elseif from then
				handAnim = GameGuideAnims:handUsePropAnim( propPos , from , from , 1 )
			else
				handAnim = GameGuideAnims:handUsePropAnim( propPos , propPos , propPos , 1 )
			end

			if handAnim then
				playUIDelegate.guideLayer:addChild(handAnim)
			end

			setTimeOut(function ( ... )
				if playUIDelegate and (not playUIDelegate.isDisposed) then
					playUIDelegate.guideLayer:removeChild(handAnim, true)
					if finishCallback then finishCallback() end
				end
			end, 3)
    	...
    	if propId == GamePropsType.kGetTempProp
    		or propId == GamePropsType.kAdd5
    		then
	    	-- 关卡加五步 或者 加五步送道具 则直接判定成功，没有底部道具栏消耗动画
			if finishCallback then finishCallback() end
	    else
			if _G.isLocalDevelopMode then print( "UserReviewLogic:showUsePropAnimation() ERROR!! item NOT Found ") end
	    end
    end
end
...
function UserReviewLogic:onStartLevel( mainLogic )
	-- printx( 1 , "UserReviewLogic:onStartLevel  " , debug.traceback() , GamePlayContext:getInstance().replayData , replayData )
	local replayData = GamePlayContext:getInstance().replayData
	...
	if replayData and replayData.showMode then
		self.context.showMode = true
		GameSpeedManager:setGameSpeedSwitch(2)
		GameSpeedManager:changeSpeedForFastPlay()
	else
		self.context.showMode = false
	end
end
...
function UserReviewLogic:getNextStepInfo( mainLogic )
	...
	local switchPlayer = mainLogic.replaySteps[mainLogic.replayStep].switchPlayer
...
function UserReviewLogic:attachMenu_1( gamePlaySceneUI )
	...
	local lPos = self:getBoardViewTilePos(gamePlaySceneUI, 10 - gamePlaySceneUI.gameBoardView.startRowIndex - gamePlaySceneUI.gameBoardView.rowCount - 0.5, 4.5)
	...
	-- printx( 1 , "UserReviewLogic:attachMenu_1 " , debug.traceback() )
	if self.context.showMode then
		menu:setVisible(false)
		setTimeOut( function() menu:notify('startReplay') end , 1 )
	end
...
function UserReviewLogic:attachMenu_2( gamePlaySceneUI )
	...
	gamePlaySceneUI:superAddChild(menu)
	local lPos = self:getBoardViewTilePos(gamePlaySceneUI, 10 - gamePlaySceneUI.gameBoardView.startRowIndex + 0.24, 4.5)
...
function UserReviewLogic:onMenuCommand(menu, message )
	...
	lua_switch(message){
		...
		stopRecord = function ( ... )
			...
			local panel = UserReviewEndPanel:create( self.context.showMode )
...
function UserReviewLogic:isEnabled( ... )
	...
	return (tonumber(topLevel) or 0) > 150
