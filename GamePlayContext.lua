...
require "zoo.gamePlay.LastGamePlayContext"
...
function GamePlayContext:init()
	...
	self.isHelpingFriendOnThisPlay = false
	...
	self.cmap = {}
	self.mctsver = nil
	self.isBonusSpeed = false
end


function GamePlayContext:reset()
	self.last_reset_log = debug.traceback()
...
function GamePlayContext:resetEndlessLoopData()
	self.endlessLoopData = {}
end
...
function GamePlayContext:setRevertData(data)
	if data then 
		...
		self.endlessLoopLog = data.endlessLoopLog and table.clone(data.endlessLoopLog, true)
...
function GamePlayContext:getRevertData()
	...
	data.endlessLoopLog = self.endlessLoopLog and table.clone(self.endlessLoopLog, true)
	-- data.endlessLoopData = self.endlessLoopData and table.clone(self.endlessLoopData, true)
	return data
end

function GamePlayContext:updateCMap( maptype , ci )
	-- printx( 1 , "GamePlayContext:updateCMap   " ,  maptype , ci , self , self.cmap )
	if not self.cmap[maptype] then
		self.cmap[maptype] = {
			[1] = 0 ,
			[2] = 0 ,
			[3] = 0 ,
	...
	local cMap = self.cmap[maptype]
	if ci > 0 and ci <= 6 then
		cMap[ ci ] = cMap[ ci ] + 1
		Notify:dispatch( "onUpdateCMap" , {maptype = maptype , ci = ci} )
	end
end
...
function GamePlayContext:addAIInterveneLog(realCostStep, realCostStepWithOutProp, leftSteps, interveneColor)
	...
	-- 补齐前面跳过的步数
	for i = realCostStep - 1, 1, -1 do
		if self.aiCostStepsArr[i] then
			break
		else
			self.aiCostStepsArr[i] = {}
			self.aiLeftStepsArr[i] = {}
			self.aiInterveneArr[i] = {}
			self.aiRepeatArr[i] = {}
			table.insert(self.aiCostStepsArr[i], realCostStepWithOutProp - (realCostStep - i))
			table.insert(self.aiLeftStepsArr[i], leftSteps + (realCostStep - i))
			table.insert(self.aiInterveneArr[i], "e")
			table.insert(self.aiRepeatArr[i], "e")
		end
	end
...
function GamePlayContext:onGainProp( propid , feature , source )
	-- gainPropList
	if self.inLevel then
		
		local mainLogic = GameBoardLogic:getInstance()
		local step = -99
		local usePropsType = nil
		...
		-- printx( 1 , "GamePlayContext:onGainProp  " , table.tostring(obj) )

		table.insert( self.gainPropList , obj )
	end
end
...
function GamePlayContext:onUseProp( propid , step , usePropsType )
	...
	-- printx( 1 , "GamePlayContext:onUseProp  " , table.tostring(obj) )
...
function GamePlayContext:setAISeedData(mainLogic)
	...
	local aiCoreInfo = LevelDifficultyAdjustManager:getAICoreInfo()
...
function GamePlayContext:startLevel( levelId , levelType )
	LastGamePlayContext:getInstance():reset()
end

function GamePlayContext:onStartLevelMessageSuccessed( levelId , levelType, preStartContext )
	self:reset()
	self.inLevel = true
	...
	self.userInfo.colorfulStar = user:getColorfulStar()

	if preStartContext then
		self.preStartContext = preStartContext
	...
	local newReturnAndLowActiveData = LevelDifficultyAdjustManager:getNewReturnAndLowActiveAdjustLogic():getLocalData()
	self.levelInfo.newReturnAndLowActiveSeqIdx = newReturnAndLowActiveData.seqIdx or 0
	self.levelInfo.newReturnAndLowActiveSeqIdx = self.levelInfo.newReturnAndLowActiveSeqIdx - 1
...
function GamePlayContext:onGameInit( mainLogic )
	...
	if GameSpeedManager:getGameSpeedSwitch() > 0 then
		if self.theGamePlayType == GameModeTypeId.CLASSIC_ID 
			-- or self.theGamePlayType == GameModeTypeId.GOLDEN_POD_BATTLE_MODE_ID 
			then
...
function GamePlayContext:endLevel()
	...
	LastGamePlayContext:getInstance():updateByContext( self )
...
function GamePlayContext:encodeContextDataForReplay()
	...
	if self.gainPropList then
		local result = {}
		for _, v in ipairs(self.gainPropList) do
			table.insert(result, {v.propId, v.step, v.pt , v.feature , v.source})
		end
		obj.gainPropList = result
	end
	...
	obj.newReturnAndLowActiveSeqIdx = self.levelInfo.newReturnAndLowActiveSeqIdx
...
function GamePlayContext:decodeContextDataForReplay(data)
	...
	if data then
		...
		if self.gainPropList then
			for _, v in ipairs(data.gainPropList or {}) do
				table.insert(self.gainPropList, {
					propId = v[1],
					step = v[2],
					pt = v[3],
					feature = v[4],
		...
		self.levelInfo.newReturnAndLowActiveSeqIdx = data.newReturnAndLowActiveSeqIdx
		return obj
	end
...
function GamePlayContext:checkHasRealUsePropInLevel(levelId)

	if self.inLevel then
		local boardLogic = GameBoardLogic:getCurrentLogic()
		local realCostMove = boardLogic.realCostMove or -1

		local realUsePropMap = {}
		local breakFlag = false
		local hasRealUseProp = false
		if self.preStartContext and self.preStartContext.prePropCostInfo then
			local count = 0 
			for k1 , v1 in pairs(self.preStartContext.prePropCostInfo) do
				hasRealUseProp = true
				breakFlag = true
				break
			end
		...
		if not breakFlag then
			local gainPropList = self.gainPropList
			local usePropList = self.usePropList

			local gainPropCountMapOnStep = {}

			for k1 , v1 in ipairs(gainPropList) do

				if v1.feature == DcFeatureType.kStagePlay and v1.source == DcSourceType.kDrop then

					if not gainPropCountMapOnStep[ v1.step ] then
						gainPropCountMapOnStep[ v1.step ] = {}
					end

					local pmap = gainPropCountMapOnStep[ v1.step ]
					if not pmap[v1.propId] then
						pmap[v1.propId] = 0
					end

					pmap[v1.propId] = pmap[v1.propId] + 1
				end
			end

			local usePropCountMapOnStep = {}

			for k1 , v1 in ipairs(usePropList) do

				if not usePropCountMapOnStep[ v1.step ] then
					usePropCountMapOnStep[ v1.step ] = {}
				end

				local pmap = usePropCountMapOnStep[ v1.step ]
				if not pmap[v1.propId] then
					pmap[v1.propId] = 0
				end
			...
			for i = 0 , realCostMove do
				if gainPropCountMapOnStep[i] then
					local _map = gainPropCountMapOnStep[i]
					for k2 , v2 in pairs( _map ) do
						if not realUsePropMap[k2] then
							realUsePropMap[k2] = 0
						end
						realUsePropMap[k2] = realUsePropMap[k2] + v2
					end
				end

				if usePropCountMapOnStep[i] then
					local _map = usePropCountMapOnStep[i]
					for k2 , v2 in pairs( _map ) do
						if not realUsePropMap[k2] then
							realUsePropMap[k2] = 0
						end
						realUsePropMap[k2] = realUsePropMap[k2] - v2
						if realUsePropMap[k2] < 0 then
							hasRealUseProp = true
							breakFlag = true
							break
						end
		...
		local isActEnergyInfiniteNew = StarBuffYEManager.getInstance():isBuffEffective(levelId) --记录刷星道具
		if isActEnergyInfiniteNew then
			if StarBuffYEManager.getInstance().curUsePropId ~= 0 then
				hasRealUseProp = true
			end
		end

		return hasRealUseProp
	end
