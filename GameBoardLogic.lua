...
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.CuckooLogic"
...
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.ActivityPlayLogic.SnowMatchFestivalLogic"
...
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.ActivityPlayLogic.SlyBunnyLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.ChannelWaterLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.ShellGiftLogic"
require "zoo.gamePlay.BoardLogic.CertainPlayLogic.PVPPlayLogic.GoldenPodBattleLogic"
...
--	self.encryptIndex = encryptIndex
	...
	self.oringinTotalScore = 0;			----被修正前的原始得分。业务逻辑用totalScore即可。当难度调整造成过多combo时候会触发意外高分所以削弱分数的获取，这里是削弱前的原始分，仅统计用
	...
	self.snailMark = false  -- 标记是否跑蜗牛，刺猬的逻辑，如果为false关卡内就不处理相关逻辑,主要为了减少计算量 --后人update：谜之标记，有蜗牛路径配置时为true，为false时也不会减少计算量
	...
	-- self.gyroGeneratedByStep = 0		--- 生成的陀螺数量（通过步数）
	-- self.gyroGeneratedByBoardMin = 0	---	生成的陀螺数量（通过棋盘最少限制）
	...
	self.generateFirecrackerTemp = 0	--活动等临时生成过爆竹
	...
	self.hadAutoSnowMonsterBuff = false -- 是否有使用雪怪技能道具
	self.hadAutoBuffActived = false -- 雪怪技能是否已触发
	...
	self.lastSlyBunnyFleeingMoves = 0	--[临时活动]上一个狡兔走动的步数

	self.cuckooEnergy = 0				-- 布谷鸟能量，出于需求允许和性能考虑全局使用
	self.skipCuckooStateType = 0 		-- [临时] 某些障碍被攻击后，不会影响stable状态并需要等到自己的state才会处理，此间屏蔽布谷鸟行动检测
	self.currMapTravelRouteLength = 0	-- 当前地图的路径长度
	self.currMapTravelStep = 0			-- 当前地图走了多少格

	self.shellGiftData = nil		-- 贝壳礼包

	self.goldenPodBattleData = GoldenPodBattleLogic:getInitGoldenPodBattleData(self)	-- 抢豆荚数据
	self.inObservingMode = false 	--观赏模式，不能操作，如对方玩家的回合
end
...
function GameBoardLogic:dispose()
	...
	self.generateFirecrackerTemp = nil
	self.generateFirecrackerTimes = nil
	self.generateFirecrackerTimesForPreBuff = nil
	self.squidOnBoard = nil
	self.globalScoreBuffVal = nil
	self.hadAutoSnowMonsterBuff = false
	self.hadAutoBuffActived = false
	
	self.collectTargetNums = nil
	self.extraGainedActCollections = nil
	self.moveBonusHarvested = false
	self.gameReachTarget = false

	self.lastSlyBunnyFleeingMoves = nil

	self.cuckooEnergy = nil
	self.skipCuckooStateType = nil
	self.currMapTravelRouteLength = nil
	self.currMapTravelStep = nil

	self.shellGiftData = nil

	self.goldenPodBattleData = nil
	self.inObservingMode = false
...
MACRO_DEV_START()
--GameBoardLogic:create 方法的实际定义在LocalBox里
MACRO_DEV_END()
...
function GameBoardLogic:getCurrentLogic()
	-- printx(11, "GameBoardLogic:getCurrentLogic", _G.UseNewGameBoardLogic, debug.traceback())
	-- printx(11, "GameBoardLogic:getCurrentLogic", _G.UseNewGameBoardLogic)
	if _G.UseNewGameBoardLogic then
		return NewGameBoardLogic:getInstance()
	else
...
function GameBoardLogic:initBoard()
	...
	self.generateFirecrackerTemp = 0
	...
	self.lastSlyBunnyFleeingMoves = 0

	self.cuckooEnergy = 0
	self.skipCuckooStateType = 0
	self.currMapTravelRouteLength = 0
	self.currMapTravelStep = 0

	self.goldenPodBattleData = GoldenPodBattleLogic:getInitGoldenPodBattleData(self)
	self.inObservingMode = false

	self.hadAutoSnowMonsterBuff = GameInitBuffLogic:isSnowMonsterBuff()	

	if ShellGiftLogic.isEnabled() then
		self.shellGiftData = ShellGiftLogic.initData(self)	
	else
		ShellGiftLogic.dispose(true)
	end
	...
	for r = 1, m do
		...
		for c = 1, n do
			...
			if item then
				...
				if self._fieldLogicPossibility[_FIELD_LOGIC_ID.NDBunnyHandle] ~= 1 
					and itemType == GameItemType.kNationalDayBunny and item:isVisibleAndFree() then
...
function GameBoardLogic:numDestroyAction()
	local count = 0
...
function GameBoardLogic:numFallingAction()
	local count = 0
...
--和getGlobalCoreActionListCount功能是一样的 注释掉了
-- function GameBoardLogic:numGlobalCoreAction()
-- 	local count = 0
...
function GameBoardLogic:getGlobalCoreActionListCount()
	local n = 0
...
function GameBoardLogic:isItemAllStableExceptSuperTotems( passPosMap )
...
function GameBoardLogic:segmentedActionCanMoveOn( )
...
function GameBoardLogic:boardStableHandler()
	...
	-- local needMix = self:addAllItemsForMatchCheck(true)
	-- print("GameBoardLogic:boardStableHandler()",needMix)
	-- if needMix then
	-- 	return
	-- end
...
function GameBoardLogic:getJamPosList()
	local posList = {} 
...
function GameBoardLogic:clearTargetTip(realRm)
	...
	if (state ~= self.theGamePlayStatus) then
		if state == GamePlayStatus.kEnd then
			...
			if self.gameMode:reachTarget() then
				...
				self.bonusTimeScore = self.totalScore
		...
		elseif state == GamePlayStatus.kBonus then
			...
			NationalDayBunnyLogic:checkNDBunnyGuide(nil,true)
			...
			if (BombItemLogic:getNumSpecialBomb(self) > 0 
				...
				or self.gameMode:is(OrderMode)
				or (self.gameMode:is(DestinationMode) and self.gameMode:hasSpecialBonusAnimation())
				)
		...
		elseif state == GamePlayStatus.kWin then
			...
			if self.theGamePlayType ~= GameModeTypeId.DIG_TIME_ID and 
				...
				self.theGamePlayType ~= GameModeTypeId.HEDGEHOG_DIG_ENDLESS_ID
				and self.theGamePlayType ~= GameModeTypeId.GOLDEN_POD_BATTLE_MODE_ID
				-- and self.theGamePlayType ~= GameModeTypeId.TRAVEL_MODE_ID
				then
...
function GameBoardLogic:_formattingScoreBeforeGameEnded()
...
function GameBoardLogic:initByConfig(level, config, levelType, forceUseDropBuff , replayMode , replayData)
	...
	self.randomSeed = config.randomSeed or 0
	self.gameDataVersion = config.gameDataVersion or 0

    self.SunmerFish3x3GetNum = config.SunmerFish3x3GetNum or 1
    if LevelType:isPopcornLevel(level) then
    	self.SunmerFish3x3GetNum = 1
    end
	...
	if not replayMode or replayMode == ReplayMode.kNone then
		...
		if GameGuide:hasGuideSeed(level) then
			...
			self.randomSeed = GuideSeeds[level]
		end
	...
	-- self.randomSeed = 47
 	if self.randomSeed == 0 then 
 		...
 		if GamePlayContext:getInstance().preStartContext then
 			local playId = GamePlayContext:getInstance().preStartContext:getPlayId()
	 		local md5str = HeMathUtils:md5( playId )
	 		self.randomSeed = hex2number( string.sub( md5str , -8 ) )
 		else
 			self.randomSeed = os.time()
 		end
 		
 		-- printx( 1 , "playId = " , playId )
 		-- printx( 1 , "md5str = " , md5str )
 		-- printx( 1 , "self.randomSeed = " , self.randomSeed )
 	end
	...
	self.GyroConfig = config.gyroCreaterData

	self.waterBucketCfg = config.waterBucketCfg

	self.NDbunnyConfig = config.NDbunnyConfig
	self.NDbunnyGlobalConfig = config.NDbunnyGlobalConfig
	self.NDbunnyGridConfig = config.NDbunnyGridConfig
	self.NDbunnySnowGridConfig = config.NDbunnySnowGridConfig
	...
	self.moveTileCarryProductRuleMode = config.moveTileCarryProductRuleMode
	...
	GameMapInitialLogic:init(self, config , replayMode , replayData)
	...
	self.gameMode:initModeSpecial(config,replayData)
	...
	-- -- TSUM 2019
	-- if true then
	-- 	local canUseBuff, canUseBuffLevel = TsumBuffManager:getInstance():getLevelCanUseBuffAndBufflevel(self.level,self.stageStartTime)
	-- 	-- print("-- Tsum GameBoardLogic---canUseBuff, canUseBuffLevel:",canUseBuff, canUseBuffLevel,"self.stageStartTime:",self.stageStartTime)
	-- 	if canUseBuff then
	-- 		TsumManager.canGetHeadIcon = true
 --            TsumManager.lastLevelStartInfo = {levelId = level,startTime = self.stageStartTime}

	-- 		DcUtil:UserTrack({category = "2019_tsum_buff",
	-- 			sub_category = "tsum_buff_stage_start",
	-- 			levelId = level,
	-- 			t1 = canUseBuffLevel,
	-- 			t2 = GamePlayContext:getInstance():getIdStr(),
	-- 			startTime = self.stageStartTime
	-- 		})

 --        else
	-- 		TsumManager.canGetHeadIcon = false
 --        	TsumManager.lastLevelStartInfo = nil
	-- 	end
 --   end
...
function GameBoardLogic:onGameInit()
	-- printx(11, "= = = GameBoardLogic:onGameInit", debug.traceback())
	...
	-- 	local log = {}
 --        log.method = "GameBoardLogic:onGameInit"
...
function GameBoardLogic:randomColor(forbidDropBuff)
	...
	-- printx( 1 , "GameBoardLogic:randomColor ~~~~~~~~~~~~~~~~~~~ self.mapColorList:")
...
function GameBoardLogic:randomColorByGlobalWeight()
	return self:_randomColorByWeight(self.mapColorWeightList)
end

function GameBoardLogic:_randomColorByWeight(colourWeightList, limitedColorList, forbidDropBuff)
...
function GameBoardLogic:safeGetBoardData(r, c)
...
function GameBoardLogic:safeGetItemData(r, c)
...
function GameBoardLogic:canUseBroom(r, c, isNewBroom)
...
function GameBoardLogic:startWaitingOperation()
	...
	self.gameMode:onModeStartWaitingState()
...
function GameBoardLogic:stopWaitingOperation()
	self.isWaitingOperation = false
	self:onWaitingOperationChanged()
	self.gameMode:onModeStopWaitingState()
end

function GameBoardLogic:onWaitingOperationChanged()
	if self.isWaitingOperation == true and not self.inObservingMode then
	...
	if self:getActionListCount() == 0 then
		...
		elseif propsType == GamePropsType.kAdd5
			...
			if self.PlayUIDelegate and (not self.PlayUIDelegate.isDisposed) then
				local pos = nil
				if r1 and c1 and r1*c1 ~= 0 then
					pos = self:getGameItemPosInView(r1, c1)
				end
				self.PlayUIDelegate:useStepPropComplete(self.theCurMoves,pos)
			end
		...
		elseif propsType == GamePropsType.kAdd10Autumn2020 then
			...
			if self.PlayUIDelegate and (not self.PlayUIDelegate.isDisposed) then
				self.PlayUIDelegate:useStepPropComplete(self.theCurMoves, nil, 10)
			end
		...
		elseif propsType == GamePropsType.kSwap 
			...
			then
			if _G.isLocalDevelopMode then printx(-1, "********Swap") end
		...
		elseif propsType == GamePropsType.kHammer 
			...
			then
			if _G.isLocalDevelopMode then printx(-1, "********Hammer") end
		...
		elseif propsType == GamePropsType.kBack 
			...
			then
			if _G.isLocalDevelopMode then printx(-1, "********TheRevert") end
			...
			if self.saveRevertData then
				...
				local step = self.theCurMoves
				self.gameMode:revertDataFromBackProp()
				local deltaStep = self.theCurMoves - step
				--[[
				...
				if self.PlayUIDelegate then
					...
					local node = TimebackAnimation:create(deltaStep<=0)
					node:setPosition(ccp(winSize.width / 2, winSize.height / 2))
					self.PlayUIDelegate.effectLayer:addChild(node)

					if self.realCostMove == 1 then
						if self.PlayUIDelegate.propList and self.PlayUIDelegate.propList.leftPropList then
		...
		elseif propsType == GamePropsType.kGetTempProp 
			then
			--  +5步ai送道具 useProp 时 aiPropId 传参必须是 int,方便写 ReplayData。如果其他地方需要临时发道具，单独处理，不需要记录的可以直接 action
			local action = GameBoardActionDataSet:createAs(
		        GameActionTargetType.kPropsAction, 
		...
		if (not self.replaying) 
			...
			if _G.isLocalDevelopMode then printx(-1, "***********ReplayLog: Remembering Property usage!") end
		end
		...
		if self.replaying then
			if self.replayMode == ReplayMode.kMcts and _G.AutoCheckLeakInLevel then
				...
				local str = "GameBoardLogic:useProps  propsType:" .. tostring(propsType) .. "   propsUseType:" .. tostring(propsUseType) .. " --> " .. tostring(r1)..tostring(c1)..tostring(r2)..tostring(c2)
				self.PlayUIDelegate:replayResult(
									self.PlayUIDelegate.levelId, 
									self.totalScore, 
									self.gameMode:getScoreStarLevel(), 
...
function GameBoardLogic:getBroomRows(r, propsType)
    local rowList = {}
	...
	-- printx(11, "getBroomRows:", table.tostring(rowList))
    return rowList
end
...
MACRO_DEV_START()
--此方法有真假两版，此处是假的，真的在LocalBox.lua  subKey2在创建收集动画时为key2指定一个子类型，支持同样的key2有不同的icon
MACRO_DEV_END()

function GameBoardLogic:tryDoOrderList(r, c, key1, key2, v1, rotation, p_pos, animate, percent, subKey2)
	...
	if key1 == 6 and ( key2 >= 5 and key2 <= 11 ) then
	...
	for i,v in ipairs(self.theOrderList) do
		if v.key1 == key1 and v.key2 == key2 then
			...
			if self.PlayUIDelegate then 				-----向UI注册函数发起调用
				...
				self.PlayUIDelegate:setTargetNumber(v.key1, v.key2, num, pos_t, rotation, animate, percent, subKey2);
			end
		end
	end
...
function GameBoardLogic:replayDoUseProp( r1, c1, r2, c2, propId , propsUseType , para )
...
function GameBoardLogic:Replay()
	...
	-- if _G.useNewAIAutoCheck then
	-- 	local log = {}
 --        log.method = "GameBoardLogic:Replay"
 --        ReplayDataManager:addMctsLogs("CC",log)
	-- end
	...
	if self.replayError then -- 发生了错误
		...
		local function doSwap(r1, c1, r2, c2, propId)
			if _G.isLocalDevelopMode and not _G.AI_CHECK_ON then
				-- 测试环境延时一会再播放下一步，可以看到步数的间隔
				setTimeOut(function() self:startTrySwapedItem(r1, c1, r2, c2) end,0.5)
			else
					...
					local function onGetNextStep( result , nextStepCmd )
						...
						if nextStepCmd then
							...
							if nextStepCmd.method == "nextStep" then
								if nextStepCmd.dataType == "rc" or nextStepCmd.dataType == "prop" then
									...
									local c2 = nextStepCmd.action.c2
         							...
         							if nextStepCmd.dataType == "rc" then
         								...
         								local propId = nextStepCmd.action.propId
         								local propPara = nextStepCmd.action.para
         								local usePropResult = self:replayDoUseProp( r1 , c1 , r2 , c2 , propId , UsePropsType.FAKE , propPara )
         							end
		...
		if self.replayStep <= #self.replaySteps then
			...
			elseif (self.gameMode:is(ClassicMode) or self.gameMode:is(MaydayEndlessMode) or self.gameMode:is(GoldenPodBattleMode)) 
				and self.replayMode ~= ReplayMode.kResume then
	...
	if self.replaySteps and self.replaySteps[self.replayStep] then
		...
		for i=self.replayStep, maxStep do
			...
			if data.prop and (tonumber(data.prop) == GamePropsType.kAdd5 or 
								...
								tonumber(data.prop) == ItemType.TIMELIMIT_ADD_FIVE_STEP_BY_WEEKLY_RACE_2020 or
								tonumber(data.prop) == ItemType.ADD_15_STEP or 
								tonumber(data.prop) == ItemType.ADD_FIVE_STEP_BY_ANGRY_BIRDS or
								tonumber(data.prop) == ItemType.ADD_10_STEP_BY_AUTUMN2020 or 
								tonumber(data.prop) == ItemType.ADD_15_STEP_BY_AUTUMN2020
							) 
			then
...
function GameBoardLogic:checkReplayCanUseResurrectProp()
	if self.replayMode == ReplayMode.kSectionResume then
		return false
	end
...
function GameBoardLogic:getGameItemPosInViewByOffset(r, c, offset)
	self.posAdd = self.posAdd or ccp(0, 0)
	local tempX = (c - 0.5) * GamePlayConfig_Tile_Width
	local tempY = (GamePlayConfig_Max_Item_Y - r - 0.5 ) * GamePlayConfig_Tile_Height
	local visibleOrigin = CCDirector:sharedDirector():getVisibleOrigin()
...
function GameBoardLogic:testEmpty()
	...
	for r=1,#self.gameItemMap do
		...
		for c=1,#self.gameItemMap[r] do
			...
			if item._encrypt.ItemColorType then
				color = AnimalTypeConfig.convertColorTypeToIndex( item._encrypt.ItemColorType )
			end

			--if item:isAvailable() then p2 = "1" end
	 		--format = format .. p2 .. "-" .. item.ItemStatus .. "-" .. item.furballLevel .. "   "
	 		format = format .. tostring(p1) .. "_" .. tostring(p3) .. "_" .. tostring(color) .. "     "
		end
...
function GameBoardLogic:hasItemOfTypeFromDigItemMap( itemType )
...
-- 两格间的冰柱削减一层
function GameBoardLogic:decChainBetween(r1, c1, r2, c2)
	if not self:isPosValid(r1, c1) or not self:isPosValid(r2, c2) then
		return
	end
...
function GameBoardLogic:hasRopeInNeighbors(r1, c1, r2, c2, isSwapCheck)
	if not self:isPosValid(r1, c1) or not self:isPosValid(r2, c2) then
		return false
	end
...
function GameBoardLogic:addAllItemsForMatchCheck(doMatchAtOnce)
	...
	if doMatchAtOnce then
		local needMix = MatchItemLogic:checkPossibleMatch(self)
		return needMix
	end
end
...
function GameBoardLogic:addNewSuperTotemPos(totemPos)
	...
	for _, v in pairs(self.newSuperTotemsPos) do
		if v.x == totemPos.x and v.y == totemPos.y then
			alreadyExist = true
			break
		end
	end
...
function GameBoardLogic:addScoreToTotal(r, c, addScore, colorType, posType, isBuffScore , oringinAddScore)
	...
	-- printx(11, "*** GameBoardLogic:addScoreToTotal, val:", addScore, self.globalScoreBuffVal)
	if self.globalScoreBuffVal and self.globalScoreBuffVal > 0 then
...
function GameBoardLogic:setInitScrollCannon(initCannon, initSpawnPrepareData)
...
function GameBoardLogic:setInitScrollSpawnData()
...
function GameBoardLogic:CheckPosCanStopJamSperad( Pos, Pos2, justRemove )
    ...
    if itemData.isBlock then
        ...
        elseif itemData:hasMeowLock() or itemData:hasMeowHitLock() then
        	bBlock = false
        -- elseif itemData.ItemType == GameItemType.kFirework then
        -- 	bBlock = false
        end
...
function GameBoardLogic:updateGlobalScoreBuffView()
	-- printx(11, "GameBoardLogic:updateGlobalScoreBuffView", self.globalScoreBuffVal)
...
function GameBoardLogic:addExtraGainedActCollection(collectionID, addAmount)
...
function GameBoardLogic:setExtraGainedActCollectionFromRevertData(revertData)
...
function GameBoardLogic:getBoardModel()
	return nil
end

function GameBoardLogic:checkReachTarget()
