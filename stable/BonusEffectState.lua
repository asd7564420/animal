...
-- @Last Modified time: 2020-08-25 19:51:15
---------------------------------------------------------------------------------------
...
function BonusEffectState:onEnter()
	...
	local function onAnimationFinished()
		...
		if _G.testBonusTimeSpeed then
			GameSpeedManager:changeSpeed( 4 , true )
		end
	end

	-- 是否是真正的BonusTime （非真正的，播放的动画没有BonusTime印记）
	local playDefaultBonusSound = true
	local useRealBonusAnimation = true
	local gameMode = self.mainLogic.gameMode
	if gameMode and gameMode:is(DestinationMode) then
		if gameMode:hasSpecialBonusAnimation() then
			-- 有布谷鸟时，有特殊的
			playDefaultBonusSound = false
			if not CuckooLogic:useRealBonusAnimation() then
				useRealBonusAnimation = false
			end
		end
	end

	if self.mainLogic.levelType == GameLevelType.kMatchFestival then
		playDefaultBonusSound = false
		GamePlayMusicPlayer:playEffect("tempFunctionResInLevel/MatchFestival/ingame/bonustime.mp3")
	end

	if playDefaultBonusSound then
	...
	local container = PlayUIDelegate and PlayUIDelegate.topEffectLayer

	local function onAnimationFinished()
		self.isPlayingBonusAnime = false		
		if _G.testBonusTimeSpeed then
			if _G.testBonusTimeSpeed == true then
				GameSpeedManager:changeSpeed( 4 , true )
			end
		end

		if not (_G.isPlayDemo or _G.isADPackage) and container and not container.isDisposed then
			self:_initSpeedBtn(container)
		end
	end

	if container and not container.isDisposed then
		PlayUIDelegate.topEffectLayer:removeChildren(true)
		if GameBoardLogic:getCurrentLogic().gameMode:is(JamSperadMode) then	
			...
			if jamNode then
				...
				container:addChild(CommonEffect:buildBonusEffect(onAnimationFinished))
			end
		elseif GameBoardLogic:getCurrentLogic().gameMode:is(TreasureHuntMode) then
			container:addChild(TreasureHuntLogic:buildBonusEffect(PlayUIDelegate,onAnimationFinished))
		elseif gameMode and gameMode:is(DestinationMode) then
			CuckooLogic:playBonusAnimationEffect(container, useRealBonusAnimation, onAnimationFinished)
		else
			container:addChild(CommonEffect:buildBonusEffect(onAnimationFinished))
		end
	else
...
function BonusEffectState:_initSpeedBtn(container)
	require "zoo.util.GameSpeedButton"
	GameSpeedButton.createLabelBtn(container)
end
