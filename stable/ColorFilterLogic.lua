...
function ColorFilterLogic:handleFilter(r, c, callback)
	...
	if boardData and boardData.colorFilterState == ColorFilterState.kStateA and boardData.colorFilterColor ~= 0 then 
    	...
    	if itemData and 
    		...
    		not itemData:hasAnyFurball() and not itemData:seizedByGhost() 
    		and not itemData.shieldToColorFilter
    		then 
    		shouldFilter = true
...
function ColorFilterLogic:canBeEffectByFilter(itemType)
	if itemType == GameItemType.kAnimal or 			--小动物
		...
		itemType == GameItemType.kShellGift or 		--礼盒
		itemType == GameItemType.kAddMove or 		--礼盒
		...
		itemType == GameItemType.kFirecracker or		--爆竹
		itemType == GameItemType.kTravelEnergyBag		--旅行能量包
		or itemType == GameItemType.kCuckooWindupKey	--布谷鸟发条钥匙
		then
		return true
	end
	return false
...
function ColorFilterLogic:doFilter(r, c, colorIndex, callback)
	...
	if itemData.ItemType == GameItemType.kShellGift then 
		ShellGiftLogic.hitShellGift(self.mainLogic, r, c, itemData)
	end
