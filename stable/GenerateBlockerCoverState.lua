			...
			if not board.isReverseSide and not board:hasSuperCuteBall() then
				...
				if item and (item:isAvailable() or item:isFreeGhost()) then
					...
					if item.isEmpty then
						...
						if (item.ItemType ~= GameItemType.kBigMonsterFrosting and item.bigMonsterFrostingType == 0)
                            and item.ItemType ~= GameItemType.kTravelHero and item.ItemType ~= GameItemType.kCuckooBird 
                            and item.ItemType ~= GameItemType.kCuckooClock 
                            then
							return true
						end
					end
