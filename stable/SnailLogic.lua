...
function SnailLogic:check()
	...
	self.completeItem = 0
	self.totalItem = 0
	if not self.mainLogic.snailMark then   -- 有无蜗牛路径配置
		-- self:handComplete()
...
function SnailLogic:checkSnailList( callback )
	...
	for r = 1, #itemMap do 
		for c = 1, #itemMap[r] do 
			...
			if self:isCanStartMovingSnail(item) then
				local list = self:getMoveList(r, c)
				if #list > 0 then
					item.snailInMoving = true
...
function SnailLogic:checkItemCanMove( prePos, nextpos )
	...
	elseif nextboard.isSnailRoadBright then
		if nextItem.ItemType ~= GameItemType.kCoin 
			...
			and  nextItem.ItemType ~= GameItemType.kShellGift
			...
			and nextItem.ItemType ~= GameItemType.kDynamiteCrate
            ...
            and nextItem.ItemType ~= GameItemType.kWater
            and nextItem.ItemType ~= GameItemType.kPlane
            and not CatteryLogic:isCatteryOrMeow(nextItem.ItemType)
            and nextItem.ItemType ~= GameItemType.kpuffedRice
            and nextItem.ItemType ~= GameItemType.kBattery
			then 
			return true
		end
	end
...
function SnailLogic:canEffectSnailRoadAt( mainLogic, r, c )
	...
	if board.snailRoadViewType and (board.snailRoadViewType ==  TileRoadType.kLine 
		...
		if item and item.isUsed 
			...
			and not CanevineLogic:is_occupy(item)
...
function SnailLogic:isCanStartMovingSnail(item)
	if item and item.isSnail and item:isAvailable() and not item.snailInMoving then
		return true
	end
	return false
end
