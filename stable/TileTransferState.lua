...
function TileTransferState:checkStateEndStatus()
	...
	if self.proceedTransmissionEnded == true and self.proceedMoveTileEnded == true then
		...
		if self.hasMoveTileItemToHandle then
			self:updatePortals()
			self:updateCarriedProductRules()
    	...
    	for c = 1, #boardmap[r] do 
            ...
            if board and board.isMoveTile then
            	if board:checkTileCanMove() and not item:hasBlocker206() and not item:hasSquidLock() then
        			...
        			action.updateGravitySkinIfNeeded = true
...
function TileTransferState:runActionView(boardView, theAction)
	if theAction.actionStatus == GameActionStatus.kWaitingForStart then
		...
		local function onMoveFinishCallback()			
			...
			-- 移动地格携带着重力地块移动后，需要检测皮肤的更新（目前风洞有需求）
			if theAction.updateGravitySkinIfNeeded then
				local fromBoardData = theAction.boardData
				if fromBoardData and fromBoardData:getGravitySkinType() == BoardGravitySkinType.kWindTunnel then
					-- printx(11, "update windtunnel skin at:", tr, tc)
					self.mainLogic.boardView.gravitySkinViewLogic:setNeedBuildGravitySkinAt(true, tr, tc)
					self.mainLogic.boardView.gravitySkinViewLogic:setIgnoreGravitySkinChangeAnimAt(true, tr, tc) -- 被移动的重力地格不播放重力切换动画
				end
			end
...
-- 移动地格搬运生成口模式下，更新掉落规则坐标
function TileTransferState:updateCarriedProductRules()
	if not self.mainLogic.moveTileCarryProductRuleMode or not self.mainLogic.productRuleConfig then return end
	if not self.moveTiles then return end

	local function isValidPos(targetVal, maxVal)
		-- 移动地格只出现于普通棋盘关卡
		if targetVal and targetVal >= 1 and targetVal <= maxVal then
			return true
		end
		return false
	end

	local newProductRuleConfig = {}
	local rowAmount, colAmount = GameBoardUtil:getRowAndColAmountOfBoard(self.mainLogic)

	for posKey, ruleID in pairs(self.mainLogic.productRuleConfig) do
		local toPos = self.moveTiles[posKey]
		if toPos and isValidPos(toPos[1], rowAmount) and isValidPos(toPos[2], colAmount) then
			local newPosKey = toPos[1].."_"..toPos[2]
			newProductRuleConfig[newPosKey] = ruleID
		else
			newProductRuleConfig[posKey] = ruleID
		end
	end
	self.mainLogic.productRuleConfig = newProductRuleConfig
end