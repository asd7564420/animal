...
--注意 注意!!!
--这个炸弹已经废弃了，变成了小导弹。小导弹用的是通用的优先级，大家不用兼容这个啦~~！！
BuffCastingPriorityOrderFlag = {
	"kDigGround", "kBalloon", "kRocket", "kGift", "kShellGift", "kSunFlask", "kFrosting", "kMagicLamp", "kMagicStone", 
	"kTurret", "kGhost", "kBottleBlocker", "kMissile", "kDynamiteCrate", "kBlocker199", "kBlocker211", "kCrystalStone", 
	"kColorFilter", "kBlocker207", "kVenom", "kLock", "kBrownFurball", "kPuffer", "kSnow", "kRoost", 
	"kGreyFurball", "kHoney", "kBlackFurball1", "kBlackFurball2", "kCoin", "kHoneyBottle", "kWindTunnelSwitch", 
	"kPlane", "kSuperCute", "kBlockerCover", "kBlockerCoverMaterial", "kMimosa", "kCommon"
}
...
function BuffBoomCastingState:tryBoom()
    ...
    local function isAvailableGrid(item , board)
		...
		if item and board and board.isUsed then
			...
			if not item:hasBlocker206() and not board.isReverseSide and not item:hasSquidLock() and (not WaterBucketLogic:hasBucket(item)) --[[and board.side < 2]] and not item.isEmpty --[[and not item:hasActiveSuperCuteBall()]] then
				...
				if item.ItemType ~= GameItemType.kPoisonBottle
					...
					and item.ItemType ~= GameItemType.kTreasureFish
				then
		...
		for r = 1, #mainLogic.gameItemMap do
			for c = 1, #mainLogic.gameItemMap[r] do
				...
				if isAvailableGrid( item , board ) then
					...
					elseif playType == GameModeTypeId.DIG_TIME_ID 
						or playType == GameModeTypeId.DIG_MOVE_ID 
						or playType == GameModeTypeId.DIG_MOVE_ENDLESS_ID 
						or playType == GameModeTypeId.WEEKLY_RACE_2020_MODE_ID 
						then
					...
					elseif playType == GameModeTypeId.CLASSIC_MOVES_ID 
						or playType == GameModeTypeId.CLASSIC_ID 
						or playType == GameModeTypeId.GOLDEN_POD_BATTLE_MODE_ID 
						then
						--do nothing
					end
					...
					if not item:hasActiveSuperCuteBall() then
						...
						elseif item.ItemType == GameItemType.kShellGift then
							addPosToTargetSortList( "kShellGift" , r , c )
						...
						elseif item.ItemType == GameItemType.kDynamiteCrate and item.missileLevel > 0 then
							addPosToTargetSortList( "kDynamiteCrate" , r , c )
						...
						elseif item.ItemType == GameItemType.kAnimal or item.ItemType == GameItemType.kCrystal 
							or item.ItemType == GameItemType.kScoreBuffBottle or item.ItemType == GameItemType.kTravelEnergyBag
						...
						elseif item.ItemType == GameItemType.kWindTunnelSwitch then
							addPosToTargetSortList( "kWindTunnelSwitch", r, c )
						elseif item.ItemType == GameItemType.kPlane then
							addPosToTargetSortList( "kPlane", r, c )
						end
					end
				end
			end
