...
require "zoo.gamePlay.stable.BeforeGameOverState"
...
require "zoo.gamePlay.stable.CanevineLevelupState"
require "zoo.gamePlay.stable.CanevineBombState"
...
require "zoo.gamePlay.stable.DynamiteCrateFireState"
...
require "zoo.gamePlay.stable.GyroGenerateState"
require "zoo.gamePlay.stable.GyroCreterRemoveState"
require "zoo.gamePlay.stable.ActAutoBombState"
require "zoo.gamePlay.stable.ActivityCollectionItemState"
require "zoo.gamePlay.stable.TravelHeroWalkState"
require "zoo.gamePlay.stable.CuckooBirdWalkState"
require "zoo.gamePlay.stable.ChangeBoardScrollState"
require "zoo.gamePlay.stable.BeforeWaitingState"
require "zoo.gamePlay.stable.PlaneFlyState"
require "zoo.gamePlay.stable.CatteryRollingSplitState"
require "zoo.gamePlay.stable.AngryBirdWalkState"
require "zoo.gamePlay.stable.MoveBonusHarvestState"
require "zoo.gamePlay.stable.SlyBunnyMoveState"
require "zoo.gamePlay.stable.WalkChickWalkState"
require "zoo.gamePlay.stable.ChannelWaterState"
require "zoo.gamePlay.stable.ShellGiftState"
require "zoo.gamePlay.stable.BattleTrophyGenerateState"
require "zoo.gamePlay.stable.NDBunnyHandleState"
require "zoo.gamePlay.stable.NDBunnyLeaveState"
require "zoo.gamePlay.stable.NDBunnyProduceState"
require "zoo.gamePlay.stable.PuffedRiceJumpState"
...
function StableStateMachine:initStates()
	...
	self.actAutoBombState = ActAutoBombState:create(self)
	...
	self.dynamiteCrateFireFirstState = DynamiteCrateFireFirstState:create(self)
	self.dynamiteCrateFireInLoopState = DynamiteCrateFireInLoopState:create(self)
	...
	-- 游戏结束前的延迟...
	self.beforeGameOverState = BeforeGameOverState:create(self)
	...
	self.shellGiftStateInBonus = ShellGiftStateInBonus:create(self)
	...
	self.travelHeroWalkState = TravelHeroWalkState:create(self)
	self.cuckooBirdWalkState = CuckooBirdWalkState:create(self)
    ...
    self.GyroGenerateState = GyroGenerateState:create(self)
    self.GyroCreterRemoveState = GyroCreterRemoveState:create(self)

    self.ActivityCollectionItemStateInLoop = ActivityCollectionItemStateInLoop:create(self)
    self.ActivityCollectionItemStateInBonus = ActivityCollectionItemStateInBonus:create(self)

	self.moveBonusHarvestStateBeforeChangeBoard = MoveBonusHarvestStateBeforeChangeBoard:create(self)
	self.moveBonusHarvestStateInBonus = MoveBonusHarvestStateInBonus:create(self)

    self.changeBoardScrollState = ChangeBoardScrollState:create(self)

	self.canevineLevelupState = CanevineLevelupState:create(self)
    self.canevineBombStateInLoop = CanevineBombStateInLoop:create(self)
    self.canevineBombStateInSwapFirst = CanevineBombStateInSwapFirst:create(self)

	self.planeFlyState = PlaneFlyState:create(self)

    self.beforeWaitingState = BeforeWaitingState:create(self)

    self.catteryRollingSplitState = CatteryRollingSplitState:create(self)

    self.angryBirdWalkState = AngryBirdWalkState:create(self)
	self.channelWaterState = ChannelWaterState:create(self)
	self.shellGiftState = ShellGiftState:create(self)
	self.slyBunnyMoveState = SlyBunnyMoveState:create(self)
    self.WalkChickWalkState = WalkChickWalkState:create(self)

    self.battleTrophyGenerateState = BattleTrophyGenerateState:create(self)

    self.NDBunnyHandleState = NDBunnyHandleState:create(self)
    self.NDBunnyLeaveState = NDBunnyLeaveState:create(self)
    self.NDBunnyProduceState = NDBunnyProduceState:create(self)

    self.PuffedRiceJumpState = PuffedRiceJumpState:create(self)
...
function StableStateMachine:changeState(target)
	if target ~= nil then
		...
		if self.currentState and self.mainLogic.isFallingStable then
...
function StableStateMachine:onEnter()
	-- printx(11, "= = = = = = = StableStateMachine:onEnter = = = = =", debug.traceback())
	...
	if self.PuffedRiceJumpState:check() > 0  then
		if self.currentState then
			self.currentState:stopUpdate()
		end
		self.priorityLogic = "PuffedRiceJumpState"
		return 
	end---爆米花跳动处理


	if self.travelHeroWalkState:check() > 0  then
		if self.currentState then
			self.currentState:stopUpdate()
		end
		self.priorityLogic = "travelHeroWalkState"
		return 
	end---检测处理旅行模式移动逻辑

	if self.cuckooBirdWalkState:check() > 0  then
		if self.currentState then
			self.currentState:stopUpdate()
		end
		self.priorityLogic = "cuckooBirdWalkState"
		return 
	end---检测处理旅行模式移动逻辑

	if self.mainLogic:getGamePlayType() == GameModeTypeId.MOLE_WEEKLY_RACE_ID
		or GameModeTypeId.WEEKLY_RACE_2020_MODE_ID == self.mainLogic:getGamePlayType()
		then
	...
	if self.angryBirdWalkState:check() > 0 then
		if self.currentState then
			self.currentState:stopUpdate()
		end
		self.priorityLogic = "angryBirdWalkState"
		return 
	end---检测处理愤怒的小鸟移动逻辑

	if self.WalkChickWalkState:check() > 0 then
		if self.currentState then
			self.currentState:stopUpdate()
		end
		return
