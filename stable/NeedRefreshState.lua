...
function NeedRefreshState:update(dt)
	if self.mainLogic.refreshAfterAddSteps then
		self.mainLogic.refreshAfterAddSteps = false
		...
		context.refreshTotalToHandle = RefreshItemLogic:runRefreshAction(self.mainLogic, false, refreshComplete) 
		return
	end
end
...
function NeedRefreshState:tryRefresh()
	...
	local needRefresh = (not self.mainLogic.gameReachTarget) and RefreshItemLogic:checkNeedRefresh(self.mainLogic) and RefreshItemLogic:checkCrystalStoneNeedRefresh(self.mainLogic)
	-- printx( 1 , "NeedRefreshState tryRefresh   self.mainLogic.theCurMoves = " , self.mainLogic.theCurMoves , " needRefresh = " , needRefresh , self)
	
	if needRefresh then
		if RefreshItemLogic.tryRefresh(self.mainLogic) then

			if self.mainLogic.theCurMoves == 0 
				and self.mainLogic.theGamePlayType ~= GameModeTypeId.MOLE_WEEKLY_RACE_ID 
				and self.mainLogic.theGamePlayType ~= GameModeTypeId.WEEKLY_RACE_2020_MODE_ID 
				then
				local gameMode = self.mainLogic.gameMode

				if gameMode and gameMode:failByRefresh() then
					-- printx( 1 , "NeedRefreshState tryRefresh 1")
					self.mainLogic.needRefreshAfterAddSteps = true
					self.mainLogic:setGamePlayStatus(GamePlayStatus.kEnd)
				else
					-- printx( 1 , "NeedRefreshState tryRefresh 2")
				...
				if self.mainLogic.PlayUIDelegate then
					-- printx( 1 , "NeedRefreshState tryRefresh 3")
					...
					-- printx( 1 , "NeedRefreshState tryRefresh 4")
		...
		-- self.mainLogic:setNeedCheckFalling()
		-- self.nextState = self.context.beforeWaitingState
		self.nextState = self.context.battleTrophyGenerateState
	end
end
